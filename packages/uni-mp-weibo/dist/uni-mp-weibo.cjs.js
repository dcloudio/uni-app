...
function createErrorHandler(app) {
  return function errorHandler(err, instance, _info) {
    ...
    {
      appInstance.proxy.$callHook(uniShared.ON_ERROR, err);
    }
  };
}
...
function setupPage(comp) {
  const weiboMixin = {
    onShow() {
      let onShareAppMessage = this.$options.onShareAppMessage;
  ...
  if (window.weibo && typeof window.weibo.sendShareMessage === "function") {
    comp.mixins = [weiboMixin];
  }
  if (process.env.NODE_ENV !== "production") {
    comp.__mpType = "page";
  }
