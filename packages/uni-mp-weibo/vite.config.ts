...
if (FORMAT === 'es') {
  ...
  rollupPlugins.push({
    ...
    generateBundle(_options, bundle) {
      const esBundle = bundle['uni-mp-weibo.es.js'] as unknown as OutputChunk
      if (esBundle) {
...
export default defineConfig({
  ...
  define: {
    ...
    __TEST__: false,
    __PLATFORM__: JSON.stringify('mp-weibo'),
    __APP_VIEW__: false,
  ...
  build: {
    ...
    rollupOptions: {
      output: {
        ...
        entryFileNames: 'uni-mp-weibo.' + FORMAT + '.js',
      },
      external(source) {
        if (
          [
        ...
        if (source.startsWith('@dcloudio/uni-mp-weibo/style')) {
          return true
        }
      },
