...
function initWrapper(protocols) {
    ...
    return function wrapper(methodName, method) {
        ...
        if (!hasProtocol && typeof ks[methodName] !== 'function') {
            return method;
        }
...
const onError = {
    args(fromArgs) {
        ...
        if (!app.$vm) {
            if (!ks.$onErrorHandlers) {
                ks.$onErrorHandlers = [];
            }
            ks.$onErrorHandlers.push(fromArgs);
        }
        else {
...
const offError = {
    args(fromArgs) {
        ...
        if (!app.$vm) {
            ...
            const index = ks.$onErrorHandlers.findIndex((fn) => fn === fromArgs);
            if (index !== -1) {
                ks.$onErrorHandlers.splice(index, 1);
            }
        }
...
const onSocketOpen = {
    args() {
        if (ks.__uni_console__) {
            if (ks.__uni_console_warned__) {
                return;
            }
            ks.__uni_console_warned__ = true;
