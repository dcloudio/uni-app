...
function normalizeEventType(type) {
    return `on${capitalize$1(camelize$1(type))}`;
}
class UniEventTarget {
    ...
    addEventListener(type, listener, options) {
        ...
        if (isOnce) {
            ...
            return this.addEventListener(type, wrapper, extend$1(options, { once: false }));
        }
...
class UniNode extends UniEventTarget {
    ...
    cloneNode(deep) {
        const cloned = extend$1(Object.create(Object.getPrototypeOf(this)), this);
...
function baseCreateRenderer(options, createHydrationFns) {
  ...
  const processText = (n1, n2, container, anchor) => {
    if (n1 == null) {
      hostInsert(
        (n2.el = hostCreateText(n2.children, container)), // fixed by xxxxxx
        container,
        anchor
      );
    } else {
  ...
  const processCommentNode = (n1, n2, container, anchor) => {
    if (n1 == null) {
      hostInsert(
        (n2.el = hostCreateComment(n2.children || '', container)), // fixed by xxxxxx
        container,
        anchor
      );
    } else {
...
const publicPropertiesMap = extend(Object.create(null), {
  ...
  $data: (i) => i.data,
  $props: (i) =>
    process.env.NODE_ENV !== 'production' ? shallowReadonly(i.props) : i.props,
  $attrs: (i) =>
    process.env.NODE_ENV !== 'production' ? shallowReadonly(i.attrs) : i.attrs,
  $slots: (i) =>
    process.env.NODE_ENV !== 'production' ? shallowReadonly(i.slots) : i.slots,
  $refs: (i) =>
