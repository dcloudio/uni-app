    ...
    function applyOptions(instance) {
      ...
      if (computedOptions) {
        var _loop = function (_key4) {
          ...
          var set = !shared.isFunction(opt) && shared.isFunction(opt.set) ? opt.set.bind(publicThis) : shared.NOOP;
          var c = computed({
            get,
            set
          });
    ...
    function updateProps(instance, rawProps, rawPrevProps, optimized) {
      ...
      /* FULL_PROPS */
        ...
        for (var _key6 in rawCurrentProps) {
          ...
          !shared.hasOwn(rawProps, _key6) && ( // it's possible the original props was passed in as kebab-case
          // and converted to camelCase (#955)
          (kebabKey = shared.hyphenate(_key6)) === _key6 || !shared.hasOwn(rawProps, kebabKey))) {
            if (options) {
        ...
        if (attrs !== rawCurrentProps) {
          for (var _key7 in attrs) {
            if (!rawProps || !shared.hasOwn(rawProps, _key7) && !false) {
              delete attrs[_key7];
              hasAttrsChanged = true;
            }
          }
    ...
    function setFullProps(instance, rawProps, props, attrs) {
      ...
      if (needCastKeys) {
        ...
        for (var i = 0; i < needCastKeys.length; i++) {
          ...
          props[_key8] = resolvePropValue(options, rawCurrentProps, _key8, castValues[_key8], instance, !shared.hasOwn(castValues, _key8));
        }
      }
        ...
        if (oldProps !== newProps) {
          ...
          if (oldProps !== shared.EMPTY_OBJ) {
            for (var _key10 in oldProps) {
              if (!shared.isReservedProp(_key10) && !(_key10 in newProps)) {
      ...
      if (isPrevObj) {
        ...
        for (var _key11 in next) {
          ...
          if (value !== prev[_key11]) {
            batchedStyles[shared.camelize(_key11)] = value;
          }
        }
      } else {
        for (var _key12 in next) {
          batchedStyles[shared.camelize(_key12)] = next[_key12];
        }
      }
