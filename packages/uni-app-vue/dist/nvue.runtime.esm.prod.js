...
function createAppAPI(render, hydrate) {
  return function createApp(rootComponent) {
    ...
    var app = context.app = {
      ...
      set config(v) {},
      use(plugin) {
        for (var _len4 = arguments.length, options = new Array(_len4 > 1 ? _len4 - 1 : 0), _key6 = 1; _key6 < _len4; _key6++) {
  ...
  (optimized || patchFlag > 0) && !(patchFlag & 16)) {
    ...
    for (var _key7 in rawCurrentProps) {
      ...
      (kebabKey = hyphenate(_key7)) === _key7 || !hasOwn(rawProps, kebabKey))) {
        if (options) {
          if (rawPrevProps && (
          // for camelCase
          rawPrevProps[_key7] !== void 0 ||
          // for kebab-case
          rawPrevProps[kebabKey] !== void 0)) {
            props[_key7] = resolvePropValue(options, rawCurrentProps, _key7, void 0, instance, true);
          }
        } else {
          delete props[_key7];
        }
    ...
    if (attrs !== rawCurrentProps) {
      for (var _key8 in attrs) {
        if (!rawProps || !hasOwn(rawProps, _key8) && true) {
          delete attrs[_key8];
          hasAttrsChanged = true;
        }
      }
...
function setFullProps(instance, rawProps, props, attrs) {
  ...
  if (needCastKeys) {
    ...
    for (var i = 0; i < needCastKeys.length; i++) {
      ...
      props[_key9] = resolvePropValue(options, rawCurrentProps, _key9, castValues[_key9], instance, !hasOwn(castValues, _key9));
    }
  }
...
function baseCreateRenderer(options, createHydrationFns) {
  ...
  var patchProps = (el, vnode, oldProps, newProps, parentComponent, parentSuspense, namespace) => {
    if (oldProps !== newProps) {
      ...
      for (var _key0 in newProps) {
        if (isReservedProp(_key0)) continue;
        var next = newProps[_key0];
        var prev = oldProps[_key0];
        if (next !== prev && _key0 !== "value" || hostForcePatchProp && hostForcePatchProp(el, _key0)) {
          hostPatchProp(el, _key0, prev, next, namespace, vnode.children, parentComponent, parentSuspense, unmountChildren,
          // fixed by xxxxxx
          vnode.hostInstance);
        }
      }
...
function patchStyle(el, prev, next) {
  ...
  if (isPrevObj) {
    ...
    for (var _key1 in next) {
      var value = next[_key1];
      if (value !== prev[_key1]) {
        batchedStyles[camelize(_key1)] = value;
      }
    }
  } else {
    for (var _key10 in next) {
      batchedStyles[camelize(_key10)] = next[_key10];
    }
  }
