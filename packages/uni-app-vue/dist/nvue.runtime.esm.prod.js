  ...
  var _loop = function (i) {
    ...
    if (isArray(slot)) {
      ...
      slots[slot.name] = slot.key ? function () {
        var res = slot.fn(...arguments); // attach branch key so each conditional branch is considered a
        // different fragment
  ...
  var patchProps = (el, vnode, oldProps, newProps, parentComponent, parentSuspense, isSVG) => {
    if (oldProps !== newProps) {
      if (oldProps !== EMPTY_OBJ) {
        for (var key in oldProps) {
          if (!isReservedProp(key) && !(key in newProps)) {
            hostPatchProp(el, key, oldProps[key], null, isSVG, vnode.children, parentComponent, parentSuspense, unmountChildren, // fixed by xxxxxx
            vnode.hostInstance);
          }
        }
      }
      ...
      for (var _key10 in newProps) {
        ...
        var prev = oldProps[_key10]; // defer patching value

        if (next !== prev && _key10 !== 'value' || hostForcePatchProp && hostForcePatchProp(el, _key10) // fixed by xxxxxx
        ) {
          hostPatchProp(el, _key10, prev, next, isSVG, vnode.children, parentComponent, parentSuspense, unmountChildren, // fixed by xxxxxx
          vnode.hostInstance);
        }
      }
