...
import { hasChanged, extend, isSymbol, isObject, toRawType, hasOwn, def, isFunction, isArray as isArray$1, isMap, makeMap, hyphenate, capitalize, EMPTY_OBJ, NOOP, toNumber, isPromise, isString, isOn, isModelListener, toHandlerKey, EMPTY_ARR, camelize, isRegExp, getGlobalThis, looseToNumber, invokeArrayFns, isSet, isPlainObject, remove, isReservedProp, NO, isGloballyAllowed, parseStringStyle, normalizeStyle as normalizeStyle$1 } from '@vue/shared';
export { camelize, capitalize, hyphenate, toDisplayString, toHandlerKey } from '@vue/shared';
import { isIntegerKey, normalizeClass, normalizeStyle, isRootHook, isRootImmediateHook, ON_LOAD, forcePatchProp, ON_SHOW, ON_HIDE, ON_LAUNCH, ON_ERROR, ON_THEME_CHANGE, ON_PAGE_NOT_FOUND, ON_UNHANDLE_REJECTION, ON_LAST_PAGE_BACK_PRESS, ON_EXIT, ON_READY, ON_UNLOAD, ON_RESIZE, ON_BACK_PRESS, ON_PAGE_SCROLL, ON_TAB_ITEM_TAP, ON_REACH_BOTTOM, ON_PULL_DOWN_REFRESH, ON_SHARE_TIMELINE, ON_SHARE_APP_MESSAGE } from '@dcloudio/uni-shared';
...
function noTracking(self, method) {
  ...
  pauseTracking();
  startBatch();
  var res = toRaw(self)[method](...args);
  endBatch();
  resetTracking();
  return res;
}
...
function emit(instance, event) {
  ...
  for (var _len1 = arguments.length, rawArgs = new Array(_len1 > 2 ? _len1 - 2 : 0), _key10 = 2; _key10 < _len1; _key10++) {
    rawArgs[_key10 - 2] = arguments[_key10];
  }
  var args = rawArgs;
...
function createAppAPI(render, hydrate) {
  return function createApp(rootComponent) {
    ...
    var app = context.app = {
      ...
      set config(v) {},
      use(plugin) {
        for (var _len12 = arguments.length, options = new Array(_len12 > 1 ? _len12 - 1 : 0), _key15 = 1; _key15 < _len12; _key15++) {
...
function updateProps(instance, rawProps, rawPrevProps, optimized) {
  ...
  (optimized || patchFlag > 0) && !(patchFlag & 16)) {
    ...
    var kebabKey;
    for (var _key16 in rawCurrentProps) {
      if (!rawProps ||
      // for camelCase
      !hasOwn(rawProps, _key16) && (
      // it's possible the original props was passed in as kebab-case
      // and converted to camelCase (#955)
      (kebabKey = hyphenate(_key16)) === _key16 || !hasOwn(rawProps, kebabKey))) {
        if (options) {
          if (rawPrevProps && (
          // for camelCase
          rawPrevProps[_key16] !== void 0 ||
          // for kebab-case
          rawPrevProps[kebabKey] !== void 0)) {
            props[_key16] = resolvePropValue(options, rawCurrentProps, _key16, void 0, instance, true);
          }
        } else {
          delete props[_key16];
        }
    ...
    if (attrs !== rawCurrentProps) {
      for (var _key17 in attrs) {
        if (!rawProps || !hasOwn(rawProps, _key17) && true) {
          delete attrs[_key17];
          hasAttrsChanged = true;
        }
      }
...
function setFullProps(instance, rawProps, props, attrs) {
  ...
  if (needCastKeys) {
    ...
    for (var i = 0; i < needCastKeys.length; i++) {
      var _key18 = needCastKeys[i];
      props[_key18] = resolvePropValue(options, rawCurrentProps, _key18, castValues[_key18], instance, !hasOwn(castValues, _key18));
    }
  }
  ...
  if (isPrevObj) {
    (function () {
      ...
      for (var _key20 in next) {
        ...
        var prevValue = prev[_key20];
        if (!isSame(prevValue, value)) {
          var _key = _key20.startsWith("--") ? _key20 : camelize(_key20);
...
var withModifiers = (fn, modifiers) => {
  return function (event) {
    ...
    for (var _len13 = arguments.length, args = new Array(_len13 > 1 ? _len13 - 1 : 0), _key21 = 1; _key21 < _len13; _key21++) {
