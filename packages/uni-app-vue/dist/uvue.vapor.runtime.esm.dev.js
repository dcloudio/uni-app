...
class CustomRefImpl {
  constructor(factory) {
    ...
    this["__v_isRef"] = true;
    this._value = void 0;
    var _factory = factory(() => trackRef(this), () => triggerRef(this)),
      get = _factory.get,
      set = _factory.set;
    this._get = get;
    this._set = set;
...
class WatcherEffect extends ReactiveEffect {
  ...
  run() {
    ...
    var _this$options = this.options,
      immediate = _this$options.immediate,
      deep = _this$options.deep,
      call = _this$options.call;
...
function traverse(value) {
  ...
  if (isRef(value)) {
    ...
    for (var _key10 of Object.getOwnPropertySymbols(value)) {
      if (Object.prototype.propertyIsEnumerable.call(value, _key10)) {
        traverse(value[_key10], depth, seen);
      }
    }
  }
  return value;
...
var transformFlex = decl => {
  ...
  if (splitResult.length === 1) {
    ...
    var _splitResult2 = _slicedToArray(splitResult, 3),
      _v = _splitResult2[0],
      _v2 = _splitResult2[1],
      v3 = _splitResult2[2];
...
function warn$1(msg) {
  ...
  for (var _len1 = arguments.length, args = new Array(_len1 > 1 ? _len1 - 1 : 0), _key11 = 1; _key11 < _len1; _key11++) {
...
function resolveCssVars(instance, vnode, expectedMap) {
  ...
  if (instance.getCssVars && (vnode === root || root && root.type === Fragment && root.children.includes(vnode))) {
    ...
    for (var key in cssVars) {
      var value = normalizeCssVarValue(cssVars[key]);
      expectedMap.set("--".concat(getEscapedCssVarName(key, false)), value);
    }
  }
  ...
  return defineComponent$1({
    ...
    __asyncHydrate(el, instance, hydrate) {
      ...
      var doHydrate = hydrateStrategy ? () => {
        var performHydrate = () => {
          if (patched) {
            warn$1("Skipping lazy hydration for component '".concat(getComponentName(resolvedComp), "': it was updated before lazy hydration performed."));
            return;
          }
          hydrate();
        };
  ...
  setup(props, _ref12) {
    ...
    onBeforeUnmount(() => {
      cache.forEach(cached => {
        var subTree = keepAliveInstance.subTree,
          suspense = keepAliveInstance.suspense;
...
function applyOptions(instance) {
  ...
  if (methods) {
    for (var _key15 in methods) {
      ...
      if (isFunction(methodHandler)) {
        ...
        {
          checkDuplicateProperties("Methods" /* METHODS */, _key15);
        }
      } else {
        warn$1("Method \"".concat(_key15, "\" has type \"").concat(typeof methodHandler, "\" in the component definition. Did you reference the function correctly?"));
      }
    }
  }
  if (dataOptions) {
    (function () {
      ...
      if (!isObject(data)) {
        ...
        {
          var _loop4 = function (_key16) {
            checkDuplicateProperties("Data" /* DATA */, _key16);
  ...
  if (computedOptions) {
    var _loop5 = function (_key17) {
      ...
      if (get === NOOP) {
        warn$1("Computed property \"".concat(_key17, "\" has no getter."));
      }
      var set = !isFunction(opt) && isFunction(opt.set) ? opt.set.bind(publicThis) : () => {
        warn$1("Write operation failed: computed property \"".concat(_key17, "\" is readonly."));
      };
      var c = computed({
        get,
        set
      ...
      {
        checkDuplicateProperties("Computed" /* COMPUTED */, _key17);
      }
    };
...
function createAppAPI(mount, unmount, getPublicInstance, render) {
  return function createApp(rootComponent) {
    ...
    var app = context.app = {
      ...
      use(plugin) {
        for (var _len13 = arguments.length, options = new Array(_len13 > 1 ? _len13 - 1 : 0), _key19 = 1; _key19 < _len13; _key19++) {
      ...
      provide(key, value) {
        if (key in context.provides) {
          if (hasOwn(context.provides, key)) {
            ...
            warn$1("App already provides property with key \"".concat(String(key), "\" inherited from its parent element. It will be overwritten with the new value."));
          }
        }
...
function updateProps(instance, rawProps, rawPrevProps, optimized) {
  ...
  !isInHmrContext(instance) && (optimized || patchFlag > 0) && !(patchFlag & 16)) {
    ...
    for (var _key20 in rawCurrentProps) {
      if (!rawProps ||
      // for camelCase
      !hasOwn(rawProps, _key20) && (
      // it's possible the original props was passed in as kebab-case
      // and converted to camelCase (#955)
      (kebabKey = hyphenate(_key20)) === _key20 || !hasOwn(rawProps, kebabKey))) {
        if (options) {
          if (rawPrevProps && (
          // for camelCase
          ...
          rawPrevProps[kebabKey] !== void 0)) {
            props[_key20] = resolvePropValue(options, _key20, void 0, instance, baseResolveDefault, true);
          }
        } else {
          delete props[_key20];
        }
    ...
    if (attrs !== rawCurrentProps) {
      for (var _key21 in attrs) {
        if (!rawProps || !hasOwn(rawProps, _key21) && true) {
          delete attrs[_key21];
          hasAttrsChanged = true;
        }
      }
...
function setFullProps(instance, rawProps, props, attrs) {
  ...
  if (needCastKeys) {
    ...
    for (var i = 0; i < needCastKeys.length; i++) {
      ...
      props[_key22] = resolvePropValue(options, _key22, castValues[_key22], instance, baseResolveDefault, !hasOwn(castValues, _key22));
    }
  }
...
function normalizePropsOptions$1(comp, appContext) {
  ...
  if (!isFunction(comp)) {
    var extendProps = raw2 => {
      hasExtends = true;
      var _normalizePropsOption = normalizePropsOptions$1(raw2, appContext, true),
...
function validateProp(key, value, propOptions, resolvedProps, isAbsent) {
  var type = propOptions.type,
    required = propOptions.required,
    validator = propOptions.validator,
    skipCheck = propOptions.skipCheck;
...
function isBoolean() {
  for (var _len14 = arguments.length, args = new Array(_len14), _key23 = 0; _key23 < _len14; _key23++) {
...
function baseCreateRenderer(options, createHydrationFns) {
  ...
  var patchProps = (el, oldProps, newProps, parentComponent, namespace, hostInstance) => {
    if (oldProps !== newProps) {
      ...
      for (var _key24 in newProps) {
        if (isReservedProp(_key24)) continue;
        var _next4 = newProps[_key24];
        var prev = oldProps[_key24];
        if (_next4 !== prev && _key24 !== "value" || hostForcePatchProp && hostForcePatchProp(el, _key24)) {
          hostPatchProp(el, _key24, prev, _next4, namespace, parentComponent,
          // fixed by xxxxxx
          hostInstance);
        }
      }
  ...
  class SetupRenderEffect extends ReactiveEffect {
    ...
    fn() {
      var instance = this.instance,
        initialVNode = this.initialVNode,
        container = this.container,
        anchor = this.anchor,
        parentSuspense = this.parentSuspense,
  ...
  var unmountComponent = (instance, parentSuspense, doRemove) => {
    ...
    var bum = instance.bum,
      ...
      a = instance.a,
      parent = instance.parent,
      slotCacheKeys = instance.slots.__;
    invalidateMount(m);
    invalidateMount(a);
    if (bum) {
      invokeArrayFns(bum);
...
function emit$1(instance, event) {
  for (var _len15 = arguments.length, rawArgs = new Array(_len15 > 2 ? _len15 - 2 : 0), _key25 = 2; _key25 < _len15; _key25++) {
    rawArgs[_key25 - 2] = arguments[_key25];
  }
...
function baseEmit(instance, props, getter, event) {
  ...
  for (var _len16 = arguments.length, rawArgs = new Array(_len16 > 4 ? _len16 - 4 : 0), _key26 = 4; _key26 < _len16; _key26++) {
    rawArgs[_key26 - 4] = arguments[_key26];
  }
  {
    var emitsOptions = instance.emitsOptions,
      propsOptions = instance.propsOptions;
    if (emitsOptions) {
...
function updateHOCHostEl(_ref22, el) {
  var vnode = _ref22.vnode,
    parent = _ref22.parent;
...
function patchSuspense(n1, n2, container, anchor, parentComponent, namespace, slotScopeIds, optimized, _ref23) {
  var patch = _ref23.p,
    unmount = _ref23.um,
    createElement = _ref23.o.createElement;
...
var normalizeKey = _ref24 => {
  var key = _ref24.key;
  return key != null ? key : null;
};
var normalizeRef = _ref25 => {
  var ref = _ref25.ref,
    ref_key = _ref25.ref_key,
    ref_for = _ref25.ref_for;
...
function validateComponentName(name, _ref26) {
  var isNativeTag = _ref26.isNativeTag;
...
function parseClassName(_ref27, parentStyles, el) {
  var styles = _ref27.styles,
    weights = _ref27.weights;
  each(parentStyles).forEach(parentSelector => {
...
function parseStyleSheet(_ref28) {
  var type = _ref28.type,
    appContext = _ref28.appContext,
    root = _ref28.root;
  var component = type;
...
function prepend(parent) {
  for (var _len19 = arguments.length, blocks = new Array(_len19 > 1 ? _len19 - 1 : 0), _key29 = 1; _key29 < _len19; _key29++) {
    blocks[_key29 - 1] = arguments[_key29];
  }
  var i = blocks.length;
  while (i--) {
...
var delegateEvents = function (doc) {
  for (var _len20 = arguments.length, names = new Array(_len20 > 1 ? _len20 - 1 : 0), _key30 = 1; _key30 < _len20; _key30++) {
...
function setDynamicProps(el, args) {
  ...
  for (var _key33 of el[cacheKey] = Object.keys(props)) {
    setDynamicProp(el, _key33, props[_key33]);
  }
}
...
function emit(instance, event) {
  for (var _len21 = arguments.length, rawArgs = new Array(_len21 > 2 ? _len21 - 2 : 0), _key34 = 2; _key34 < _len21; _key34++) {
    rawArgs[_key34 - 2] = arguments[_key34];
  }
...
function resolveDynamicProps(props) {
  ...
  if (props.$) {
    for (var source of props.$) {
      ...
      for (var _key36 in resolved) {
        var value = isDynamic ? resolved[_key36] : resolved[_key36]();
        if (_key36 === "class" || _key36 === "style") {
          var existing = mergedRawProps[_key36];
          if (isArray$1(existing)) {
            existing.push(value);
          } else {
            mergedRawProps[_key36] = [existing, value];
          }
        } else {
          mergedRawProps[_key36] = value;
        }
      }
    }
  }
...
function hmrReload(instance, newComp) {
  ...
  var prev = setCurrentInstance(instance.parent);
...
function createComponent(component, rawProps, rawSlots, isSingleRoot) {
  ...
  var _ref36 = appContext.config.uniX || {},
    beforeSetupPage = _ref36.beforeSetupPage,
    initNativePage = _ref36.initNativePage,
    initFontFace = _ref36.initFontFace;
...
function devRender(instance) {
  instance.block = (instance.type.render ? callWithErrorHandling(instance.type.render, instance, 1, [instance.setupState, instance.props, instance.emit, instance.attrs, instance.slots]) : callWithErrorHandling(isFunction(instance.type) ? instance.type : instance.type.setup, instance, 0, [instance.props, {
    slots: instance.slots,
    attrs: instance.attrs,
    emit: instance.emit,
...
class VaporComponentInstance {
  constructor(comp, rawProps, rawSlots, appContext) {
    ...
    if (rawProps || comp.props) {
      var _getPropsProxyHandler = getPropsProxyHandlers(comp),
        _getPropsProxyHandler2 = _slicedToArray(_getPropsProxyHandler, 2),
        propsHandlers = _getPropsProxyHandler2[0],
        attrsHandlers = _getPropsProxyHandler2[1];
...
function getRootElement(_ref37) {
  var block = _ref37.block;
  if (block instanceof UniElement) {
    return block;
  }
...
var createFor = function (doc, src, renderItem, getKey) {
  ...
  var renderList = () => {
    ...
    if (!isMounted) {
      ...
      if (!oldLength) {
        ...
        for (var _i0 = startOffset; _i0 < oldLength - endOffset; _i0++) {
          previousKeyIndexPairs[previousKeyIndexInsertIndex++] = [oldBlocks[_i0].key, _i0];
        }
        var preparationBlockCount = Math.min(newLength - endOffset, sharedBlockCount);
        for (var _i1 = startOffset; _i1 < preparationBlockCount; _i1++) {
          var blockItem = getItem(source, _i1);
          var blockKey = getKey(...blockItem);
          queuedBlocks[queuedBlocksInsertIndex++] = [_i1, blockItem, blockKey];
        }
        if (!queuedBlocksInsertIndex && !previousKeyIndexInsertIndex) {
          for (var _i10 = preparationBlockCount; _i10 < newLength - endOffset; _i10++) {
            var _blockItem = getItem(source, _i10);
            var _blockKey = getKey(..._blockItem);
            mount(source, _i10, anchorFallback, _blockItem, _blockKey);
          }
        } else {
          ...
          for (var _i11 = queuedBlocks.length - 1; _i11 >= 0; _i11--) {
            var _queuedBlocks$_i = _slicedToArray(queuedBlocks[_i11], 3),
              blockIndex = _queuedBlocks$_i[0],
              _blockItem2 = _queuedBlocks$_i[1],
              _blockKey2 = _queuedBlocks$_i[2];
          ...
          for (var _i12 = preparationBlockCount; _i12 < newLength - endOffset; _i12++) {
            var _blockItem3 = getItem(source, _i12);
            var _blockKey3 = getKey(..._blockItem3);
            relocateOrMountBlock(_i12, _blockItem3, _blockKey3, -1);
          }
          ...
          for (var _ref40 of blocksToMount) {
            var _ref39 = _slicedToArray(_ref40, 4);
            var _blockIndex = _ref39[0];
            var _blockItem4 = _ref39[1];
            var _blockKey4 = _ref39[2];
            var anchorOffset = _ref39[3];
  ...
  var mount = function (source, idx) {
    ...
    var _ref41 = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : getItem(source, idx),
  ...
  var update = (_ref43, newItem, newKey, newIndex) => {
    var itemRef = _ref43.itemRef,
      keyRef = _ref43.keyRef,
      indexRef = _ref43.indexRef;
...
function normalizeSource(source) {
  ...
  if (isArray$1(source)) {
    ...
    if (source[Symbol.iterator]) {
      ...
      for (var _i13 = 0, l = keys.length; _i13 < l; _i13++) {
        values[_i13] = source[keys[_i13]];
      }
    }
  }
  return {
...
function getItem(_ref44, idx) {
  var keys = _ref44.keys,
    values = _ref44.values,
    needsWrap = _ref44.needsWrap,
    isReadonlySource = _ref44.isReadonlySource;
...
function withVaporDirectives(node, dirs) {
  for (var _ref48 of dirs) {
    var _ref47 = _slicedToArray(_ref48, 4);
    var dir = _ref47[0];
    var value = _ref47[1];
    var argument = _ref47[2];
    var modifiers = _ref47[3];
    if (dir) {
...
function patchAttr(el, key, value) {
  var instance = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : null;
  if (instance) {
    var _transformAttr = transformAttr(el, key, value, instance);
    var _transformAttr2 = _slicedToArray(_transformAttr, 2);
    key = _transformAttr2[0];
    value = _transformAttr2[1];
  }
...
function patchEvent(el, rawName, prevValue, nextValue) {
  ...
  if (nextValue && existingInvoker) {
    ...
    var _parseName = parseName(rawName),
      _parseName2 = _slicedToArray(_parseName, 1),
      name = _parseName2[0];
    if (nextValue) {
...
var vModelText = {
  created(el, _ref49, vnode) {
    var _ref49$modifiers = _ref49.modifiers,
      lazy = _ref49$modifiers.lazy,
      trim = _ref49$modifiers.trim,
      number = _ref49$modifiers.number;
  ...
  beforeUpdate(el, _ref50, vnode) {
    var value = _ref50.value,
      oldValue = _ref50.oldValue,
      _ref50$modifiers = _ref50.modifiers,
      lazy = _ref50$modifiers.lazy,
      trim = _ref50$modifiers.trim,
      number = _ref50$modifiers.number;
    el[assignKey] = getModelAssigner(vnode);
...
function unmountPage(pageInstance) {
  var __page_container__ = pageInstance.__page_container__;
