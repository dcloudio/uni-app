...
function nvueFactory(exports, document) {
  (function (shared, reactivity) {
        ...
        if (emitsOptions) {
          if (!(event in emitsOptions) && !false) {
            if (!propsOptions || !(shared.toHandlerKey(event) in propsOptions)) {
              warn("Component emitted event \"".concat(event, "\" but it is neither declared in ") + "the emits option nor as an \"".concat(shared.toHandlerKey(event), "\" prop."));
            }
          } else {
    ...
    function createSuspenseBoundary(vnode, parent, parentComponent, container, hiddenContainer, anchor, isSVG, slotScopeIds, optimized, rendererInternals) {
      ...
      var timeout = shared.toNumber(vnode.props && vnode.props.timeout);
      var suspense = {
        vnode,
        parent,
        parentComponent,
    ...
    function doWatch(source, cb) {
      var {
        ...
        onTrack,
        onTrigger
      } = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : shared.EMPTY_OBJ;
    ...
    function instanceWatch(source, value, options) {
      ...
      var getter = shared.isString(source) ? source.includes('.') ? createPathGetter(publicThis, source) : () => publicThis[source] : source.bind(publicThis, publicThis);
      var cb;
    ...
    function injectHook(type, hook) {
      ...
      if (target) {
        ...
        // fixed by xxxxxx
        var apiName = shared.toHandlerKey((ErrorTypeStrings[type] || type.replace(/^on/, '')).replace(/ hook$/, ''));
    ...
    function applyOptions(instance) {
      ...
      if (computedOptions) {
        var _loop2 = function (_key7) {
          ...
          var set = !shared.isFunction(opt) && shared.isFunction(opt.set) ? opt.set.bind(publicThis) : () => {
    ...
    function resolveInjections(injectOptions, ctx) {
      var checkDuplicateProperties = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : shared.NOOP;
    ...
    function createWatcher(raw, ctx, publicThis, key) {
      ...
      if (shared.isString(raw)) {
        ...
        if (shared.isArray(raw)) {
          ...
          var _handler = shared.isFunction(raw.handler) ? raw.handler.bind(publicThis) : ctx[raw.handler];

          if (shared.isFunction(_handler)) {
    ...
    function mergeWatchOptions(to, from) {
      if (!to) return from;
      if (!from) return to;
      var merged = shared.extend(Object.create(null), to);
    ...
    function updateProps(instance, rawProps, rawPrevProps, optimized) {
      ...
      /* FULL_PROPS */
        ...
        for (var _key9 in rawCurrentProps) {
          ...
          !shared.hasOwn(rawProps, _key9) && ( // it's possible the original props was passed in as kebab-case
          // and converted to camelCase (#955)
          (kebabKey = shared.hyphenate(_key9)) === _key9 || !shared.hasOwn(rawProps, kebabKey))) {
            if (options) {
        ...
        if (attrs !== rawCurrentProps) {
          for (var _key10 in attrs) {
            if (!rawProps || !shared.hasOwn(rawProps, _key10) && !false) {
              delete attrs[_key10];
              hasAttrsChanged = true;
            }
          }
    ...
    function setFullProps(instance, rawProps, props, attrs) {
      ...
      if (needCastKeys) {
        ...
        for (var i = 0; i < needCastKeys.length; i++) {
          ...
          props[_key11] = resolvePropValue(options, rawCurrentProps, _key11, castValues[_key11], instance, !shared.hasOwn(castValues, _key11));
        }
      }
    ...
    function resolvePropValue(options, props, key, value, instance, isAbsent) {
      ...
      if (opt != null) {
        var hasDefault = shared.hasOwn(opt, 'default'); // default values
    ...
    function normalizePropsOptions(comp, appContext) {
      ...
      if (shared.isArray(raw)) {
        ...
        for (var key in raw) {
          var _normalizedKey = shared.camelize(key);

          if (validatePropName(_normalizedKey)) {
            var opt = raw[key];
            var prop = normalized[_normalizedKey] = shared.isArray(opt) || shared.isFunction(opt) ? {
              type: opt
            } : opt;
    ...
    function validateProp(name, value, prop, isAbsent) {
      ...
      if (type != null && type !== true) {
        var isValid = false;
        var types = shared.isArray(type) ? type : [type];
    ...
    */
    ...
    function withDirectives(vnode, directives) {
      ...
      for (var i = 0; i < directives.length; i++) {
        var [dir, value, arg, modifiers = shared.EMPTY_OBJ] = directives[i];
    ...
    function baseCreateRenderer(options, createHydrationFns) {
        ...
        if (oldProps !== newProps) {
          ...
          if (oldProps !== shared.EMPTY_OBJ) {
            for (var _key14 in oldProps) {
              if (!shared.isReservedProp(_key14) && !(_key14 in newProps)) {
      ...
      var setupRenderEffect = (instance, initialVNode, container, anchor, parentSuspense, isSVG, optimized) => {
        ...
        toggleRecurse(instance, true);
        {
          ...
          effect.onTrigger = instance.rtg ? e => shared.invokeArrayFns(instance.rtg, e) : void 0; // @ts-ignore (for scheduler)
    ...
    function resolveAsset(type, name) {
      ...
      if (instance) {
        ...
        warn("resolve".concat(shared.capitalize(type.slice(0, -1)), " ") + "can only be used in render() or setup().");
      }
    }
    ...
    function _createVNode(type) {
      ...
      var shapeFlag = shared.isString(type) ? 1
      /* ELEMENT */
      : isSuspense(type) ? 128
      /* SUSPENSE */
      : isTeleport(type) ? 64
    ...
    var RuntimeCompiledPublicInstanceProxyHandlers = /*#__PURE__*/shared.extend({}, PublicInstanceProxyHandlers, {
      ...
      has(_, key) {
        var has = key[0] !== '_' && !shared.isGloballyWhitelisted(key);
    ...
    function validateComponentName(name, config) {
      var appIsNativeTag = config.isNativeTag || shared.NO;
    ...
    function mergeDefaults(raw, defaults) {
      var props = shared.isArray(raw) ? raw.reduce((normalized, p) => (normalized[p] = {}, normalized), {}) : raw;
    ...
    function useCssStyles(styles) {
      ...
      if (shared.isArray(styles)) {
        styles.forEach(style => {
          Object.keys(style).forEach(name => {
            if (shared.hasOwn(normalized, name)) {
              shared.extend(normalized[name], style[name]);
            } else {
    ...
    function parseStylesheet(_ref21) {
      ...
      if (!component.__styles) {
        if (component.mpType === 'page' && appContext) {
          ...
          if (shared.isArray(component.styles)) {
    ...
    function transformAttr(el, key, value, instance) {
      ...
      if (opts) {
        var camelized = shared.camelize(key);
        ...
        if (opts['style'].indexOf(key) > -1) {
          if (shared.isString(value)) {
            return [camelized, shared.parseStringStyle(value)];
          }

          return [camelized, shared.normalizeStyle(value)];
        }
      }
    ...
    function createInvoker(initialValue, instance) {
      ...
      if (shared.isArray(invoker.value)) {
    ...
    function patchStyle(el, prev, next) {
      ...
      if (shared.isString(next)) {
        next = shared.parseStringStyle(next);
      }

      var batchedStyles = {};
      var isPrevObj = prev && !shared.isString(prev);

      if (isPrevObj) {
        for (var key in prev) {
          if (next[key] == null) {
            batchedStyles[shared.camelize(key)] = '';
          }
        }
        ...
        for (var _key17 in next) {
          ...
          if (value !== prev[_key17]) {
            batchedStyles[shared.camelize(_key17)] = value;
          }
        }
      } else {
        for (var _key18 in next) {
          batchedStyles[shared.camelize(_key18)] = next[_key18];
        }
      }
    ...
    var rendererOptions = shared.extend({
      patchProp
    ...
    var Vue = {
      ...
      camelize: shared.camelize,
      capitalize: shared.capitalize,
      hyphenate: shared.hyphenate,
      normalizeClass: shared.normalizeClass,
      normalizeProps: shared.normalizeProps,
      normalizeStyle: shared.normalizeStyle,
      toDisplayString: shared.toDisplayString,
      toHandlerKey: shared.toHandlerKey,
    ...
    exports.Vue = Vue;
  })(exports.VueShared, exports.VueReactivity);
}
