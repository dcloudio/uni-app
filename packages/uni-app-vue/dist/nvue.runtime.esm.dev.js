  ...
  function applyOptions(instance) {
    ...
    if (methods) {
      for (var _key8 in methods) {
        ...
        if (isFunction(methodHandler)) {
          ...
          warn$1("Method \"".concat(_key8, "\" has type \"").concat(typeof methodHandler, "\" in the component definition. ") + "Did you reference the function correctly?");
        }
      }
    }
    ...
    if (computedOptions) {
      var _loop2 = function (_key10) {
        ...
        if (get === NOOP) {
          warn$1("Computed property \"".concat(_key10, "\" has no getter."));
        }

        var set = !isFunction(opt) && isFunction(opt.set) ? opt.set.bind(publicThis) : () => {
          warn$1("Write operation failed: computed property \"".concat(_key10, "\" is readonly."));
        };
        var c = computed({
          get,
          set
