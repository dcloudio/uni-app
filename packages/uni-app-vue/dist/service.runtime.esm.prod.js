...
function vueFactory(exports) {
  (function (reactivity, shared) {
    ...
    function doWatch(source, cb) {
      ...
      var effect = new reactivity.ReactiveEffect(getter, scheduler); // initial run
    ...
    function setupStatefulComponent(instance, isSSR) {
      ...
      instance.proxy = reactivity.markRaw(new Proxy(instance.ctx, PublicInstanceProxyHandlers)); // 2. call setup()
        ...
        if (isPrevObj) {
          ...
          for (var _key13 in next) {
            ...
            if (value !== prev[_key13]) {
              batchedStyles[_key13] = value;
            }
          }
        } else {
          for (var _key14 in next) {
            batchedStyles[_key14] = next[_key14];
          }
        }
