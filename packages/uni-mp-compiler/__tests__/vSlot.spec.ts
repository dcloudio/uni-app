...
describe('compiler: transform v-slot', () => {
  ...
  return { a: _w((slotProps, s0, i0) => { return { a: _t(slotProps.item), b: i0, c: s0 }; }, { name: 'd', path: 'a', vueId: '2a9ec0b0-0' }) }
    ...
    assert(
      `<custom><template v-slot:default="slotProps"><view>{{ slotProps.item }}</view></template></custom>`,
      `<custom u-s="{{['d']}}" style="{{'--status-bar-height:' + b}}" u-i="2a9ec0b0-0" u-p="{{c||''}}"><view wx:for="{{a}}" wx:for-item="slotProps" wx:key="b" slot="{{slotProps.c}}" style="overflow: visible" class="uni__inherit_flex_box_style"><view>{{slotProps.a}}</view></view></custom>`,
      `(_ctx, _cache) => { "raw js"
  const __returned__ = { a: _w((slotProps, s0, i0) => { return { a: _t(slotProps.item), b: i0, c: s0 }; }, { name: 'd', path: 'a', vueId: '2a9ec0b0-0' }), b: \`\${_ctx.u_s_b_h}px\`, c: _p({ style: _normalizeStyle({ '--status-bar-height': \`\${_ctx.u_s_b_h}px\` }) }) }
  return __returned__
}`,
      {
        isX: true,
