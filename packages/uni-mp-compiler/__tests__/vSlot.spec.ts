...
describe('compiler: transform v-slot', () => {
  ...
  return { a: _w((slotProps, s0, i0) => { return { a: _t(slotProps.item), b: i0, c: s0 }; }, { name: 'd', path: 'a', vueId: '2a9ec0b0-0' }) }
    ...
    assert(
      ...
      `(_ctx, _cache) => { "raw js"
  const __returned__ = { a: _w((slotProps, s0, i0) => { return { a: _t(slotProps.item), b: i0, c: s0 }; }, { name: 'd', path: 'a', vueId: '2a9ec0b0-0' }), b: \`\${_ctx.u_s_b_h}px\`, c: _p({ style: { '--status-bar-height': \`\${_ctx.u_s_b_h}px\` } }) }
  return __returned__
}`,
      {
        isX: true,
