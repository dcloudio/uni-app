...
function useMovableViewState(props, trigger, rootRef, setTouchMovableViewContext) {
	...
	function _setX(val) {
		if (xMove.value) {
			if (val + _scaleOffset.x === _translateX) return _translateX;
			else {
				if (_SFA) _SFA.cancel();
	...
	function _setY(val) {
		if (yMove.value) {
			if (val + _scaleOffset.y === _translateY) return _translateY;
			else {
				if (_SFA) _SFA.cancel();
...
//#endregion
//#region src/nvue/picker-view/index.tsx
var nvuePickerViewProps = extend({}, {
	value: {
		type: Array,
		default() {
			return [];
...
var picker_view_default = /* @__PURE__ */ defineComponent({
	...
	setup(props, { slots, emit }) {
		...
		return () => {
			...
			const style = props.height ? { height: `${parseFloat(props.height)}px` } : {};
			return createVNode("view", mergeProps({
				"ref": rootRef,
			...
			reset: () => {
				switch (props.mode) {
					...
					case mode.MULTISELECTOR:
						...
						break;
					case mode.DATE:
					case mode.TIME: valueSync.value = "";
				}
			}
		};
		if (uniForm) {
	...
	const getOrClearNativeValue = (outResult, nodes) => {
		(nodes || children || []).forEach(function(node) {
			if (NATIVE_COMPONENTS.indexOf(String(node.type)) >= 0 && node.el && node.el.attr && node.el.attr.name) {
				if (outResult) outResult[node.el.attr.name] = modulePlus.getValue(node.el.nodeId);
