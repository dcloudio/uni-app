...
function getLocaleLanguage() {
    ...
    {
        localeLanguage =
            normalizeLocale(qq.getSystemInfoSync().language) || LOCALE_EN;
    }
...
function initWrapper(protocols) {
    ...
    return function wrapper(methodName, method) {
        ...
        if (!hasProtocol && typeof qq[methodName] !== 'function') {
            return method;
        }
...
const offError = {
    args(fromArgs) {
        ...
        if (!app.$vm) {
            ...
            const index = qq.$onErrorHandlers.findIndex((fn) => fn === fromArgs);
            if (index !== -1) {
                qq.$onErrorHandlers.splice(index, 1);
            }
        }
...
const onSocketOpen = {
    args() {
        if (qq.__uni_console__) {
            if (qq.__uni_console_warned__) {
                return;
            }
            qq.__uni_console_warned__ = true;
