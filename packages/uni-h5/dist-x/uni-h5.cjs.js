...
function normalizeCustomEvent(name, domEvt, el, detail) {
	const target = (0, _dcloudio_uni_shared.normalizeTarget)(el);
	return {
...
var UniElement = class extends Object {
	...
	get dataset() {
		if (!this.__uniDatasetMap) this.__uniDatasetMap = (0, _dcloudio_uni_shared.createUniDOMStringMap)(this.__uniDataset || {});
...
function invokeApi(method, api, options, params) {
	...
	if (interceptor && Object.keys(interceptor).length) {
		if ((0, _vue_shared.isArray)(interceptor.invoke)) return queue(interceptor.invoke, options).then((options) => {
...
function normalizeFormatApiParams(args) {
	const params = args[0];
	if ((0, _vue_shared.isPlainObject)(params)) return params;
...
function formatApiArgs(args, options) {
	...
	if (!options || !options.formatArgs || !(0, _vue_shared.isPlainObject)(options.formatArgs) && (0, _vue_shared.isPlainObject)(rawParams)) return;
...
function invokeFail(id, name, errMsg, errRes = {}) {
	...
	if (typeof UniError !== "undefined") {
		const errOptions = (0, _vue_shared.extend)({}, errRes);
...
function checkCallback(callback) {
	if (!(0, _vue_shared.isFunction)(callback)) throw new Error("Invalid args: type check failed for args \"callback\". Expected Function");
}
...
var eventTransport = /*#__PURE__*/ new _dcloudio_uni_shared.Emitter();
...
function stringifyQuery$3(url, data) {
	let str = url.split("#");
	const hash = str[1] || "";
	str = str[0].split("?");
	let query = str[1] || "";
...
var RequestOptions = { formatArgs: {
	...
	url(value, params) {
		if (params.method === HTTP_METHODS[0] && (0, _vue_shared.isPlainObject)(params.data) && Object.keys(params.data).length) params.url = stringifyQuery$3(value, params.data);
	},
	header(value, params) {
...
function encodeQueryString(url) {
	if (!(0, _vue_shared.isString)(url)) return url;
...
function normalizeAppRoutePath(path) {
	...
	return typeof pagePath === "string" ? pagePath : (0, _dcloudio_uni_shared.removeLeadingSlash)((route === null || route === void 0 ? void 0 : route.path) || path);
}
function normalizeRewriteRoute({ url, preserveQuery }, event) {
	if (preserveQuery) url = (0, _dcloudio_uni_shared.parseUrl)(url).path + (0, _dcloudio_uni_shared.stringifyQuery)(event.query);
	const params = {
		url,
		openType: event.openType
	};
	...
	if (errMsg) return errMsg;
	const { path, query } = (0, _dcloudio_uni_shared.parseUrl)(params.url);
	return {
		url: params.url,
		path: normalizeAppRoutePath(path),
		query: (0, _dcloudio_uni_shared.decodedQuery)(query),
		notFound: false
	};
}
...
new Promise((resolve) => {});
appRouteRuntime.onAppRoute;
appRouteRuntime.offAppRoute;
appRouteRuntime.onBeforeAppRoute;
appRouteRuntime.offBeforeAppRoute;
appRouteRuntime.rewriteRoute;
...
function resolveAppRoute(url, openType, notFound = false) {
	...
	let rewriteCount = 0;
	while (true) {
		const { path, query } = (0, _dcloudio_uni_shared.parseUrl)(routeUrl);
...
function isCurrentTabBarPage(url) {
	...
	const path = (0, _dcloudio_uni_shared.parseUrl)(url).path;
...
function navigate({ type, url, tabBarText, events, isAutomatedTesting }, __id__) {
	...
	return new Promise((resolve, reject) => {
		...
		router[type === "navigateTo" ? "push" : "replace"]({
			...
			if ((0, vue_router.isNavigationFailure)(failure)) {
...
var _tabBar;
function useTabBar() {
	if (!_tabBar) _tabBar = __uniConfig.tabBar && (0, vue.reactive)(initTabBarI18n(__uniConfig.tabBar));
...
function setupPage(comp, path) {
	...
	return setupComponent(comp, {
		...
		setup(instance) {
			...
			(0, vue_router.useRouter)();
			const query = (0, _dcloudio_uni_shared.decodedQuery)(route.query);
...
function flatVNode(nodes) {
	...
	if ((0, _vue_shared.isArray)(nodes)) nodes.forEach((vnode) => {
		if ((0, vue.isVNode)(vnode)) {
			if (vnode.type === vue.Fragment) array.push(...flatVNode(vnode.children));
...
function useState$2(props) {
	const value = (0, vue.reactive)([...props.value]);
	const state = (0, vue.reactive)({
		value,
		height: 34
	});
...
function useProgressState(props) {
	...
	const outerBarStyle = (0, vue.computed)(() => `background-color: ${props.backgroundColor}; height: ${rpx2px(props.strokeWidth)}px;`);
	...
	const realPercent = (0, vue.computed)(() => {
...
var nodeList2VNode = (scopeId, triggerItemClick, nodeList) => {
	...
	return nodeList.map((node) => {
		...
		if ((!(0, _vue_shared.hasOwn)(node, "type") || node.type === "text") && (0, _vue_shared.isString)(node.text) && node.text !== "") return (0, vue.createTextVNode)(decodeEntities(node.text || ""));
	...
	setup(props, { emit }) {
		...
		return () => {
			...
			return (0, vue.createVNode)("uni-switch", (0, vue.mergeProps)({
				...
				"style": thumbStyle
			}, null, 6)], 6), [[vue.vShow, type === "switch"]]), (0, vue.withDirectives)((0, vue.createVNode)("div", { "class": "uni-checkbox-input" }, [realCheckValue ? createSvgIconVNode(ICON_PATH_SUCCESS_NO_CIRCLE, props.foreColor || props.color || "currentColor", 22) : ""], 512), [[vue.vShow, type === "checkbox"]])])], 16, ["id", "onClick"]);
		};
	}
});
...
var list_view_default = /*#__PURE__*/ defineBuiltInComponent({
	...
	setup(props, { slots, emit }) {
		...
		*/
		...
		const placeholderHeadStyle = (0, vue.computed)(() => {
			return `${isVertical.value ? "height" : "width"}: ${state.headPlaceholderSize}px;`;
		});
		const placeholderTailStyle = (0, vue.computed)(() => {
		...
		return () => {
			...
			return (0, vue.createVNode)("uni-list-view", {
				...
				"refresherDefaultStyle",
				"refresherBackground"
			]) : null, (0, vue.createVNode)("div", { "class": "uni-list-view-content" }, [
				(0, vue.createVNode)("div", { "style": placeholderHeadStyle.value }, null, 4),
				visibleVNode,
				(0, vue.createVNode)("div", { "style": placeholderTailStyle.value }, null, 4)
			])], 6), (0, vue.createVNode)(resize_sensor_default, {
				"initial": true,
				"onResize": onResize
			}, null, 8, ["onResize"])], 512);
		};
...
var sticky_section_default = /*#__PURE__*/ defineBuiltInComponent({
	...
	setup(props, { slots, expose }) {
		...
		*/
		const headPlaceholderSize = (0, vue.ref)(0);
		const tailPlaceholderSize = (0, vue.ref)(0);
...
var createLifeCycleHook = (lifecycle, flag = 0) => (hook, target = (0, vue.getCurrentInstance)()) => {
	if (vue.isInSSRComponentSetup) return;
	(0, vue.injectHook)(lifecycle, hook, target);
};
...
function useLoadingStyle(targetElement, bold) {
	const loadingSize = (0, vue.ref)("16px");
	const loadingBorderWidth = (0, vue.ref)("1px");
	(0, vue.ref)("8px");
	return {
		width: loadingSize,
		height: loadingSize,
...
var index_x_default$1 = /* @__PURE__ */ (0, vue.defineComponent)(_objectSpread2(_objectSpread2({}, {
	...
	setup(__props) {
		...
		return (_ctx, _cache) => {
			const _component_view = view_default;
			return (0, vue.openBlock)(), (0, vue.createElementBlock)("uni-loading-element", {
				class: "default __uni_loading_container__",
				ref_key: "LoadingRef",
				ref: LoadingRef,
				style: { "display": "flex" }
			}, [(0, vue.createVNode)(_component_view, {
				class: (0, vue.normalizeClass)(["__uni-loading__ __loading-4-3__", { "__uni-loading__paused": props.paused }]),
				style: (0, vue.normalizeStyle)(loadingStyle)
	...
	if (mpType === "page") {
		...
		try {
			...
			if (($basePage === null || $basePage === void 0 ? void 0 : $basePage.openType) !== "preloadPage") {
				if (isDialogPageInstance(getPageInstanceByChild(instance))) invokeNewDialogPageHook(publicThis.$page, _dcloudio_uni_shared.ON_SHOW);
...
function useControls(props, videoState, seek, seeking) {
	...
	const controlsShow = (0, vue.computed)(() => !centerPlayBtnShow.value && props.controls && controlsVisible.value);
	const state = (0, vue.reactive)({
		seeking: false,
		touching: false,
...
var request = /*#__PURE__*/ defineTaskApi(API_REQUEST, ({ url, data, header = {}, method, dataType, responseType, enableChunked, withCredentials, timeout = __uniConfig.networkTimeout.request }, { resolve, reject }) => {
	...
	if (method !== "GET") {
		if ((0, _vue_shared.isString)(data) || data instanceof ArrayBuffer) body = data;
...
function useState() {
	...
	const showTopWindow = (0, vue.computed)(() => __UNI_FEATURE_TOPWINDOW__ && route.meta.topWindow !== false && topWindowMediaQuery.value);
	const showLeftWindow = (0, vue.computed)(() => __UNI_FEATURE_LEFTWINDOW__ && route.meta.leftWindow !== false && leftWindowMediaQuery.value);
	const showRightWindow = (0, vue.computed)(() => __UNI_FEATURE_RIGHTWINDOW__ && route.meta.rightWindow !== false && rightWindowMediaQuery.value);
	const layoutState = (0, vue.reactive)({
		topWindowMediaQuery,
		showTopWindow,
...
function useTopWindow(layoutState) {
	...
	(0, vue.watch)(() => layoutState.topWindowStyle, () => (0, vue.nextTick)(updateWindow));
...
function useLeftWindow(layoutState) {
	...
	(0, vue.watch)(() => layoutState.leftWindowStyle, () => (0, vue.nextTick)(updateWindow));
...
function useRightWindow(layoutState) {
	...
	(0, vue.watch)(() => layoutState.rightWindowStyle, () => (0, vue.nextTick)(updateWindow));
