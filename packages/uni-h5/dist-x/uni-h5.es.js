...
function resolveJsonObj(jsonObj, names) {
	if (names.length === 1) {
		if (jsonObj) {
			const _isI18nStr = (value$1) => isString(value$1) && isI18nStr(value$1, I18N_JSON_DELIMITERS);
			const _name = names[0];
			let filterJsonObj = [];
...
function normalizeMessages(module$1, keys, values) {
	return keys.reduce((res, name, index$4) => {
		res[module$1 + name] = values[index$4];
		return res;
	}, {});
}
...
function initBridge(subscribeNamespace) {
	...
	return {
		...
		subscribe(event, callback, once$1 = false) {
			emitter[once$1 ? "once" : "on"](`${subscribeNamespace}.${event}`, callback);
		},
...
const invokeServiceMethod = (name, args, callback) => {
	...
	const id$2 = callback ? invokeServiceMethodId++ : 0;
	callback && subscribe("invokeServiceApi." + id$2, callback, true);
...
function onInvokeViewMethod({ id: id$2, name, args }, pageId) {
	name = normalizeViewMethodName(pageId, name);
	const publish = (res) => {
		id$2 && UniViewJSBridge.publishHandler("invokeViewApi." + id$2, res);
	};
	const handler = viewMethods[name];
	if (handler) handler(args, publish);
	else publish({});
}
const ViewJSBridge = /* @__PURE__ */ extend(/* @__PURE__ */ initBridge("service"), { invokeServiceMethod });
...
function getWindowWidth$1() {
	const isApple$2 = /^Apple/.test(navigator.vendor);
	var screenWidth = isApple$2 && window.matchMedia("(orientation:landscape)").matches ? Math.max(screen.width, screen.height) : screen.width;
	return isApple$2 ? Math.min(window.innerWidth, document.documentElement.clientWidth, screenWidth) || screenWidth : Math.min(window.innerWidth, document.documentElement.clientWidth);
}
function useRem() {
	const config = __uniConfig.globalStyle || {};
	const maxWidth$1 = checkValue$1(config.rpxCalcMaxDeviceWidth, 960);
	const baseWidth$1 = checkValue$1(config.rpxCalcBaseDeviceWidth, 375);
	function updateRem() {
...
var require_out = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function offChange(callback) {
		var index$4 = callbacks.indexOf(callback);
		if (index$4 >= 0) callbacks.splice(index$4, 1);
	}
	module.exports = {
		get support() {
...
var import_out$4 = /* @__PURE__ */ __toESM(require_out());
const onEventPrevent = /* @__PURE__ */ withModifiers(() => {}, ["prevent"]);
const onEventStop = /* @__PURE__ */ withModifiers((_event) => {}, ["stop"]);
...
function getWindowTop() {
	...
	return top ? top + import_out$4.default.top : 0;
}
function getWindowOffset() {
	...
	return {
		top,
		bottom: bottom ? bottom + import_out$4.default.bottom : 0,
		left: left ? left + import_out$4.default.left : 0,
		right: right ? right + import_out$4.default.right : 0,
...
function updateStyle(id$2, content) {
	let style = sheetsMap.get(id$2);
	...
	sheetsMap.set(id$2, style);
}
function removeStyle(id$2) {
	let style = sheetsMap.get(id$2);
	if (style) {
...
function initRouteMeta(pageMeta, id$2) {
	...
	const res = extend({ id: id$2 }, globalStyle, pageMeta);
...
function normalizeTabBarRoute(index$4, oldPagePath, newPagePath) {
	...
	if (newTabBarRoute) {
		...
		meta.tabBarIndex = index$4;
		meta.isQuit = meta.isTabBar = true;
		const tabBar$1 = __uniConfig.tabBar;
		if (tabBar$1 && tabBar$1.list && tabBar$1.list[index$4]) tabBar$1.list[index$4].pagePath = removeLeadingSlash(newPagePath);
	}
}
...
var ComponentDescriptor = class {
	...
	addClass(clazz$1) {
		if (!this.$el || !clazz$1) return this;
		const __wxsAddClass = this.$el.__wxsAddClass || (this.$el.__wxsAddClass = []);
		if (__wxsAddClass.indexOf(clazz$1) === -1) {
			__wxsAddClass.push(clazz$1);
	...
	removeClass(clazz$1) {
		...
		const { __wxsAddClass } = this.$el;
		if (__wxsAddClass) {
			const index$4 = __wxsAddClass.indexOf(clazz$1);
			if (index$4 > -1) __wxsAddClass.splice(index$4, 1);
		}
		const __wxsRemoveClass = this.$el.__wxsRemoveClass || (this.$el.__wxsRemoveClass = []);
		if (__wxsRemoveClass.indexOf(clazz$1) === -1) {
			__wxsRemoveClass.push(clazz$1);
			this.forceUpdate("class");
		}
		return this;
	}
...
var componentInstance_exports = /* @__PURE__ */ __export({
	$nne: () => $nne,
...
function createNativeEvent(evt, htmlElement = false) {
	...
	return new Proxy(evt, { get(target$1, key) {
		if (key in event) return event[key];
		const value = target$1[key];
		return isFunction(value) ? value.bind(target$1) : value;
	} });
}
...
const invokeViewMethod = (name, args, pageId, callback) => {
	...
	const id$2 = callback ? invokeViewMethodId++ : 0;
	callback && subscribe("invokeViewApi." + id$2, callback, true);
...
const invokeViewMethodKeepAlive = (name, args, callback, pageId) => {
	...
	const id$2 = invokeViewMethodId++;
	const subscribeName = INVOKE_VIEW_API + "." + id$2;
...
function initOn() {
	const { on: on$1 } = UniServiceJSBridge;
	on$1(ON_RESIZE, onResize$2);
	on$1(ON_APP_ENTER_FOREGROUND, onAppEnterForeground);
	on$1(ON_APP_ENTER_BACKGROUND, onAppEnterBackground);
}
...
var componentWx_exports = /* @__PURE__ */ __export({
...
function startAnimation(context) {
	...
	function animate() {
		...
		index$4 += 1;
		if (index$4 < length) setTimeout(animate, transition.duration + transition.delay);
	}
	setTimeout(() => {
		animate();
	}, 0);
...
const defineBuiltInComponent = ((options) => {
	...
	const { props: props$31, mixins } = options;
	if (!props$31 || !props$31.animation) (mixins || (options.mixins = [])).push(animation_default);
	{
...
function useCustomEvent(ref$1, emit$2) {
	return (name, evt, detail) => {
		if (ref$1.value) emit$2(name, normalizeCustomEvent(name, evt, ref$1.value, detail || {}));
	};
}
...
function useHover(props$31) {
	...
	function hoverReset() {
		requestAnimationFrame(() => {
			...
			}, parseInt(props$31.hoverStayTime));
		});
	}
	...
	function handleHoverStart(evt) {
		...
		if (!props$31.hoverClass || props$31.hoverClass === "none" || props$31.disabled) return;
		if (props$31.hoverStopPropagation) evt._hoverPropagationStopped = true;
		hoverTouch = true;
		hoverStartTimer = setTimeout(() => {
			hovering.value = true;
			if (!hoverTouch) hoverReset();
		}, parseInt(props$31.hoverStartTime));
	}
	function onTouchend() {
		handleHoverEnd();
	}
...
function useBooleanAttr(props$31, keys) {
	if (isString(keys)) keys = [keys];
	return keys.reduce((res, key) => {
		if (props$31[key]) res[key] = true;
		return res;
	}, Object.create(null));
}
...
var UniElement = class extends HTMLElement {
	...
	attachVmProps(props$31) {
		this._props = props$31;
	}
	getAttribute(qualifiedName) {
		const name = camelize(qualifiedName);
		const attr$1 = name in this._props ? this._props[name] + "" : super.getAttribute(qualifiedName);
...
var label_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { slots }) {
		const rootRef = ref(null);
		const pageId = useCurrentPageId();
		const handlers = useProvideLabel();
		const pointer = computed(() => props$31.for || slots.default && slots.default.length);
		const _onClick = withWebEvent(($event) => {
			...
			if (props$31.for) UniViewJSBridge.emit("uni-label-click-" + pageId + "-" + props$31.for, $event, true);
			else handlers.length && handlers[0]($event, true);
		});
		onMounted(() => {
			rootRef.value.attachVmProps(props$31);
		});
...
function useListeners(props$31, listeners$1) {
	_addListeners(props$31.id, listeners$1);
	watch(() => props$31.id, (newId, oldId) => {
		_removeListeners(oldId, listeners$1, true);
		_addListeners(newId, listeners$1, true);
	});
	onUnmounted(() => {
		_removeListeners(props$31.id, listeners$1);
	});
}
function _addListeners(id$2, listeners$1, watch$1) {
	const pageId = useCurrentPageId();
	if (watch$1 && !id$2) return;
	if (!isPlainObject(listeners$1)) return;
	Object.keys(listeners$1).forEach((name) => {
		if (watch$1) {
			if (name.indexOf("@") !== 0 && name.indexOf("uni-") !== 0) UniViewJSBridge.on(`uni-${name}-${pageId}-${id$2}`, listeners$1[name]);
		} else if (name.indexOf("uni-") === 0) UniViewJSBridge.on(name, listeners$1[name]);
		else if (id$2) UniViewJSBridge.on(`uni-${name}-${pageId}-${id$2}`, listeners$1[name]);
	});
}
function _removeListeners(id$2, listeners$1, watch$1) {
	const pageId = useCurrentPageId();
	if (watch$1 && !id$2) return;
	if (!isPlainObject(listeners$1)) return;
	Object.keys(listeners$1).forEach((name) => {
		if (watch$1) {
			if (name.indexOf("@") !== 0 && name.indexOf("uni-") !== 0) UniViewJSBridge.off(`uni-${name}-${pageId}-${id$2}`, listeners$1[name]);
		} else if (name.indexOf("uni-") === 0) UniViewJSBridge.off(name, listeners$1[name]);
		else if (id$2) UniViewJSBridge.off(`uni-${name}-${pageId}-${id$2}`, listeners$1[name]);
	});
}
const buttonProps = {
	id: {
...
var button_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { slots }) {
		...
		const { hovering, binding } = useHover(props$31);
		const { t: t$1 } = /* @__PURE__ */ useI18n();
		const onClick = withWebEvent((e$1, isLabelClick) => {
			if (props$31.disabled) return e$1.stopImmediatePropagation();
			if (isLabelClick) rootRef.value.click();
			const formType = props$31.formType;
			if (formType) {
				if (!uniForm) return;
		...
		useListeners(props$31, { "label-click": onClick });
		onMounted(() => {
			rootRef.value.attachVmProps(props$31);
		});
		return () => {
			const hoverClass = props$31.hoverClass;
			const booleanAttrs = useBooleanAttr(props$31, "disabled");
			const loadingAttrs = useBooleanAttr(props$31, "loading");
			const plainAttrs = useBooleanAttr(props$31, "plain");
...
var checkbox_group_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { emit: emit$2, slots }) {
		const rootRef = ref(null);
		useProvideCheckGroup(props$31, useCustomEvent(rootRef, emit$2));
		onMounted(() => {
			rootRef.value.attachVmProps(props$31);
		});
		return () => {
...
function useProvideCheckGroup(props$31, trigger) {
	const fields$1 = [];
...
var checkbox_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { slots }) {
		const rootRef = ref(null);
		const checkboxChecked = ref(props$31.checked);
		const checkboxCheckedBool = computed(() => {
			return checkboxChecked.value === "true" || checkboxChecked.value === true;
		});
		const checkboxValue = ref(props$31.value);
		const initialCheckedValue = props$31.checked;
		function getCheckBoxStyle(checked) {
			if (props$31.disabled) return {
				backgroundColor: "#E1E1E1",
				borderColor: "#D1D1D1"
			};
			const style = {};
			if (checked) {
				if (props$31.activeBorderColor) style.borderColor = props$31.activeBorderColor;
				if (props$31.activeBackgroundColor) style.backgroundColor = props$31.activeBackgroundColor;
			} else {
				if (props$31.borderColor) style.borderColor = props$31.borderColor;
				if (props$31.backgroundColor) style.backgroundColor = props$31.backgroundColor;
			}
			return style;
		}
		...
		watch([() => props$31.checked, () => props$31.value], ([newChecked, newModelValue]) => {
		...
		useListeners(props$31, { "label-click": _onClick });
		...
		onMounted(() => {
			...
			rootElement.attachVmProps(props$31);
		});
		return () => {
			const booleanAttrs = useBooleanAttr(props$31, "disabled");
			let realCheckValue;
			...
			return createVNode("uni-checkbox", mergeProps(booleanAttrs, {
				...
				"style": { "--HOVER-BD-COLOR": props$31.activeBorderColor }
			}, [createVNode("div", {
				"class": ["uni-checkbox-input", { "uni-checkbox-input-disabled": props$31.disabled }],
				"style": checkboxStyle.value
			}, [realCheckValue ? createSvgIconVNode(ICON_PATH_SUCCESS_NO_CIRCLE, props$31.disabled ? "#ADADAD" : props$31.foreColor || props$31.iconColor || props$31.color, 22) : ""], 6), slots.default && slots.default()], 4)], 16, ["id", "onClick"]);
		};
	}
});
...
function useKeyboard$1(props$31, elRef, trigger) {
	function initKeyboard(el) {
		const isApple$2 = computed(() => String(navigator.vendor).indexOf("Apple") === 0);
...
var import_out$3 = /* @__PURE__ */ __toESM(require_out());
var pageMetaKey = PolySymbol(process.env.NODE_ENV !== "production" ? "UniPageMeta" : "upm");
function usePageMeta() {
	return inject(pageMetaKey);
}
function providePageMeta(id$2) {
	const pageMeta = initPageMeta(id$2);
...
function initPageMeta(id$2) {
	if (__UNI_FEATURE_PAGES__) return reactive(normalizePageMeta(JSON.parse(JSON.stringify(initRouteMeta(useRoute().meta, id$2)))));
	return reactive(normalizePageMeta(JSON.parse(JSON.stringify(initRouteMeta(__uniRoutes[0].meta, id$2)))));
}
function normalizePageMeta(pageMeta) {
	if (__UNI_FEATURE_PULL_DOWN_REFRESH__) {
		const { enablePullDownRefresh, navigationBar } = pageMeta;
		{
			...
			if (style !== "custom" && type !== "transparent") pullToRefresh.offset += NAVBAR_HEIGHT + import_out$3.default.top;
...
var lookup = /* @__PURE__ */ (function() {
	const lookup$1 = new Uint8Array(256);
	for (var i = 0; i < chars.length; i++) lookup$1[chars.charCodeAt(i)] = i;
	return lookup$1;
})();
...
function decode(base64) {
	var bufferLength = base64.length * .75, len = base64.length, i, p$1 = 0, encoded1, encoded2, encoded3, encoded4;
...
function validateProp(name, value, prop, isAbsent) {
	...
	const { type, required, validator: validator$1 } = prop;
	...
	if (validator$1) return validator$1(value);
}
...
var invokeCallbacks = {};
function addInvokeCallback(id$2, name, callback, keepAlive = false) {
...
function invokeCallback(id$2, res, extras) {
	if (typeof id$2 === "number") {
		const opts = invokeCallbacks[id$2];
		if (opts) {
...
function wrapperOptions(interceptors$1, options = {}) {
	[
		HOOK_SUCCESS,
		HOOK_FAIL,
		HOOK_COMPLETE
	].forEach((name) => {
		const hooks = interceptors$1[name];
		if (!isArray(hooks)) return;
...
function invokeFail(id$2, name, errMsg, errRes = {}) {
...
function beforeInvokeApi(name, args, protocol, options) {
	...
	if (options && options.beforeInvoke) {
		...
		if (isString(errMsg$1)) return errMsg$1;
	}
...
function wrapperTaskApi(name, fn, protocol, options) {
	return (args) => {
		const id$2 = createAsyncApiCallback(name, args, options);
...
function checkDeviceWidth() {
	...
	{
		const { windowWidth: w, pixelRatio: p$1, platform: pf } = getBaseSystemInfo();
		windowWidth = w;
		pixelRatio = p$1;
		platform = pf;
	}
...
function mergeInterceptorHook(interceptors$1, interceptor) {
	Object.keys(interceptor).forEach((hook) => {
		if (isFunction(interceptor[hook])) interceptors$1[hook] = mergeHook(interceptors$1[hook], interceptor[hook]);
	});
}
function removeInterceptorHook(interceptors$1, interceptor) {
	if (!interceptors$1 || !interceptor) return;
...
const $on = /* @__PURE__ */ defineSyncApi("$on", (name, callback) => {
...
const createVideoContext = /* @__PURE__ */ defineSyncApi(API_CREATE_VIDEO_CONTEXT, (id$2, context) => {
	if (context) return new VideoContext(id$2, getPageIdByVm(context));
	return new VideoContext(id$2, getPageIdByVm(getCurrentPageVm()));
});
...
var operateMapWrap = (id$2, pageId, type, options) => {
	operateMap(id$2, pageId, type, options, (res) => {
...
const createMapContext = /* @__PURE__ */ defineSyncApi(API_CREATE_MAP_CONTEXT, (id$2, context) => {
	if (context) return new MapContext(id$2, getPageIdByVm(context));
	return new MapContext(id$2, getPageIdByVm(getCurrentPageVm()));
...
const CanvasGetImageDataOptions = { formatArgs: {
	x: getInt("x"),
	y: getInt("y"),
	width: formatWidth,
	height: formatHeight
...
const CanvasPutImageDataProtocol = /* @__PURE__ */ extend({ data: {
...
const CanvasToTempFilePathOptions = { formatArgs: {
	x: getInt("x", 0),
	y: getInt("y", 0),
	width: formatWidth,
	height: formatHeight,
...
function checkColor(e$1) {
	e$1 = e$1 || "#000000";
	let t$1 = null;
	if ((t$1 = /^#([0-9|A-F|a-f]{6})$/.exec(e$1)) != null) return [
		parseInt(t$1[1].slice(0, 2), 16),
		parseInt(t$1[1].slice(2, 4), 16),
		parseInt(t$1[1].slice(4), 16),
		255
	];
	if ((t$1 = /^#([0-9|A-F|a-f]{3})$/.exec(e$1)) != null) {
		let n = t$1[1].slice(0, 1);
		let o$1 = t$1[1].slice(1, 2);
		let r = t$1[1].slice(2, 3);
		n = parseInt(n + n, 16);
		o$1 = parseInt(o$1 + o$1, 16);
		r = parseInt(r + r, 16);
		return [
			n,
			o$1,
	...
	if ((t$1 = /^rgb\((.+)\)$/.exec(e$1)) != null) return t$1[1].split(",").map(function(e$2) {
		return Math.min(255, parseInt(e$2.trim()));
	}).concat(255);
	if ((t$1 = /^rgba\((.+)\)$/.exec(e$1)) != null) return t$1[1].split(",").map(function(e$2, t$2) {
		return t$2 === 3 ? Math.floor(255 * parseFloat(e$2.trim())) : Math.min(255, parseInt(e$2.trim()));
	});
	var i = e$1.toLowerCase();
	if (hasOwn(predefinedColor, i)) {
		t$1 = /^#([0-9|A-F|a-f]{6,8})$/.exec(predefinedColor[i]);
		const n = parseInt(t$1[1].slice(0, 2), 16);
		const o$1 = parseInt(t$1[1].slice(2, 4), 16);
		const r = parseInt(t$1[1].slice(4, 6), 16);
		let a$1 = parseInt(t$1[1].slice(6, 8), 16);
		a$1 = a$1 >= 0 ? a$1 : 255;
		return [
			n,
			o$1,
	...
	console.error("unsupported color:" + e$1);
	return [
		0,
		0,
		0,
...
var CanvasContext = class {
	...
	strokeText(text, x, y, maxWidth$1) {
		console.log("initCanvasContextProperty implemented.");
	}
	...
	set font(value) {
		...
		if (fontFormat) {
			...
			var actions = [];
			style.forEach(function(value$1, index$4) {
				if ([
					"italic",
					"oblique",
					"normal"
					...
					self.state.fontStyle = value$1;
				} else if ([
					"bold",
					"normal",
					"lighter",
					"bolder"
				].indexOf(value$1) > -1 || /^\d+$/.test(value$1)) {
					actions.push({
						method: "setFontWeight",
						data: [value$1]
					});
					self.state.fontWeight = value$1;
				} else if (index$4 === 0) {
					actions.push({
						method: "setFontStyle",
						data: ["normal"]
...
var initCanvasContextProperty = /* @__PURE__ */ once(() => {
	[...methods1, ...methods2].forEach(function(method) {
		function get(method$1) {
			switch (method$1) {
				case "fill":
				case "stroke": return function() {
					this.actions.push({
						method: method$1 + "Path",
						data: [...this.path]
					});
				};
				...
				case "fillText":
				case "strokeText": return function(text, x, y, maxWidth$1) {
					var data = [
						text.toString(),
						x,
						y
					];
					if (typeof maxWidth$1 === "number") data.push(maxWidth$1);
					this.actions.push({
						method: method$1,
						data
					});
...
const canvasPutImageData = /* @__PURE__ */ defineAsyncApi(API_CANVAS_PUT_IMAGE_DATA, ({ canvasId, data, x, y, width, height }, { resolve, reject }) => {
	...
	const operate = () => {
		operateCanvas(canvasId, pageId, "putImageData", {
			...
			if (data$1.errMsg && data$1.errMsg.indexOf("fail") !== -1) {
				reject();
				return;
			}
			resolve(data$1);
...
const canvasToTempFilePath = /* @__PURE__ */ defineAsyncApi(API_CANVAS_TO_TEMP_FILE_PATH, ({ x = 0, y = 0, width, height, destWidth, destHeight, canvasId, fileType, quality }, { resolve, reject }) => {
	var pageId = getPageIdByVm(getCurrentPageVm());
	if (!pageId) {
		reject();
		return;
...
function operateEditor(componentId, pageId, type, options) {
	...
	UniServiceJSBridge.invokeViewMethod(`editor.${componentId}`, {
		type,
		data
	}, pageId, ({ callbackId, data: data$1 }) => {
		if (needCallOptions) {
			callOptions(optionsCache[callbackId], data$1);
...
function convertContext(result) {
	if (result && result.contextInfo) {
		const { id: id$2, type, page } = result.contextInfo;
		const ContextClass = ContextClasss[type];
		result.context = new ContextClass(id$2, page);
...
var NodesRef = class {
	...
	fields(fields$1, callback) {
		this._selectorQuery._push(this._selector, this._component, this._single, fields$1, callback);
		return this._selectorQuery;
	}
...
var SelectorQuery = class {
	...
	_push(selector, component, single, fields$1, callback) {
		this._queue.push({
			component,
			selector,
			single,
...
const CreateAnimationOptions = { formatArgs: {} };
...
const setPageMeta = /* @__PURE__ */ defineAsyncApi("setPageMeta", (options, { resolve }) => {
...
const getEnterOptionsSync = /* @__PURE__ */ defineSyncApi("getEnterOptionsSync", () => {
	return getEnterOptions();
});
const getLaunchOptionsSync = /* @__PURE__ */ defineSyncApi("getLaunchOptionsSync", () => {
...
function invokeGetPushCidCallbacks(cid$1, errMsg) {
	getPushCidCallbacks.forEach((callback) => {
		callback(cid$1, errMsg);
	});
	getPushCidCallbacks.length = 0;
}
const getPushClientId = /* @__PURE__ */ defineAsyncApi("getPushClientId", (_, { resolve, reject }) => {
	Promise.resolve().then(() => {
		...
		getPushCidCallbacks.push((cid$1, errMsg) => {
			if (cid$1) resolve({ cid: cid$1 });
			else reject(errMsg);
		});
...
const offPushMessage = (fn) => {
	if (!fn) onPushMessageCallbacks.length = 0;
		...
		const index$4 = onPushMessageCallbacks.indexOf(fn);
		if (index$4 > -1) onPushMessageCallbacks.splice(index$4, 1);
	}
};
...
const MakePhoneCallProtocol = { phoneNumber: String };
...
const SetClipboardDataOptions = {
	...
	beforeSuccess(res, params) {
		...
		const { t: t$1 } = useI18n();
		const title = t$1("uni.setClipboardData.success");
...
const GetFileInfoOptions = { formatArgs: { filePath(filePath, params) {
	params.filePath = getRealPath(filePath);
} } };
const GetFileInfoProtocol = { filePath: {
	type: String,
	required: true
} };
const API_OPEN_DOCUMENT = "openDocument";
const OpenDocumentOptions = { formatArgs: { filePath(filePath, params) {
	params.filePath = getRealPath(filePath);
} } };
...
const GetLocationOptions = { formatArgs: {
	type(value, params) {
...
const OpenLocationOptions = { formatArgs: {
...
const ChooseImageOptions = { formatArgs: {
	count(value, params) {
...
const ChooseVideoOptions = { formatArgs: {
	sourceType(sourceType, params) {
		params.sourceType = elemsInArray(sourceType, CHOOSE_SOURCE_TYPES);
	},
	compressed: true,
...
const ChooseFileOptions = { formatArgs: {
	count(count, params) {
...
const GetImageInfoOptions = { formatArgs: { src(src, params) {
	params.src = getRealPath(src);
} } };
const GetImageInfoProtocol = { src: {
	type: String,
	required: true
} };
const API_PREVIEW_IMAGE = "previewImage";
const PreviewImageOptions = { formatArgs: {
	urls(urls, params) {
...
const GetVideoInfoOptions = { formatArgs: { src(src, params) {
	params.src = getRealPath(src);
} } };
const GetVideoInfoProtocol = { src: {
	type: String,
	required: true
} };
...
function stringifyQuery$1(url, data) {
	...
	for (const key in data) if (hasOwn(data, key)) {
		...
		else if (isPlainObject(v$1)) v$1 = JSON.stringify(v$1);
...
const DownloadFileOptions = { formatArgs: { header(value, params) {
	params.header = value || {};
} } };
...
const UploadFileOptions = { formatArgs: {
...
const ConnectSocketOptions = { formatArgs: {
	header(value, params) {
		params.header = value || {};
	},
	method(value, params) {
...
const SendSocketMessageProtocol = { data: [String, ArrayBuffer] };
...
const StartLocationUpdateProtocol = { type: String };
const StartLocationUpdateOptions = { formatArgs: { type(value, params) {
	value = (value || "").toLowerCase();
...
function encodeQueryString(url) {
	...
	const index$4 = url.indexOf("?");
	if (index$4 === -1) return url;
	const query = url.slice(index$4 + 1).trim().replace(/^(\?|#|&)/, "");
	if (!query) return url;
...
const NavigateBackProtocol = /* @__PURE__ */ extend({ delta: { type: Number } }, createAnimationProtocol(ANIMATION_OUT$1));
...
const NavigateBackOptions = { formatArgs: { delta(value, params) {
...
const SetNavigationBarColorOptions = { formatArgs: { animation(animation, params) {
...
const PageScrollToOptions = { formatArgs: { duration: 300 } };
...
const ShowToastOptions = { formatArgs: {
	title: "",
	icon(type, params) {
...
const HideTabBarProtocol = { animation: Boolean };
...
const SetTabBarBadgeProtocol = /* @__PURE__ */ extend({ text: {
	type: String,
	required: true
} }, IndexProtocol);
...
const initIntersectionObserverPolyfill = function() {
	...
	var document$1 = (function(startDoc) {
		var doc = startDoc;
	...
	IntersectionObserver$1.prototype._initThresholds = function(opt_threshold) {
		...
		return threshold.sort().filter(function(t$1, i, a$1) {
	...
	IntersectionObserver$1.prototype._monitorIntersections = function(doc) {
		...
		if (doc != (this.root && (this.root.ownerDocument || this.root) || document$1)) {
			var frame = getFrameElement(doc);
	...
	IntersectionObserver$1.prototype._unmonitorIntersections = function(doc) {
		var index$4 = this._monitoringDocuments.indexOf(doc);
		if (index$4 == -1) return;
		var rootDoc = this.root && (this.root.ownerDocument || this.root) || document$1;
		if (this._observationTargets.some(function(item) {
		...
		var unsubscribe = this._monitoringUnsubscribes[index$4];
		this._monitoringDocuments.splice(index$4, 1);
		this._monitoringUnsubscribes.splice(index$4, 1);
		unsubscribe();
		if (doc != rootDoc) {
			var frame = getFrameElement(doc);
	...
	IntersectionObserver$1.prototype._computeTargetAndRootIntersection = function(target, targetRect, rootRect) {
		...
		while (!atRoot && parent) {
			...
			if (parent == this.root || parent.nodeType == 9) {
				atRoot = true;
				if (parent == this.root || parent == document$1) if (crossOriginUpdater && !this.root) if (!crossOriginRect || crossOriginRect.width == 0 && crossOriginRect.height == 0) {
					parent = null;
					parentRect = null;
	...
	IntersectionObserver$1.prototype._getRootRect = function() {
		...
		if (this.root && !isDoc(this.root)) rootRect = getBoundingClientRect(this.root);
			...
			var doc = isDoc(this.root) ? this.root : document$1;
	...
	IntersectionObserver$1.prototype._unregisterInstance = function() {
		var index$4 = registry.indexOf(this);
		if (index$4 != -1) registry.splice(index$4, 1);
	};
	function now() {
	...
	function getParentNode(node) {
		...
		if (node.nodeType == 9 && node != document$1) return getFrameElement(node);
...
function requestComponentObserver($el, options, callback) {
	...
	const intersectionObserver = new IntersectionObserver((entries$1) => {
...
const switchTab = /* @__PURE__ */ defineAsyncApi(API_SWITCH_TAB, ({ url, tabBarText, isAutomatedTesting }, { resolve, reject }) => {
...
const redirectTo = /* @__PURE__ */ defineAsyncApi(API_REDIRECT_TO, ({ url, isAutomatedTesting }, { resolve, reject }) => {
	if (!entryPageState.handledBeforeEntryPageRoutes) {
...
const reLaunch = /* @__PURE__ */ defineAsyncApi(API_RE_LAUNCH, ({ url, isAutomatedTesting }, { resolve, reject }) => {
	if (!entryPageState.handledBeforeEntryPageRoutes) {
...
function navigate({ type, url, tabBarText, events, isAutomatedTesting }, __id__) {
	...
	return new Promise((resolve, reject) => {
		const state$2 = createPageState(type, __id__);
		router[type === "navigateTo" ? "push" : "replace"]({
			...
			if (type === "navigateTo") {
				...
				if (!meta.eventChannel) meta.eventChannel = new EventChannel(state$2.__id__, events);
				else if (events) {
				...
				return isAutomatedTesting ? resolve({ __id__: state$2.__id__ }) : resolve({ eventChannel: meta.eventChannel });
			}
			return isAutomatedTesting ? resolve({ __id__: state$2.__id__ }) : resolve();
		});
	});
}
...
function updateCurPageCssVar(pageMeta) {
	...
	if (__UNI_FEATURE_TABBAR__ && pageMeta.isTabBar) {
		...
		tabBar$1.shown && (windowBottomValue = parseInt(tabBar$1.height));
	}
	updatePageCssVar({
...
var import_out$2 = /* @__PURE__ */ __toESM(require_out());
...
const getSystemSafeAreaInsets = function() {
	return {
		top: import_out$2.default.top,
		right: import_out$2.default.right,
		bottom: import_out$2.default.bottom,
		left: import_out$2.default.left
	};
};
...
var UniPageImpl = class {
	get statusBarHeight() {
		return import_out$1.default.top;
	}
	get width() {
	...
	getElementById(id$2) {
		...
		return uniPageBody ? uniPageBody.querySelector(`#${id$2}`) : null;
	}
...
const closeDialogPage = (options) => {
	...
	if (options === null || options === void 0 ? void 0 : options.dialogPage) {
		...
		if (!isSystemDialogPage(dialogPage)) if (parentPage && (parentPage.vm.$basePage.meta.isTabBar || currentPages.indexOf(parentPage) !== -1)) {
			...
			const index$4 = parentDialogPages.indexOf(dialogPage);
			invokeHook(dialogPage.vm, ON_UNLOAD);
			parentDialogPages.splice(index$4, 1);
			if (index$4 === parentDialogPages.length) dialogPageTriggerPrevDialogPageLifeCycle(parentPage, ON_SHOW);
			...
			var _parentPage$vm;
			const parentSystemDialogPages = (_parentPage$vm = parentPage.vm) === null || _parentPage$vm === void 0 || (_parentPage$vm = _parentPage$vm.$pageLayoutInstance) === null || _parentPage$vm === void 0 || (_parentPage$vm = _parentPage$vm.$systemDialogPages) === null || _parentPage$vm === void 0 ? void 0 : _parentPage$vm.value;
			if (!parentSystemDialogPages) {
				triggerFailCallback$1(options, "dialogPage is not a valid page");
				return;
			}
			const index$4 = parentSystemDialogPages.indexOf(dialogPage);
			if (index$4 > -1) {
				invokeHook(parentSystemDialogPages[index$4].vm, ON_UNLOAD);
				parentSystemDialogPages.splice(index$4, 1);
				if (index$4 === parentSystemDialogPages.length) dialogPageTriggerPrevDialogPageLifeCycle(parentPage, ON_SHOW);
				dialogPageTriggerParentShow(dialogPage, 1);
...
function pruneCurrentPages() {
	currentPagesMap.forEach((page, id$2) => {
		if (page.$.isUnmounted) currentPagesMap.delete(id$2);
	});
}
...
function normalizeRouteKey(path, id$2) {
	return path + SEP + id$2;
}
...
function operateMap(id$2, pageId, type, data, operateMapCallback$1) {
	UniServiceJSBridge.invokeViewMethod("map." + id$2, {
		type,
		data
	}, pageId, operateMapCallback$1);
}
...
function getNodeInfo(el, fields$1) {
	...
	if (fields$1.scrollOffset) if (el.tagName === "SCROLL-VIEW") {
...
function matches(element, selectors) {
	return (element.matches || element.matchesSelector || element.mozMatchesSelector || element.msMatchesSelector || element.oMatchesSelector || element.webkitMatchesSelector || function(selectors$1) {
		const matches$1 = this.parentElement.querySelectorAll(selectors$1);
		let i = matches$1.length;
		while (--i >= 0 && matches$1.item(i) !== this);
		return i > -1;
...
var QuerySelectorHelper = class QuerySelectorHelper {
	...
	static queryElement(element, selector, fields$1, all, vnode) {
		return new QuerySelectorHelper(element, vnode).query(selector, all, fields$1);
	}
	query(selector, all, fields$1) {
		if (this._element.nodeType === 3 || this._element.nodeType === 8) return this.queryFragment(this._element, selector, all, fields$1);
		else return all ? this.querySelectorAll(this._element, selector, fields$1) : this.querySelector(this._element, selector, fields$1);
	}
	queryFragment(el, selector, all, fields$1) {
		...
		if (all) {
			...
			while (depth > 0) {
				...
				const queryResult = this.querySelectorAll(current, selector, fields$1);
			...
			while (depth > 0) {
				...
				result2 = this.querySelector(current, selector, fields$1);
				current = current.nextSibling;
	...
	querySelector(element, selector, fields$1) {
		let element2 = this.querySelf(element, selector);
		if (element2 == null) element2 = element.querySelector(selector);
		if (element2 != null) return this.getNodeInfo(element2, fields$1);
		return null;
	}
	querySelectorAll(element, selector, fields$1) {
		const nodesInfoArray = [];
		if (this.querySelf(element, selector) != null) nodesInfoArray.push(this.getNodeInfo(element, fields$1));
		const findNodes = element.querySelectorAll(selector);
		findNodes === null || findNodes === void 0 || findNodes.forEach((el) => {
			nodesInfoArray.push(this.getNodeInfo(el, fields$1));
		});
		return nodesInfoArray;
	}
	querySelf(element, selector) {
		if (element == null || selector.length < 2) return null;
		const selectorType$1 = selector.charAt(0);
		const selectorName = selector.slice(1);
		if (selectorType$1 == "." && element.classList.contains(selectorName)) return element;
		if (selectorType$1 == "#" && element.getAttribute("id") == selectorName) return element;
		if (selector.toUpperCase() == element.nodeName.toUpperCase()) return element;
		return null;
	}
	getNodeInfo(element, fields$1) {
		...
		if (fields$1.node) nodeInfo.node = element;
		if (fields$1.dataset) nodeInfo.dataset = {};
		return nodeInfo;
	}
};
function getNodesInfo(pageVm, component, selector, single, fields$1) {
	...
	if (maybeFragment) return QuerySelectorHelper.queryElement(selfElement, selector, fields$1, !single, (component || pageVm).$.subTree);
	if (single) {
		...
		if (nodeList && nodeList.length) [].forEach.call(nodeList, (node) => {
			infos.push(getNodeInfo(node, fields$1));
		});
...
function requestComponentInfo(page, reqs, callback) {
	const result = [];
	reqs.forEach(({ component, selector, single, fields: fields$1 }) => {
...
function addMediaQueryObserver({ reqId, component, options, callback }, _pageId) {
	...
	const listener$2 = listeners[reqId] = (observer) => callback(observer.matches);
...
function removeMediaQueryObserver({ reqId, component }, _pageId) {
	const listener$2 = listeners[reqId];
...
function wrapperComponentSetup(comp, { type, clone, init: init$1, setup, before, options }) {
	...
	comp.setup = (props$31, ctx) => {
		const instance = getCurrentInstance();
		init$1(instance.proxy);
		setup(instance);
		if (oldSetup) return oldSetup(props$31, ctx);
	};
...
function setupWindow(comp, id$2) {
	...
	return setupComponent(comp, {
		...
		setup(instance) {
			...
			onMounted(() => {
				...
				const { onReady: onReady$1 } = instance;
				onReady$1 && invokeArrayFns$1(onReady$1);
	...
	return setupComponent(comp, {
		...
		setup(instance) {
			...
			const onLaunch = () => {
				...
				const { onLaunch: onLaunch$1, onShow, onPageNotFound: onPageNotFound$1 } = instance;
				const launchOptions$1 = initLaunchOptions({
					path: route.path.slice(1) || __uniRoutes[0].meta.route,
					query: decodedQuery(route.query)
				});
				onLaunch$1 && invokeArrayFns$1(onLaunch$1, launchOptions$1);
				onShow && invokeArrayFns$1(onShow, launchOptions$1);
				if (__UNI_FEATURE_PAGES__) {
					if (!route.matched.length) {
						...
						handleBeforeEntryPageRoutes();
						onPageNotFound$1 && invokeArrayFns$1(onPageNotFound$1, pageNotFoundOptions);
					}
				}
			};
		...
		before(comp$1) {
			...
			comp$1.setup = (props$31, ctx) => {
				const res = setup && setup(props$31, ctx);
...
function onVisibilityChange$1() {
	const { emit: emit$2 } = UniServiceJSBridge;
	if (document.visibilityState === "visible") emit$2(ON_APP_ENTER_FOREGROUND, getEnterOptions());
	else emit$2(ON_APP_ENTER_BACKGROUND);
}
function onThemeChange$2() {
	...
	if (mediaQueryList) {
		let callback = (e$1) => {
			UniServiceJSBridge.emit(ON_THEME_CHANGE, { theme: e$1.matches ? "dark" : "light" });
		};
...
function getBrowserInfo() {
	...
	if (isIOS) {
		...
		for (let i = 0; i < infos.length; i++) {
			...
			let other;
			for (let o$1 = 0; o$1 < otherInfo.length; o$1++) if (otherInfo[o$1].test(info)) {
				other = true;
				break;
			}
			if (!other) {
	...
	if (browserVersion !== "-1") browserName = "IE";
		...
		for (let index$4 = 0; index$4 < browseVendors.length; index$4++) {
			const vendor = browseVendors[index$4];
			const reg = /* @__PURE__ */ new RegExp(`(${vendor})/(\\S*)\\b`);
			if (reg.test(ua)) {
				browserName = vendors[index$4];
...
function usePageHeadTransparent(headRef, { id: id$2, navigationBar: { titleColor, coverage, backgroundColor } }) {
	let A = 0;
	...
	useOn(id$2 + ".onPageScroll", ({ scrollTop }) => {
		...
		borderRadiusElemsStyles.forEach(function(borderRadiusElemStyle, index$4) {
			const rgba = oldColors[index$4].match(/[\d+\.]+/g);
...
function createButtonsTsx(btns) {
	return btns.map(({ onClick, btnClass, btnStyle, btnText, btnIconPath, badgeText, iconStyle, btnSelect }, index$4) => {
		return createVNode("div", {
			"key": index$4,
			"class": btnClass,
			"style": btnStyle,
...
function usePageHeadButtons({ id: id$2, navigationBar }) {
	const left = [];
	const right = [];
	...
	if (isArray(buttons)) {
		...
		buttons.forEach((btn, index$4) => {
			...
			const pageHeadBtn = usePageHeadButton(id$2, index$4, btn, isTransparent);
...
function usePageHeadButton(pageId, index$4, btn, isTransparent) {
	const iconStyle = {
		color: btn.color,
	...
	return new Proxy({
		...
		iconStyle,
		onClick() {
			invokeHook(pageId, ON_NAVIGATION_BAR_BUTTON_TAP, extend({ index: index$4 }, btn));
		},
...
function usePageHeadSearchInput({ id: id$2, navigationBar: { searchInput } }) {
	...
	if (disabled) {
		const onClick = () => {
			invokeHook(id$2, ON_NAVIGATION_BAR_SEARCH_INPUT_CLICKED);
		};
		return {
			focus,
			text,
	...
	const onFocus = () => {
		focus.value = true;
		invokeHook(id$2, ON_NAVIGATION_BAR_SEARCH_INPUT_FOCUS_CHANGED, { focus: true });
	};
	const onBlur = () => {
		focus.value = false;
		invokeHook(id$2, ON_NAVIGATION_BAR_SEARCH_INPUT_FOCUS_CHANGED, { focus: false });
	};
	const onInput = (evt) => {
		text.value = evt.detail.value;
		invokeHook(id$2, ON_NAVIGATION_BAR_SEARCH_INPUT_CHANGED, { text: text.value });
	};
	const onConfirm = (evt) => {
		invokeHook(id$2, ON_NAVIGATION_BAR_SEARCH_INPUT_CONFIRMED, { text: text.value });
	};
	return {
		focus,
		text,
...
var __plugin_vue_export_helper_default = (sfc, props$31) => {
	const target = sfc.__vccOpts || sfc;
	for (const [key, val] of props$31) target[key] = val;
	return target;
};
...
function processDeltaY(ev, identifier, startY) {
	const touch = Array.prototype.slice.call(ev.changedTouches).filter((touch$1) => touch$1.identifier === identifier)[0];
...
function usePageRefresh(refreshRef) {
	const pageMeta = usePageMeta();
	const { id: id$2, pullToRefresh } = pageMeta;
	...
	}, API_START_PULL_DOWN_REFRESH, false, id$2);
	useSubscribe(() => {
		if (!pageMeta.enablePullDownRefresh) return;
		if (state$2 === REFRESHING) {
			removeClass();
			state$2 = RESTORING;
			addClass();
			restoring(() => {
				removeClass();
				state$2 = distance = offset = null;
			});
		}
	}, API_STOP_PULL_DOWN_REFRESH, false, id$2);
	...
	function toggleClass(type) {
		if (!state$2) return;
		if (refreshContainerElem) refreshContainerElem.classList[type]("uni-page-refresh--" + state$2);
	}
	function addClass() {
		toggleClass("add");
	}
	...
	const onTouchstartPassive = withWebEvent((ev) => {
		...
		if ([
			...
			REFRESHING,
			RESTORING
		].indexOf(state$2) >= 0) canRefresh = false;
	...
	const onTouchmove = withWebEvent((ev) => {
		...
		distance = deltaY;
		if (deltaY >= range && state$2 !== REACHED || deltaY < range && state$2 !== PULLING) {
			removeClass();
			state$2 = state$2 === REACHED ? PULLING : REACHED;
			addClass();
		}
		pulling(deltaY);
	});
	...
	function refreshing() {
		...
		invokeHook(id$2, ON_PULL_DOWN_REFRESH);
	}
...
function createDialogPageVNode(normalDialogPages, systemDialogPages) {
	...
	dialogPages.sort((a$1, b) => {
		var _a$page$vm, _b$page$vm;
		return (((_a$page$vm = a$1.page.vm) === null || _a$page$vm === void 0 || (_a$page$vm = _a$page$vm.$basePage) === null || _a$page$vm === void 0 ? void 0 : _a$page$vm.id) || Number.MAX_SAFE_INTEGER) - (((_b$page$vm = b.page.vm) === null || _b$page$vm === void 0 || (_b$page$vm = _b$page$vm.$basePage) === null || _b$page$vm === void 0 ? void 0 : _b$page$vm.id) || Number.MAX_SAFE_INTEGER);
	});
...
function renderPage(component, props$31) {
	return openBlock(), createBlock(page_default, null, {
		page: withCtx(() => [createVNode(component, extend({}, props$31, { ref: "page" }), null, 512)]),
		_: 1
	});
}
var systemRoutes = [];
function registerSystemRoute(route, page, meta = {}) {
	...
	if (isArray(page.styles) && page.styles.length > 0) page.styles.forEach((style, index$4) => {
		updateStyle(`${route}-style-${index$4}`, style);
	});
...
function HTMLParser(html, handler) {
	...
	while (html) {
		...
		if (!stack.last() || !special[stack.last()]) {
			if (html.indexOf("<!--") == 0) {
				index$4 = html.indexOf("-->");
				if (index$4 >= 0) {
					if (handler.comment) handler.comment(html.substring(4, index$4));
					html = html.substring(index$4 + 3);
					chars$1 = false;
				}
			...
			if (chars$1) {
				index$4 = html.indexOf("<");
				var text = index$4 < 0 ? html : html.substring(0, index$4);
				html = index$4 < 0 ? "" : html.substring(index$4);
	...
	class List extends Container {
		...
		insertBefore(blot, ref$1) {
			if (blot instanceof ListItem) super.insertBefore(blot, ref$1);
				...
				const index$4 = ref$1 == null ? this.length() : ref$1.offset(this);
				const after = this.split(index$4);
...
function background_default(Quill) {
	...
	const BackgroundColorStyle = new (Quill.import("formats/background")).constructor("backgroundColor", "background-color", { scope: Scope.INLINE });
...
function text_default$1(Quill) {
	...
	const result = {};
	text.forEach(({ name, scope: scope$1 }) => {
		result[`formats/${name}`] = new Attributor.Style(name, hyphenate(name), { scope: scope$1 });
	});
	return result;
}
function image_default$1(Quill) {
	const Image$1 = Quill.import("formats/image");
	const ATTRIBUTES = [
		"alt",
		"height",
		"width",
	...
	const format = Image$1.prototype.format;
...
function mention_default(Quill) {
	...
	class MentionBlot extends Embed {
		...
		static value(node) {
			...
			SupportStyleList.forEach((item) => {
				const styleValue$1 = getMentionStyleValue(node, item);
				if (styleValue$1) value[item] = styleValue$1;
			});
			return value;
		}
	}
...
function useQuill(props$31, rootRef, trigger) {
	let quillReady;
	let skipMatcher;
	let quill;
	watch(() => props$31.readOnly, (value) => {
		if (quillReady) {
			quill.enable(!value);
			if (value) quill.blur();
		}
	});
	watch(() => props$31.placeholder, (value) => {
		if (quillReady) setPlaceHolder(value);
	});
	watch(() => props$31.type, (value) => {
	...
	function html2delta(html) {
		...
		HTMLParser(html, {
			start: function(tag, attrs$1, unary) {
				...
				disable = false;
				const start = `<${tag} ${attrs$1.map(({ name, value }) => `${name}="${value}"`).join(" ")} ${unary ? "/" : ""}>`;
				content += start;
			},
			end: function(tag) {
	...
	function initQuill(imageResizeModules) {
		...
		const options = {
			...
			placeholder: props$31.placeholder
		};
			...
			$el.addEventListener(name, ($event) => {
				...
				if (name === "input") {
					if (getBaseSystemInfo().platform === "ios") {
						...
						setPlaceHolder(regExpContent && regExpContent.replace(/\s/g, "") ? "" : props$31.placeholder);
					}
					$event.stopPropagation();
		...
		if (quillReady) {
			...
			switch (type) {
				...
				case "insertImage":
					{
						range = quill.getSelection(true);
						const { src = "", alt = "", width = "", height = "", extClass = "", data: data$1 = {} } = options;
						...
						quill.formatText(range.index, 1, "data-custom", Object.keys(data$1).map((key) => `${key}=${data$1[key]}`).join("&"), "silent");
	...
	onMounted(() => {
		...
		if (props$31.showImgSize) imageResizeModules.push("DisplaySize");
		if (props$31.showImgToolbar) imageResizeModules.push("Toolbar");
		if (props$31.showImgResize) imageResizeModules.push("Resize");
...
var props$27 = /* @__PURE__ */ extend({}, props$25, {
	id: {
		type: String,
		default: ""
	},
...
var editor_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { emit: emit$2 }) {
		const rootRef = ref(null);
		const trigger = useCustomEvent(rootRef, emit$2);
		useQuill(props$31, rootRef, trigger);
		useKeyboard$1(props$31, rootRef, trigger);
		onMounted(() => {
			rootRef.value.attachVmProps(props$31);
		});
		return () => {
...
var icon_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31) {
		const rootRef = ref(null);
		const path = computed(() => ICONS[props$31.type]);
		onMounted(() => {
			rootRef.value.attachVmProps(props$31);
		});
		return () => {
			const { value } = path;
			return createVNode("uni-icon", { "ref": rootRef }, [value && value.d && createSvgIconVNode(value.d, props$31.color || value.c, rpx2px(props$31.size))], 512);
		};
	}
});
var resize_sensor_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { emit: emit$2 }) {
		...
		useResizeSensorLifecycle(rootRef, props$31, update, reset);
...
function useResizeSensorUpdate(rootRef, emit$2, reset) {
	...
	watch(() => extend({}, size), (value) => emit$2("resize", value));
	return () => {
		const rootEl = rootRef.value;
		if (!rootEl) return;
...
function useResizeSensorLifecycle(rootRef, props$31, update, reset) {
	onActivated(reset);
	onMounted(() => {
		if (props$31.initial) nextTick(update);
		const rootEl = rootRef.value;
...
var image_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { emit: emit$2 }) {
		const rootRef = ref(null);
		const state$2 = useImageState(rootRef, props$31);
		const trigger = useCustomEvent(rootRef, emit$2);
		const { fixSize } = useImageSize(rootRef, props$31, state$2);
		useImageLoader(state$2, props$31, rootRef, fixSize, trigger);
		onMounted(() => {
			const rootElement = rootRef.value;
		...
		return () => {
			return createVNode("uni-image", { "ref": rootRef }, [createVNode("div", { "style": state$2.modeStyle }, null, 4), FIX_MODES[props$31.mode] ? createVNode(resize_sensor_default, { "onResize": fixSize }, null, 8, ["onResize"]) : createVNode("span", null, null)], 512);
		};
	}
});
function useImageState(rootRef, props$31) {
	const imgSrc = ref("");
	const modeStyleRef = computed(() => {
		let size = "auto";
		let position = "";
		const opts = IMAGE_MODES[props$31.mode];
		if (!opts) {
			position = "0% 0%";
			size = "100% 100%";
		} else {
	...
	const state$2 = reactive({
		rootEl: rootRef,
		src: computed(() => props$31.src ? getRealPath(props$31.src) : ""),
		origWidth: 0,
		origHeight: 0,
		origStyle: {
			width: "",
	...
	onMounted(() => {
		const rootEl = rootRef.value;
		state$2.origWidth = rootEl.clientWidth || 0;
		state$2.origHeight = rootEl.clientHeight || 0;
	});
	return state$2;
}
function useImageLoader(state$2, props$31, rootRef, fixSize, trigger) {
	let img;
	let draggableImg;
	const setState = (width = 0, height = 0, imgSrc = "") => {
		state$2.origWidth = width;
		state$2.origHeight = height;
		state$2.imgSrc = imgSrc;
	};
	const loadImage = (src) => {
		if (!src) {
			resetImage();
		...
		img.onload = (evt) => {
			...
			img.draggable = props$31.draggable;
		...
		img.onerror = (evt) => {
			setState();
			resetImage();
			trigger("error", evt, { errMsg: `GET ${state$2.src} 404 (Not Found)` });
		};
		img.src = src;
	};
	const resetImage = () => {
	...
	watch(() => state$2.src, (value) => loadImage(value));
	watch(() => state$2.imgSrc, (value) => {
		if (!value && draggableImg) {
			draggableImg.remove();
			draggableImg = null;
		}
	});
	onMounted(() => loadImage(state$2.src));
...
function useImageSize(rootRef, props$31, state$2) {
	const fixSize = () => {
		const { mode: mode$1 } = props$31;
		const names = FIX_MODES[mode$1];
		if (!names) return;
		const { origWidth, origHeight } = state$2;
	...
	const resetSize = () => {
		...
		const { origStyle: { width, height } } = state$2;
		style.width = width;
		style.height = height;
	};
	watch(() => props$31.mode, (value, oldValue) => {
...
function removeInteractListener(vm) {
	const index$4 = states.indexOf(vm);
	if (index$4 >= 0) states.splice(index$4, 1);
}
const getInteractStatus = () => !!userInteract;
function useUserAction() {
	const state$2 = reactive({ userAction: false });
	onMounted(() => {
		addInteractListener(state$2);
	});
	onBeforeUnmount(() => {
		removeInteractListener(state$2);
	});
	return { state: state$2 };
}
function useScopedAttrs() {
	const state$2 = reactive({ attrs: {} });
	onMounted(() => {
		let instance = getCurrentInstance();
		while (instance) {
			const scopeId = instance.type.__scopeId;
			if (scopeId) state$2.attrs[scopeId] = "";
...
const props$14 = /* @__PURE__ */ extend({}, {
	name: {
		type: String,
		default: ""
	},
...
function useBase(props$31, rootRef, emit$2) {
	const fieldRef = ref(null);
	const trigger = useCustomEvent(rootRef, emit$2);
	const selectionStart = computed(() => {
		const selectionStart$1 = Number(props$31.selectionStart);
		return isNaN(selectionStart$1) ? -1 : selectionStart$1;
	});
	const selectionEnd = computed(() => {
		const selectionEnd$1 = Number(props$31.selectionEnd);
		return isNaN(selectionEnd$1) ? -1 : selectionEnd$1;
	});
	const cursor = computed(() => {
		const cursor$1 = Number(props$31.cursor);
		return isNaN(cursor$1) ? -1 : cursor$1;
	});
	const maxlength = computed(() => {
		var maxlength$1 = Number(props$31.maxlength);
		return isNaN(maxlength$1) || maxlength$1 < 0 ? Infinity : Math.floor(maxlength$1);
	});
	let value = "";
	{
		const modelValueString = getValueString(props$31.modelValue, props$31.type, maxlength.value);
		const valueString = getValueString(props$31.value, props$31.type, maxlength.value);
		value = props$31.modelValue !== void 0 ? modelValueString !== null && modelValueString !== void 0 ? modelValueString : valueString : valueString;
	}
	const state$2 = reactive({
		value,
		valueOrigin: value,
		maxlength,
		focus: props$31.focus,
		composing: false,
		selectionStart,
		selectionEnd,
		cursor
	});
	watch(() => state$2.focus, (val) => emit$2("update:focus", val));
	watch(() => state$2.maxlength, (val) => state$2.value = state$2.value.slice(0, val), { immediate: true });
	return {
		fieldRef,
		state: state$2,
		trigger
...
function useValueSync(props$31, state$2, emit$2, trigger, fieldRef) {
	let valueChangeFn = null;
	valueChangeFn = throttle((val) => {
		state$2.value = getValueString(val, props$31.type, state$2.maxlength);
	}, 100);
	watch(() => props$31.modelValue, valueChangeFn);
	watch(() => props$31.value, valueChangeFn);
...
function useAutoFocus(props$31, fieldRef) {
	const { state: userActionState } = useUserAction();
	const needFocus = computed(() => props$31.autoFocus || props$31.focus);
	function focus() {
...
function useEvent(fieldRef, state$2, props$31, trigger, triggerInput, beforeInput) {
	function checkSelection() {
		const field = fieldRef.value;
		if (field && state$2.focus && state$2.selectionStart > -1 && state$2.selectionEnd > -1 && field.type !== "number") {
			field.selectionStart = state$2.selectionStart;
			field.selectionEnd = state$2.selectionEnd;
		}
	}
	function checkCursor() {
		const field = fieldRef.value;
		if (field && state$2.focus && state$2.selectionStart < 0 && state$2.selectionEnd < 0 && state$2.cursor > -1 && field.type !== "number") field.selectionEnd = field.selectionStart = state$2.cursor;
	}
	...
	function initField() {
		...
		const onFocus = function(event) {
			state$2.focus = true;
			trigger("focus", event, { value: state$2.value });
			checkSelection();
			checkCursor();
		};
		const onInput = function(event, force) {
			event.stopPropagation();
			if (isFunction(beforeInput) && beforeInput(event, state$2) === false) return;
			state$2.value = field.value;
			if (!state$2.composing || !props$31.ignoreCompositionEvent) triggerInput(event, {
				value: field.value,
				cursor: getFieldSelectionEnd(field)
			}, force);
		};
		const onBlur = function(event) {
			if (state$2.composing) {
				state$2.composing = false;
				onInput(event, true);
			}
			state$2.focus = false;
			trigger("blur", event, {
		...
		function _onComposition(event) {
			if (!props$31.ignoreCompositionEvent) trigger(event.type, event, { value: event.data });
		}
	}
	watch([() => state$2.selectionStart, () => state$2.selectionEnd], checkSelection);
	watch(() => state$2.cursor, checkCursor);
	watch(() => fieldRef.value, initField);
}
function useField(props$31, rootRef, emit$2, beforeInput) {
	UniViewJSBridgeSubscribe();
	const { fieldRef, state: state$2, trigger } = useBase(props$31, rootRef, emit$2);
	const { triggerInput } = useValueSync(props$31, state$2, emit$2, trigger, fieldRef);
	useAutoFocus(props$31, fieldRef);
	useKeyboard$1(props$31, fieldRef, trigger);
	const { state: scopedAttrsState } = useScopedAttrs();
	useFormField("name", state$2);
	useEvent(fieldRef, state$2, props$31, trigger, triggerInput, beforeInput);
	return {
		fieldRef,
		state: state$2,
		scopedAttrsState,
...
once(() => {
	{
		const ua$1 = navigator.userAgent;
		let osVersion = "";
		const osVersionFind = ua$1.match(/OS\s([\w_]+)\slike/);
		if (osVersionFind) osVersion = osVersionFind[1].replace(/_/g, ".");
		else if (/Macintosh|Mac/i.test(ua$1) && navigator.maxTouchPoints > 0) {
			const versionMatched = ua$1.match(/Version\/(\S*)\b/);
			if (versionMatched) osVersion = versionMatched[1];
		}
		return !!osVersion && parseInt(osVersion) >= 16 && parseFloat(osVersion) < 17.2;
	}
});
var props$24 = /* @__PURE__ */ extend({}, props$14, {
	placeholderClass: {
		type: String,
...
function useCache(props$31, type) {
	if (type.value === "number") {
		const value = typeof props$31.modelValue === "undefined" ? props$31.value : props$31.modelValue;
		const cache = ref(typeof value !== "undefined" && value !== null ? value.toLocaleString() : "");
		watch(() => props$31.modelValue, (value$1) => {
			cache.value = typeof value$1 !== "undefined" && value$1 !== null ? value$1.toLocaleString() : "";
		});
		watch(() => props$31.value, (value$1) => {
			cache.value = typeof value$1 !== "undefined" && value$1 !== null ? value$1.toLocaleString() : "";
		});
		return cache;
	} else return ref("");
...
var input_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { emit: emit$2, expose }) {
		const INPUT_TYPES = [
			"text",
			"number",
			"idcard",
		...
		const type = computed(() => {
			let type$1 = "";
			switch (props$31.type) {
				case "text":
					type$1 = "text";
					if (props$31.confirmType === "search") type$1 = "search";
					break;
				case "idcard":
					type$1 = "text";
					break;
				...
				case "none":
					...
					break;
				default:
					type$1 = INPUT_TYPES.includes(props$31.type) ? props$31.type : "text";
					break;
			}
			return props$31.password ? "password" : type$1;
		});
		const autocomplete = computed(() => {
			const camelizeIndex = AUTOCOMPLETES.indexOf(props$31.textContentType);
			const kebabCaseIndex = AUTOCOMPLETES.indexOf(hyphenate(props$31.textContentType));
			return AUTOCOMPLETES[camelizeIndex !== -1 ? camelizeIndex : kebabCaseIndex !== -1 ? kebabCaseIndex : 0];
		});
		const inputmode = computed(() => {
			if (props$31.inputmode !== void 0) return props$31.inputmode;
			if (INPUT_MODES.includes(props$31.type)) return props$31.type;
			return {
				number: "numeric",
				digit: "decimal",
				idcard: "text"
		...
		let cache = useCache(props$31, type);
		const rootRef = ref(null);
		const { fieldRef, state: state$2, scopedAttrsState, fixDisabledColor, trigger } = useField(props$31, rootRef, emit$2, (event, state$3) => {});
		watch(() => state$2.value, (value) => {
			if (props$31.type === "number" && !(cache.value === "-" && value === "")) cache.value = value.toString();
		});
		watch(() => props$31.maxlength, (length) => {
			length = parseInt(length, 10);
			const realValue = state$2.value.slice(0, length);
			realValue !== state$2.value && (state$2.value = realValue);
		});
		const NUMBER_TYPES = ["number", "digit"];
		const step = computed(() => NUMBER_TYPES.includes(props$31.type) ? props$31.step : "");
		function onKeyUpEnter(event) {
			if (event.key !== "Enter") return;
			const input = event.target;
			event.stopPropagation();
			trigger("confirm", event, { value: input.value });
			!props$31.confirmHold && input.blur();
		}
		expose({ $triggerInput: (detail) => {
			emit$2("update:modelValue", detail.value);
			emit$2("update:value", detail.value);
			state$2.value = detail.value;
		} });
		onMounted(() => {
			const rootElement = rootRef.value;
		...
		return () => {
			let inputNode = props$31.disabled && fixDisabledColor ? createVNode("input", {
				"key": "disabled-input",
				"ref": fieldRef,
				"value": state$2.value,
				"tabindex": "-1",
				"readonly": !!props$31.disabled,
				"type": type.value,
				"maxlength": state$2.maxlength,
				"step": step.value,
				"class": "uni-input-input",
				"style": props$31.cursorColor ? { caretColor: props$31.cursorColor } : {},
				...
				"onInput": withModifiers((event) => {
					...
					if (type.value === "number" && state$2.maxlength > 0 && value.length > state$2.maxlength) {
						if (isPaste(event)) state$2.value = value.slice(0, state$2.maxlength);
						return;
					}
					if (value.length === 0 && event.inputType === "insertText" && event.data === ".") return;
					state$2.value = value;
				}, ["stop"]),
				"disabled": !!props$31.disabled,
				"type": type.value,
				"maxlength": state$2.maxlength,
				"step": step.value,
				"enterkeyhint": props$31.confirmType,
				"pattern": props$31.type === "number" ? "[0-9]*" : void 0,
				"class": "uni-input-input",
				"style": props$31.cursorColor ? { caretColor: props$31.cursorColor } : {},
			...
			return createVNode("uni-input", { "ref": rootRef }, [createVNode("div", { "class": "uni-input-wrapper" }, [withDirectives(createVNode("div", mergeProps(scopedAttrsState.attrs, {
				"style": props$31.placeholderStyle,
				"class": ["uni-input-placeholder", props$31.placeholderClass]
			}), [props$31.placeholder], 16), [[vShow, !(state$2.value.length || cache.value === "-" || cache.value.includes("."))]]), props$31.confirmType === "search" ? createVNode("form", {
				"action": "",
...
const useAttrs = (params = {}) => {
	...
	const instance = getCurrentInstance();
	const attrs$1 = shallowRef({});
	const listeners$1 = shallowRef({});
...
var movable_area_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { slots }) {
		...
		let { setContexts, events: movableAreaEvents } = useMovableAreaState(props$31, rootRef);
		...
		function updateMovableViewContexts() {
			const contexts = [];
			for (let index$4 = 0; index$4 < movableViewItems.length; index$4++) {
				let movableViewItem = movableViewItems[index$4];
		...
		const removeMovableViewContext = (movableViewContext) => {
			const index$4 = originMovableViewContexts.indexOf(movableViewContext);
			if (index$4 >= 0) {
				originMovableViewContexts.splice(index$4, 1);
				updateMovableViewContexts();
			}
		};
...
function useMovableAreaState(props$31, rootRef) {
	const width = ref(0);
	const height = ref(0);
	const gapV = reactive({
		x: null,
	...
	function _updateScale(e$1) {
		if (e$1 && e$1 !== 1) {
			if (props$31.scaleArea) movableViewContexts.forEach(function(item) {
				item._setScale(e$1);
			});
	...
	function _resize() {
		_getWH();
		movableViewContexts.forEach(function(item, index$4) {
			item.setParent();
		});
	}
	function _getWH() {
...
function useTouchtrack(element, method, useCancel) {
	...
	let x1 = 0;
	let y1 = 0;
	const fn = function($event, state$2, x, y) {
		if (method({
...
Spring$1.prototype._solve = function(e$1, t$1) {
	...
	if (o$1 === 0) {
		...
		return {
			...
			dx: function(e$2) {
				const t$2 = Math.pow(Math.E, a$1 * e$2);
				return a$1 * (s + l * e$2) * t$2 + l * t$2;
			}
		};
	}
	if (o$1 > 0) {
		const c = (-n - Math.sqrt(o$1)) / (2 * i);
		const u = (-n + Math.sqrt(o$1)) / (2 * i);
...
Spring$1.prototype.setEnd = function(e$1, n, i) {
...
Spring$1.prototype.reconfigure = function(m, t$1, c) {
	this._m = m;
	this._k = t$1;
	this._c = c;
	if (!this.done()) {
...
var movable_view_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { slots, emit: emit$2 }) {
		const rootRef = ref(null);
		const { setParent } = useMovableViewState(props$31, useCustomEvent(rootRef, emit$2), rootRef);
		onMounted(() => {
			rootRef.value.attachVmProps(props$31);
		});
		return () => {
...
function useMovableViewTransform(rootRef, props$31, _scaleOffset, _scale, maxX, maxY, minX, minY, _translateX, _translateY, _SFA, _FA, _adjustScale, trigger) {
	const dampingNumber = computed(() => {
		let val = Number(props$31.damping);
		return isNaN(val) ? 20 : val;
	});
	const xMove = computed(() => props$31.direction === "all" || props$31.direction === "horizontal");
	const yMove = computed(() => props$31.direction === "all" || props$31.direction === "vertical");
	const xSync = ref(_getPx(props$31.x));
	const ySync = ref(_getPx(props$31.y));
	watch(() => props$31.x, (val) => {
		xSync.value = _getPx(val);
	});
	watch(() => props$31.y, (val) => {
	...
	function _animationTo(x, y, scale, source, r, o$1) {
		...
		if (!props$31.scale) scale = _scale.value;
		let limitXY = _getLimitXY(x, y);
		x = limitXY.x;
		y = limitXY.y;
		if (!props$31.animation) {
	...
	function _setTransform(x, y, scale, source = "", r, o$1) {
		...
		if (o$1 && scale !== _scale.value) trigger("scale", {}, {
			x,
			y,
			scale
		});
...
function useMovableViewInit(props$31, rootRef, trigger, _scale, _oldScale, _isScaling, _translateX, _translateY, _SFA, _FA) {
	const scaleMinNumber = computed(() => {
		let val = Number(props$31.scaleMin);
		return isNaN(val) ? .1 : val;
	});
	const scaleMaxNumber = computed(() => {
		let val = Number(props$31.scaleMax);
		return isNaN(val) ? 10 : val;
	});
	const scaleValueSync = ref(Number(props$31.scaleValue) || 1);
	...
	const { FAandSFACancel, _getLimitXY, _animationTo, _setTransform, _revise, dampingNumber, xMove, yMove, xSync, ySync, _STD } = useMovableViewTransform(rootRef, props$31, _scaleOffset, _scale, maxX, maxY, minX, minY, _translateX, _translateY, _SFA, _FA, _adjustScale, trigger);
	...
	function _setScaleMinOrMax() {
		if (!props$31.scale) return false;
...
function useMovableViewState(props$31, trigger, rootRef) {
	...
	const frictionNumber = computed(() => {
		let val = Number(props$31.friction);
		return isNaN(val) || val <= 0 ? 2 : val;
	});
	const _friction = new Friction$1(1, frictionNumber.value);
	watch(() => props$31.disabled, () => {
		__handleTouchStart();
	});
	const { _updateOldScale, _endScale, _setScale, scaleValueSync, _updateBoundary, _updateOffset, _updateWH, _scaleOffset, minX, minY, maxX, maxY, FAandSFACancel, _getLimitXY, _setTransform, _revise, dampingNumber, xMove, yMove, xSync, ySync, _STD } = useMovableViewInit(props$31, rootRef, trigger, _scale, _oldScale, _isScaling, _translateX, _translateY, _SFA, _FA);
	function __handleTouchStart() {
		if (!_isScaling.value) {
			if (!props$31.disabled) {
				disableScrollBounce({ disable: true });
				FAandSFACancel();
	...
	function __handleTouchMove(event) {
		if (!_isScaling.value && !props$31.disabled && _isTouching) {
			...
			if (!_checkCanMove) {
				...
				let source = "touch";
				if (x < minX.value) if (props$31.outOfBounds) {
					source = "touch-out-of-bounds";
					x = minX.value - _declineX.x(minX.value - x);
				} else x = minX.value;
				else if (x > maxX.value) if (props$31.outOfBounds) {
					source = "touch-out-of-bounds";
					x = maxX.value + _declineX.x(x - maxX.value);
				} else x = maxX.value;
				if (y < minY.value) if (props$31.outOfBounds) {
					source = "touch-out-of-bounds";
					y = minY.value - _declineY.x(minY.value - y);
				} else y = minY.value;
				else if (y > maxY.value) if (props$31.outOfBounds) {
					source = "touch-out-of-bounds";
	...
	function __handleTouchEnd() {
		if (!_isScaling.value && !props$31.disabled && _isTouching) {
			...
			_isTouching = false;
			if (!_checkCanMove && !_revise("out-of-bounds") && props$31.inertia) {
		...
		if (!props$31.outOfBounds && !props$31.inertia) FAandSFACancel();
	}
	function setParent() {
		if (!_isMounted.value) return;
		FAandSFACancel();
		let scale = props$31.scale ? scaleValueSync.value : 1;
		_updateOffset();
		_updateWH(scale);
		_updateBoundary();
...
function createNavigatorOnClick(props$31) {
	return () => {
		if (props$31.openType !== "navigateBack" && !props$31.url) {
			console.error("<navigator/> should have url attribute when using navigateTo, redirectTo, reLaunch or switchTab");
			return;
		}
		const animationDuration = parseInt(props$31.animationDuration);
		const onFail = (error) => {
			console.error(error.errMsg);
		};
		switch (props$31.openType) {
			case "navigate":
				uni.navigateTo({
					url: props$31.url,
					animationType: props$31.animationType || "pop-in",
					animationDuration,
					fail: onFail
				});
				break;
			...
			case "navigateBack":
				uni.navigateBack({
					delta: props$31.delta,
					animationType: props$31.animationType || "pop-out",
					animationDuration,
					fail: onFail
				});
				break;
...
var navigator_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { slots }) {
		...
		const { hovering, binding } = useHover(props$31);
		const onClick = createNavigatorOnClick(props$31);
		onMounted(() => {
			rootRef.value.attachVmProps(props$31);
		});
		return () => {
			const { hoverClass, url } = props$31;
			const hasHoverClass = props$31.hoverClass && props$31.hoverClass !== "none";
			const innerNode = props$31.renderLink ? createVNode("a", {
...
function useState$3(props$31) {
	const state$2 = reactive({
		value: reactive([...props$31.value]),
		height: 34
	});
	watch(() => props$31.value, (val, oldVal) => {
		state$2.value.length = val.length;
		val.forEach((val$1, index$4) => {
			if (val$1 !== state$2.value[index$4]) state$2.value.splice(index$4, 1, val$1);
		});
	});
	return state$2;
}
...
var picker_view_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { slots, emit: emit$2 }) {
		...
		const wrapperRef = ref(null);
		const trigger = useCustomEvent(rootRef, emit$2);
		const state$2 = useState$3(props$31);
		const resizeSensorRef = ref(null);
		const onMountedCallback = () => {
			const resizeSensor = resizeSensorRef.value;
			resizeSensor && (state$2.height = resizeSensor.$el.offsetHeight);
		};
		...
		function getItemIndex(vnode) {
			...
			columnVNodes = columnVNodes.filter((vnode$1) => vnode$1.type !== Comment);
			let index$4 = columnVNodes.indexOf(vnode);
			return index$4 !== -1 ? index$4 : ColumnsPreRef.value.indexOf(vnode);
		}
		const getPickerViewColumn = function(columnInstance) {
			return computed({
				get() {
					const index$4 = getItemIndex(columnInstance.vnode);
					return state$2.value[index$4] || 0;
				},
				set(current) {
					const index$4 = getItemIndex(columnInstance.vnode);
					if (index$4 < 0) return;
					if (state$2.value[index$4] !== current) {
						state$2.value[index$4] = current;
						const value = state$2.value.map((val) => val);
		...
		provide("pickerViewProps", props$31);
		provide("pickerViewState", state$2);
		onMounted(() => {
			const rootElement = rootRef.value;
			Object.defineProperty(rootElement, "value", {
				get() {
				...
				set(value) {
					const columns = rootElement.querySelectorAll("uni-picker-view-column");
					Array.from(columns).forEach((item, index$4) => {
						item.current = value[index$4] || 0;
					});
				}
			});
			rootElement.attachVmProps(props$31);
		...
		return () => {
			...
			return createVNode("uni-picker-view", { "ref": rootRef }, [createVNode(resize_sensor_default, {
				"ref": resizeSensorRef,
				"onResize": ({ height }) => state$2.height = height
...
var Friction = class {
	...
	x(e$1) {
		...
		const t$1 = e$1 === this._dt && this._powDragDt ? this._powDragDt : this._powDragDt = Math.pow(this._drag, e$1);
		this._dt = e$1;
...
var Spring = class {
	...
	_solve(e$1, t$1) {
		...
		if (o$1 === 0) {
			const a$2 = -n / (2 * i);
			const s$1 = e$1;
			const l$1 = t$1 / (a$2 * e$1);
			return {
				x: function(e$2) {
					return (s$1 + l$1 * e$2) * Math.pow(Math.E, a$2 * e$2);
				},
				dx: function(e$2) {
					const t$2 = Math.pow(Math.E, a$2 * e$2);
					return a$2 * (s$1 + l$1 * e$2) * t$2 + l$1 * t$2;
				}
			};
		}
		if (o$1 > 0) {
		...
		return {
			...
			dx: function(e$2) {
				const t$2 = Math.pow(Math.E, a$1 * e$2);
				const n$1 = Math.cos(d * e$2);
				const i$1 = Math.sin(d * e$2);
				return t$2 * (l * d * n$1 - s * d * i$1) + a$1 * t$2 * (l * i$1 + s * n$1);
			}
		};
	}
	x(e$1) {
	...
	setEnd(e$1, t$1, n) {
		...
		if (e$1 !== this._endPosition || !a(t$1, .4)) {
			...
			if (!(this._solution && a(i - e$1, .4) && a(t$1, .4))) {
				this._endPosition = e$1;
				this._solution = this._solve(i - this._endPosition, t$1);
				this._startTime = n;
			}
		}
	}
...
var Scroll = class {
	...
	configuration() {
		const e$1 = this._friction.configuration();
...
function createAnimation$1(scroll, onScroll, onEnd) {
	...
	function startAnimation$1(state$3, scroll$1, onScroll$1, onEnd$1) {
		if (!state$3 || !state$3.cancelled) {
			onScroll$1(scroll$1);
			const isDone = scroll$1.done();
			if (!isDone) {
				if (!state$3.cancelled) state$3.id = requestAnimationFrame(startAnimation$1.bind(null, state$3, scroll$1, onScroll$1, onEnd$1));
			}
			if (isDone && onEnd$1) onEnd$1(scroll$1);
		}
	}
	function cancel(state$3) {
		if (state$3 && state$3.id) cancelAnimationFrame(state$3.id);
		if (state$3) state$3.cancelled = true;
	}
	startAnimation$1(state$2, scroll, onScroll, onEnd);
	return {
		cancel: cancel.bind(null, state$2),
		model: scroll
	};
}
var Scroller = class {
	...
	onTouchEnd(x, y, o$1) {
		...
		this._animation = createAnimation$1(this._scroll, () => {
			const e$1 = Date.now();
			const i = (e$1 - this._scroll._startTime) / 1e3;
			this._position = this._scroll.x(i);
			this.updatePosition();
			const o$2 = this._scroll.dx(i);
...
function useScroller(element, options) {
	...
	function handleTouchEnd(event) {
		if (touchInfo.trackingID !== -1) {
			...
			const delta = findDelta(event);
			if (delta) {
				const listener$2 = touchInfo.listener;
				...
				if (length > 2) for (let i = touchInfo.historyTime.length - 1, time1 = touchInfo.historyTime[i], x = touchInfo.historyX[i], y = touchInfo.historyY[i]; i > 0;) {
					...
					if (time > 30 && time < 50) {
						o$1.x = (x - touchInfo.historyX[i]) / (time / 1e3);
						o$1.y = (y - touchInfo.historyY[i]) / (time / 1e3);
						break;
					}
				}
				...
				touchInfo.historyX = [];
				touchInfo.historyY = [];
				if (listener$2 && listener$2.onTouchEnd) listener$2.onTouchEnd(delta.x, delta.y, o$1);
			}
		}
	}
	return {
...
var picker_view_column_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { slots, emit: emit$2 }) {
		...
		function updatesScroller() {
			if (scroller && !updatesScrollerRequest) {
				...
				nextTick(() => {
					...
					let current = Math.min(state$2.current, state$2.length - 1);
		...
		watch(() => currentRef.value, (current) => {
			if (current !== state$2.current) {
				state$2.current = current;
				updatesScroller();
			}
		});
		watch(() => state$2.current, (current) => currentRef.value = current);
		watch([
		...
		function handleWheel(event) {
			...
			if (Math.abs(deltaY) > 10) {
				oldDeltaY = 0;
				let current = Math.min(state$2.current + (deltaY < 0 ? -1 : 1), state$2.length - 1);
				state$2.current = current = Math.max(current, 0);
		...
		function handleTap({ clientY }) {
			...
			if (!scroller.isScrolling()) {
				...
				const o$1 = indicatorHeight.value / 2;
				if (!(Math.abs(r) <= o$1)) {
					const a$1 = Math.ceil((Math.abs(r) - o$1) / indicatorHeight.value);
					const s = r < 0 ? -a$1 : a$1;
					let current = Math.min(state$2.current + s, state$2.length - 1);
					state$2.current = current = Math.max(current, 0);
					scroller.scrollTo(current * indicatorHeight.value);
				}
			}
		...
		const initScroller = () => {
			...
			const { scroller: scrollerOrigin, handleTouchStart, handleTouchMove, handleTouchEnd } = useScroller(content, {
				...
				onSnap: (index$4) => {
					if (!isNaN(index$4) && index$4 !== state$2.current) state$2.current = index$4;
				}
			});
		...
		let currentCache = state$2.current;
		watch(() => state$2.current, (newCurrent) => {
		...
		return () => {
			const defaultSlots = slots.default && slots.default();
			state$2.length = flatVNode(defaultSlots).length;
...
var progress_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31) {
		const rootRef = ref(null);
		const state$2 = useProgressState(props$31);
		_activeAnimation(state$2, props$31);
		watch(() => state$2.realPercent, (newValue, oldValue) => {
			state$2.strokeTimer && clearInterval(state$2.strokeTimer);
			state$2.lastPercent = oldValue || 0;
			_activeAnimation(state$2, props$31);
		});
		let percentCache = state$2.currentPercent;
		watch(() => state$2.currentPercent, (newPercent) => {
		...
		onMounted(() => {
			...
			Object.defineProperty(rootElement, "percent", {
				...
				set(value) {
					...
					if (props$31.showInfo) rootElement.querySelector(".uni-progress-info").innerText = `${value}%`;
				}
			});
			rootElement.attachVmProps(props$31);
		});
		return () => {
			const { showInfo } = props$31;
			const { outerBarStyle, innerBarStyle, currentPercent } = state$2;
...
function useProgressState(props$31) {
	const currentPercent = ref(0);
	return reactive({
		outerBarStyle: computed(() => `background-color: ${props$31.backgroundColor}; height: ${rpx2px(props$31.strokeWidth)}px;`),
		innerBarStyle: computed(() => {
			const backgroundColor = props$31.color !== PROGRESS_VALUES.activeColor && props$31.activeColor === PROGRESS_VALUES.activeColor ? props$31.color : props$31.activeColor;
			return `width: ${currentPercent.value}%;background-color: ${backgroundColor}`;
		}),
		realPercent: computed(() => {
			if (typeof props$31.percent === "string" && !/^-?\d*\.?\d*$/.test(props$31.percent)) return 0;
			let realValue = parseFloat(props$31.percent);
...
function _activeAnimation(state$2, props$31) {
	if (props$31.active) {
		state$2.currentPercent = props$31.activeMode === PROGRESS_VALUES.activeMode ? 0 : state$2.lastPercent;
		state$2.strokeTimer = setInterval(() => {
			if (state$2.currentPercent + 1 > state$2.realPercent) {
				state$2.currentPercent = state$2.realPercent;
				state$2.strokeTimer && clearInterval(state$2.strokeTimer);
			} else state$2.currentPercent += 1;
		}, parseFloat(props$31.duration));
	} else state$2.currentPercent = state$2.realPercent;
}
...
var radio_group_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { emit: emit$2, slots }) {
		const rootRef = ref(null);
		useProvideRadioGroup(props$31, useCustomEvent(rootRef, emit$2));
		onMounted(() => {
			rootRef.value.attachVmProps(props$31);
		});
		return () => {
...
function useProvideRadioGroup(props$31, trigger) {
	const fields$1 = [];
	onMounted(() => {
	...
	function _resetRadioGroupValue(key, change) {
		fields$1.forEach((value, index$4) => {
			if (index$4 === key) return;
			if (change) setFieldChecked(fields$1[index$4], false);
			else fields$1.forEach((v$1, i) => {
				if (index$4 >= i) return;
				if (fields$1[i].value.radioChecked) setFieldChecked(fields$1[index$4], false);
			});
		});
	}
	return fields$1;
...
var index_x_default$2 = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { slots }) {
		const rootRef = ref(null);
		const radioChecked = ref(props$31.checked);
		const radioValue = ref(props$31.value);
		const initialCheckedValue = props$31.checked;
		function getRadioStyle(checked) {
			...
			if (radioChecked.value) {
				style.backgroundColor = props$31.activeBackgroundColor || props$31.color;
				style.borderColor = props$31.activeBorderColor || style.backgroundColor;
			} else {
				if (props$31.borderColor) style.borderColor = props$31.borderColor;
				if (props$31.backgroundColor) style.backgroundColor = props$31.backgroundColor;
			}
		...
		const _onClick = ($event) => {
			if (props$31.disabled || radioChecked.value) return;
		...
		onMounted(() => {
			...
			Object.defineProperty(rootElement, "checked", {
				...
				set(value) {
					...
					const checkboxInputElement = rootElement.querySelector(".uni-checkbox-input");
					for (const key in style) {
						const value$1 = style[key];
						value$1 && checkboxInputElement.style.setProperty(key, value$1);
					}
				}
			});
			rootElement.attachVmProps(props$31);
		...
		return () => {
			...
			return createVNode("uni-radio", mergeProps(booleanAttrs, {
				...
				"style": { "--HOVER-BD-COLOR": !radioChecked.value ? props$31.activeBorderColor : radioStyle.value.borderColor }
			}), [createVNode("div", {
				"class": ["uni-radio-input", { "uni-radio-input-disabled": props$31.disabled }],
				"style": radioStyle.value
			}, [realCheckValue ? createSvgIconVNode(ICON_PATH_SUCCESS_NO_CIRCLE, props$31.disabled ? "#ADADAD" : props$31.foreColor || props$31.iconColor, 18) : ""], 6), slots.default && slots.default()], 16, ["onClick", "id"]);
		};
	}
});
...
function processClickEvent(node, triggerItemClick) {
	if (node.name && ["a", "img"].includes(node.name) && triggerItemClick) return { onClickCapture: (e$1) => {
		if (node.name === "a") triggerItemClick(e$1, { href: (node.attrs || {}).href });
		else triggerItemClick(e$1, { src: (node.attrs || {}).src });
		e$1.stopPropagation();
		e$1.preventDefault();
...
function normalizeAttrs(tagName, attrs$1) {
	...
	Object.keys(attrs$1).forEach((name) => {
		normalizedAttrs[name] = normalizeValue(tagName, name, attrs$1[name]);
	});
...
var rich_text_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { emit: emit$2 }) {
		...
		function renderVNode() {
			let nodeList = props$31.nodes;
			if (isString(nodeList)) nodeList = parseHtml(props$31.nodes);
			_vnode.value = nodeList2VNode(scopeId, triggerItemClick, nodeList);
		}
		watch(() => props$31.nodes, renderVNode, {
			immediate: true,
			deep: true
		});
		onMounted(() => {
...
var refresher_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { slots }) {
		...
		const rootStyle = computed(() => {
			const style = { backgroundColor: props$31.refresherBackground };
			switch (props$31.refreshState) {
				case "pulling":
					style.height = props$31.refresherHeight + "px";
					break;
				case "refreshing":
					style.height = props$31.refresherThreshold + "px";
		...
		const refreshRotate = computed(() => {
			const route = props$31.refresherHeight / props$31.refresherThreshold;
			return (route > 1 ? 1 : route) * 360;
		});
		return () => {
			const { refreshState, refresherDefaultStyle, refresherThreshold } = props$31;
			return createVNode("div", {
				"ref": rootRef,
...
var scroll_view_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { emit: emit$2, slots, expose }) {
		const rootRef = ref(null);
		const main = ref(null);
		const wrap = ref(null);
		const content = ref(null);
		const trigger = useCustomEvent(rootRef, emit$2);
		const { state: state$2, scrollTopNumber, scrollLeftNumber } = useScrollViewState(props$31);
		const { realScrollX, realScrollY, _scrollLeftChanged, _scrollTopChanged } = useScrollViewLoader(props$31, state$2, scrollTopNumber, scrollLeftNumber, trigger, rootRef, main, content, emit$2);
		...
		const scrollBarClassName = computed(() => {
			...
			if (props$31.showScrollbar === false) className += " uni-scroll-view-scrollbar-hidden";
			return className;
		});
		onMounted(() => {
			const rootElement = rootRef.value;
		...
		return () => {
			const { refresherEnabled, refresherBackground, refresherDefaultStyle, refresherThreshold } = props$31;
			const { refresherHeight, refreshState } = state$2;
...
function useScrollViewState(props$31) {
	const scrollTopNumber = computed(() => {
		return Number(props$31.scrollTop) || 0;
	});
	const scrollLeftNumber = computed(() => {
		return Number(props$31.scrollLeft) || 0;
	});
	return {
		state: reactive({
...
function useScrollViewLoader(props$31, state$2, scrollTopNumber, scrollLeftNumber, trigger, rootRef, main, content, emit$2) {
	...
	const realScrollX = computed(() => {
		if (props$31.direction === "horizontal" || props$31.direction === "all") return true;
		return false;
	});
	const realScrollY = computed(() => {
		if (props$31.direction === "vertical" || props$31.direction === "all") return true;
		return false;
	});
	const upperThresholdNumber = computed(() => {
		let val = Number(props$31.upperThreshold);
		return isNaN(val) ? 50 : val;
	});
	const lowerThresholdNumber = computed(() => {
		let val = Number(props$31.lowerThreshold);
		return isNaN(val) ? 50 : val;
	});
	...
	function _handleScroll($event) {
		...
		trigger("scroll", $event, {
			...
			deltaX: state$2.lastScrollLeft - target.scrollLeft,
			deltaY: state$2.lastScrollTop - target.scrollTop
		});
		if (realScrollY.value) {
			if (target.scrollTop <= upperThresholdNumber.value && state$2.lastScrollTop - target.scrollTop > 0 && $event.timeStamp - state$2.lastScrollToUpperTime > 200) {
				trigger("scrolltoupper", $event, { direction: "top" });
				state$2.lastScrollToUpperTime = $event.timeStamp;
			}
			if (target.scrollTop + target.offsetHeight + lowerThresholdNumber.value >= target.scrollHeight && state$2.lastScrollTop - target.scrollTop < 0 && $event.timeStamp - state$2.lastScrollToLowerTime > 200) {
				trigger("scrolltolower", $event, { direction: "bottom" });
				state$2.lastScrollToLowerTime = $event.timeStamp;
			}
		}
		if (realScrollX.value) {
			if (target.scrollLeft <= upperThresholdNumber.value && state$2.lastScrollLeft - target.scrollLeft > 0 && $event.timeStamp - state$2.lastScrollToUpperTime > 200) {
				trigger("scrolltoupper", $event, { direction: "left" });
				state$2.lastScrollToUpperTime = $event.timeStamp;
			}
			if (target.scrollLeft + target.offsetWidth + lowerThresholdNumber.value >= target.scrollWidth && state$2.lastScrollLeft - target.scrollLeft < 0 && $event.timeStamp - state$2.lastScrollToLowerTime > 200) {
				trigger("scrolltolower", $event, { direction: "right" });
				state$2.lastScrollToLowerTime = $event.timeStamp;
			}
		}
		state$2.lastScrollTop = target.scrollTop;
		state$2.lastScrollLeft = target.scrollLeft;
	}
	function _scrollTopChanged(val) {
		if (realScrollY.value) if (_innerSetScrollTop) _innerSetScrollTop = false;
		else if (props$31.scrollWithAnimation) scrollTo$1(val, "y");
	...
	function _scrollLeftChanged(val) {
		...
		else if (props$31.scrollWithAnimation) scrollTo$1(val, "x");
		...
		if (val) {
			...
			if (element) {
				...
				if (realScrollX.value) {
					...
					if (props$31.scrollWithAnimation) scrollTo$1(x, "x");
					else main.value.scrollLeft = x;
				}
				if (realScrollY.value) {
					let top = elRect.top - mainRect.top;
					let y = main.value.scrollTop + top;
					if (props$31.scrollWithAnimation) scrollTo$1(y, "y");
	...
	function _setRefreshState(_state) {
		if (!props$31.refresherEnabled) return;
		switch (_state) {
			case "refreshing":
				state$2.refresherHeight = props$31.refresherThreshold;
				if (!beforeRefreshing) {
					beforeRefreshing = true;
					trigger("refresherpulling", {}, {
						deltaY: state$2.refresherHeight,
						dy: state$2.refresherHeight
					});
			...
			case "refresherabort":
				...
				state$2.refresherHeight = toUpperNumber = 0;
				if (_state === "restore") {
					triggerAbort = false;
	...
	onMounted(() => {
		...
		_scrollIntoViewChanged(props$31.scrollIntoView);
		...
		let __handleTouchMove = function(event) {
			...
			if (Math.abs(x - touchStart.x) > Math.abs(y - touchStart.y)) if (realScrollX.value) {
				...
				needStop = false;
				if (props$31.refresherEnabled && event.cancelable !== false) event.preventDefault();
			...
			if (props$31.refresherEnabled && state$2.refreshState === "pulling") {
				const dy = y - touchStart.y;
				if (toUpperNumber === 0) toUpperNumber = y;
				if (!beforeRefreshing) {
					state$2.refresherHeight = y - toUpperNumber;
					if (state$2.refresherHeight > 0) {
						triggerAbort = true;
					...
					state$2.refresherHeight = dy + props$31.refresherThreshold;
					triggerAbort = false;
				}
			}
		};
		...
		let __handleTouchEnd = function(event) {
			...
			disableScrollBounce({ disable: false });
			if (state$2.refresherHeight >= props$31.refresherThreshold) _setRefreshState("refreshing");
	...
	onActivated(() => {
		realScrollY.value && (main.value.scrollTop = state$2.lastScrollTop);
		realScrollX.value && (main.value.scrollLeft = state$2.lastScrollLeft);
	});
	...
	watch(() => props$31.scrollIntoView, (val) => {
		_scrollIntoViewChanged(val);
	});
	watch(() => props$31.refresherTriggered, (val) => {
...
var index_x_default$3 = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { emit: emit$2 }) {
		...
		watch(() => props$31.value, (val) => {
			uniSliderElement.value = Number(val);
		});
		const trigger = useCustomEvent(sliderRef, emit$2);
		const state$2 = useSliderState(props$31);
		const { _onInput, _onChange } = useSliderLoader(props$31, sliderRef, trigger);
		onMounted(() => {
			uniSliderElement = sliderRef.value;
			uniSliderElement._initialValue = props$31.value;
			uniSliderElement.init();
			uniSliderElement.attachVmProps(props$31);
		});
		return () => {
			const { setTrackBgColor, setActiveColor, setThumbStyle, thumbTrackStyle, setValueStyle } = state$2;
			...
			}, null, 4), [[vShow, props$31.showValue]])])], 512);
		};
	}
});
function useSliderState(props$31) {
	const _getBgColor = () => {
		return props$31.backgroundColor !== "#e9e9e9" ? props$31.backgroundColor : props$31.color !== "#007aff" ? props$31.color : "#007aff";
	};
	const _getActiveColor = () => {
		const activeColor = props$31.activeBackgroundColor || props$31.activeColor;
		return activeColor !== "#007aff" ? activeColor : props$31.selectedColor !== "#e9e9e9" ? props$31.selectedColor : "#e9e9e9";
	};
	const _getBlockSizeString = () => {
		return Math.min(Math.max(Number(props$31.blockSize), SLIDER_BLOCK_SIZE_MIN_VALUE), SLIDER_BLOCK_SIZE_MAX_VALUE) + "px";
	};
	return {
		...
		setThumbStyle: computed(() => ({
			...
			backgroundColor: props$31.foreColor || props$31.blockColor
		})),
		setValueStyle: computed(() => ({ color: props$31.valueColor }))
	};
}
function useSliderLoader(props$31, sliderRef, trigger) {
...
function useState$2(props$31) {
	return reactive({
		interval: computed(() => {
			const interval = Number(props$31.interval);
			return isNaN(interval) ? 5e3 : interval;
		}),
		duration: computed(() => {
			const duration = Number(props$31.duration);
			return isNaN(duration) ? 500 : duration;
		}),
		displayMultipleItems: computed(() => {
			const displayMultipleItems = Math.round(props$31.displayMultipleItems);
			return isNaN(displayMultipleItems) ? 1 : displayMultipleItems;
		}),
		current: Math.round(props$31.current) || 0,
		currentItemId: props$31.currentItemId,
		userTracking: false
	});
}
function useLayout(props$31, state$2, swiperContexts, slideFrameRef, emit$2, trigger) {
	...
	let animationFrame;
	const swiperEnabled = computed(() => swiperContexts.value.length > state$2.displayMultipleItems);
	const circularEnabled = computed(() => props$31.circular && swiperEnabled.value);
	function checkCircularLayout(index$4) {
		if (!invalid) for (let items = swiperContexts.value, n = items.length, i = index$4 + state$2.displayMultipleItems, r = 0; r < n; r++) {
			const item = items[r];
			const s = Math.floor(index$4 / n) * n + r;
			const l = s + n;
			const c = s - n;
			const u = Math.max(index$4 - (s + 1), s - i, 0);
			const d = Math.max(index$4 - (l + 1), l - i, 0);
			const h$1 = Math.max(index$4 - (c + 1), c - i, 0);
			const p$1 = Math.min(u, d, h$1);
			const position = [
				s,
				l,
				c
			...
			item.updatePosition(position, props$31.vertical);
		}
	}
	function updateViewport(index$4) {
		if (!(Math.floor(2 * viewportPosition) === Math.floor(2 * index$4) && Math.ceil(2 * viewportPosition) === Math.ceil(2 * index$4))) {
			if (circularEnabled.value) checkCircularLayout(index$4);
		}
		const x = props$31.vertical ? "0" : 100 * -index$4 * viewportMoveRatio + "%";
		const y = props$31.vertical ? 100 * -index$4 * viewportMoveRatio + "%" : "0";
		...
		viewportPosition = index$4;
		if (!transitionStart) {
			if (index$4 % 1 === 0) return;
			transitionStart = index$4;
		}
		index$4 -= Math.floor(transitionStart);
		const items = swiperContexts.value;
		if (index$4 <= -(items.length - 1)) index$4 += items.length;
		else if (index$4 >= items.length) index$4 -= items.length;
		index$4 = transitionStart % 1 > .5 || transitionStart < 0 ? index$4 - 1 : index$4;
		trigger("transition", {}, {
			dx: props$31.vertical ? 0 : index$4 * slideFrame.offsetWidth,
			dy: props$31.vertical ? index$4 * slideFrame.offsetHeight : 0
		});
	}
	...
	function normalizeCurrentValue(current) {
		...
		if (!length) return -1;
		const index$4 = (Math.round(current) % length + length) % length;
		if (circularEnabled.value) {
			if (length <= state$2.displayMultipleItems) return 0;
		} else if (index$4 > length - state$2.displayMultipleItems) return length - state$2.displayMultipleItems;
		return index$4;
	}
	...
	function animateFrameFuncProto() {
		...
		if (time <= 0) {
			...
			const item = swiperContexts.value[state$2.current];
			if (item) {
	...
	function animateViewport(current, source, n) {
		...
		const duration = state$2.duration;
		const length = swiperContexts.value.length;
		...
		else if (source === "click") current = current + state$2.displayMultipleItems - 1 < length ? current : 0;
		animating = {
			toPos: current,
	...
	function scheduleAutoplay() {
		...
		const callback = function() {
			...
			if (circularEnabled.value) state$2.current = normalizeCurrentValue(state$2.current + 1);
			else state$2.current = state$2.current + state$2.displayMultipleItems < items.length ? state$2.current + 1 : 0;
			animateViewport(state$2.current, "autoplay", circularEnabled.value ? 1 : 0);
			timer = setTimeout(callback, state$2.interval);
		};
		if (!(invalid || items.length <= state$2.displayMultipleItems)) timer = setTimeout(callback, state$2.interval);
	}
	function resetLayout() {
		cancelSchedule();
		endViewportAnimation();
		const items = swiperContexts.value;
		for (let i = 0; i < items.length; i++) items[i].updatePosition(i, props$31.vertical);
		viewportMoveRatio = 1;
		const slideFrameEl = slideFrameRef.value;
		if (state$2.displayMultipleItems === 1 && items.length) {
		...
		viewportPosition = -2;
		const current = state$2.current;
		if (current >= 0) {
			invalid = false;
			if (state$2.userTracking) {
				updateViewport(position + current - contentTrackViewport);
				contentTrackViewport = current;
			} else {
				updateViewport(current);
				if (props$31.autoplay) scheduleAutoplay();
			}
		} else {
			invalid = true;
			updateViewport(-state$2.displayMultipleItems - 1);
		}
	}
	watch([
		() => props$31.current,
		() => props$31.currentItemId,
		() => [...swiperContexts.value]
	], () => {
		let current = -1;
		if (props$31.currentItemId) {
			for (let i = 0, items = swiperContexts.value; i < items.length; i++) if (items[i].getItemId() === props$31.currentItemId) {
				current = i;
				break;
			}
		}
		if (current < 0) current = Math.round(props$31.current) || 0;
		current = current < 0 ? 0 : current;
		if (state$2.current !== current) {
			currentChangeSource = "";
			state$2.current = current;
		}
	});
	watch([
		...
		() => state$2.displayMultipleItems,
		() => [...swiperContexts.value]
	], resetLayout);
	watch(() => state$2.interval, () => {
		if (timer) {
			cancelSchedule();
			scheduleAutoplay();
		}
	});
	function currentChanged(current, history$1) {
		const source = currentChangeSource;
		currentChangeSource = "";
		const items = swiperContexts.value;
		if (!source) {
			const length = items.length;
			animateViewport(current, "", circularEnabled.value && history$1 + (length - current) % length > length / 2 ? 1 : 0);
		}
		const item = items[current];
		if (item) {
			const currentItemId = state$2.currentItemId = item.getItemId();
			trigger("change", {}, {
				current: state$2.current,
				currentItemId,
				source
	...
	watch(() => state$2.current, (val, oldVal) => {
		currentChanged(val, oldVal);
		emit$2("update:current", val);
	});
	watch(() => state$2.currentItemId, (val) => {
	...
	watch(() => props$31.autoplay && !state$2.userTracking, inintAutoplay);
	inintAutoplay(props$31.autoplay && !state$2.userTracking);
	...
	onMounted(() => {
		...
		function handleTrackMove(data) {
			...
			contentTrackT = Date.now();
			const other = swiperContexts.value.length - state$2.displayMultipleItems;
			function calc$1(val) {
				return .5 - .25 / (val + .5);
			}
			...
			const slideFrameEl = slideFrameRef.value;
			if (props$31.vertical) move(-data.dy / slideFrameEl.offsetHeight, -data.ddy / time);
			else move(-data.dx / slideFrameEl.offsetWidth, -data.ddx / time);
		}
		function handleTrackEnd(isCancel) {
			debouncedTrackEndFallback === null || debouncedTrackEndFallback === void 0 || debouncedTrackEndFallback.cancel();
			state$2.userTracking = false;
			const t$1 = contentTrackSpeed / Math.abs(contentTrackSpeed);
			let n = 0;
			if (!isCancel && Math.abs(contentTrackSpeed) > .2) n = .5 * t$1;
			const current = normalizeCurrentValue(viewportPosition + n);
			if (isCancel) animateViewport(state$2.current, "", 0);
			else {
		...
		useTouchtrack(slideFrameRef.value, (event) => {
			if (props$31.disableTouch) return;
			if (!invalid) {
				if (event.detail.state === "start") {
					state$2.userTracking = true;
				...
				if (state$2.userTracking) {
					if (!userDirectionChecked) {
						...
						const t$1 = Math.abs(event.detail.dx);
						const n = Math.abs(event.detail.dy);
						if (t$1 >= n && props$31.vertical) state$2.userTracking = false;
						else if (t$1 <= n && !props$31.vertical) state$2.userTracking = false;
						if (!state$2.userTracking) {
							if (props$31.autoplay) scheduleAutoplay();
							return;
						}
	...
	function onSwiperDotClick(index$4) {
		animateViewport(state$2.current = index$4, currentChangeSource = "click", circularEnabled.value ? 1 : 0);
	}
	return {
		onSwiperDotClick,
		circularEnabled,
...
var swiper_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { slots, emit: emit$2 }) {
		...
		const state$2 = useState$2(props$31);
		const slidesStyle = computed(() => {
			let style = {};
			if (props$31.nextMargin || props$31.previousMargin) style = props$31.vertical ? {
				left: 0,
				right: 0,
				top: rpx2px(props$31.previousMargin, true),
				bottom: rpx2px(props$31.nextMargin, true)
			} : {
				top: 0,
				bottom: 0,
				left: rpx2px(props$31.previousMargin, true),
				right: rpx2px(props$31.nextMargin, true)
			};
			return style;
		});
		const slideFrameStyle = computed(() => {
			const value = Math.abs(100 / state$2.displayMultipleItems) + "%";
			return {
				width: props$31.vertical ? "100%" : value,
				height: !props$31.vertical ? "100%" : value
			};
		});
		let swiperItems = [];
		...
		function updateSwiperContexts() {
			const contexts = [];
			for (let index$4 = 0; index$4 < swiperItems.length; index$4++) {
				let swiperItem = swiperItems[index$4];
		...
		const removeSwiperContext = function(swiperContext) {
			const index$4 = originSwiperContexts.indexOf(swiperContext);
			if (index$4 >= 0) {
				originSwiperContexts.splice(index$4, 1);
				updateSwiperContexts();
			}
		};
		provide("removeSwiperContext", removeSwiperContext);
		const { onSwiperDotClick, circularEnabled, swiperEnabled } = useLayout(props$31, state$2, swiperContexts, slideFrameRef, emit$2, trigger);
		let createNavigationTsx = () => null;
		createNavigationTsx = useSwiperNavigation(rootRef, props$31, state$2, onSwiperDotClick, swiperContexts, circularEnabled, swiperEnabled);
		onMounted(() => {
			rootRef.value.attachVmProps(props$31);
		});
		return () => {
			...
			return createVNode("uni-swiper", { "ref": rootRef }, [createVNode("div", {
				...
				props$31.indicatorDots && createVNode("div", { "class": ["uni-swiper-dots", props$31.vertical ? "uni-swiper-dots-vertical" : "uni-swiper-dots-horizontal"] }, [swiperContexts.value.map((_, index$4, array) => createVNode("div", {
					"onClick": () => onSwiperDotClick(index$4),
					"class": {
						"uni-swiper-dot": true,
						"uni-swiper-dot-active": index$4 < state$2.current + state$2.displayMultipleItems && index$4 >= state$2.current || index$4 < state$2.current + state$2.displayMultipleItems - array.length
					},
					"style": { background: index$4 === state$2.current ? props$31.indicatorActiveColor : props$31.indicatorColor }
				}, null, 14, ["onClick"]))], 2),
				createNavigationTsx()
			], 512)], 512);
		};
...
var useSwiperNavigation = (rootRef, props$31, state$2, onSwiperDotClick, swiperContext, circularEnabled, swiperEnabled) => {
	let isNavigationAuto = false;
	let prevDisabled = false;
	let nextDisabled = false;
	let hideNavigation = ref(false);
	watchEffect(() => {
		isNavigationAuto = props$31.navigation === "auto";
		hideNavigation.value = props$31.navigation !== true || isNavigationAuto;
		swiperAddMouseEvent();
	});
	watchEffect(() => {
		const swiperItemLength = swiperContext.value.length;
		const notCircular = !circularEnabled.value;
		prevDisabled = state$2.current === 0 && notCircular;
		nextDisabled = state$2.current === swiperItemLength - 1 && notCircular || notCircular && state$2.current + state$2.displayMultipleItems >= swiperItemLength;
	...
	function navigationHover(event, type) {
		...
		if (!target) return;
		target.style.backgroundColor = type === "over" ? props$31.navigationActiveColor : "";
	}
	...
	const createNavigationSVG = () => createSvgIconVNode(ICON_PATH_BACK, props$31.navigationColor, 26);
	...
	const _mousemove = (e$1) => {
		...
		let hide = false;
		if (props$31.vertical) hide = !(clientY - top < height / 3 || bottom - clientY < height / 3);
	...
	function createNavigationTsx() {
		const navigationClass = {
			...
			"uni-swiper-navigation-vertical": props$31.vertical
		};
		if (props$31.navigation) return createVNode(Fragment, null, [createVNode("div", mergeProps({
...
var index_x_default$4 = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { emit: emit$2 }) {
		const rootRef = ref(null);
		const switchChecked = ref(props$31.checked);
		const uniLabel = useSwitchInject(rootRef, props$31, switchChecked);
		const trigger = useCustomEvent(rootRef, emit$2);
		watch(() => props$31.checked, (val) => {
		...
		return () => {
			const { activeBackgroundColor, activeForeColor, backgroundColor, color, foreColor, type } = props$31;
			const booleanAttrs = useBooleanAttr(props$31, "disabled");
			...
			return createVNode("uni-switch", mergeProps({
				...
				"style": thumbStyle
			}, null, 6)], 6), [[vShow, type === "switch"]]), withDirectives(createVNode("div", { "class": "uni-checkbox-input" }, [realCheckValue ? createSvgIconVNode(ICON_PATH_SUCCESS_NO_CIRCLE, props$31.color, 22) : ""], 512), [[vShow, type === "checkbox"]])])], 16, ["id", "onClick"]);
		};
	}
});
function useSwitchInject(rootRef, props$31, switchChecked) {
	const initialCheckedValue = props$31.checked;
	const uniForm = inject(uniFormKey, false);
	const uniLabel = inject(uniLabelKey, false);
	const formField = {
...
function normalizeText(text, { space, decode: decode$1 }) {
	let result = "";
	let isEscape = false;
	...
	setup(props$31, { slots }) {
		...
		return () => {
			...
			if (slots.default) slots.default().forEach((vnode) => {
				if (vnode.shapeFlag & 8 && vnode.type !== Comment) {
					...
					lines = [parseTextIgnoreLinefeed(vnode.children, {
						space: props$31.space,
						decode: props$31.decode
					})];
					const len = lines.length - 1;
					lines.forEach((line, index$4) => {
						if (index$4 === 0 && !line) {} else children.push(createTextVNode(line));
						if (index$4 !== len) children.push(createVNode("br"));
					});
				} else {
			...
			return createVNode("uni-text", {
				"ref": rootRef,
				"selectable": props$31.selectable ? true : null
			}, [createVNode("span", null, children)], 8, ["selectable"]);
		};
	}
});
var props$15 = /* @__PURE__ */ extend({}, props$14, {
	placeholderClass: {
		type: String,
		default: "input-placeholder"
	},
...
var textarea_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { emit: emit$2, expose }) {
		const rootRef = ref(null);
		const wrapperRef = ref(null);
		const { fieldRef, state: state$2, scopedAttrsState, fixDisabledColor, trigger } = useField(props$31, rootRef, emit$2);
		const valueCompute = computed(() => state$2.value.split(LINEFEED));
		const isDone = computed(() => ConfirmTypes.includes(props$31.confirmType));
		...
		watch(() => heightRef.value, (height) => {
			...
			if (props$31.autoHeight) wrapper.style.height = height + "px";
		});
		watch(() => props$31.autoHeight, (autoHeight) => {
			const wrapper = wrapperRef.value;
			if (autoHeight) wrapper.style.height = heightRef.value + "px";
			else wrapper.style.height = "";
		});
		function onResize$3({ height }) {
			heightRef.value = height;
		}
		function onChange$1(event) {
			trigger("change", event, { value: state$2.value });
		}
		function confirm(event) {
			trigger("confirm", event, { value: state$2.value });
		}
		...
		function onKeyUpEnter(event) {
			...
			if (isDone.value) {
				...
				!props$31.confirmHold && textarea.blur();
			}
		}
		setFixMargin();
		expose({ $triggerInput: (detail) => {
		...
		return () => {
			let textareaNode = props$31.disabled && fixDisabledColor ? createVNode("textarea", {
				...
				"style": {
					overflowY: props$31.autoHeight ? "hidden" : "auto",
					...props$31.cursorColor && { caretColor: props$31.cursorColor }
				},
				"onFocus": (event) => event.target.blur()
			}, null, 46, [
				"value",
			...
			return createVNode("uni-textarea", {
				"ref": rootRef,
				"auto-height": props$31.autoHeight
			}, [createVNode("div", {
				"ref": wrapperRef,
				...
				withDirectives(createVNode("div", mergeProps(scopedAttrsState.attrs, {
					"style": props$31.placeholderStyle,
					"class": ["uni-textarea-placeholder", props$31.placeholderClass]
				}), [props$31.placeholder], 16), [[vShow, !state$2.value.length]]),
				createVNode("div", {
					"ref": lineRef,
				...
				createVNode("div", { "class": {
					...
					"uni-textarea-compute-auto-height": props$31.autoHeight
				...
				props$31.confirmType === "search" ? createVNode("form", {
					"action": "",
					"onSubmit": () => false,
					"class": "uni-input-form"
...
var list_view_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { slots, emit: emit$2 }) {
		...
		const { isVertical, state: state$2 } = useListViewState(props$31);
		provide("__listViewIsVertical", isVertical);
		provide("__listViewDefaultItemSize", state$2.defaultItemSize);
		provide("__listViewDefaultHeaderSize", state$2.defaultHeaderSize);
		...
		provide("__listViewUnregisterItem", (status) => {
			const index$4 = childStatus.indexOf(status);
			childStatus.splice(index$4, 1);
			rearrangeDebounce();
		});
		provide("__listViewFirstItemRendered", (status) => {
			state$2.defaultItemSize = status.cachedSize;
			state$2.defaultItemSizeUpdated = true;
		});
		watch(() => {
			return state$2.defaultHeaderSize;
		}, (value) => {
			rearrangeDebounce();
		});
		watch(() => {
			return state$2.defaultItemSize;
		}, () => {
			childStatus.forEach((status) => {
				if (status.cachedSizeUpdated) return;
				status.cachedSize = state$2.defaultItemSize;
			});
			rearrangeDebounce();
		});
		const trigger = useCustomEvent(rootRef, emit$2);
		handleTouchEvent(isVertical, containerRef, props$31, state$2, trigger, emit$2);
		function getOffset() {
			return isVertical.value ? containerRef.value.scrollTop : containerRef.value.scrollLeft;
		}
		function resetContainerSize() {
			const containerEl = containerRef.value;
			state$2.containerSize = isVertical.value ? containerEl.clientHeight : containerEl.clientWidth;
			rearrangeDebounce();
		}
		watch(isVertical, () => {
			resetContainerSize();
		...
		const upperThresholdNumber = computed(() => {
			const val = Number(props$31.upperThreshold);
			return isNaN(val) ? 50 : val;
		});
		const lowerThresholdNumber = computed(() => {
			const val = Number(props$31.lowerThreshold);
			return isNaN(val) ? 50 : val;
		});
		const scrollTopNumber = computed(() => {
			return Number(props$31.scrollTop) || 0;
		...
		function _rearrange() {
			rearrange(visibleVNode, containerRef, isVertical, state$2);
		}
		function _shouldRearrange() {
			return shouldRearrange(containerRef, isVertical, state$2);
		}
		const containerStyle = computed(() => {
			return `${props$31.direction === "none" ? "overflow: hidden;" : props$31.direction === "all" ? "overflow: auto;" : isVertical.value ? "overflow: hidden auto;" : "overflow: auto hidden;"}scroll-behavior: ${props$31.scrollWithAnimation ? "smooth" : "auto"};`;
		});
		const contentStyle = computed(() => {
			return `position: relative; ${isVertical.value ? "height" : "width"}: ${state$2.totalSize}px;`;
		});
		const visibleStyle = computed(() => {
			return `position: absolute; ${isVertical.value ? "width" : "height"}: 100%; ${isVertical.value ? "top" : "left"}: ${state$2.placehoderSize}px;`;
		});
		...
		return () => {
			...
			return createVNode("uni-list-view", {
				...
				"ref": containerRef,
				"class": `uni-list-view-container ${props$31.showScrollbar === false ? "uni-list-view-scrollbar-hidden" : ""}`,
			...
			}, [visibleVNode], 4)], 6), createVNode(resize_sensor_default, { "onResize": onResize$3 }, null, 8, ["onResize"])], 512);
		};
	}
});
function useListViewState(props$31) {
	const isVertical = computed(() => {
		return props$31.direction !== "horizontal";
	});
	return {
		state: reactive({
			defaultItemSize: 40,
...
function shouldRearrange(containerRef, isVertical, state$2) {
	const offset = isVertical.value ? containerRef.value.scrollTop : containerRef.value.scrollLeft;
	const loadScreenThresholdSize = state$2.containerSize * state$2.loadScreenThreshold;
	const rearrangeOffsetMin = state$2.lastRenderOffsetMin + loadScreenThresholdSize;
	const rearrangeOffsetMax = state$2.lastRenderOffsetMax - loadScreenThresholdSize;
	return offset < rearrangeOffsetMin || offset > rearrangeOffsetMax;
}
function rearrange(visibleVNode, containerRef, isVertical, state$2) {
	if (!visibleVNode) return;
	const containerEl = containerRef.value;
	if (!containerEl) return;
	const offset = isVertical.value ? containerEl.scrollTop : containerEl.scrollLeft;
	const offsetMin = Math.max(offset - state$2.containerSize * state$2.cacheScreenCount, 0);
	const offsetMax = Math.max(offset + state$2.containerSize * (state$2.cacheScreenCount + 1), offsetMin + 1);
	state$2.lastRenderOffsetMin = offsetMin;
	state$2.lastRenderOffsetMax = offsetMax;
	let tempTotalSize = 0;
	...
	function callback(child) {
		...
		if (childType === "StickySection") {
			...
			traverseStickySection(child, (child$1) => {
				var _child$component3, _child$component4;
				const childType$1 = (_child$component3 = child$1.component) === null || _child$component3 === void 0 ? void 0 : _child$component3.type.name;
				const status$1 = (_child$component4 = child$1.component) === null || _child$component4 === void 0 || (_child$component4 = _child$component4.exposed) === null || _child$component4 === void 0 ? void 0 : _child$component4.__listViewChildStatus;
				if (childType$1 === "StickyHeader") {
					const { cachedSize, cachedSizeUpdated } = status$1;
					if (cachedSizeUpdated && cachedSize > 0 && !state$2.defaultHeaderSizeUpdated) {
						state$2.defaultHeaderSize = cachedSize;
						state$2.defaultHeaderSizeUpdated = true;
					}
					tempTotalSize += cachedSize || state$2.defaultHeaderSize;
					tempVisibleSize += cachedSize;
				} else if (childType$1 === "ListItem") {
					const { cachedSize, cachedSizeUpdated } = status$1;
					if (cachedSizeUpdated && cachedSize > 0 && !state$2.defaultItemSizeUpdated) {
						state$2.defaultItemSize = cachedSize;
						state$2.defaultItemSizeUpdated = true;
					}
					const itemSize = cachedSize || state$2.defaultItemSize;
					tempTotalSize += itemSize;
					if (!start && tempTotalSize > offsetMin) start = true;
					if (start && !end) {
						tempVisibleSize += itemSize;
						status$1.visible.value = true;
					} else {
						status$1.visible.value = false;
	...
	state$2.totalSize = tempTotalSize;
	state$2.visibleSize = tempVisibleSize;
	state$2.placehoderSize = tempPlaceholderSize;
}
function handleTouchEvent(isVertical, containerRef, props$31, state$2, trigger, emit$2) {
	let beforeRefreshing = false;
	let triggerAbort = false;
	let toUpperNumber = 0;
	let touchStart = {
...
var list_item_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { slots, expose, attrs: attrs$1 }) {
		if (attrs$1.slot === "refresher") return () => {
...
var sticky_section_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { slots, expose }) {
		const rootRef = ref(null);
		const isVertical = inject("__listViewIsVertical");
		const placeholderSize = ref(0);
		const style = computed(() => {
			const padding = props$31.padding;
		...
		expose({ __listViewChildStatus: {
			type: "StickySection",
			headSize: computed(() => {
				return isVertical ? props$31.padding[0] : props$31.padding[3];
			}),
			tailSize: computed(() => {
				return isVertical ? props$31.padding[2] : props$31.padding[1];
			}),
			placeholderSize
		} });
		onMounted(() => {
...
var sticky_header_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { slots, expose }) {
		...
		const style = computed(() => {
			return {
				paddingTop: props$31.padding[0] + "px",
				paddingRight: props$31.padding[1] + "px",
				paddingBottom: props$31.padding[2] + "px",
				paddingLeft: props$31.padding[3] + "px",
				top: 0 - props$31.padding[0] + "px"
			};
		});
		const status = {
			type: "StickyHeader",
...
var page_container_default = /* @__PURE__ */ defineComponent({
	name: "page-container",
	rootElement: {
	...
	setup(__props, { emit: __emit }) {
		...
		const overlayStyleMap = computed(() => {
			const styleObj = {
				"z-index": props$31.zIndex,
				"transition-duration": props$31.duration + "ms"
			};
			if (isEntered.value) {
				styleObj["opacity"] = "1";
		...
		const innerStyleMap = computed(() => {
			const styleObj = {
				"z-index": props$31.zIndex + 1,
				"transition-duration": props$31.duration + "ms"
			};
			if (translateValue.value != 0 && isDragging) {
				let transformValue = "";
				switch (props$31.position) {
					case "bottom":
					case "top":
			...
			} else if (translateValue.value != 0 && !isDragging) styleObj["transition"] = `transform ${props$31.duration}ms ease`;
			return styleObj;
		});
		const popupClasses = computed(() => {
			const classes = [];
			if (props$31.position != null) classes.push(`uni-page-container-popup-${props$31.position}`);
			if (props$31.round) classes.push("uni-page-container-popup-round");
		...
		function onTouchStart(e$1) {
			if (!props$31.closeOnSlideDown) return;
			if (e$1.touches.length > 0) {
				const { clientX, clientY } = e$1.touches[0];
				touchStartX = clientX;
				touchStartY = clientY;
		...
		onMounted(() => {
			if (props$31.show) openContainer();
		});
		onBeforeUnmount(() => {
			clearTransitionTimer();
		});
...
function useLoadingStyle(targetElement, bold) {
	...
	let observer = null;
	const calculateLoadingWidth = (element, bold$1) => {
		const { width, height } = element.getBoundingClientRect();
		const coefficient = bold$1 ? 2 : 1;
	...
	const setupObserver = (cb) => {
		...
		if (!el) return;
		observer = new ResizeObserver((entries$1) => {
			cb(el);
		});
		observer.observe(el);
	};
...
var index_x_default$1 = /* @__PURE__ */ defineComponent({
	name: "loading",
	rootElement: {
	...
	setup(__props) {
		...
		const loadingStyle = reactive(useLoadingStyle(LoadingRef, computed(() => props$31.bold)));
		return (_ctx, _cache) => {
			const _component_view = view_default;
			return openBlock(), createBlock(_component_view, {
				...
				default: withCtx(() => [createVNode(_component_view, {
					class: normalizeClass(["__uni-loading__ __loading-4-3__", { "__uni-loading__paused": props$31.paused }]),
...
function normalizeEvent(vm, id$2) {
	if (!id$2) id$2 = vm.id;
	if (!id$2) return;
	return vm.$options.name.toLowerCase() + "." + id$2;
}
...
function useContextInfo(_id) {
	...
	const id$2 = _id || vm.id || `context${index$2++}`;
	onMounted(() => {
		const el = vm.$el;
		el.__uniContextInfo = {
			id: id$2,
...
function initHooks(options, instance, publicThis) {
	...
	if (mpType === "page") {
		...
		try {
			...
			console.error(e$1.message + LINEFEED + e$1.stack);
		}
	}
}
...
function usePopupStyle(props$31) {
	...
	const popupStyle = computed(() => {
		...
		const popover = props$31.popover;
...
function useKeyboard() {
	...
	const onKeyup = (evt) => {
		...
		const res = KEYS.find((key$1) => KEY_MAPS[key$1].indexOf(evt.key) !== -1);
...
function ensureRoot(id$2) {
	let rootEl = document.getElementById(id$2);
	if (!rootEl) {
...
function usePopup(props$31, { onEsc, onEnter }) {
	const visible = ref(props$31.visible);
	const { key, disable } = useKeyboard();
	watch(() => props$31.visible, (value) => visible.value = value);
...
function getTabBarScrollPosition(id$2) {
	return positionStore[id$2];
}
function saveTabBarScrollPosition(id$2) {
	if (typeof window !== "undefined") positionStore[id$2] = {
...
function initHistory() {
	...
	const history$1 = __UNI_FEATURE_ROUTER_MODE__ === "history" ? createWebHistory(routerBase) : createWebHashHistory(routerBase);
	history$1.listen((_to, _from, info) => {
...
function formatTime(val) {
	...
	const h$1 = Math.floor(val / 3600);
	const m = Math.floor(val % 3600 / 60);
	const s = Math.floor(val % 3600 % 60);
	const hStr = (h$1 < 10 ? "0" : "") + h$1;
	const mStr = (m < 10 ? "0" : "") + m;
	const sStr = (s < 10 ? "0" : "") + s;
	let str = mStr + ":" + sStr;
...
function useGesture(props$31, videoState, videoRef, fullscreenState) {
	const state$2 = reactive({
		seeking: false,
		gestureType: "none",
		volumeOld: 0,
	...
	const changeToastThin = () => {
		if (state$2.gestureType !== "none" && changeToastThinTimer != null) return;
		changeToastThinTimer = setTimeout(() => {
			state$2.toastThin = true;
		}, 500);
	};
	...
	function onTouchmove(event) {
		...
		const gestureType = state$2.gestureType;
		...
		if (Math.abs(pageX - origin.x) > Math.abs(pageY - origin.y)) {
			if (!props$31.enableProgressGesture) {
				state$2.gestureType = "stop";
				return;
			}
			state$2.gestureType = "progress";
			state$2.currentTimeOld = state$2.currentTimeNew = video.currentTime;
			if (!fullscreenState.fullscreen) stop();
		} else {
			if (!props$31.pageGesture && !props$31.vslideGesture) {
				state$2.gestureType = "stop";
				return;
			}
			changeToastThin();
			state$2.gestureType = "volume";
			state$2.volumeOld = video.volume;
			if (!fullscreenState.fullscreen) stop();
		}
	}
	function onTouchend(event) {
		const video = videoRef.value;
		if (state$2.gestureType !== "none" && state$2.gestureType !== "stop") {
			event.stopPropagation();
			event.preventDefault();
		}
		if (state$2.gestureType === "progress" && state$2.currentTimeOld !== state$2.currentTimeNew) video.currentTime = state$2.currentTimeNew;
		state$2.gestureType = "none";
	}
	function changeProgress(x) {
		const duration = videoState.currentDuration;
		let currentTimeNew = x / 600 * duration + state$2.currentTimeOld;
		if (currentTimeNew < 0) currentTimeNew = 0;
		else if (currentTimeNew > duration) currentTimeNew = duration;
		state$2.currentTimeNew = currentTimeNew;
	}
	function changeVolume(y) {
		const video = videoRef.value;
		const valueOld = state$2.volumeOld;
		let value;
...
function useFullscreen(trigger, containerRef, videoRef, userActionState, rootRef) {
	const state$2 = reactive({ fullscreen: false });
...
function useVideo(props$31, attrs$1, trigger) {
	const videoRef = ref(null);
	const src = computed(() => getRealPath(props$31.src));
	const muted = computed(() => props$31.muted === "true" || props$31.muted === true);
	const state$2 = reactive({
		start: false,
		src,
		playing: false,
	...
	watch(() => state$2.buffered, (buffered) => {
	...
	watch([() => state$2.duration, () => props$31.duration], () => {
		let _duration = Number(props$31.duration);
		isNaN(_duration) && (_duration = 0);
		state$2.currentDuration = _duration > 0 ? _duration : state$2.duration;
	});
	function onDurationChange({ target }) {
		state$2.duration = target.duration;
	}
	function onLoadedMetadata($event) {
		const initialTime = Number(props$31.initialTime) || 0;
	...
	function onProgress($event) {
		...
		if (buffered.length) state$2.buffered = buffered.end(buffered.length - 1) / video.duration * 100;
	}
	...
	function onTimeUpdate($event) {
		const video = $event.target;
		if (!state$2.pauseUpdatingCurrentTime) state$2.currentTime = video.currentTime;
	...
	function toggle() {
		const video = videoRef.value;
		if (state$2.playing) video.pause();
		else video.play();
	}
	function play() {
		const video = videoRef.value;
...
function useControls(props$31, videoState, seek, seeking) {
	const progressRef = ref(null);
	const ballRef = ref(null);
	const centerPlayBtnShow = computed(() => props$31.showCenterPlayBtn && !videoState.start);
	...
	const state$2 = reactive({
		...
		centerPlayBtnShow,
		controlsShow: computed(() => !centerPlayBtnShow.value && props$31.controls && controlsVisible.value),
		controlsVisible
	});
	...
	function toggleControls() {
		state$2.controlsVisible = !state$2.controlsVisible;
	}
	let hideTiming;
	function autoHideStart() {
		hideTiming = setTimeout(() => {
			state$2.controlsVisible = false;
		}, 3e3);
	}
	...
	watch(() => state$2.controlsShow && videoState.playing && !state$2.controlsTouching, (val) => {
		if (val) autoHideStart();
		else autoHideEnd();
	});
	onMounted(() => {
		const passiveOptions$4 = passive(false);
		let originX;
		let originY;
		let moveOnce = true;
		let originProgress;
		...
		function touchend(event) {
			state$2.controlsTouching = false;
			if (state$2.touching) {
				ball.removeEventListener("touchmove", touchmove$1, passiveOptions$4);
				if (!moveOnce) {
					event.preventDefault();
					event.stopPropagation();
					seek(videoState.currentDuration * videoState.progress / 100);
				}
				state$2.touching = false;
			}
		}
		ball.addEventListener("touchstart", (event) => {
			state$2.controlsTouching = true;
			const toucher = event.targetTouches[0];
			originX = toucher.pageX;
			originY = toucher.pageY;
			originProgress = videoState.progress;
			moveOnce = true;
			state$2.touching = true;
			ball.addEventListener("touchmove", touchmove$1, passiveOptions$4);
		});
...
function useDanmu(props$31, videoState) {
	const danmuRef = ref(null);
	const state$2 = reactive({ enable: Boolean(props$31.enableDanmu) });
	let danmuIndex = {
		time: 0,
		index: -1
	};
	const danmuList = isArray(props$31.danmuList) ? JSON.parse(JSON.stringify(props$31.danmuList)) : [];
	danmuList.sort(function(a$1, b) {
		return (a$1.time || 0) - (b.time || 0);
	});
	function toggleDanmu() {
		state$2.enable = !state$2.enable;
	}
	function updateDanmu(event) {
		...
		if (currentTime > oldDanmuIndex.time) for (let index$4 = oldDanmuIndex.index + 1; index$4 < danmuList.length; index$4++) {
			const element = danmuList[index$4];
			if (currentTime >= (element.time || 0)) {
				newDanmuIndex.index = index$4;
				if (videoState.playing && state$2.enable) playDanmu(element);
			} else break;
		}
		else if (currentTime < oldDanmuIndex.time) for (let index$4 = oldDanmuIndex.index - 1; index$4 > -1; index$4--) if (currentTime <= (danmuList[index$4].time || 0)) newDanmuIndex.index = index$4 - 1;
		else break;
...
var video_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { emit: emit$2, attrs: attrs$1, slots }) {
		const rootRef = ref(null);
		const containerRef = ref(null);
		const trigger = useCustomEvent(rootRef, emit$2);
		const { state: userActionState } = useUserAction();
		...
		const { videoRef, state: videoState, play, pause, stop, seek, playbackRate, toggle, onDurationChange, onLoadedMetadata, onProgress, onWaiting, onVideoError, onPlay, onPause, onEnded, onTimeUpdate } = useVideo(props$31, attrs$1, trigger);
		const { state: danmuState, danmuRef, updateDanmu, toggleDanmu, sendDanmu } = useDanmu(props$31, videoState);
		...
		const { state: gestureState, onTouchstart, onTouchend, onTouchmove } = useGesture(props$31, videoState, videoRef, fullscreenState);
		const { state: controlsState, progressRef, ballRef, clickProgress, toggleControls, autoHideEnd, autoHideStart } = useControls(props$31, videoState, seek, (currentTimeNew) => {
		...
		return () => {
			return createVNode("uni-video", {
				...
				createVNode("video", mergeProps({
					"ref": videoRef,
					"style": { "object-fit": props$31.objectFit },
					"muted": !!props$31.muted,
					"loop": !!props$31.loop,
					"src": videoState.src,
					"poster": props$31.poster,
					"autoplay": !!props$31.autoplay
				}, videoAttrs.value, {
					"class": {
				...
				withDirectives(createVNode("div", {
					...
					createVNode("div", { "class": "uni-video-controls" }, [
						...
						}, null, 10, ["onClick"]), [[vShow, props$31.showPlayBtn]]),
						withDirectives(createVNode("div", { "class": "uni-video-current-time" }, [formatTime(videoState.currentTime)], 512), [[vShow, props$31.showProgress]]),
						withDirectives(createVNode("div", {
							"ref": progressRef,
						...
						], 2)], 8, ["onClick"]), [[vShow, props$31.showProgress]]),
						withDirectives(createVNode("div", { "class": "uni-video-duration" }, [formatTime(videoState.currentDuration)], 512), [[vShow, props$31.showProgress]])
					]),
					withDirectives(createVNode("div", {
						"class": {
							"uni-video-icon": true,
					...
					}, null, 10, ["onClick"]), [[vShow, props$31.danmuBtn]]),
					withDirectives(createVNode("div", {
						"class": {
							"uni-video-icon": true,
					...
					}, null, 10, ["onClick"]), [[vShow, props$31.showFullscreenBtn]])
...
var index_x_default$5 = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { emit: emit$2 }) {
		...
		const renderIframe = () => {
			...
			watchEffect(() => {
				iframe.src = getRealPath(props$31.src);
			});
...
function createCallout(maps$1) {
	...
	if (!getIsAMap() && !getIsBMap()) {
		const overlay = new (maps$1.OverlayView || maps$1.Overlay)();
...
function loadMaps(libraries, callback) {
	...
	const callbacks$1 = callbacksMap[mapInfo.type] = callbacksMap[mapInfo.type] || [];
...
function translateCoordinateSystem(type, coords, skip) {
	...
	if (type && type.toUpperCase() === "WGS84" || wgs84Map.includes(mapInfo.type) || skip) return Promise.resolve(coords);
	if (mapInfo.type === MapType.QQ) return new Promise((resolve) => {
	...
	if (mapInfo.type === MapType.AMAP) return new Promise((resolve) => {
		loadMaps([], () => {
...
function useMarkerLabelStyle(id$2) {
	const className = "uni-map-marker-label-" + id$2;
...
var MapMarker_default = /* @__PURE__ */ defineSystemComponent({
	...
	props: props$10,
	setup(props$31) {
		const id$2 = String(!isNaN(Number(props$31.id)) ? props$31.id : "");
		const onMapReady = inject("onMapReady");
		const updateMarkerLabelStyle = useMarkerLabelStyle(id$2);
		let marker;
		...
		onMapReady((map, maps$1, trigger) => {
                ...
                <div>`;
					...
					if (calloutOpt.content || title) {
						...
						if (callout) callout.setOption(calloutStyle);
							...
							const callback = () => {
								if (id$2 !== "") trigger("callouttap", {}, { markerId: Number(id$2) });
							};
							...
							callout.div.onclick = function($event) {
								if (id$2 !== "") trigger("callouttap", $event, { markerId: Number(id$2) });
								$event.stopPropagation();
								$event.preventDefault();
							};
			...
			function addMarker(props$32) {
				if (!getIsBMap()) marker = new maps$1.Marker({
					map,
					flat: true,
					autoRotation: false
				});
				updateMarker(props$32);
				const MapsEvent = maps$1.event || maps$1.Event;
				if (getIsBMap()) {} else MapsEvent.addListener(marker, "click", () => {
					...
					if (id$2) trigger("markertap", {}, {
						markerId: Number(id$2),
						latitude: props$32.latitude,
						longitude: props$32.longitude
					});
				});
			}
			addMarker(props$31);
			watch(props$31, updateMarker);
		});
		if (id$2) {
			...
			const context = {
				...
				translate(data) {
					onMapReady((map, maps$1, trigger) => {
						...
						const a$1 = marker.getPosition();
						const b = new maps$1.LatLng(destination.latitude, destination.longitude);
						const speed = maps$1.geometry.spherical.computeDistanceBetween(a$1, b) / 1e3 / ((typeof duration === "number" ? duration : 1e3) / (1e3 * 60 * 60));
						const MapsEvent = maps$1.event || maps$1.Event;
...
var MapPolyline_default = /* @__PURE__ */ defineSystemComponent({
	...
	setup(props$31) {
		...
		onMapReady((map, maps$1) => {
			...
			addPolyline(props$31);
			watch(props$31, updatePolyline);
		});
	...
	setup(props$31) {
		...
		onMapReady((map, maps$1) => {
			...
			function addCircle(option) {
				...
				if (getIsBMap()) {
					let pt = new maps$1.Point(circleOptions.center[0], circleOptions.center[1]);
...
var MapControl_default = /* @__PURE__ */ defineSystemComponent({
	...
	setup(props$31) {
		const imgPath = computed(() => getRealPath(props$31.iconPath));
		const positionStyle = computed(() => {
			let positionStyle$1 = `top:${props$31.position.top || 0}px;left:${props$31.position.left || 0}px;`;
			if (props$31.position.width) positionStyle$1 += `width:${props$31.position.width}px;`;
			if (props$31.position.height) positionStyle$1 += `height:${props$31.position.height}px;`;
			return positionStyle$1;
		});
		const handleClick = ($event) => {
			if (props$31.clickable) props$31.trigger("controltap", $event, { controlId: props$31.id });
		};
		return () => {
...
var initInnerAudioContextEventOnce = /* @__PURE__ */ once(() => {
	...
	innerAudioContextOffEventNames.forEach((eventName) => {
		InnerAudioContext.prototype[eventName] = function(callback) {
			...
			var index$4 = handle.indexOf(callback);
			if (index$4 >= 0) handle.splice(index$4, 1);
		};
	});
});
var InnerAudioContext = class {
	constructor() {
		...
		audio.addEventListener("loadedmetadata", () => {
			var startTime$1 = Number(this.startTime) || 0;
			if (startTime$1 > 0) audio.currentTime = startTime$1;
		});
		var stopEventNames = [
			"canplay",
			"pause",
...
const getSystemInfoSync = /* @__PURE__ */ defineSyncApi("getSystemInfoSync", () => {
	...
	_initBrowserInfo = true;
	const { ua: ua$1, browserName, browserVersion, osname, osversion } = browserInfo;
...
const getClipboardData = /* @__PURE__ */ defineAsyncApi(API_GET_CLIPBOARD_DATA, async (_, { resolve, reject }) => {
	...
	const { t: t$1 } = useI18n();
	try {
		resolve({ data: await navigator.clipboard.readText() });
	} catch (error) {
		_getClipboardData(resolve, () => {
			reject(`${error} ${t$1("uni.getClipboardData.fail")}`);
		});
	}
});
...
const onThemeChange = /* @__PURE__ */ defineOnApi(ON_THEME_CHANGE, () => {
	UniServiceJSBridge.on(ON_THEME_CHANGE, themeChangeCallBack);
});
const offThemeChange = /* @__PURE__ */ defineOffApi(OFF_THEME_CHANGE, () => {
	UniServiceJSBridge.off(ON_THEME_CHANGE, themeChangeCallBack);
});
var THEME_CALLBACK = [];
const onHostThemeChange = /* @__PURE__ */ defineSyncApi(ON_HOST_THEME_CHANGE, (callback) => {
	const onHostThemeChangeCallback = (res) => {
		callback({ hostTheme: res.theme });
	};
	const index$4 = THEME_CALLBACK.push([callback, onHostThemeChangeCallback]) - 1;
	UniServiceJSBridge.on(ON_THEME_CHANGE, onHostThemeChangeCallback);
	return index$4;
});
const offHostThemeChange = /* @__PURE__ */ defineSyncApi(OFF_HOST_THEME_CHANGE, (callbackId) => {
...
const getStorageInfoSync = /* @__PURE__ */ defineSyncApi("getStorageInfoSync", () => {
	...
	const keys = [];
	let currentSize = 0;
	for (let index$4 = 0; index$4 < length; index$4++) {
		const key = localStorage.key(index$4);
...
function createInput_default({ count, sourceType, type, extension }) {
	...
	inputEl.accept = extension.map((item) => {
		if (type !== ALL) {
			...
			return `${type}/${MIMEType_default[type][MIMEKey] || MIMEKey}`;
		} else {
			if (isWXEnv()) return ".";
...
var fileInput = null;
const chooseFile = /* @__PURE__ */ defineAsyncApi(API_CHOOSE_FILE, ({ count, sourceType, type, extension }, { resolve, reject }) => {
	...
	fileInput.click();
	if (!getInteractStatus()) console.warn(t$1("uni.chooseFile.notUserActivation"));
}, ChooseFileProtocol, ChooseFileOptions);
var imageInput = null;
const chooseImage = /* @__PURE__ */ defineAsyncApi(API_CHOOSE_IMAGE, ({ count, sourceType, extension }, { resolve, reject }) => {
	initI18nChooseFileMsgsOnce();
	const { t: t$1 } = useI18n();
	if (imageInput) {
...
var ImageView_default = /* @__PURE__ */ defineSystemComponent({
	...
	setup(props$31) {
		const state$2 = reactive({ direction: "none" });
		let scale = 1;
		let imgWidth = 0;
		let imgHeight = 0;
		let width = 0;
		...
		function onTouchEnd(event) {
			...
			if (horizontal && vertical) state$2.direction = "all";
			else if (horizontal) state$2.direction = "horizontal";
			else if (vertical) state$2.direction = "vertical";
			else state$2.direction = "none";
			checkDirection(event);
		}
		function checkDirection(event) {
			if (state$2.direction === "all" || state$2.direction === "horizontal") event.stopPropagation();
		}
		return () => {
			const viewStyle = {
				position: "absolute",
...
function getIndex(props$31) {
	let index$4 = typeof props$31.current === "number" ? props$31.current : props$31.urls.indexOf(props$31.current);
	index$4 = index$4 < 0 ? 0 : index$4;
	return index$4;
}
var ImagePreview_default = /* @__PURE__ */ defineSystemComponent({
	...
	setup(props$31, { emit: emit$2 }) {
		...
		const rootRef = ref(null);
		const indexRef = ref(getIndex(props$31));
		watch(() => props$31.current, () => indexRef.value = getIndex(props$31));
		...
		return () => {
			...
			}, _isSlot$1(_slot = props$31.urls.map((src) => createVNode(swiper_item_default, null, { default: () => [createVNode(ImageView_default, { "src": src }, null, 8, ["src"])] }))) ? _slot : {
				default: () => [_slot],
				_: 1
...
const chooseVideo = /* @__PURE__ */ defineAsyncApi(API_CHOOSE_VIDEO, ({ sourceType, extension }, { resolve, reject }) => {
	...
	videoInput.addEventListener("change", function(event) {
		...
		const video = document.createElement("video");
		if (video.onloadedmetadata !== void 0) {
			const filePath$1 = fileToUrl(file);
			video.onloadedmetadata = function() {
				revokeObjectURL(filePath$1);
...
var RequestTask = class {
	...
	offHeadersReceived(callback) {
		...
		if (typeof callback === "function") {
			this._requestOnHeadersReceiveCallbacks.forEach((cb, id$2) => {
				if (cb === callback) {
					this._requestOnHeadersReceiveCallbacks.delete(id$2);
	...
	offChunkReceived(callback) {
		...
		if (typeof callback === "function") {
			this._requestOnChunkReceiveCallbacks.forEach((cb, id$2) => {
				if (cb === callback) {
					this._requestOnChunkReceiveCallbacks.delete(id$2);
...
var DownloadTask = class {
	...
	offProgressUpdate(callback) {
		const index$4 = this._callbacks.indexOf(callback);
		if (index$4 >= 0) this._callbacks.splice(index$4, 1);
	}
	abort() {
		if (this._xhr) {
			this._xhr.abort();
...
const uploadFile = /* @__PURE__ */ defineTaskApi(API_UPLOAD_FILE, ({ url, file, filePath, name, files: files$1, header = {}, formData = {}, timeout = __uniConfig.networkTimeout.uploadFile }, { resolve, reject }) => {
	timeout = timeout == null ? __uniConfig.networkTimeout.uploadFile : timeout;
	var uploadTask = new UploadTask();
	if (!isArray(files$1) || !files$1.length) {
	...
	function upload(realFiles) {
		...
		Object.values(files$1).forEach(({ name: name$1 }, index$4) => {
			const file$1 = realFiles[index$4];
			form.append(name$1 || "file", file$1, file$1.name || `file-${Date.now()}`);
		});
		xhr.open("POST", url);
		Object.keys(header).forEach((key) => {
			xhr.setRequestHeader(key, header[key]);
		...
		xhr.onload = function() {
			...
			const header$1 = responseHeaders ? responseHeaders.trim().split(/[\r\n]+/).reduce((acc, line) => {
				const parts = line.split(": ");
				const header$2 = parts.shift();
				acc[header$2] = parts.join(": ");
				return acc;
			}, {}) : {};
			resolve({
				statusCode,
	...
	Promise.all(files$1.map(({ file: file$1, uri }) => file$1 instanceof Blob ? Promise.resolve(blobToFile(file$1)) : urlToFile(uri))).then(upload).catch(() => {
		setTimeout(() => {
			reject("file error");
		}, 0);
	});
		...
		try {
			...
			[
				...
				webSocket.addEventListener(name, (event) => {
					...
					this._callbacks[name].forEach((callback$1) => {
						try {
							callback$1(res);
						} catch (e$1) {
							console.error(`thirdScriptError\n${e$1};at socketTask.on${capitalize(name)} callback function\n`, e$1);
						}
					});
					if (this === socketTasks[0] && globalEvent[name]) UniServiceJSBridge.invokeOnCallback(globalEvent[name], res);
					if (name === "error" || name === "close") {
						const index$4 = socketTasks.indexOf(this);
						if (index$4 >= 0) socketTasks.splice(index$4, 1);
					}
				});
			});
			[
...
const getLocation = /* @__PURE__ */ defineAsyncApi(API_GET_LOCATION, ({ type, altitude, highAccuracyExpireTime, isHighAccuracy }, { resolve, reject }) => {
	...
	new Promise((resolve$1, reject$1) => {
		if (navigator.geolocation) navigator.geolocation.getCurrentPosition((res) => resolve$1({ coords: res.coords }), reject$1, {
			enableHighAccuracy: isHighAccuracy || altitude,
			timeout: highAccuracyExpireTime || 1e3 * 100
		});
		else reject$1(/* @__PURE__ */ new Error("device nonsupport geolocation"));
	}).catch((error) => {
		return new Promise((resolve$1, reject$1) => {
			if (mapInfo.type === MapType.QQ) getJSONP(`https://apis.map.qq.com/ws/location/v1/ip?output=jsonp&key=${mapInfo.key}`, { callback: "callback" }, (res) => {
				if ("result" in res && res.result.location) {
					const location$1 = res.result.location;
					resolve$1({
						coords: {
				...
				} else reject$1(new Error(res.message || JSON.stringify(res)));
			}, () => reject$1(/* @__PURE__ */ new Error("network error")));
			else if (mapInfo.type === MapType.GOOGLE) request({
				method: "POST",
				url: `https://www.googleapis.com/geolocation/v1/geolocate?key=${mapInfo.key}`,
				success(res) {
					const data = res.data;
					if ("location" in data) resolve$1({
						coords: {
					...
					else reject$1(new Error(data.error && data.error.message || JSON.stringify(res)));
				},
				fail() {
					reject$1(/* @__PURE__ */ new Error("network error"));
				}
				...
				window.AMap.plugin("AMap.Geolocation", () => {
					new window.AMap.Geolocation({
						...
						else reject$1(new Error(data.message));
					});
				});
			});
			else reject$1(error);
		...
		translateCoordinateSystem(type, coords, skip).then((coords$1) => {
			resolve({
				latitude: coords$1.latitude,
				longitude: coords$1.longitude,
				accuracy: coords$1.accuracy,
				speed: coords$1.altitude || 0,
				altitude: coords$1.altitude || 0,
				verticalAccuracy: coords$1.altitudeAccuracy || 0,
				horizontalAccuracy: coords$1.accuracy || 0
			});
		}).catch((error) => {
			reject(error.message);
		});
...
function useState$1(props$31) {
	...
	function updatePosition() {
		if (props$31.latitude && props$31.longitude) {
			state$2.center.latitude = props$31.latitude;
			state$2.center.longitude = props$31.longitude;
			state$2.marker.latitude = props$31.latitude;
			state$2.marker.longitude = props$31.longitude;
		}
	}
	watch([() => props$31.latitude, () => props$31.longitude], updatePosition);
	updatePosition();
	return state$2;
}
var LocationView_default = /* @__PURE__ */ defineSystemComponent({
	...
	emits: ["close"],
	setup(props$31, { emit: emit$2 }) {
		const state$2 = useState$1(props$31);
		usePreventScroll();
		getLocation({
			type: "gcj02",
			success: ({ latitude, longitude }) => {
				state$2.location.latitude = latitude;
				state$2.location.longitude = longitude;
			}
		});
		function onRegionChange(event) {
			const centerLocation = event.detail.centerLocation;
			if (centerLocation) {
				state$2.center.latitude = centerLocation.latitude;
				state$2.center.longitude = centerLocation.longitude;
			}
		}
		function nav() {
			const mapInfo = getMapInfo();
			let url = "";
			if (mapInfo.type === MapType.GOOGLE) url = `https://www.google.com/maps/dir/?api=1${state$2.location.latitude ? `&origin=${state$2.location.latitude}%2C${state$2.location.longitude}` : ""}&destination=${props$31.latitude}%2C${props$31.longitude}`;
			else if (mapInfo.type === MapType.QQ) url = `https://apis.map.qq.com/uri/v1/routeplan?type=drive${state$2.location.latitude ? `&fromcoord=${state$2.location.latitude}%2C${state$2.location.longitude}&from=${encodeURIComponent("我的位置")}` : ""}&tocoord=${props$31.latitude}%2C${props$31.longitude}&to=${encodeURIComponent(props$31.name || "目的地")}&ref=${mapInfo.key}`;
			else if (mapInfo.type === MapType.AMAP) url = `https://uri.amap.com/navigation?${state$2.location.latitude ? `from=${state$2.location.longitude},${state$2.location.latitude},${encodeURIComponent("我的位置")}&` : ""}to=${props$31.longitude},${props$31.latitude},${encodeURIComponent(props$31.name || "目的地")}`;
			window.open(url);
		}
		function back() {
			emit$2("close");
		...
		function setCenter({ latitude, longitude }) {
			state$2.center.latitude = latitude;
			state$2.center.longitude = longitude;
		}
		return () => {
			return createVNode("div", { "class": "uni-system-open-location" }, [
				createVNode(map_default, {
					"latitude": state$2.center.latitude,
					"longitude": state$2.center.longitude,
					"class": "map",
					"markers": [state$2.marker, state$2.location],
					"onRegionchange": onRegionChange
				}, { default: () => [createVNode("div", {
					"class": "map-move",
					"onClick": () => setCenter(state$2.location)
				...
				createVNode("div", { "class": "info" }, [
					createVNode("div", {
						"class": "name",
						"onClick": () => setCenter(state$2.marker)
					}, [props$31.name], 8, ["onClick"]),
					createVNode("div", {
						"class": "address",
						"onClick": () => setCenter(state$2.marker)
					}, [props$31.address], 8, ["onClick"]),
					createVNode("div", {
						"class": "nav",
						"onClick": nav
...
var started = false;
var watchId = 0;
const startLocationUpdate = /* @__PURE__ */ defineAsyncApi(API_START_LOCATION_UPDATE, (options, { resolve, reject }) => {
...
const stopLocationUpdate = /* @__PURE__ */ defineAsyncApi(API_STOP_LOCATION_UPDATE, (_, { resolve }) => {
	if (watchId) {
...
const offLocationChangeError = /* @__PURE__ */ defineOffApi(API_OFF_LOCATION_CHANGE_ERROR, () => {});
...
const navigateTo = /* @__PURE__ */ defineAsyncApi(API_NAVIGATE_TO, ({ url, events, isAutomatedTesting }, { resolve, reject }) => {
	if (!entryPageState.handledBeforeEntryPageRoutes) {
...
var toast_default = /* @__PURE__ */ defineComponent({
	...
	setup(props$31) {
		...
		const { Icon } = useToastIcon(props$31);
		const visible = usePopup(props$31, {});
		return () => {
			const { mask, duration, title, image } = props$31;
...
function hidePopup(type) {
	...
	if (!showType) return;
	let warnMsg = "";
	if (type === "onHideToast" && showType !== "onShowToast") warnMsg = t$1("uni.showToast.unpaired");
	else if (type === "onHideLoading" && showType !== "onShowLoading") warnMsg = t$1("uni.showLoading.unpaired");
...
function setProperties(item, props$31, propsData) {
	props$31.forEach(function(name) {
...
function setTabBar(type, args, resolve, reject) {
	...
	const { index: index$4 } = args;
	if (typeof index$4 === "number") {
		var __uniConfig2;
		...
		if (!tabBarListLength || index$4 >= tabBarListLength) return reject(`tabbar item not found`);
	}
	const tabBar$1 = useTabBar();
	switch (type) {
		...
		case API_HIDE_TAB_BAR:
			tabBar$1.shown = false;
			break;
		case API_SET_TAB_BAR_ITEM:
			const tabBarItem = tabBar$1.list[index$4];
			...
			if (pagePath) {
				...
				if (newPagePath !== oldPagePath) normalizeTabBarRoute(index$4, oldPagePath, newPagePath);
			}
			break;
		case API_SET_TAB_BAR_STYLE:
			setProperties(tabBar$1, setTabBarStyleProps, args);
			break;
		case API_SHOW_TAB_BAR_RED_DOT:
			setProperties(tabBar$1.list[index$4], setTabBarBadgeProps, {
				badge: "",
				redDot: true
			});
			break;
...
const setTabBarItem = /* @__PURE__ */ defineAsyncApi(API_SET_TAB_BAR_ITEM, (args, { resolve, reject }) => {
	setTabBar(API_SET_TAB_BAR_ITEM, args, resolve, reject);
}, SetTabBarItemProtocol, SetTabBarItemOptions);
const setTabBarStyle = /* @__PURE__ */ defineAsyncApi(API_SET_TAB_BAR_STYLE, (args, { resolve, reject }) => {
	setTabBar(API_SET_TAB_BAR_STYLE, args, resolve, reject);
}, SetTabBarStyleProtocol, SetTabBarStyleOptions);
const hideTabBar = /* @__PURE__ */ defineAsyncApi(API_HIDE_TAB_BAR, (args, { resolve, reject }) => {
	setTabBar(API_HIDE_TAB_BAR, args ? args : {}, resolve, reject);
}, HideTabBarProtocol);
const showTabBar = /* @__PURE__ */ defineAsyncApi(API_SHOW_TAB_BAR, (args, { resolve, reject }) => {
	setTabBar(API_SHOW_TAB_BAR, args ? args : {}, resolve, reject);
}, ShowTabBarProtocol);
const hideTabBarRedDot = /* @__PURE__ */ defineAsyncApi(API_HIDE_TAB_BAR_RED_DOT, (args, { resolve, reject }) => {
	setTabBar(API_HIDE_TAB_BAR_RED_DOT, args, resolve, reject);
}, HideTabBarRedDotProtocol, HideTabBarRedDotOptions);
const showTabBarRedDot = /* @__PURE__ */ defineAsyncApi(API_SHOW_TAB_BAR_RED_DOT, (args, { resolve, reject }) => {
	setTabBar(API_SHOW_TAB_BAR_RED_DOT, args, resolve, reject);
}, ShowTabBarRedDotProtocol, ShowTabBarRedDotOptions);
const removeTabBarBadge = /* @__PURE__ */ defineAsyncApi(API_REMOVE_TAB_BAR_BADGE, (args, { resolve, reject }) => {
	setTabBar(API_REMOVE_TAB_BAR_BADGE, args, resolve, reject);
}, RemoveTabBarBadgeProtocol, RemoveTabBarBadgeOptions);
const setTabBarBadge = /* @__PURE__ */ defineAsyncApi(API_SET_TAB_BAR_BADGE, (args, { resolve, reject }) => {
...
var tabBar_default = /* @__PURE__ */ defineSystemComponent({
	...
	setup() {
		...
		const tabBar$1 = useTheme(_tabBar, () => {
			...
			if (tabBarStyle.list && tabBarStyle.list.length) tabBarStyle.list.forEach((item, index$4) => {
				tabBar$1.list[index$4].iconPath = item.iconPath;
				tabBar$1.list[index$4].selectedIconPath = item.selectedIconPath;
			});
		});
		useVisibleList(tabBar$1, visibleList);
		useTabBarCssVar(tabBar$1);
		const onSwitchTab = useSwitchTab(useRoute(), tabBar$1, visibleList);
		const { style, borderStyle, placeholderStyle } = useTabBarStyle(tabBar$1);
		onMounted(() => {
			if (tabBar$1.iconfontSrc) loadFontFace({
				family: UNI_TABBAR_ICON_FONT,
				source: `url("${tabBar$1.iconfontSrc}")`
			});
		});
		return () => {
			const tabBarItemsTsx = createTabBarItemsTsx(tabBar$1, onSwitchTab, visibleList);
			return createVNode("uni-tabbar", { "class": "uni-tabbar-" + tabBar$1.position }, [createVNode("div", {
				"class": "uni-tabbar",
				"style": style.value
			}, [createVNode("div", {
...
function useVisibleList(tabBar$1, visibleList) {
	const internalMidButton = ref(extend({ type: "midButton" }, tabBar$1.midButton));
	function setVisibleList() {
		...
		if (__UNI_FEATURE_TABBAR_MIDBUTTON__ && tabBar$1.midButton) {
			internalMidButton.value = extend({}, _middleButton, internalMidButton.value, tabBar$1.midButton);
...
function createTabBarItemsTsx(tabBar$1, onSwitchTab, visibleList) {
	...
	return visibleList.value.map((item, index$4) => {
		const selected = selectedIndex === index$4;
		...
		if (!__UNI_FEATURE_TABBAR_MIDBUTTON__) return createTabBarItemTsx(textColor, iconPath, iconfontText, iconfontColor, item, tabBar$1, index$4, onSwitchTab);
		return isMidButton(item) ? createTabBarMidButtonTsx(textColor, iconPath, iconfontText, iconfontColor, item, tabBar$1, index$4, onSwitchTab) : createTabBarItemTsx(textColor, iconPath, iconfontText, iconfontColor, item, tabBar$1, index$4, onSwitchTab);
	});
}
function createTabBarItemTsx(color, iconPath, iconfontText, iconfontColor, tabBarItem, tabBar$1, index$4, onSwitchTab) {
	return createVNode("div", {
		"key": index$4,
		"class": "uni-tabbar__item",
		"onClick": onSwitchTab(tabBarItem, index$4)
...
function createTabBarItemRedDotTsx(badge) {
	...
	return createVNode("div", { "class": clazz$1 }, [badge], 2);
}
function createTabBarMidButtonTsx(color, iconPath, iconfontText, iconfontColor, midButton, tabBar$1, index$4, onSwitchTab) {
	...
	return createVNode("div", {
		...
		"onClick": onSwitchTab(midButton, index$4)
	}, [createVNode("div", {
...
var layout_default = /* @__PURE__ */ defineSystemComponent({
	...
	setup(_props, { emit: emit$2 }) {
		...
		const showTabBar$1 = __UNI_FEATURE_TABBAR__ && useShowTabBar(emit$2);
		const clazz$1 = useAppClass(showTabBar$1);
		...
		return () => {
			...
			const tabBarTsx = __UNI_FEATURE_TABBAR__ && createTabBarTsx(showTabBar$1);
...
function useAppClass(showTabBar$1) {
	const showMaxWidth = ref(false);
	return computed(() => {
		return {
			"uni-app--showtabbar": showTabBar$1 && showTabBar$1.value,
...
function useMaxWidth(layoutState, rootRef) {
	...
	function checkMaxWidth$1() {
		...
		const maxWidth$1 = parseInt(String((hasOwn(meta, "maxWidth") ? meta.maxWidth : __uniConfig.globalStyle.maxWidth) || Number.MAX_SAFE_INTEGER));
		let showMaxWidth = false;
		if (windowWidth > maxWidth$1) showMaxWidth = true;
		else showMaxWidth = false;
		if (showMaxWidth && maxWidth$1) {
			layoutState.marginWidth = (windowWidth - maxWidth$1) / 2;
			nextTick(() => {
				const rootEl = rootRef.value;
				if (rootEl) rootEl.setAttribute("style", "max-width:" + maxWidth$1 + "px;margin:0 auto;");
			});
		} else {
	...
	watch([() => route.path], checkMaxWidth$1);
	onMounted(() => {
		checkMaxWidth$1();
		window.addEventListener("resize", checkMaxWidth$1);
	});
}
function useState() {
	const route = usePageRoute();
	if (!__UNI_FEATURE_RESPONSIVE__) {
		const layoutState$1 = reactive({
			marginWidth: 0,
			leftWindowWidth: 0,
			rightWindowWidth: 0
		});
		watch(() => layoutState$1.marginWidth, (value) => updateCssVar({ "--window-margin": value + "px" }));
		watch(() => layoutState$1.leftWindowWidth + layoutState$1.marginWidth, (value) => {
			updateCssVar({ "--window-left": value + "px" });
		});
		watch(() => layoutState$1.rightWindowWidth + layoutState$1.marginWidth, (value) => {
	...
	[
		...
		const matches$1 = initMediaQuery(topWindowMinWidth, (ev) => {
			layoutState[`${prop}MediaQuery`] = ev.matches;
		});
		layoutState[`${prop}MediaQuery`] = matches$1;
	});
...
function useShowTabBar(emit$2) {
	const route = usePageRoute();
	const tabBar$1 = useTabBar();
	const showTabBar$1 = computed(() => route.meta.isTabBar && tabBar$1.shown);
	updateCssVar({ "--tab-bar-height": tabBar$1.height });
	return showTabBar$1;
}
function createTabBarTsx(showTabBar$1) {
	return withDirectives(createVNode(tabBar_default, null, null, 512), [[vShow, showTabBar$1.value]]);
}
...
const showTopWindow = /* @__PURE__ */ defineAsyncApi("showTopWindow", (_, { resolve, reject }) => {
	const state$2 = getLayoutState();
	if (!state$2) {
		reject();
		return;
	}
	state$2.apiShowTopWindow = true;
	nextTick(resolve);
});
const hideTopWindow = /* @__PURE__ */ defineAsyncApi("hideTopWindow", (_, { resolve, reject }) => {
	...
	state$2.apiShowTopWindow = false;
	nextTick(resolve);
});
const showLeftWindow = /* @__PURE__ */ defineAsyncApi("showLeftWindow", (_, { resolve, reject }) => {
	...
	state$2.apiShowLeftWindow = true;
	nextTick(resolve);
});
const hideLeftWindow = /* @__PURE__ */ defineAsyncApi("hideLeftWindow", (_, { resolve, reject }) => {
	...
	state$2.apiShowLeftWindow = false;
	nextTick(resolve);
});
const showRightWindow = /* @__PURE__ */ defineAsyncApi("showRightWindow", (_, { resolve, reject }) => {
	...
	state$2.apiShowRightWindow = true;
	nextTick(resolve);
});
...
async function releaseWakeLock() {
	if (wakeLockRequest) await wakeLockRequest.catch(() => null);
...
const setKeepScreenOn = /* @__PURE__ */ defineAsyncApi(API_SET_KEEP_SCREEN_ON, ({ keepScreenOn: value }, { resolve, reject }) => {
	keepScreenOn = !!value;
	if (keepScreenOn) {
	...
	nextTick(() => {
		...
		requestComponentInfo(currentPage, [{
			...
			if (result.length > 0) {
				...
				const id$2 = contextInfo === null || contextInfo === void 0 ? void 0 : contextInfo.id;
				const page = contextInfo === null || contextInfo === void 0 ? void 0 : contextInfo.page;
				if (id$2 != null && page != null) {
					var _options$success;
					(_options$success = options.success) === null || _options$success === void 0 || _options$success.call(options, new EditorContext(id$2, page));
				} else {
					var _options$fail;
	...
	setup() {
		...
		});
		{
			...
			function compassChangeHandler(res) {
				state$2.rotate = res.direction;
			}
			function updateLocation() {
				getLocation({
					type: "gcj02",
					success: (res) => {
						state$2.latitude = res.latitude;
						state$2.longitude = res.longitude;
					},
					complete: () => {
		...
		return () => {
			return state$2.latitude ? createVNode(MapMarker_default, mergeProps({
				"anchor": {
					x: .5,
					y: .5
				},
...
var map_polygon_default = /* @__PURE__ */ defineSystemComponent({
	...
	setup(props$31) {
		let polygonIns;
		inject("onMapReady")((map, maps$1, trigger) => {
			function drawPolygon() {
				const { points, strokeWidth, strokeColor, dashArray, fillColor, zIndex } = props$31;
...
function useMap(props$31, rootRef, emit$2) {
	const trigger = useCustomEvent(rootRef, emit$2);
	const mapRef = ref(null);
	let maps$1;
	let map;
	const state$2 = reactive({
		latitude: Number(props$31.latitude),
		longitude: Number(props$31.longitude),
		includePoints: getPoints(props$31.includePoints)
	});
	const onMapReadyCallbacks = [];
	let isMapReady;
	function onMapReady(callback) {
		if (isMapReady) callback(map, maps$1, trigger);
		else onMapReadyCallbacks.push(callback);
	}
	function emitMapReady() {
		isMapReady = true;
		onMapReadyCallbacks.forEach((callback) => callback(map, maps$1, trigger));
	...
	watch([() => props$31.latitude, () => props$31.longitude], ([latitudeVlaue, longitudeVlaue]) => {
		const latitude = Number(latitudeVlaue);
		const longitude = Number(longitudeVlaue);
		if (latitude !== state$2.latitude || longitude !== state$2.longitude) {
			state$2.latitude = latitude;
			state$2.longitude = longitude;
			if (map) {
				const centerPosition = getMapPosition(maps$1, state$2.latitude, state$2.longitude);
				map.setCenter(centerPosition);
			}
		}
	});
	watch(() => props$31.includePoints, (points) => {
		state$2.includePoints = getPoints(points);
	...
	function updateBounds() {
		if (getIsAMap()) {
			const points = [];
			state$2.includePoints.forEach((point) => {
				points.push([point.longitude, point.latitude]);
			});
			const bounds = new maps$1.Bounds(...points);
			map.setBounds(bounds);
		} else if (getIsBMap()) {} else {
			const bounds = new maps$1.LatLngBounds();
			state$2.includePoints.forEach(({ latitude, longitude }) => {
				const latLng = new maps$1.LatLng(latitude, longitude);
	...
	function initMap() {
		...
		const center = getMapPosition(maps$1, state$2.latitude, state$2.longitude);
		const event = maps$1.event || maps$1.Event;
		const map$1 = new maps$1.Map(mapEl, {
			center,
			zoom: Number(props$31.scale),
		...
		if (getIsBMap()) {
			map$1.centerAndZoom(center, Number(props$31.scale));
			map$1.enableScrollWheelZoom();
			map$1._printLog && map$1._printLog("uniapp");
		}
		watch(() => props$31.scale, (scale) => {
			map$1.setZoom(Number(scale) || 16);
		});
		onBoundsReady(() => {
			if (state$2.includePoints.length) {
				updateBounds();
				updateCenter();
			}
		});
		if (getIsBMap()) {
			...
			const boundsChangedEvent = event.addListener(map$1, "bounds_changed", () => {
				boundsChangedEvent.remove();
				emitBoundsReady();
			});
			event.addListener(map$1, "complete", () => {
				emitBoundsReady();
			});
			...
			const zoomChangedCallback = () => {
				emit$2("update:scale", map$1.getZoom());
				trigger("regionchange", {}, extend({
					type: "end",
					causedBy: "scale"
				}, getMapInfo$1()));
			...
			event.addListener(map$1, "center_changed", () => {
				const center$1 = map$1.getCenter();
				const latitude = getLat(center$1);
				const longitude = getLng(center$1);
				emit$2("update:latitude", latitude);
				emit$2("update:longitude", longitude);
			});
		}
		return map$1;
	}
		...
		useSubscribe((type, data = {}) => {
			switch (type) {
				...
				case "moveToLocation":
					{
						...
						if (latitude && longitude) {
							...
							state$2.longitude = longitude;
							if (map) {
								const centerPosition = getMapPosition(maps$1, latitude, longitude);
								map.setCenter(centerPosition);
							}
							onMapReady(() => {
				...
				case "translateMarker":
					...
					break;
				case "includePoints":
					state$2.includePoints = getPoints(data.includePoints);
	...
	onMounted(() => {
		loadMaps(props$31.libraries, (result) => {
			maps$1 = result;
			map = initMap();
			emitMapReady();
			trigger("updated", {}, {});
...
var map_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { emit: emit$2, slots }) {
		const rootRef = ref(null);
		const { mapRef, trigger } = useMap(props$31, rootRef, emit$2);
		onMounted(() => {
			rootRef.value.attachVmProps(props$31);
		});
		return () => {
			return createVNode("uni-map", {
				...
				props$31.markers.map((item) => createVNode(MapMarker_default, mergeProps({ "key": item.id }, item), null, 16)),
				props$31.polyline.map((item) => createVNode(MapPolyline_default, item, null, 16)),
				props$31.circles.map((item) => createVNode(MapCircle_default, item, null, 16)),
				props$31.controls.map((item) => createVNode(MapControl_default, mergeProps(item, { "trigger": trigger }), null, 16, ["trigger"])),
				props$31.showLocation && createVNode(MapLocation_default, null, null),
				props$31.polygons.map((item) => createVNode(map_polygon_default, item, null, 16)),
...
var cover_view_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { slots }) {
		...
		onMounted(() => {
			setScrollTop(props$31.scrollTop);
		});
		onMounted(() => {
			root.value.attachVmProps(props$31);
		});
		return () => {
			return createVNode("uni-cover-view", {
				"scroll-top": props$31.scrollTop,
				"ref": root
			}, [createVNode("div", {
				"ref": content,
...
function getDefaultStartValue(props$31) {
	if (props$31.mode === mode.TIME) return "00:00";
	if (props$31.mode === mode.DATE) {
		const year = (/* @__PURE__ */ new Date()).getFullYear() - 150;
		switch (props$31.fields) {
...
function getDefaultEndValue(props$31) {
	if (props$31.mode === mode.TIME) return "23:59";
	if (props$31.mode === mode.DATE) {
		const year = (/* @__PURE__ */ new Date()).getFullYear() + 150;
		switch (props$31.fields) {
			case fields.YEAR: return year.toString();
...
function getDateValueArray(props$31, state$2, valueStr, defaultValue) {
	const splitStr = props$31.mode === mode.DATE ? "-" : ":";
	const array = props$31.mode === mode.DATE ? state$2.dateArray : state$2.timeArray;
	let max;
	if (props$31.mode === mode.TIME) max = 2;
	else switch (props$31.fields) {
		case fields.YEAR:
			max = 1;
			break;
		case fields.MONTH:
	...
	if (value.indexOf(-1) >= 0) value = defaultValue ? getDateValueArray(props$31, state$2, defaultValue) : value.map(() => 0);
	return value;
}
var mode = {
	SELECTOR: "selector",
...
var props = {
	...
	start: {
		type: String,
		default: (props$31) => {
			return getDefaultStartValue(props$31);
		}
	},
	end: {
		type: String,
		default: (props$31) => {
			return getDefaultEndValue(props$31);
		}
	},
	disabled: {
		type: [Boolean, String],
...
var picker_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { emit: emit$2, slots }) {
		...
		const { state: state$2, rangeArray } = usePickerState(props$31);
		const { system, selectorTypeComputed, _show, _l10nColumn, _l10nItem, _input, _fixInputPosition, _pickerViewChange, _cancel, _change, _resetFormData, _getFormData, _createTime, _createDate, _setValueSync } = usePickerMethods(props$31, state$2, useCustomEvent(rootRef, emit$2), rootRef, pickerRef, selectRef, inputRef);
		usePickerWatch(state$2, _cancel, _change);
		usePickerForm(_resetFormData, _getFormData);
		_createTime();
		_createDate();
		_setValueSync();
		const popup = usePopupStyle(state$2);
		watchEffect(() => {
			state$2.isDesktop = popup.isDesktop.value;
			state$2.popupStyle = popup.popupStyle.value;
		});
		onBeforeUnmount(() => {
		...
		return () => {
			let _slot2;
			const { visible, contentVisible, valueArray, popupStyle, valueSync } = state$2;
			const { rangeKey, mode: mode$1, start, end } = props$31;
			const booleanAttrs = useBooleanAttr(props$31, "disabled");
			return createVNode("uni-picker", mergeProps({ "ref": rootRef }, booleanAttrs, { "onClick": withWebEvent(_show) }), [
				pickerRender.value ? createVNode("div", {
					...
					createVNode("div", {
						...
						"onClick": withWebEvent(_cancel)
					}, [t$1("uni.picker.cancel")], 8, ["onClick"]), createVNode("div", {
					...
					contentVisible ? createVNode(picker_view_default, {
						...
						let _slot;
						return createVNode(picker_view_column_default, { "key": index0 }, _isSlot(_slot = renderList(rangeItem, (item, index$4) => createVNode("div", {
							"key": index$4,
					...
					}, [renderList(rangeArray.value[0], (item, index$4) => createVNode("div", {
						"key": index$4,
						"class": ["uni-picker-item", { selected: valueArray[0] === index$4 }],
						"onClick": () => {
							valueArray[0] = index$4;
							_change();
						}
...
function usePickerState(props$31) {
	const state$2 = reactive({
		valueSync: void 0,
		visible: false,
	...
	return {
		state: state$2,
		rangeArray: computed(() => {
			let val = props$31.range;
			switch (props$31.mode) {
				case mode.SELECTOR: return [val];
				case mode.MULTISELECTOR: return val;
				case mode.TIME: return state$2.timeArray;
				case mode.DATE: {
					const dateArray = state$2.dateArray;
					switch (props$31.fields) {
...
function usePickerMethods(props$31, state$2, trigger, rootRef, pickerRef, selectRef, inputRef) {
	const isiPad = useIsiPad();
	const _system = useSystem();
	const selectorTypeComputed = computed(() => {
		const type = props$31.selectorType;
		if (Object.values(selectorType).includes(type)) return type;
		return isiPad.value ? selectorType.PICKER : selectorType.SELECT;
	});
	const system = computed(() => {
		if (props$31.mode === mode.DATE && !Object.values(fields).includes(props$31.fields) && state$2.isDesktop) return _system.value;
		return "";
	});
	const startArray = computed(() => {
		return getDateValueArray(props$31, state$2, props$31.start, getDefaultStartValue(props$31));
	});
	const endArray = computed(() => {
		return getDateValueArray(props$31, state$2, props$31.end, getDefaultEndValue(props$31));
	});
	function _show(event) {
		if (props$31.disabled) return;
		state$2.valueChangeSource = "";
		...
		setTimeout(() => {
			state$2.visible = true;
		}, 20);
	}
	function _getFormData() {
		return {
			value: state$2.valueSync,
			key: props$31.name
		};
	}
	function _resetFormData() {
		switch (props$31.mode) {
			...
			case mode.MULTISELECTOR:
				state$2.valueSync = props$31.value.map((val) => 0);
				break;
			case mode.DATE:
			case mode.TIME:
				state$2.valueSync = "";
	...
	function _createTime() {
		...
		state$2.timeArray.push(hours, minutes);
	}
	function getYearStartEnd() {
		...
		let end = year + 150;
		if (props$31.start) {
			const _year = new Date(props$31.start).getFullYear();
			if (!isNaN(_year) && _year < start) start = _year;
		}
		if (props$31.end) {
			const _year = new Date(props$31.end).getFullYear();
	...
	function _createDate() {
		...
		state$2.dateArray.push(years, months, days);
	}
	...
	function _setValueSync() {
		let val = props$31.value;
		switch (props$31.mode) {
			case mode.MULTISELECTOR:
				{
					if (!isArray(val)) val = state$2.valueArray;
					if (!isArray(state$2.valueSync)) state$2.valueSync = [];
					const length = state$2.valueSync.length = Math.max(val.length, props$31.range.length);
					for (let index$4 = 0; index$4 < length; index$4++) {
						const val0 = Number(val[index$4]);
						const val1 = Number(state$2.valueSync[index$4]);
						const val2 = isNaN(val0) ? isNaN(val1) ? 0 : val1 : val0;
						const maxVal = props$31.range[index$4] ? props$31.range[index$4].length - 1 : 0;
						state$2.valueSync.splice(index$4, 1, val2 < 0 || val2 > maxVal ? 0 : val2);
					}
				}
				break;
			case mode.TIME:
			case mode.DATE:
				state$2.valueSync = String(val);
				break;
			default: {
				const valueSync = Number(val);
				state$2.valueSync = valueSync < 0 ? 0 : valueSync;
				break;
			}
		}
	}
	function _setValueArray() {
		let val = state$2.valueSync;
		let valueArray;
		switch (props$31.mode) {
			case mode.MULTISELECTOR:
				valueArray = [...val];
				break;
			case mode.TIME:
				valueArray = getDateValueArray(props$31, state$2, val, formatDateTime({ mode: mode.TIME }));
				break;
			case mode.DATE:
				valueArray = getDateValueArray(props$31, state$2, val, formatDateTime({ mode: mode.DATE }));
				break;
			default:
				valueArray = [val];
				break;
		}
		state$2.oldValueArray = [...valueArray];
		state$2.valueArray = [...valueArray];
	}
	function _getValue() {
		let val = state$2.valueArray;
		switch (props$31.mode) {
			case mode.SELECTOR: return val[0];
			case mode.MULTISELECTOR: return val.map((val$1) => val$1);
			case mode.TIME: return state$2.valueArray.map((val$1, i) => state$2.timeArray[i][val$1]).join(":");
			case mode.DATE: return state$2.valueArray.map((val$1, i) => state$2.dateArray[i][val$1]).join("-");
		}
	}
	function _change() {
		_close();
		state$2.valueChangeSource = "click";
		const value = _getValue();
		state$2.valueSync = isArray(value) ? value.map((val) => val) : value;
		trigger("change", {}, { value });
	}
	function _cancel($event) {
		if (system.value === "firefox" && $event) {
			const { top, left, width, height } = state$2.popover;
	...
	function _select() {
		if (props$31.mode === mode.SELECTOR && selectorTypeComputed.value === selectorType.SELECT) selectRef.value.scrollTop = state$2.valueArray[0] * 34;
	}
	function _input($event) {
		state$2.valueSync = $event.target.value;
		nextTick(() => {
			_change();
		});
	}
	...
	function _pickerViewChange(event) {
		state$2.valueArray = _l10nColumn(event.detail.value, true);
	}
	function _l10nColumn(array, normalize) {
		const { getLocale: getLocale$1 } = useI18n();
		if (props$31.mode === mode.DATE) {
			const locale = getLocale$1();
			if (!locale.startsWith("zh")) switch (props$31.fields) {
	...
	function _l10nItem(item, index$4) {
		const { getLocale: getLocale$1 } = useI18n();
		if (props$31.mode === mode.DATE) {
			const locale = getLocale$1();
			...
			else if (props$31.fields !== fields.YEAR && index$4 === (props$31.fields !== fields.MONTH && (locale === "es" || locale === "fr") ? 1 : 0)) {
				let array;
				switch (locale) {
					case "es":
						array = [
	...
	watch(() => state$2.visible, (val) => {
		if (val) {
			clearTimeout(__contentVisibleDelay);
			state$2.contentVisible = val;
			_select();
	...
	watch(() => state$2.valueSync, _setValueArray, { deep: true });
	watch(() => state$2.valueArray, (val) => {
		if (props$31.mode === mode.TIME || props$31.mode === mode.DATE) {
			const getValue = props$31.mode === mode.TIME ? _getTimeValue : _getDateValue;
			const valueArray = state$2.valueArray;
		...
		val.forEach((value, column) => {
			if (value !== state$2.oldValueArray[column]) {
				state$2.oldValueArray[column] = value;
				if (props$31.mode === mode.MULTISELECTOR) trigger("columnchange", {}, {
					column,
					value
				});
			}
...
function usePickerWatch(state$2, _cancel, _change) {
	const { key, disable } = useKeyboard();
	watchEffect(() => {
		disable.value = !state$2.visible;
	});
	watch(key, (value) => {
...
var AdConfig = class AdConfig {
	...
	_doCallback(adpid, success, fail) {
		AdConfig.IS++;
		var { a: a$1, b } = this._adConfig;
		const adData = a$1[adpid];
...
var AdRender = class {
	constructor(props$31, trigger, rootRef, options) {
		...
		this._adpid = props$31.adpid;
		this._adpidWidescreen = props$31.adpidWidescreen;
		this._widescreenWidth = props$31.widescreenWidth;
	...
	load(adpid) {
		...
		this._reset();
		AdConfig.instance.get(this._currentAdpid, (b, a$1) => {
			this._b = b;
			this._pl = a$1;
			this._renderAd();
		}, (err) => {
	...
	_renderAd() {
		...
		const id$2 = this._randomId();
		this._createView(id$2);
		...
		if (providerId === AD_PROVIDER.TUIA) {
			AdScript.instance.load({
				...
				this._renderTuiaMaterial(id$2, data);
			}, (err) => {
				this._trigger("error", {}, err);
				this._renderNext();
			});
	...
	_createView(id$2) {
		...
		adView.setAttribute("id", id$2);
		adView.setAttribute("class", id$2);
		this._rootRef.value.innerHTML = "";
		this._rootRef.value.append(adView);
		return adView;
	}
	_renderGdt(id$2, data) {
		window.TencentGDT.push({
			...
			onComplete: (res) => {
				if (res && res.constructor === Array && res.length > 0) {
					window.TencentGDT.NATIVE.renderAd(res[0], id$2);
					this._trigger("load", {}, {});
				} else {
	...
	_renderTuiaMaterial(id$2, data) {
		const adView = document.getElementById(id$2);
		if (!adView) {
...
var ad_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$31, { emit: emit$2, slots }) {
		...
		const ad = new AdRender(props$31, useCustomEvent(rootRef, emit$2), rootRef, {
		...
		watch(() => props$31.adpid, (val) => {
			ad.load(val);
		});
		watch(() => props$31.adpidWidescreen, (val) => {
			ad.load(val);
		});
		onMounted(() => {
		...
		return () => {
			const { adpid, adpidWidescreen, widescreenWidth } = props$31;
...
var UniMatchMediaElement = class UniMatchMediaElement extends UniViewElement {
	...
	attributeChangedCallback(name, oldValue, newValue) {
		...
		const matches$1 = name.match(RE_MQ_FEATURE);
		if (matches$1 == null || matches$1.length == 0) return;
		const modifier = matches$1[1] != null ? matches$1[1] : "";
		const feature = matches$1[2] != null ? matches$1[2].toLowerCase() : "";
...
const UniViewJSBridge$1 = /* @__PURE__ */ extend(ViewJSBridge, { publishHandler(event, args, pageId) {
	UniServiceJSBridge.subscribeHandler(event, args, pageId);
} });
const getElementById = /* @__PURE__ */ defineSyncApi("getElementById", (id$2) => {
	const uniPageBody = document.querySelector("uni-page-body");
	return uniPageBody ? uniPageBody.querySelector(`#${id$2}`) : null;
});
...
const openDialogPage = (options) => {
	...
	if (!isSystemDialogPage(dialogPage)) {
		...
		if (!currentPages.length) targetSystemDialogPages = homeSystemDialogPages;
			...
			var _parentPage$vm$$pageL;
			if (!parentPage) parentPage = currentPages[currentPages.length - 1];
			dialogPageTriggerPrevDialogPageLifeCycle(parentPage, ON_HIDE);
			dialogPage.getParentPage = () => parentPage;
			targetSystemDialogPages = (_parentPage$vm$$pageL = parentPage.vm.$pageLayoutInstance) === null || _parentPage$vm$$pageL === void 0 ? void 0 : _parentPage$vm$$pageL.$systemDialogPages.value;
		}
...
var actionSheet_default = /* @__PURE__ */ __plugin_vue_export_helper_default(/* @__PURE__ */ defineComponent({
	__name: "actionSheet",
	setup(__props) {
		...
		onLoad((options) => {
			...
			isLandscape$1.value = systemInfo.deviceOrientation == "landscape";
		});
		...
		onResize((_) => {
			isLandscape$1.value = uni.getSystemInfoSync().deviceOrientation == "landscape";
		});
		onBeforeUnmount(() => {
			...
			return openBlock(), createBlock(_component_view, null, {
				default: withCtx(() => [createVNode(_component_view, {
					...
					class: normalizeClass(["uni-action-sheet_dialog__container", {
						...
						"uni-action-sheet_landscape__mode": isLandscape$1.value
					}])
				}, {
					default: withCtx(() => [
						createVNode(_component_view, {
							...
							default: withCtx(() => [title.value ? (openBlock(), createElementBlock(Fragment, { key: 0 }, [createVNode(_component_view, { class: normalizeClass(["uni-action-sheet_dialog__title border-b", {
								...
								_: 1
							}, 8, ["class"]), createVNode(_component_view, { class: normalizeClass(["divider", { "uni-action-sheet_dark__mode": theme.value == "dark" }]) }, null, 8, ["class"])], 64)) : createCommentVNode("", true), createVNode(_component_view, { class: normalizeClass(["uni-action-sheet_dialog__cell__container", { "uni-action-sheet_landscape__mode": isLandscape$1.value }]) }, {
								default: withCtx(() => [(openBlock(true), createElementBlock(Fragment, null, renderList(itemList.value, (item, index$4) => {
									return openBlock(), createElementBlock(Fragment, { key: index$4 }, [index$4 !== 0 ? (openBlock(), createBlock(_component_view, {
										key: 0,
										...
										onClick: ($event) => handleMenuItemClick(index$4)
									}, {
										default: withCtx(() => [createVNode(_component_text, {
						...
						!isLandscape$1.value ? (openBlock(), createBlock(_component_view, {
							key: 0,
...
const showActionSheet$1 = (options) => {
	...
	uni.$on(successEventName, (index$4) => {
		var _options$success, _options$complete;
		const res = new ShowActionSheetSuccessImpl(index$4);
...
var chooseLocation_default = /* @__PURE__ */ __plugin_vue_export_helper_default(/* @__PURE__ */ defineComponent({
	...
	setup(__props) {
		...
		const calcDirection = (poi1, poi2) => {
			const toRadians = (angle$1) => angle$1 * (Math.PI / 180);
			const toDegrees = (angle$1) => angle$1 * (180 / Math.PI);
		...
		const safeSetTimeout = (callback, delay) => {
			...
			timerId = setTimeout(() => {
				callback();
				const index$4 = timeoutTimers.value.indexOf(timerId);
				if (index$4 > -1) timeoutTimers.value.splice(index$4, 1);
			}, delay);
		...
		const poiHandle = (poisData) => {
			let list = poisData.map((item, index$4) => {
				const location$1 = item["location"];
				let distance = item["distance"];
				let lat = location$1["lat"];
				let lng = location$1["lng"];
				if (distance == 0) {
					lat = latitude.value;
					lng = longitude.value;
				}
		...
		const getPoi = (type) => {
			...
			if (searchVal != "" && searchVal.length > 0) callUniMapCo("search", {
				...
				if (pois.value.length > 0 && index$4 == 1) {
		...
		const regionchange = (e$1) => {
			let causedBy = e$1.causedBy;
			if (!causedBy) causedBy = e$1.detail.causedBy;
			if (e$1.type !== "end" || causedBy != "drag" || locationComplete.value == false) return;
		...
		const selectPoi = (item, index$4) => {
			isFocus.value = false;
			selected.value = index$4;
		...
		const getSystemInfo$1 = () => {
			...
			isLandscape$1.value = systemInfo.windowWidth >= 900 ? true : false;
		...
		const landscapeClassCom = computed(() => {
			return isLandscape$1.value ? "uni-choose-location-landscape" : "uni-choose-location-vertical";
		});
		const mapBoxStyleCom = computed(() => {
			let list = [];
			if (!useUniCloud.value) list.push(`flex: 1;`);
			if (!isLandscape$1.value) {
					...
					useUniCloud.value ? (openBlock(), createBlock(_component_view, {
						...
						default: withCtx(() => [createVNode(_component_view, { class: normalizeClass(["uni-choose-location-poi-search", [darkClassCom.value]]) }, {
							...
							default: withCtx(() => [errMsg.value != "" ? (openBlock(), createBlock(_component_view, {
								...
								_: 1
							})) : (openBlock(true), createElementBlock(Fragment, { key: 3 }, renderList(pois.value, (item, index$4) => {
								return openBlock(), createBlock(_component_view, {
									key: index$4,
									class: normalizeClass(["uni-choose-location-poi-item", [landscapeClassCom.value]]),
									onClick: ($event) => selectPoi(item, index$4)
								}, {
									default: withCtx(() => [
										...
										selected.value == index$4 ? (openBlock(), createBlock(_component_text, {
											key: 0,
...
var uniModal_default = /* @__PURE__ */ __plugin_vue_export_helper_default(/* @__PURE__ */ defineComponent({
	__name: "uniModal",
	setup(__props) {
		const theme = ref("light");
		const language = ref("zh-Hans");
		...
		const onInputBlur = (e$1) => {
			setTimeout(() => {
				inputBottom.value = "0px";
			}, 220);
		};
		const onInputKeyboardChange = (e$1) => {
			let keyBoardHeight = e$1.detail.height;
		...
		const updateUI = () => {
			...
			if (theme.value == "dark") hoverClassName.value = "uni-modal_dialog__content__bottom__button__hover__uni-modal_dark__mode";
			else hoverClassName.value = "uni-modal_dialog__content__bottom__button__hover";
		};
		const closeModal = () => {
		...
		return (_ctx, _cache) => {
			...
			const _component_scroll_view = scroll_view_default;
			const _component_view = view_default;
			return openBlock(), createBlock(_component_view, { class: normalizeClass(["uni-modal_dialog__mask", {
				"uni-modal_dialog__mask__show": showAnim.value,
				"uni-modal_dialog__mask__hide": !showAnim.value
			}]) }, {
				default: withCtx(() => [createVNode(_component_view, {
					class: normalizeClass(["uni-modal_dialog__container", {
						"uni-modal_dialog__show": showAnim.value,
						"uni-modal_dark__mode": theme.value == "dark"
					}]),
					style: normalizeStyle({ bottom: inputBottom.value })
				}, {
					default: withCtx(() => [createVNode(_component_view, { class: normalizeClass(["uni-modal_dialog__container__wrapper", { "uni-modal_dark__mode": theme.value == "dark" }]) }, {
						default: withCtx(() => [
							...
							createVNode(_component_view, { class: "uni-modal_dialog__content" }, {
								default: withCtx(() => [editable.value ? (openBlock(), createBlock(_component_textarea, {
									...
									"auto-height",
									"placeholder"
								])) : createCommentVNode("", true), !editable.value && content.value.length > 0 ? (openBlock(), createBlock(_component_scroll_view, {
									key: 1,
									class: "uni-modal_dialog__content__scrollview",
									"show-scrollbar": "true",
									style: normalizeStyle({ maxHeight: maxScrollHeight.value })
								}, {
									default: withCtx(() => [createVNode(_component_text, { class: "uni-modal_dialog__content__scrollview__text" }, {
							...
							createVNode(_component_view, { class: normalizeClass(["uni-modal_dialog__content__topline", { "uni-modal_dark__mode": theme.value == "dark" }]) }, null, 8, ["class"]),
...
const showModal$1 = (options) => {
	...
	if (openRet != null) return openRet;
		...
		const res = new ShowModalFailImpl();
		(_options$fail3 = options.fail) === null || _options$fail3 === void 0 || _options$fail3.call(options, res);
		(_options$complete4 = options.complete) === null || _options$complete4 === void 0 || _options$complete4.call(options, res);
		return null;
	}
};
const hideModal$1 = function(options) {
	var _currentPage$vm$$page, _options$success2, _options$complete6;
	const pages = getCurrentPages();
	const currentPage = pages[pages.length - 1];
	if (!currentPage) {
		var _options$fail4, _options$complete5;
		const res$1 = new HideModalFailImpl();
		options === null || options === void 0 || (_options$fail4 = options.fail) === null || _options$fail4 === void 0 || _options$fail4.call(options, res$1);
		options === null || options === void 0 || (_options$complete5 = options.complete) === null || _options$complete5 === void 0 || _options$complete5.call(options, res$1);
		return;
	}
	const systemDialogPages = (_currentPage$vm$$page = currentPage.vm.$pageLayoutInstance) === null || _currentPage$vm$$page === void 0 ? void 0 : _currentPage$vm$$page.$systemDialogPages.value;
	let shallClosePages = [];
	for (let perPage of systemDialogPages) if (isSystemModalDialogPage(perPage)) {
		if ((options === null || options === void 0 ? void 0 : options.modalPage) == null) shallClosePages.push(perPage);
		else if (perPage.options["optionsEventName"] === options.modalPage.options["optionsEventName"]) {
			shallClosePages.push(perPage);
			break;
		}
	}
	shallClosePages.forEach((item) => {
		const index$4 = systemDialogPages.indexOf(item);
		if (index$4 > -1) {
			notifyClosedDialog(systemDialogPages[index$4]);
			systemDialogPages.splice(index$4, 1);
		}
	});
	let res = {};
...
function notifyClosedDialog(perPage) {
	...
	if (perPage.options["successEventName"] != null) uni.$emit(perPage.options["successEventName"], JSON.stringify(ret));
}
...
const hideModal = /* @__PURE__ */ defineAsyncApi(API_HIDE_MODAL, (args, { resolve, reject }) => {
	registerModalOnce();
	hideModal$1(extend({
		success: (res) => {
			resolve(res);
...
const showModal = /* @__PURE__ */ defineAsyncApi(API_SHOW_MODAL, (args, { resolve, reject }) => {
	registerModalOnce();
...
var showLoading_default = /* @__PURE__ */ __plugin_vue_export_helper_default(/* @__PURE__ */ defineComponent({
	__name: "showLoading",
	setup(__props) {
		const readyEventName = ref("");
		const optionsEventName = ref("");
		...
		return (_ctx, _cache) => {
			...
			const _component_text = text_default;
			const _component_view = view_default;
			return openBlock(), createBlock(_component_view, { class: normalizeClass(["uni-loading_dialog__mask", { "uni-loading_dialog__mask__show": showAnim.value }]) }, {
				default: withCtx(() => [createVNode(_component_view, { class: normalizeClass(["uni-loading_dialog__container", { "uni-loading_dialog__show": showAnim.value }]) }, {
...
const hideLoading$1 = function(options) {
	...
	if (!currentPage) {
		var _options$fail4, _options$complete5;
		const res$1 = new HideLoadingFailImpl();
		options === null || options === void 0 || (_options$fail4 = options.fail) === null || _options$fail4 === void 0 || _options$fail4.call(options, res$1);
		options === null || options === void 0 || (_options$complete5 = options.complete) === null || _options$complete5 === void 0 || _options$complete5.call(options, res$1);
		return;
	}
	const systemDialogPages = (_currentPage$vm$$page = currentPage.vm.$pageLayoutInstance) === null || _currentPage$vm$$page === void 0 ? void 0 : _currentPage$vm$$page.$systemDialogPages.value;
	let shallClosePages = [];
	for (let perPage of systemDialogPages) if (isSystemShowLoadingDialogPage(perPage)) {
		if ((options === null || options === void 0 ? void 0 : options.loadingPage) == null) shallClosePages.push(perPage);
		else if (perPage.options["optionsEventName"] === options.loadingPage.options["optionsEventName"]) {
			shallClosePages.push(perPage);
			break;
		}
	}
	shallClosePages.forEach((item) => {
		const index$4 = systemDialogPages.indexOf(item);
		if (index$4 > -1) systemDialogPages.splice(index$4, 1);
	});
	let res = {};
	options === null || options === void 0 || (_options$success2 = options.success) === null || _options$success2 === void 0 || _options$success2.call(options, res);
	options === null || options === void 0 || (_options$complete6 = options.complete) === null || _options$complete6 === void 0 || _options$complete6.call(options, res);
...
var id = 0;
const createWorker = /* @__PURE__ */ defineSyncApi(API_CREATE_WORKER, (url) => {
	...
	class WorkerImpl {
		...
		onMessage(listener$2) {
			$on(MESSAGE_EVENT_NAME, listener$2);
		}
		onError(listener$2) {
			$on(ERROR_EVENT_NAME, listener$2);
		}
		postMessage(message, options = null) {
			this._threadWorker.postMessage(message, (options === null || options === void 0 ? void 0 : options.transfer) ?? void 0);
		}
		terminate() {
...
const UniServiceJSBridge$1 = /* @__PURE__ */ extend(ServiceJSBridge, { publishHandler(event, args, pageId) {
