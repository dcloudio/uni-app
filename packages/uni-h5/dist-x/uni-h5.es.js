...
function createScrollListener({ onPageScroll, onReachBottom, onReachBottomDistance }) {
	...
	const trigger = () => {
		...
		if (onReachBottom && reachBottomLocking) {
			if (testReachBottom()) {} else testReachBottomTimer = setTimeout(testReachBottom, 300);
		}
		ticking = false;
	};
...
function createNativeEvent(evt, htmlElement = false) {
	...
	if (typeof CustomEvent !== "undefined" && evt instanceof CustomEvent && isPlainObject(evt.detail)) event.detail = evt.detail;
...
var UniElement = class extends HTMLElement {
	...
	get dataset() {
		if (!this.__uniDatasetMap) this.__uniDatasetMap = createUniDOMStringMap(this.__uniDataset || {});
		return this.__uniDatasetMap;
	}
	setAttribute(qualifiedName, value) {
		super.setAttribute(qualifiedName, value);
		if (qualifiedName.startsWith("data-") && this.__uniDatasetMap) this.__uniDatasetMap.set(qualifiedName, value);
	}
	removeAttribute(qualifiedName) {
		super.removeAttribute(qualifiedName);
		if (qualifiedName.startsWith("data-") && this.__uniDatasetMap) this.__uniDatasetMap.delete(qualifiedName);
	}
...
var checkbox_default = /*#__PURE__*/ defineBuiltInComponent({
	...
	setup(props, { slots }) {
		...
		return () => {
			...
			}, [realCheckValue ? createSvgIconVNode(ICON_PATH_SUCCESS_NO_CIRCLE, props.disabled ? "currentColor" : props.foreColor || props.iconColor || props.color, 22) : ""], 6), slots.default && slots.default()], 4)], 16, ["id", "onClick"]);
		};
	}
});
...
function removeAllKeepAliveApiCallbacks(name) {
	for (const key in invokeCallbacks) if (invokeCallbacks[key].name === name) delete invokeCallbacks[key];
}
function offKeepAliveApiCallback(name, eventTransport) {
	const eventName = eventTransport ? name : "api." + name;
	(eventTransport || UniServiceJSBridge).off(eventName);
}
function onKeepAliveApiCallback(name, eventTransport) {
	const eventName = eventTransport ? name : "api." + name;
	(eventTransport || UniServiceJSBridge).on(eventName, (res) => {
		for (const key in invokeCallbacks) {
...
function formatApiArgs(args, options) {
	...
	if (!options || !options.formatArgs || !isPlainObject(options.formatArgs) && isPlainObject(rawParams)) return;
...
function invokeFail(id, name, errMsg, errRes = {}) {
	...
	if (typeof UniError !== "undefined") {
		...
		if (typeof errOptions.errSubject === "undefined") errOptions.errSubject = name;
...
function wrapperOffApi(name, fn, options) {
	return (callback) => {
		...
		if (!clearAll) checkCallback(callback);
		const errMsg = beforeInvokeApi(name, clearAll ? [] : [callback], void 0, options);
		if (errMsg) throw new Error(errMsg);
		const onApiName = name.replace("off", "on");
		if (clearAll) removeAllKeepAliveApiCallbacks(onApiName);
		else removeKeepAliveApiCallback(onApiName, callback);
		if (!findInvokeCallbackByName(onApiName)) {
...
var upx2px = /*#__PURE__*/ defineSyncApi(API_UPX2PX, (number, newDeviceWidth) => {
	...
	if (result === 0) {
		if (deviceDPR === 1 || !isIOS$1) result = 1;
		else result = .5;
	}
...
//#endregion
//#region ../uni-api/src/service/route/appRoute.ts
...
function createAppRouteRuntime(options = {}) {
	let routeEventId = 0;
	const onAppRoute = /*#__PURE__*/ defineOnApi(API_ON_APP_ROUTE, () => {}, { eventTransport });
	const offAppRoute = /*#__PURE__*/ defineOffApi(API_OFF_APP_ROUTE, () => {}, {
		allowClearAll: true,
		eventTransport
	});
	const onBeforeAppRoute = /*#__PURE__*/ defineOnApi(API_ON_BEFORE_APP_ROUTE, () => {}, { eventTransport });
	const offBeforeAppRoute = /*#__PURE__*/ defineOffApi(API_OFF_BEFORE_APP_ROUTE, () => {}, {
		allowClearAll: true,
		eventTransport
	});
	const rewriteRoute = /*#__PURE__*/ defineAsyncApi(API_REWRITE_ROUTE, ({ url, preserveQuery }, { resolve, reject }) => {
	...
	function createAppRouteContext(event) {
		...
		const timeStamp = (_event$timeStamp = event.timeStamp) !== null && _event$timeStamp !== void 0 ? _event$timeStamp : Date.now();
		return {
			event: {
				path: event.path,
				...
				timeStamp,
				routeEventId: (_event$routeEventId = event.routeEventId) !== null && _event$routeEventId !== void 0 ? _event$routeEventId : `${timeStamp}-${++routeEventId}`
			},
...
var ChooseFileOptions = { formatArgs: {
	...
	extension(extension, params) {
		if (extension instanceof Array && extension.length === 0) return "param extension should not be empty.";
		if (!extension) {
			if (params.type === "all" || !params.type) params.extension = [""];
...
function createNormalizeUrl(type, options = {}) {
	return function normalizeUrl(url, params) {
		...
		if (!options.skipNavigatorLock && navigatorLock === url && params.openType !== "appLaunch") return `${navigatorLock} locked`;
		if (!options.skipNavigatorLock && __uniConfig.ready) navigatorLock = url;
	};
}
//#endregion
...
function removeNonTabBarPages(targetPageId) {
	...
	for (const routeKey of keys) {
		...
		else page.$.__isActive = targetPageId !== void 0 && getPage$BasePage(page).id === targetPageId;
	}
...
var switchTab = /*#__PURE__*/ defineAsyncApi(API_SWITCH_TAB, ({ url, tabBarText, isAutomatedTesting }, { resolve, reject }) => {
	...
	return navigate({
		...
		tabBarText,
		isAutomatedTesting
	}, void 0).then(resolve).catch(reject);
...
//#endregion
//#region src/service/api/route/appRoute.ts
function normalizeAppRoutePath(path) {
	const route = getRouteOptions(path, true);
	const pagePath = route === null || route === void 0 ? void 0 : route.meta.route;
	return typeof pagePath === "string" ? pagePath : removeLeadingSlash((route === null || route === void 0 ? void 0 : route.path) || path);
}
function normalizeRewriteRoute({ url, preserveQuery }, event) {
	if (preserveQuery) url = parseUrl(url).path + stringifyQuery(event.query);
	const params = {
		url,
		openType: event.openType
	};
	const errMsg = createNormalizeUrl(event.openType, { skipNavigatorLock: true })(url, params);
	if (errMsg) return errMsg;
...
var appRouteRuntime = createAppRouteRuntime({ normalizeRewriteRoute });
var pendingProgrammaticRoutes = [];
var routeTransactions = /* @__PURE__ */ new WeakMap();
var pendingHistoryRoute;
var appRouteStarted = false;
var launchExecutor;
var resolveLaunchExecutor;
var launchExecutorReady = new Promise((resolve) => {
	resolveLaunchExecutor = resolve;
});
var singlePageAppRouteContext;
...
function dispatchAppRoute(context) {
	const event = context.event;
	if (event.notFound) invokeHook(getApp().vm, ON_PAGE_NOT_FOUND, {
		path: event.path,
		query: Object.assign({}, event.query),
...
function discardWebAppRouteTransaction(transaction) {
	...
	if (index !== -1) pendingProgrammaticRoutes.splice(index, 1);
}
function takePendingProgrammaticRoute(to) {
	const redirectedFrom = getOriginalRoute(to.redirectedFrom);
	const index = pendingProgrammaticRoutes.findIndex((transaction) => !transaction.cancelled && (transaction.finalFullPath === to.fullPath || transaction.finalFullPath === (redirectedFrom === null || redirectedFrom === void 0 ? void 0 : redirectedFrom.fullPath)));
	if (index !== -1) return pendingProgrammaticRoutes.splice(index, 1)[0];
}
function takePendingHistoryRoute(to) {
	const redirectedFrom = getOriginalRoute(to.redirectedFrom);
	if (pendingHistoryRoute && (pendingHistoryRoute.fullPath === to.fullPath || pendingHistoryRoute.fullPath === (redirectedFrom === null || redirectedFrom === void 0 ? void 0 : redirectedFrom.fullPath))) {
...
function getOriginalRoute(route) {
	while (route === null || route === void 0 ? void 0 : route.redirectedFrom) route = route.redirectedFrom;
	return route;
}
...
function bindRouteTransaction(route, transaction) {
	...
	if (originalRoute) routeTransactions.set(originalRoute, transaction);
}
function findRouteTransaction(route) {
	...
	return (transaction === null || transaction === void 0 ? void 0 : transaction.cancelled) ? void 0 : transaction;
}
function replaceTransactionRoute(router, transaction, route) {
	const resolved = resolveAppRoute(getRouteUrl(route), transaction.openType, route.matched.length === 0);
...
function createLaunchTransaction(_x, _x2) {
	return _createLaunchTransaction.apply(this, arguments);
}
function _createLaunchTransaction() {
	_createLaunchTransaction = _asyncToGenerator(function* (router, to) {
		...
		const transaction = createWebAppRouteTransaction(resolveFullPath(router, resolved.url), "appLaunch", resolved.context);
...
function registerWebAppRouteLaunchExecutor(executor) {
	...
	resolveLaunchExecutor === null || resolveLaunchExecutor === void 0 || resolveLaunchExecutor();
...
function setSinglePageAppRoute(resolved, originalUrl) {
	...
	if (resolved.url === originalUrl) return;
...
function dispatchWebAppRoute(route) {
	...
	if (route) {
		...
		if (transaction && !transaction.cancelled) {
			...
			const originalRoute = getOriginalRoute(route.redirectedFrom);
			if (originalRoute) routeTransactions.delete(originalRoute);
		}
	} else {
		context = singlePageAppRouteContext;
		singlePageAppRouteContext = void 0;
	}
	if (context) dispatchAppRoute(context);
}
function setWebAppRouteHistoryDirection(fullPath, direction, delta = 0) {
	pendingHistoryRoute = {
		fullPath,
...
function initWebAppRouteListener(router, { onRouteConfirmed, onMissingRoute }) {
	router.beforeEach(function() {
		var _ref = _asyncToGenerator(function* (to) {
			...
			if (!appRouteStarted) {
				...
				if (launch.redirect) return launch.redirect;
			...
			if (!transaction) {
				...
				if (historyRoute) {
					...
					const resolved = resolveAppRoute(historyRoute.fullPath, historyRoute.openType, originalRoute.matched.length === 0);
					transaction = createWebAppRouteTransaction(resolveFullPath(router, resolved.url), historyRoute.openType, resolved.context);
			...
			if (!transaction) transaction = findRouteTransaction(route);
			if (!transaction) {
				const originalRoute = getOriginalRoute(route.redirectedFrom) || route;
				const resolved = resolveAppRoute(getRouteUrl(originalRoute), API_NAVIGATE_TO, originalRoute.matched.length === 0);
				transaction = createWebAppRouteTransaction(resolveFullPath(router, resolved.url), API_NAVIGATE_TO, resolved.context);
	...
	router.afterEach((to, _from, failure) => {
		...
		if ((_transaction$context = transaction.context) === null || _transaction$context === void 0 ? void 0 : _transaction$context.event.notFound) {
...
function isCurrentTabBarPage(url) {
	...
	const currentPage = pages[pages.length - 1];
	if (!(currentPage === null || currentPage === void 0 ? void 0 : currentPage.$.__isTabBar)) return false;
...
function findTabBarPageId(url) {
	...
	for (const page of pages) {
		const $page = getPage$BasePage(page);
		if (path === $page.path || path === "/" && $page.meta.isEntry) return $page.id;
	}
}
function navigate({ type, url, tabBarText, events, isAutomatedTesting }, __id__) {
	...
	return new Promise((resolve, reject) => {
		...
		let transaction;
		{
			const appRoute = type !== "switchTab" || !isCurrentTabBarPage(url) ? resolveAppRoute(url, type) : void 0;
			routeUrl = (appRoute === null || appRoute === void 0 ? void 0 : appRoute.url) || url;
			const { path, query } = parseUrl(routeUrl);
			transaction = createWebAppRouteTransaction(router.resolve({
				path,
				query
			}).fullPath, type, appRoute === null || appRoute === void 0 ? void 0 : appRoute.context);
		}
		const { path, query } = parseUrl(routeUrl);
		const state = createPageState(type, type === "switchTab" ? findTabBarPageId(routeUrl) : __id__);
		if (transaction) {
		...
		router[type === "navigateTo" ? "push" : "replace"]({
			...
			if (type === "switchTab") {
				const finalTabBarText = routeUrl === url ? tabBarText : router.resolve({
					path,
					query
				}).meta.tabBarText;
...
var isHarmony2in1 = isHarmony && /PC/i.test(ua);
var isHarmonyTablet = isHarmony && /Tablet/i.test(ua);
...
function getTheme() {
	if (__uniConfig.darkmode == null || __uniConfig.darkmode === false) return void 0;
...
var _tabBar;
function useTabBar() {
	if (!_tabBar) _tabBar = __uniConfig.tabBar && reactive(initTabBarI18n(__uniConfig.tabBar));
...
var SAFE_AREA_INSET_PROPERTY_PREFIX = "--uni-safe-area-inset-";
function getSafeAreaInset(style, position) {
	const value = parseFloat(style.getPropertyValue(`${SAFE_AREA_INSET_PROPERTY_PREFIX}${position}`));
	return isNaN(value) ? import_out.default[position] : value;
}
	...
	setPageStyle(style) {
		...
		for (const key in style) switch (key) {
			...
			case "navigationStyle":
				...
				break;
			default: pageMeta[key] = style[key];
		}
	}
	$setPageStyle(style) {
...
function onPageShow(instance, pageMeta) {
	...
	if (isDialogPageInstance(getPageInstanceByChild(instance))) return;
	initPageScrollListener(instance, pageMeta);
}
function onPageReady(instance) {
	if (!updateCurPageBodyScopeId(instance) && process.env.NODE_ENV !== "production") console.warn("uni-page-body not found");
}
function updateCurPageBodyScopeId(instance) {
	var _getPageInstanceByChi, _pageRoot$querySelect;
	const pageRoot = (_getPageInstanceByChi = getPageInstanceByChild(instance)) === null || _getPageInstanceByChi === void 0 || (_getPageInstanceByChi = _getPageInstanceByChi.subTree) === null || _getPageInstanceByChi === void 0 ? void 0 : _getPageInstanceByChi.el;
	const pageBodyEl = pageRoot === null || pageRoot === void 0 || (_pageRoot$querySelect = pageRoot.querySelector) === null || _pageRoot$querySelect === void 0 ? void 0 : _pageRoot$querySelect.call(pageRoot, "uni-page-body");
	if (!pageBodyEl) return false;
	if (instance.type.styleIsolation !== "isolated") {
...
function getRealPath(filePath) {
	...
	if (filePath.indexOf("/") === 0) {
		if (filePath.indexOf("//") === 0) filePath = "https:" + filePath;
...
function getRootInfo(fields) {
	...
	if (fields.dataset) info.dataset = createDatasetSnapshot({});
	if (fields.rect) {
		info.left = 0;
		info.right = 0;
		info.top = 0;
...
function getNodeInfo(el, fields) {
	...
	if (fields.dataset) info.dataset = createDatasetSnapshot(getCustomDataset(el));
...
var QuerySelectorHelper = class QuerySelectorHelper {
	...
	getNodeInfo(element, fields) {
		...
		if (fields.dataset) nodeInfo.dataset = createDatasetSnapshot(getCustomDataset(element));
		return nodeInfo;
	}
};
	...
	return setupComponent(comp, {
		...
		setup(instance) {
			...
			onBeforeMount(() => {
				...
				if (!isDialogPageInstance(getPageInstanceByChild(instance))) dispatchWebAppRoute(__UNI_FEATURE_PAGES__ ? router.currentRoute.value : void 0);
			});
			onMounted(() => {
				var _instance$proxy;
			...
			onBeforeActivate(() => {
				if (!instance.__isVisible) {
					...
					if (!isDialogPageInstance(getPageInstanceByChild(instance))) {
						...
						dispatchWebAppRoute(__UNI_FEATURE_PAGES__ ? router.currentRoute.value : void 0);
		...
		setup(instance) {
			...
			const onLaunch = (launchRoute = route) => {
				...
				const launchOptions = initLaunchOptions({
					path: launchRoute.path.slice(1) || __uniRoutes[0].meta.route,
				...
				const originalUrl = __UNI_FEATURE_PAGES__ ? launchRoute.fullPath : launchRoute.path + stringifyQuery(launchRoute.query);
				const resolved = resolveAppRoute(originalUrl, "appLaunch", __UNI_FEATURE_PAGES__ && !launchRoute.matched.length);
				if (!__UNI_FEATURE_PAGES__) setSinglePageAppRoute(resolved, originalUrl);
				return resolved;
			};
			if (__UNI_FEATURE_PAGES__) registerWebAppRouteLaunchExecutor(function() {
				var _ref = _asyncToGenerator(function* (launchRoute) {
	...
	class List extends Container {
		...
		static formats(domNode) {
			...
			if (domNode.tagName === "UL") {
				if (domNode.hasAttribute("data-checked")) return domNode.getAttribute("data-checked") === "true" ? "checked" : "unchecked";
				else return "bullet";
			}
		}
		constructor(domNode) {
		...
		if (quillReady) {
			...
			switch (type) {
				...
				case "getSelectionText":
					...
					break;
				case "scrollIntoView": quill.scrollIntoView();
			}
			updateStatus(range);
...
var props$24 = /*#__PURE__*/ extend({}, props$25, {
	id: {
		type: String,
		default: ""
	},
...
var props$21 = /*#__PURE__*/ extend({}, props$22, {
	placeholderClass: {
		type: String,
		...
		const type = computed(() => {
			...
			switch (props.type) {
				...
				case "none":
					type = "text";
					break;
				default: type = INPUT_TYPES.includes(props.type) ? props.type : "text";
			}
...
function flatVNode(nodes) {
	...
	if (isArray(nodes)) nodes.forEach((vnode) => {
		if (isVNode(vnode)) {
			if (vnode.type === Fragment) array.push(...flatVNode(vnode.children));
...
Friction$2.prototype.s = function(t) {
...
Friction$2.prototype.ds = function(t) {
	if (void 0 === t) t = ((/* @__PURE__ */ new Date()).getTime() - this._startTime) / 1e3;
...
Friction$2.prototype.reconfigure = function(m, f) {
	this._m = m;
	this._f = 1e3 * f;
};
...
function useMovableViewTransform(rootRef, props, _scaleOffset, _scale, maxX, maxY, minX, minY, _translateX, _translateY, _SFA, _FA, _adjustScale, trigger) {
	...
	function _setX(val) {
		if (xMove.value) {
			if (val + _scaleOffset.x === _translateX.value) return _translateX;
			else {
				if (_SFA) _SFA.cancel();
	...
	function _setY(val) {
		if (yMove.value) {
			if (val + _scaleOffset.y === _translateY.value) return _translateY;
			else {
				if (_SFA) _SFA.cancel();
	...
	return () => {
		...
		switch (props.openType) {
			...
			case "reLaunch":
				...
				break;
			case "navigateBack": uni.navigateBack({
				delta: props.delta,
...
var Scroll = class {
	constructor(extent, friction, spring) {
		this._extent = extent;
		this._friction = friction || new Friction$1(.01);
...
function useProgressState(props) {
	...
	const outerBarStyle = computed(() => `background-color: ${props.backgroundColor}; height: ${rpx2px(props.strokeWidth)}px;`);
	...
	setup(props, { slots }) {
		...
		function getRadioStyle(checked) {
			...
			if (checked) {
				...
				} else if (props.activeBorderColor) style.borderColor = props.activeBorderColor;
			} else {
				if (props.borderColor) style.borderColor = props.borderColor;
				if (props.backgroundColor) style.backgroundColor = props.backgroundColor;
			}
		...
		return () => {
			...
			const hoverBorderColor = realCheckValue ? radioStyle === null || radioStyle === void 0 ? void 0 : radioStyle.borderColor : props.activeBorderColor;
			...
			return createVNode("uni-radio", mergeProps(booleanAttrs, {
				...
				"style": radioStyle
			}, [realCheckValue ? createSvgIconVNode(ICON_PATH_SUCCESS_NO_CIRCLE, props.disabled ? "currentColor" : iconColor, 18) : ""], 6), slots.default && slots.default()], 16, ["onClick", "id"]);
		};
	}
});
...
var nodeList2VNode = (scopeId, triggerItemClick, nodeList) => {
	...
	return nodeList.map((node) => {
		...
		if ((!hasOwn(node, "type") || node.type === "text") && isString(node.text) && node.text !== "") return createTextVNode(decodeEntities(node.text || ""));
...
function parseAttrs(attrs) {
	return attrs.reduce(function(pre, attr) {
		...
		if (pre[name]) {
			if (Array.isArray(pre[name])) pre[name].push(value);
...
function useScrollViewLoader(props, state, scrollTopNumber, scrollLeftNumber, trigger, rootRef, main, content, emit) {
	...
	function _scrollTopChanged(val) {
		if (realScrollY.value) {
			if (_innerSetScrollTop) _innerSetScrollTop = false;
	...
	function _scrollLeftChanged(val) {
		if (realScrollX.value) {
			if (_innerSetScrollLeft) _innerSetScrollLeft = false;
...
//#endregion
//#region ../uni-components/src/vue/slider/utils.ts
...
function useSliderState(props) {
	const _getBgColor = () => {
		...
		if (backgroundColor && backgroundColor !== "#e9e9e9") return backgroundColor;
	...
	const _getActiveColor = () => {
		...
		if (selectedColor && selectedColor !== "#e9e9e9") return selectedColor;
	...
	return {
		...
		setValueStyle: () => props.valueColor ? { color: props.valueColor } : void 0
	};
}
	...
	setup(props, { emit }) {
		...
		return () => {
			...
			return createVNode("uni-switch", mergeProps({
				...
				"style": thumbStyle
			}, null, 6)], 6), [[vShow, type === "switch"]]), withDirectives(createVNode("div", { "class": "uni-checkbox-input" }, [realCheckValue ? createSvgIconVNode(ICON_PATH_SUCCESS_NO_CIRCLE, props.foreColor || props.color || "currentColor", 22) : ""], 512), [[vShow, type === "checkbox"]])])], 16, ["id", "onClick"]);
		};
	}
});
...
var props$12 = /*#__PURE__*/ extend({}, props$22, {
	placeholderClass: {
		type: String,
		default: "input-placeholder"
	},
		...
		*/
		...
		return () => {
			...
			return createVNode("uni-list-view", {
				...
				"refresherDefaultStyle",
				"refresherBackground"
			]) : null, createVNode("div", { "class": "uni-list-view-content" }, [
				createVNode("div", { "style": placeholderHeadStyle.value }, null, 4),
				visibleVNode,
				createVNode("div", { "style": placeholderTailStyle.value }, null, 4)
			])], 6), createVNode(resize_sensor_default, {
				"initial": true,
				"onResize": onResize
			}, null, 8, ["onResize"])], 512);
		};
...
function rearrange(visibleVNode, containerRef, isVertical, state) {
	...
	function callback(child) {
		...
		if (childType === "StickySection") {
			...
			tempTotalSize += itemSize;
			if (!start && tempTotalSize > offsetMin) start = true;
			if (!start) tempHeadPlaceholderSize += itemSize;
			else if (start && end) tempTailPlaceholderSize += itemSize;
			if (start && !end) {
				tempVisibleSize += itemSize;
				status.visible.value = true;
...
var list_item_default = /*#__PURE__*/ defineBuiltInComponent({
	...
	setup(props, { slots, expose, attrs }) {
		...
		function updateSize() {
			if (!visible.value || status.cachedSizeUpdated) return;
			...
			if (translateValue.value != 0 && isDragging) {
				...
				switch (props.position) {
					...
					case "top":
						...
						break;
					case "left":
					case "right": transformValue = `translateX(${translateValue.value}px)`;
				}
				if (transformValue != "") {
	...
	if (mpType === "page") {
		...
		try {
			...
			if (($basePage === null || $basePage === void 0 ? void 0 : $basePage.openType) !== "preloadPage") {
				if (isDialogPageInstance(getPageInstanceByChild(instance))) invokeNewDialogPageHook(publicThis.$page, ON_SHOW);
...
function usePopupStyle(props, triangleColor = "#fcfcfd") {
...
function cleanupWebAppRoute(transaction) {
	switch (transaction.openType) {
		...
		case API_SWITCH_TAB:
			...
			break;
		case API_NAVIGATE_BACK: removeCurrentPages(transaction.delta || 1);
	}
}
...
function useGesture(props, videoState, videoRef, fullscreenState) {
	...
	function onTouchstart(event) {
		...
		state.volumeOld = 0;
		if (fullscreenState.fullscreen) event.stopPropagation();
	}
...
function useFullscreen(trigger, containerRef, videoRef, userActionState, rootRef) {
	...
	function toggleFullscreen(val) {
		...
		let mockFullScreen;
		if (val) {
			if ((document.fullscreenEnabled || document.webkitFullscreenEnabled) && (!isSafari || userActionState.userAction)) container[document.fullscreenEnabled ? "requestFullscreen" : "webkitRequestFullscreen"]();
	...
	useSubscribe((type, data) => {
		...
		switch (type) {
			...
			case "sendDanmu":
				options = data;
				break;
			case "playbackRate": options = data.rate;
		}
...
var onWebInvokeAppService = ({ name, arg }) => {
	if (name === "postMessage") {} else switch (name) {
		...
		case "reLaunch":
			uni.reLaunch(arg);
			break;
		case "redirectTo": uni.redirectTo(arg);
	}
};
//#endregion
...
var index_x_default$5 = /*#__PURE__*/ defineBuiltInComponent({
	...
	setup(props, { emit }) {
		...
		const renderIframe = () => {
			...
			watchEffect(() => {
				for (const key in $attrs.value) if (hasOwn($attrs.value, key)) {
					const attr = $attrs.value[key];
					iframe[key] = attr;
				}
			});
			watchEffect(() => {
			...
			const context = {
				...
				translate(data) {
					onMapReady((map, maps, trigger) => {
						...
						const speed = maps.geometry.spherical.computeDistanceBetween(a, b) / 1e3 / ((typeof duration === "number" ? duration : 1e3) / 36e5);
						const MapsEvent = maps.event || maps.Event;
...
var request = /*#__PURE__*/ defineTaskApi(API_REQUEST, ({ url, data, header = {}, method, dataType, responseType, enableChunked, withCredentials, timeout = __uniConfig.networkTimeout.request }, { resolve, reject }) => {
	...
	if (method !== "GET") {
		if (isString(data) || data instanceof ArrayBuffer) body = data;
	...
	if (!enableChunked) {
		...
		*/
		if (typeof window.fetch === void 0 || typeof window.AbortController === void 0) throw new Error("fetch or AbortController is not supported in this environment");
...
var getLocation = /*#__PURE__*/ defineAsyncApi(API_GET_LOCATION, ({ type, altitude, highAccuracyExpireTime, isHighAccuracy }, { resolve, reject }) => {
	...
	new Promise((resolve, reject) => {
		if (navigator.geolocation) navigator.geolocation.getCurrentPosition((res) => resolve({ coords: res.coords }), reject, {
			...
			timeout: highAccuracyExpireTime || 1e5
		});
...
function useToastIcon(props) {
	...
	watchEffect(() => {
		if (props.visible) onThemeChange$2(_onThemeChange);
...
function setTabBar(type, args, resolve, reject) {
	...
	switch (type) {
		...
		case API_REMOVE_TAB_BAR_BADGE: setProperties(tabBar.list[index], setTabBarBadgeProps, {
			badge: "",
			redDot: false
		});
	}
...
function useState() {
	...
	const showTopWindow = computed(() => __UNI_FEATURE_TOPWINDOW__ && route.meta.topWindow !== false && topWindowMediaQuery.value);
	const showLeftWindow = computed(() => __UNI_FEATURE_LEFTWINDOW__ && route.meta.leftWindow !== false && leftWindowMediaQuery.value);
	const showRightWindow = computed(() => __UNI_FEATURE_RIGHTWINDOW__ && route.meta.rightWindow !== false && rightWindowMediaQuery.value);
	const layoutState = reactive({
		topWindowMediaQuery,
		showTopWindow,
	...
	function _resetFormData() {
		switch (props.mode) {
			...
			case mode.MULTISELECTOR:
				...
				break;
			case mode.DATE:
			case mode.TIME: state.valueSync = "";
		}
	}
...
var actionSheet_default = /*#__PURE__*/ _plugin_vue_export_helper_default(/* @__PURE__ */ defineComponent({
	...
	setup(__props) {
		...
		onLoad((options) => {
			...
			if (appInfo.appLanguage != null) language.value = appInfo.appLanguage;
			else if (deviceInfo.osLanguage != null) language.value = deviceInfo.osLanguage;
		...
		const containerStyle = computed(() => {
			...
			res["left"] = `${Math.max(0, center - 150)}px`;
		...
		const triangleStyle = computed(() => {
			var _backgroundColor$valu;
			if (Object.keys(popover).length == 0) return {};
			const res = {};
			const borderColor = (_backgroundColor$valu = backgroundColor.value) !== null && _backgroundColor$valu !== void 0 ? _backgroundColor$valu : theme.value == "dark" ? "#2C2C2C" : "#fcfcfd";
			const top = popover.top;
			const left = popover.left;
			const width = popover.width;
			const height = popover.height;
			const center = left + width / 2;
			const contentLeft = Math.max(0, center - 150);
			let triangleLeft = Math.max(12, center - contentLeft);
		...
		const computedBackgroundColor = computed(() => {
			var _backgroundColor$valu2;
			return (_backgroundColor$valu2 = backgroundColor.value) !== null && _backgroundColor$valu2 !== void 0 ? _backgroundColor$valu2 : theme.value == "dark" ? "#2C2C2C" : "#ffffff";
		});
	...
	setup(__props) {
		...
		const getPoi = (type) => {
			...
			if (searchVal != "" && searchVal.length > 0) callUniMapCo("search", {
				...
				var _res$getJSON;
				let poisData = (_res$getJSON = res.getJSON("result")) === null || _res$getJSON === void 0 || (_res$getJSON = _res$getJSON.getJSON("result")) === null || _res$getJSON === void 0 ? void 0 : _res$getJSON.getArray("data");
				poiHandle(poisData);
				...
				var _res$getJSON2;
				let poisData = (_res$getJSON2 = res.getJSON("result")) === null || _res$getJSON2 === void 0 || (_res$getJSON2 = _res$getJSON2.getJSON("result")) === null || _res$getJSON2 === void 0 ? void 0 : _res$getJSON2.getArray("pois");
				poiHandle(poisData);
		...
		const getSystemInfo = () => {
			...
			mapHeight.value = (screenHeight - safeArea.top - safeArea.bottom) * .6;
...
var uniModal_default = /*#__PURE__*/ _plugin_vue_export_helper_default(/* @__PURE__ */ defineComponent({
	...
	setup(__props) {
		...
		const updateUI = () => {
			...
			else confirmColor.value = theme.value == "dark" ? "#7388a2" : "#4A5E86";
			if (isValidColor(inputCancelColor.value)) cancelColor.value = inputCancelColor.value;
			else cancelColor.value = theme.value == "dark" ? "#a5a5a5" : "#000000";
		...
		onLoad((options) => {
			...
			*/
			const deviceInfo = uni.getDeviceInfo();
			const windowInfo = uni.getWindowInfo();
			maxScrollHeight.value = `${Math.floor(windowInfo.screenHeight * .55)}px`;
			/**
			* add since 2025-04-03 目前暂不支持设置app language
			*/
			const appBaseInfo = uni.getAppBaseInfo();
			if (appBaseInfo.appLanguage != null) language.value = appBaseInfo.appLanguage;
			else if (deviceInfo.osLanguage != null) language.value = deviceInfo.osLanguage;
...
//#endregion
//#region ../../uni-ext-api/uni_modules/uni-previewImage/components/uni-previewImageItem/Friction.uts.ts
var Friction = class {
	constructor(mass, frictionForce) {
		_defineProperty(this, "mass", void 0);
		_defineProperty(this, "frictionForce", void 0);
		...
		_defineProperty(this, "endPosition", null);
		_defineProperty(this, "lastTimeElapsed", null);
		_defineProperty(this, "totalTimeToStop", 0);
		this.mass = mass;
	...
	setVelocity(x, y) {
		...
		if (Number.isNaN(this.totalTimeToStop)) this.totalTimeToStop = Math.abs(y / this.acceleration.y);
		if (Number.isNaN(this.totalTimeToStop)) this.totalTimeToStop = 0;
	...
	positionAtTime(t) {
		if (t == null) t = (Date.now() - this.startTime) / 1e3;
		if (t > this.totalTimeToStop) {
			t = this.totalTimeToStop;
			this.lastTimeElapsed = t;
		}
		var x = this.velocity.x * t + .5 * this.acceleration.x * t * t + this.startPosition.x;
		var y = this.velocity.y * t + .5 * this.acceleration.y * t * t + this.startPosition.y;
		if (this.acceleration.x > 0 && x < this.endPosition.x || this.acceleration.x < 0 && x > this.endPosition.x) x = this.endPosition.x;
		if (this.acceleration.y > 0 && y < this.endPosition.y || this.acceleration.y < 0 && y > this.endPosition.y) y = this.endPosition.y;
		return {
			x,
			y
		};
	...
	velocityAtTime(t) {
		if (t == null) t = (Date.now() - this.startTime) / 1e3;
		if (t > this.totalTimeToStop) t = this.totalTimeToStop;
		return {
...
//#endregion
//#region ../../uni-ext-api/uni_modules/uni-previewImage/components/uni-previewImageItem/uni-previewImageItem.vue?vue&type=script&setup=true&lang.ts
var DEFAULT_DISTANCE = 4;
var FAST_SLIDE_LENGTH = 10;
var SCALE_BOUNDARY_BUFFER = 2;
var MIN_SCALE = 1;
var MAX_SCALE = 3;
var ANIMATION_DURATION = 200;
//#endregion
//#region ../../uni-ext-api/uni_modules/uni-previewImage/components/uni-previewImageItem/uni-previewImageItem.vue
var uni_previewImageItem_default = /*#__PURE__*/ _plugin_vue_export_helper_default(/* @__PURE__ */ defineComponent({
	...
	setup(__props) {
		...
		const resetImageState = () => {
			var _imageView$value, _imageView$value2, _imageView$value3, _imageView$value4;
			...
			(_imageView$value = imageView.value) === null || _imageView$value === void 0 || _imageView$value.style.setProperty("height", "100%");
			(_imageView$value2 = imageView.value) === null || _imageView$value2 === void 0 || _imageView$value2.style.setProperty("transition-duration", "0ms");
			(_imageView$value3 = imageView.value) === null || _imageView$value3 === void 0 || _imageView$value3.style.setProperty("transform-origin", "0px 0px");
			(_imageView$value4 = imageView.value) === null || _imageView$value4 === void 0 || _imageView$value4.style.setProperty("transform", "translate(0px,0px) scale(1)");
		};
		watch(() => props.src, (newValue, oldValue) => {
			if (props.resetOnSrcChange) resetImageState();
			if (newValue != "") getSrcLocalPath(newValue);
			else {
				srcPath.value = "";
		...
		onMounted(() => {
			...
			if (dpr == null) devicePixelRatio.value = 1;
			else devicePixelRatio.value = dpr;
			const systemInfo = uni.getSystemInfoSync();
		...
		const previewImageError = (e) => {
			var _mask$value;
			(_mask$value = mask.value) === null || _mask$value === void 0 || _mask$value.style.setProperty("point-events", "none");
			loadingFinished.value = true;
			loadError.value = true;
		};
		const onLongPressAction = () => {
			var longPressAction = props.longPressAction;
			if (longPressAction != null && longPressAction.itemList.length > 0) uni.showActionSheet({
				itemList: longPressAction.itemList,
				itemColor: longPressAction.itemColor,
				success: (e) => {
					var _success;
					(_success = longPressAction.success) === null || _success === void 0 || _success.call(longPressAction, {
						tapIndex: e.tapIndex,
						index: props.index
					});
				...
				fail() {
					var _fail;
					(_fail = longPressAction.fail) === null || _fail === void 0 || _fail.call(longPressAction, new PreviewImageErrorImpl(1101001, UniErrorPreviewImage));
					uni.$emit("__UNIPREVIEWLONGPRESS", {
						type: "fail",
						tapIndex: -1,
						index: -1
		...
		const preventDefaultScall = (e) => {
			e === null || e === void 0 || e.preventDefault();
			e === null || e === void 0 || e.stopPropagation();
		};
		const setSwiperTouchDisabled = (disabled) => {
			if (swiperTouchDisabled.value == disabled) return;
		...
		const shouldDelegateMoveToParent = (slideX, slideY) => {
			if (scaleSize.value <= 1 || screenWidth.value <= 0) return false;
			if (Math.abs(slideX) <= Math.abs(slideY)) return false;
		...
		const delegateCurrentMoveToParent = (e, currentSlideTouch) => {
			...
			downPoint.value = {
				x: currentSlideTouch.clientX,
				y: currentSlideTouch.clientY
			};
		...
		const caculatorTransformOrigin = (e) => {
			var originalCenterX;
			var originalCenterY;
			if (e != null) {
				if (e.touches.length >= 2) {
					...
					if (scaleSize.value * imageHeight.value < screenHeight.value) originalCenterY = screenHeight.value / 2;
					if (imageHeight.value > screenHeight.value && scaleSize.value >= 1) originalCenterY = originalCenterY - imageTop.value / scaleSize.value;
					...
					if (inDoubleTapMode.value && scaleSize.value == 2 && lastSlideTouch.value != null && lastSlideTouch.value.length == 1) {
						...
						if (scaleSize.value * imageHeight.value < screenHeight.value) originalCenterY = screenHeight.value / 2;
						if (imageHeight.value > screenHeight.value) originalCenterY = originalCenterY - imageTop.value;
						transformOrigin.value = [originalCenterX, originalCenterY];
					}
				}
			}
		};
		const updateStyle = (e, xDistance, yDistance) => {
			var _imageView$value5, _imageView$value6, _imageView$value7;
			...
			if (scaleSize.value <= 1 && imageHeight.value > screenHeight.value) {
				...
				if (imageTop.value > 0) imageTop.value = 0;
				else if (imageTop.value < minTop) imageTop.value = minTop;
				if (!Number.isNaN(yDistance) && Math.abs(yDistance - downPoint.value.y) > DEFAULT_DISTANCE) preventDefaultScall(e);
				...
				if (visualBottom < screenHeight.value) imageTop.value = imageTop.value + (screenHeight.value - visualBottom);
				else if (visualTop > 0 && !inScaleBounceBack.value) imageTop.value = imageTop.value - visualTop;
				if (!Number.isNaN(yDistance) && Math.abs(yDistance - downPoint.value.y) > DEFAULT_DISTANCE) preventDefaultScall(e);
			} else if (!inScaleMode.value) {
				var scaledTopWithoutTranslate = transformOrigin.value[1] + (imageContentTop - transformOrigin.value[1]) * scaleSize.value;
			...
			(_imageView$value5 = imageView.value) === null || _imageView$value5 === void 0 || _imageView$value5.style.setProperty("transition-duration", shouldAnimate ? "200ms" : "0ms");
			(_imageView$value6 = imageView.value) === null || _imageView$value6 === void 0 || _imageView$value6.style.setProperty("transform-origin", transformOrigin.value[0] + "px " + transformOrigin.value[1] + "px");
			(_imageView$value7 = imageView.value) === null || _imageView$value7 === void 0 || _imageView$value7.style.setProperty("transform", "translate(" + imageLeft.value + "px," + imageTop.value + "px) scale(" + scaleSize.value + ")");
		};
		...
		function doTransform(callback) {
			requestId.value = requestAnimationFrame(() => {
				callback();
				if (!_friction.value.isDone()) doTransform(callback);
			});
		}
		...
		const onstart = (e) => {
			...
			if (e.touches.length >= 2) {
				...
				markScaleGesture();
				setSwiperTouchDisabled(true);
			} else setSwiperTouchDisabled(scaleSize.value > 1 && !isAtHorizontalBoundary());
			withAnimation.value = false;
			cancelAnimationFrame(requestId.value);
			clearTimeout(clickTimeoutId.value);
			lastSlideTouch.value = e.touches;
			...
			if (needExecLongPress.value) longPressActionTimeoutId.value = setTimeout(() => {
				if (needExecLongPress.value) onLongPressAction();
			}, 350);
		};
		const onmove = (e) => {
			if (isPreviewImaqeClosed.value) return;
			if (e.touches.length == 1) {
				...
				if (lastSlideTouch.value != null && lastSlideTouch.value.length != 1) {
					...
					historyY.value = [imageTop.value, imageTop.value];
					historyT.value = [e.timeStamp, e.timeStamp];
					if (scaleSize.value > 1 || inScaleMode.value || scaleGestureActive.value) preventDefaultScall(e);
					else onInterceptTouchEvent(e);
					return;
				}
				if (lastSlideTouch.value != null) {
					...
					if (downX > DEFAULT_DISTANCE || downY > DEFAULT_DISTANCE) {
						...
						lastSlideTouch.value = e.touches;
						needExecLongPress.value = false;
					} else needExecLongPress.value = true;
				} else lastSlideTouch.value = e.touches;
		...
		const onend = (e) => {
			...
			if (wasScaleMode) markScaleGesture();
			inScaleMode.value = false;
			needExecLongPress.value = false;
			clearTimeout(longPressActionTimeoutId.value);
			...
			if (wasScaleMode) {
				...
				lastSlideTouch.value = null;
				scaleGestureActive.value = false;
				if (e.touches.length == 0) scaleGestureInProgress.value = false;
				delegateMoveToParent.value = false;
				setSwiperTouchDisabled(false);
				return;
			}
			if (historyY.value[0] == 0 && historyY.value[1] == 0 && historyX.value[0] == 0 && historyX.value[1] == 0) {
				...
				if (current - lastTouchEndTime.value < 350) {
					...
					if (lastSlideTouch.value != null && lastSlideTouch.value.length == 1 && e.timeStamp - startTimestamp.value < 160) {
						var downX = Math.abs(lastSlideTouch.value[0].clientX - downPoint.value.x);
						var downY = Math.abs(lastSlideTouch.value[0].clientY - downPoint.value.y);
						if (downX < FAST_SLIDE_LENGTH && downY < FAST_SLIDE_LENGTH && !shouldIgnoreClick()) clickTimeoutId.value = setTimeout(() => {
							closePreviewImage();
						}, 200);
					}
					lastTouchEndTime.value = current;
				...
				if (!Number.isNaN(x0)) x = x0 + imageLeft.value;
				var y = imageTop.value;
				if (!Number.isNaN(y0)) y = y0 + imageTop.value;
				_friction.value.setEndPosition(x, y);
				doTransform(() => {
					var p = _friction.value.positionAtTime(null);
					if (Number.isNaN(p.x) && Number.isNaN(p.y)) cancelAnimationFrame(requestId.value);
					if (!Number.isNaN(p.x)) imageLeft.value = p.x;
					if (!Number.isNaN(p.y)) imageTop.value = p.y;
					updateStyle(e, NaN, NaN);
				});
			}
			lastSlideTouch.value = null;
		...
		const caculatorImageSize = (imgWidth, imgHeight) => {
			...
			if (scaleImageSize > screenHeight.value) {
				var _imageView$value8;
				imageHeight.value = scaleImageSize;
				imageMode.value = "aspectFill";
				(_imageView$value8 = imageView.value) === null || _imageView$value8 === void 0 || _imageView$value8.style.setProperty("height", scaleImageSize + "px");
			} else {
				var _imageView$value9;
				imageMode.value = "aspectFit";
				(_imageView$value9 = imageView.value) === null || _imageView$value9 === void 0 || _imageView$value9.style.setProperty("height", "100%");
			}
		...
		const onImageLoad = (e) => {
			var _mask$value2;
			(_mask$value2 = mask.value) === null || _mask$value2 === void 0 || _mask$value2.style.setProperty("point-events", "none");
		...
		const reloadImage = (e) => {
			...
			(_mask$value3 = mask.value) === null || _mask$value3 === void 0 || _mask$value3.style.setProperty("point-events", "none");
			loadingFinished.value = false;
			loadError.value = false;
		...
		return (_ctx, _cache) => {
			const _component_image = image_default;
			const _component_view = view_default;
			const _component_loading = index_x_default$1;
			return openBlock(), createBlock(_component_view, {
				style: { "flex": "1" },
				...
				default: withCtx(() => [
					...
					!loadingFinished.value ? (openBlock(), createBlock(_component_view, {
						...
						default: withCtx(() => [createVNode(_component_loading, { style: {
							"margin": "auto",
							"width": "54px",
							"height": "54px",
							"border-color": "#d3d3d3"
					...
					loadError.value ? (openBlock(), createBlock(_component_view, {
						...
						default: withCtx(() => [createVNode(_component_image, {
...
//#endregion
//#region ../../uni-ext-api/uni_modules/uni-previewImage/components/uni-previewImageNonVapor/uni-previewImageNonVapor.vue
var uni_previewImageNonVapor_default = /*#__PURE__*/ _plugin_vue_export_helper_default(/* @__PURE__ */ defineComponent({
	...
	setup(__props, { emit: __emit }) {
		...
		const shouldRender = (index) => {
			var _props$urls;
			const len = (_props$urls = props.urls) === null || _props$urls === void 0 ? void 0 : _props$urls.length;
			...
			if (props.loop) {
				...
				if (index == prev || index == next || index == prev2 || index == next2) return true;
			} else if (Math.abs(index - c) <= 2) return true;
			return false;
		};
		const handlePreviewImageChanged = (e) => {
			emit("previewImageChanged", e);
		};
		const handlePreviewImageFinish = (e) => {
			emit("previewImageFinish", e);
		};
		return (_ctx, _cache) => {
			const _component_swiper_item = swiper_item_default;
			const _component_swiper = swiper_default;
			const _component_text = text_default;
			const _component_view = view_default;
			return openBlock(), createElementBlock(Fragment, null, [
				createVNode(_component_swiper, {
					...
					default: withCtx(() => [props.urls != null ? (openBlock(true), createElementBlock(Fragment, { key: 0 }, renderList(props.urls, (item, index) => {
						return openBlock(), createBlock(_component_swiper_item, { class: "uni-preview-image-swiper-item" }, {
							default: withCtx(() => [shouldRender(index) ? (openBlock(), createBlock(uni_previewImageItem_default, {
								key: 0,
								index,
								src: item,
				...
				props.indicator == "number" ? (openBlock(), createBlock(_component_view, {
					key: 0,
					class: "uni-preview-image-number-indicator-layout"
				}, {
					default: withCtx(() => [createVNode(_component_text, { class: "uni-preview-image-number-indicator" }, {
						default: withCtx(() => [createTextVNode(toDisplayString(props.numberIndicator), 1)]),
						_: 1
					})]),
					_: 1
				})) : createCommentVNode("", true),
				props.indicator == "default" ? withDirectives((openBlock(), createBlock(_component_view, {
					key: 1,
					class: "uni-preview-image-default-indicator-layout"
				}, {
					default: withCtx(() => [(openBlock(true), createElementBlock(Fragment, null, renderList(props.urls.length, (i) => {
						return openBlock(), createBlock(_component_view, { class: normalizeClass(["uni-preview-image-default-indicator", props.indicatorIndex + 1 == i ? "uni-preview-image-default-indicator-active" : "uni-preview-image-default-indicator-default"]) }, null, 8, ["class"]);
					}), 256))]),
					_: 1
				}, 512)), [[vShow, props.urls != null]]) : createCommentVNode("", true)
			], 64);
		};
	}
...
//#endregion
//#region ../../uni-ext-apiuni:previewImage.vue
var previewImage_default = /* @__PURE__ */ defineComponent({
	__name: "previewImage",
	setup(__props) {
		const urls = ref(null);
		const current = ref(0);
		...
		const updateIndicator = (index) => {
			var _urls$value;
			numberIndicator.value = index + 1 + " / " + ((_urls$value = urls.value) === null || _urls$value === void 0 ? void 0 : _urls$value.length);
		...
		const __onPreviewLoadCallback = (result) => {
			...
			if (result["current"] != null) {
				...
				if (typeof c == "number") {
					var d = c;
					if (urls.value == null || d < 0 || d >= urls.value.length) d = 0;
					current.value = d;
			...
			if (result["indicator"] != null) indicator.value = result["indicator"];
			if (result["longPressActions"] != null) longPressAction.value = {
				itemList: result["longPressActions"]["itemList"],
				itemColor: result["longPressActions"]["itemColor"]
			};
			if (result["loop"] != null) loop.value = result["loop"];
		...
		return (_ctx, _cache) => {
			return openBlock(), createBlock(uni_previewImageNonVapor_default, {
				urls: unref(urls),
...
//#endregion
//#region ../../uni-ext-api/uni_modules/uni-previewImage/utssdk/unierror.uts.ts
/**
* 错误主题
*/
var UniErrorPreviewImage$1 = "uni-previewImage";
/**
* 错误码
* @UniError
*/
var PreviewImageUniErrors = /* @__PURE__ */ new Map([
	[1101001, "user cancel"],
...
var PreviewImageErrorImpl$1 = class extends UniError {
	constructor(errCode, uniErrorSubject) {
		var _PreviewImageUniError;
		super();
		_defineProperty(this, "errCode", void 0);
		this.errSubject = uniErrorSubject;
		this.errCode = errCode;
		this.errMsg = (_PreviewImageUniError = PreviewImageUniErrors.get(errCode)) !== null && _PreviewImageUniError !== void 0 ? _PreviewImageUniError : "";
	}
};
//#endregion
//#region ../../uni-ext-api/uni_modules/uni-previewImage/utssdk/PreviewImage.uts.ts
function __previewImage(option) {
	if (option.urls.length == 0) {
		var _option$fail, _option$complete;
		let error = new PreviewImageErrorImpl$1(1001, UniErrorPreviewImage$1);
		(_option$fail = option.fail) === null || _option$fail === void 0 || _option$fail.call(option, error);
		(_option$complete = option.complete) === null || _option$complete === void 0 || _option$complete.call(option, error);
		return;
	}
	uni.$once("__onPreviewLoad", () => {
		...
		if (option.longPressActions != null) object.longPressActions = {
	...
	uni.$on("__UNIPREVIEWLONGPRESS", (value) => {
		if (option.longPressActions != null) {
			...
			if (type == "success") {
				var _option$longPressActi, _option$longPressActi2, _option$longPressActi3, _option$longPressActi4;
				var success = {
					tapIndex,
					index
				};
				(_option$longPressActi = option.longPressActions) === null || _option$longPressActi === void 0 || (_option$longPressActi2 = _option$longPressActi.success) === null || _option$longPressActi2 === void 0 || _option$longPressActi2.call(_option$longPressActi, success);
				(_option$longPressActi3 = option.longPressActions) === null || _option$longPressActi3 === void 0 || (_option$longPressActi4 = _option$longPressActi3.complete) === null || _option$longPressActi4 === void 0 || _option$longPressActi4.call(_option$longPressActi3, success);
			} else {
				var _option$longPressActi5, _option$longPressActi6, _option$longPressActi7, _option$longPressActi8;
				var fail = new PreviewImageErrorImpl$1(1101001, UniErrorPreviewImage$1);
				(_option$longPressActi5 = option.longPressActions) === null || _option$longPressActi5 === void 0 || (_option$longPressActi6 = _option$longPressActi5.fail) === null || _option$longPressActi6 === void 0 || _option$longPressActi6.call(_option$longPressActi5, fail);
				(_option$longPressActi7 = option.longPressActions) === null || _option$longPressActi7 === void 0 || (_option$longPressActi8 = _option$longPressActi7.complete) === null || _option$longPressActi8 === void 0 || _option$longPressActi8.call(_option$longPressActi7, fail);
			}
		}
	});
	uni.openDialogPage({
		...
		success(_) {
			var _option$success, _option$complete2;
			let success = {
				errMsg: "ok",
				"errSubject": UniErrorPreviewImage$1
			};
			(_option$success = option.success) === null || _option$success === void 0 || _option$success.call(option, success);
			(_option$complete2 = option.complete) === null || _option$complete2 === void 0 || _option$complete2.call(option, success);
		},
		fail(_) {
			var _option$fail2, _option$complete3;
			let error = new PreviewImageErrorImpl$1(1101010, UniErrorPreviewImage$1);
			(_option$fail2 = option.fail) === null || _option$fail2 === void 0 || _option$fail2.call(option, error);
			(_option$complete3 = option.complete) === null || _option$complete3 === void 0 || _option$complete3.call(option, error);
		}
	});
}
function __closePreviewImage(option) {
	var _option$success2, _option$complete4;
	uni.$emit("__CLOSEPREVIEWIMAGE", null);
	let callback = { errMsg: "ok" };
	(_option$success2 = option.success) === null || _option$success2 === void 0 || _option$success2.call(option, callback);
	(_option$complete4 = option.complete) === null || _option$complete4 === void 0 || _option$complete4.call(option, callback);
}
//#endregion
//#region ../../uni-ext-api/uni_modules/uni-previewImage/utssdk/index.uts.ts
var previewImage$1 = (option) => {
	__previewImage(option);
};
var closePreviewImage$1 = (option) => {
	__closePreviewImage(option);
};
//#endregion
//#region src/x/service/api/pages/previewImage.ts
function mergeComponentStyles(page, components) {
	const styles = isArray(page.styles) ? [...page.styles] : [];
	components.forEach((component) => {
		if (isArray(component.styles)) styles.push(...component.styles);
	});
	page.styles = styles;
}
var registerPreviewImageOnce = /* @__PURE__ */ once(() => {
	mergeComponentStyles(previewImage_default, [uni_previewImageNonVapor_default, uni_previewImageItem_default]);
	registerSystemRoute("uni:previewImage", previewImage_default);
});
var closePreviewImage = () => {
	registerPreviewImageOnce();
	closePreviewImage$1();
};
var previewImage = /*#__PURE__*/ defineAsyncApi(API_PREVIEW_IMAGE, (args, { resolve, reject }) => {
	registerPreviewImageOnce();
	previewImage$1(extend({
		success: (res) => {
			resolve(res);
		},
		fail: (err) => {
...
//#endregion
//#region src/x/service/api/base/uasm.ts
var uasmCache = /* @__PURE__ */ new Map();
function loadUasm(module) {
	const descriptor = module;
	if (!descriptor || typeof descriptor.id !== "string" || typeof descriptor.loader !== "function") return Promise.reject(/* @__PURE__ */ new Error("uni.loadUasm 参数未经过编译处理"));
	let promise = uasmCache.get(descriptor.id);
	if (!promise) {
		promise = descriptor.loader().then((loaded) => {
			if (typeof loaded.default !== "function") throw new Error(`uasm 插件[${descriptor.id}]的默认导出必须是函数`);
			return loaded.default();
		});
		uasmCache.set(descriptor.id, promise);
		promise.catch(() => {
			if (uasmCache.get(descriptor.id) === promise) uasmCache.delete(descriptor.id);
		});
	}
	return promise;
}
