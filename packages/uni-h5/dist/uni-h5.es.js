	...
	setup(props, { slots }) {
		...
		return () => {
			...
			return createVNode("uni-radio", mergeProps(booleanAttrs, {
				...
				"style": radioStyle
			}, [realCheckValue ? createSvgIconVNode(ICON_PATH_SUCCESS_NO_CIRCLE, props.disabled ? "currentColor" : iconColor, 18) : ""], 6), slots.default && slots.default()], 4)], 16, ["id", "onClick"]);
		};
	}
});
...
function useSliderState(props, sliderValue) {
	...
	return {
		...
		setActiveColor: () => withBackgroundColor({ width: _getValueWidth() }, _getActiveColor()),
	...
	function getList() {
		...
		if (mapInfo.type === MapType.GOOGLE) {
			...
			new google.maps.places.PlacesService(document.createElement("div"))[state.searching ? "textSearch" : "nearbySearch"]({
				...
				if (page) {
					if (!page.hasNextPage) listState.hasNextPage = false;
...
var pageBody_default = /*#__PURE__*/ defineSystemComponent({
	...
	setup(props, ctx) {
		...
		return () => {
			...
			return createVNode(Fragment, null, [pageRefreshTsx, createVNode("uni-page-wrapper", mergeProps({ "ref": wrapperRef }, pageRefresh.value), [createVNode("uni-page-body", null, [renderSlot(ctx.slots, "default")]), null], 16)]);
		};
	}
});
