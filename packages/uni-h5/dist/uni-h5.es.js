...
function onInvokeViewMethod({ id: id$1, name, args }, pageId) {
	name = normalizeViewMethodName(pageId, name);
	const publish = (res) => {
...
function getWindowTop() {
	...
	return top ? top + import_out$2.default.top : 0;
}
function getWindowOffset() {
	...
	return {
		top,
		bottom: bottom ? bottom + import_out$2.default.bottom : 0,
		left: left ? left + import_out$2.default.left : 0,
		right: right ? right + import_out$2.default.right : 0,
...
function updateStyle(id$1, content) {
	let style = sheetsMap.get(id$1);
	...
	sheetsMap.set(id$1, style);
}
function removeStyle(id$1) {
	let style = sheetsMap.get(id$1);
	if (style) {
...
function initRouteMeta(pageMeta, id$1) {
	...
	const res = extend({ id: id$1 }, globalStyle, pageMeta);
...
function onAppEnterForeground(enterOptions$1) {
	const page = getCurrentPage();
	invokeHook(getApp(), ON_SHOW, enterOptions$1);
...
const defineBuiltInComponent = ((options) => {
	...
	const { props: props$33, mixins } = options;
	if (!props$33 || !props$33.animation) (mixins || (options.mixins = [])).push(animation_default);
...
function useHover(props$33) {
	...
	function hoverReset() {
		requestAnimationFrame(() => {
			...
			}, parseInt(props$33.hoverStayTime));
		});
	}
	...
	function handleHoverStart(evt) {
		...
		if (!props$33.hoverClass || props$33.hoverClass === "none" || props$33.disabled) return;
		if (props$33.hoverStopPropagation) evt._hoverPropagationStopped = true;
		hoverTouch = true;
		hoverStartTimer = setTimeout(() => {
			hovering.value = true;
			if (!hoverTouch) hoverReset();
		}, parseInt(props$33.hoverStartTime));
	}
	function onTouchend() {
		handleHoverEnd();
	}
...
function useBooleanAttr(props$33, keys) {
	if (isString(keys)) keys = [keys];
	return keys.reduce((res, key) => {
		if (props$33[key]) res[key] = true;
		return res;
	}, Object.create(null));
}
...
var label_default = /* @__PURE__ */ defineBuiltInComponent({
	name: "Label",
	props: labelProps,
	setup(props$33, { slots }) {
		const rootRef = ref(null);
		const pageId = useCurrentPageId();
		const handlers = useProvideLabel();
		const pointer = computed(() => props$33.for || slots.default && slots.default.length);
		const _onClick = withWebEvent(($event) => {
			...
			if (props$33.for) UniViewJSBridge.emit("uni-label-click-" + pageId + "-" + props$33.for, $event, true);
...
function useListeners(props$33, listeners$1) {
	_addListeners(props$33.id, listeners$1);
	watch(() => props$33.id, (newId, oldId) => {
		_removeListeners(oldId, listeners$1, true);
		_addListeners(newId, listeners$1, true);
	});
	onUnmounted(() => {
		_removeListeners(props$33.id, listeners$1);
	});
}
function _addListeners(id$1, listeners$1, watch$1) {
	...
	Object.keys(listeners$1).forEach((name) => {
		if (watch$1) {
			if (name.indexOf("@") !== 0 && name.indexOf("uni-") !== 0) UniViewJSBridge.on(`uni-${name}-${pageId}-${id$1}`, listeners$1[name]);
		} else if (name.indexOf("uni-") === 0) UniViewJSBridge.on(name, listeners$1[name]);
		else if (id$1) UniViewJSBridge.on(`uni-${name}-${pageId}-${id$1}`, listeners$1[name]);
	});
}
function _removeListeners(id$1, listeners$1, watch$1) {
	...
	Object.keys(listeners$1).forEach((name) => {
		if (watch$1) {
			if (name.indexOf("@") !== 0 && name.indexOf("uni-") !== 0) UniViewJSBridge.off(`uni-${name}-${pageId}-${id$1}`, listeners$1[name]);
		} else if (name.indexOf("uni-") === 0) UniViewJSBridge.off(name, listeners$1[name]);
		else if (id$1) UniViewJSBridge.off(`uni-${name}-${pageId}-${id$1}`, listeners$1[name]);
	});
}
var button_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$33, { slots }) {
		...
		const { hovering, binding } = useHover(props$33);
		const { t: t$1 } = /* @__PURE__ */ useI18n();
		const onClick = withWebEvent((e$1, isLabelClick) => {
			if (props$33.disabled) return e$1.stopImmediatePropagation();
			if (isLabelClick) rootRef.value.click();
			const formType = props$33.formType;
			if (formType) {
				if (!uniForm) return;
		...
		useListeners(props$33, { "label-click": onClick });
		return () => {
			const hoverClass = props$33.hoverClass;
			const booleanAttrs = useBooleanAttr(props$33, "disabled");
			const loadingAttrs = useBooleanAttr(props$33, "loading");
			const plainAttrs = useBooleanAttr(props$33, "plain");
...
function initPageMeta(id$1) {
	if (__UNI_FEATURE_PAGES__) return reactive(normalizePageMeta(JSON.parse(JSON.stringify(initRouteMeta(useRoute().meta, id$1)))));
	return reactive(normalizePageMeta(JSON.parse(JSON.stringify(initRouteMeta(__uniRoutes[0].meta, id$1)))));
}
function normalizePageMeta(pageMeta) {
	if (__UNI_FEATURE_PULL_DOWN_REFRESH__) {
		...
		if (enablePullDownRefresh) {
			...
			if (style !== "custom" && type !== "transparent") pullToRefresh.offset += NAVBAR_HEIGHT + import_out$1.default.top;
...
function invokeSuccess(id$1, name, res) {
	...
	return invokeCallback(id$1, extend(res || {}, result));
}
function invokeFail(id$1, name, errMsg, errRes = {}) {
	...
	return invokeCallback(id$1, extend({ errMsg: apiErrMsg }, errRes));
}
...
function checkDeviceWidth() {
	let windowWidth, pixelRatio$1, platform;
	{
...
const createVideoContext = /* @__PURE__ */ defineSyncApi(API_CREATE_VIDEO_CONTEXT, (id$1, context) => {
	if (context) return new VideoContext(id$1, getPageIdByVm(context));
	return new VideoContext(id$1, getPageIdByVm(getCurrentPageVm()));
});
...
var operateMapWrap = (id$1, pageId, type, options) => {
	operateMap(id$1, pageId, type, options, (res) => {
...
const createMapContext = /* @__PURE__ */ defineSyncApi(API_CREATE_MAP_CONTEXT, (id$1, context) => {
	if (context) return new MapContext(id$1, getPageIdByVm(context));
	return new MapContext(id$1, getPageIdByVm(getCurrentPageVm()));
...
function convertContext(result) {
	if (result && result.contextInfo) {
		const { id: id$1, type, page } = result.contextInfo;
		const ContextClass = ContextClasss[type];
		result.context = new ContextClass(id$1, page);
...
const ShowActionSheetOptions = { formatArgs: { itemColor: "#000" } };
const API_SHOW_LOADING = "showLoading";
const ShowLoadingProtocol = {
	title: String,
	mask: Boolean
};
const ShowLoadingOptions = { formatArgs: {
	title: "",
	mask: false
} };
...
function navigate({ type, url, tabBarText, events, isAutomatedTesting }, __id__) {
	...
	return new Promise((resolve, reject) => {
		const state$3 = createPageState(type, __id__);
		router[type === "navigateTo" ? "push" : "replace"]({
			...
			if (type === "navigateTo") {
				...
				if (!meta.eventChannel) meta.eventChannel = new EventChannel(state$3.__id__, events);
				else if (events) {
				...
				return isAutomatedTesting ? resolve({ __id__: state$3.__id__ }) : resolve({ eventChannel: meta.eventChannel });
			}
			return isAutomatedTesting ? resolve({ __id__: state$3.__id__ }) : resolve();
		});
	});
}
...
function operateMap(id$1, pageId, type, data, operateMapCallback$1) {
	UniServiceJSBridge.invokeViewMethod("map." + id$1, {
		type,
		data
...
function getSameOriginUrl(url) {
	...
	a$1.href = url;
	if (a$1.origin === location.origin) return Promise.resolve(url);
...
function useResizeSensorLifecycle(rootRef, props$33, update, reset) {
	onActivated(reset);
	onMounted(() => {
		if (props$33.initial) nextTick(update);
		const rootEl = rootRef.value;
...
var canvas_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$33, { emit: emit$2, slots }) {
		initHidpiOnce();
		const rootRef = ref(null);
		...
		const { _listeners } = useListeners$1(props$33, $listeners, trigger);
		const { _handleSubscribe, _resize } = useMethods(props$33, canvas, actionsWaiting);
		useSubscribe(_handleSubscribe, useContextInfo(props$33.canvasId), true);
		onMounted(() => {
			_resize();
		});
		return () => {
			const { canvasId, disableScroll } = props$33;
...
function useListeners$1(props$33, Listeners, trigger) {
	return { _listeners: computed(() => {
		...
		events.forEach((event) => {
			...
			if (props$33.disableScroll && event === "onTouchmove") eventHandler.push(onEventPrevent);
			$listeners[event] = eventHandler;
		});
		return $listeners;
	}) };
}
function useMethods(props$33, canvasRef, actionsWaiting) {
	let _actionsDefer = [];
	let _images = {};
	const _pixelRatio = computed(() => props$33.hidpi ? pixelRatio : 1);
	function _resize(size) {
		...
		if (canvas.width > 0 && canvas.height > 0) {
			...
			wrapper(canvas, props$33.hidpi);
			context.putImageData(imageData, 0, 0);
		} else wrapper(canvas, props$33.hidpi);
	}
	function actionsChanged({ actions, reserve }, resolve) {
		...
		for (let index$4 = 0; index$4 < actions.length; index$4++) {
			const action = actions[index$4];
	...
	function getImageData({ x = 0, y = 0, width, height, destWidth, destHeight, hidpi = true, dataType: dataType$1, quality = 1, type = "png" }, resolve) {
		...
		let data;
		const maxWidth$1 = canvas.offsetWidth - x;
...
var checkbox_group_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$33, { emit: emit$2, slots }) {
		const rootRef = ref(null);
		useProvideCheckGroup(props$33, useCustomEvent(rootRef, emit$2));
		return () => {
			return createVNode("uni-checkbox-group", { "ref": rootRef }, [slots.default && slots.default()], 512);
		};
	}
});
function useProvideCheckGroup(props$33, trigger) {
	const fields$1 = [];
...
var checkbox_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$33, { slots }) {
		const rootRef = ref(null);
		const checkboxChecked = ref(props$33.checked);
		const checkboxCheckedBool = computed(() => {
			return checkboxChecked.value === "true" || checkboxChecked.value === true;
		});
		const checkboxValue = ref(props$33.value);
		function getCheckBoxStyle(checked) {
			if (props$33.disabled) return {
				backgroundColor: "#E1E1E1",
				borderColor: "#D1D1D1"
			};
			const style = {};
			if (checked) {
				if (props$33.activeBorderColor) style.borderColor = props$33.activeBorderColor;
				if (props$33.activeBackgroundColor) style.backgroundColor = props$33.activeBackgroundColor;
			} else {
				if (props$33.borderColor) style.borderColor = props$33.borderColor;
				if (props$33.backgroundColor) style.backgroundColor = props$33.backgroundColor;
			}
			return style;
		}
		...
		watch([() => props$33.checked, () => props$33.value], ([newChecked, newModelValue]) => {
		...
		useListeners(props$33, { "label-click": _onClick });
		return () => {
			const booleanAttrs = useBooleanAttr(props$33, "disabled");
			let realCheckValue;
			...
			return createVNode("uni-checkbox", mergeProps(booleanAttrs, {
				...
				"style": { "--HOVER-BD-COLOR": props$33.activeBorderColor }
			}, [createVNode("div", {
				"class": ["uni-checkbox-input", { "uni-checkbox-input-disabled": props$33.disabled }],
				"style": checkboxStyle.value
			}, [realCheckValue ? createSvgIconVNode(ICON_PATH_SUCCESS_NO_CIRCLE, props$33.disabled ? "#ADADAD" : props$33.foreColor || props$33.iconColor || props$33.color, 22) : ""], 6), slots.default && slots.default()], 4)], 16, ["id", "onClick"]);
		};
	}
});
...
function useKeyboard$1(props$33, elRef, trigger) {
...
function mention_default(Quill) {
	...
	class MentionBlot extends Embed {
		static create(data) {
			...
			const id$1 = data.id == null ? "" : data.id;
...
function useQuill(props$33, rootRef, trigger) {
	let quillReady;
	let skipMatcher;
	let quill;
	watch(() => props$33.readOnly, (value) => {
		if (quillReady) {
			quill.enable(!value);
			if (value) quill.blur();
		}
	});
	watch(() => props$33.placeholder, (value) => {
		if (quillReady) setPlaceHolder(value);
	});
	watch(() => props$33.type, (value) => {
	...
	function initQuill(imageResizeModules) {
		...
		const options = {
			...
			placeholder: props$33.placeholder
		};
			...
			$el.addEventListener(name, ($event) => {
				...
				if (name === "input") {
					if (getBaseSystemInfo().platform === "ios") {
						...
						setPlaceHolder(regExpContent && regExpContent.replace(/\s/g, "") ? "" : props$33.placeholder);
					}
					$event.stopPropagation();
	...
	onMounted(() => {
		...
		if (props$33.showImgSize) imageResizeModules.push("DisplaySize");
		if (props$33.showImgToolbar) imageResizeModules.push("Toolbar");
		if (props$33.showImgResize) imageResizeModules.push("Resize");
...
var editor_default = /* @__PURE__ */ defineBuiltInComponent({
	name: "Editor",
	props: /* @__PURE__ */ extend({}, props$27, {
		id: {
			type: String,
			default: ""
		},
	...
	setup(props$33, { emit: emit$2 }) {
		...
		useQuill(props$33, rootRef, trigger);
		useKeyboard$1(props$33, rootRef, trigger);
		return () => {
...
var icon_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$33) {
		const rootRef = ref(null);
		const path = computed(() => ICONS[props$33.type]);
		return () => {
			const { value } = path;
			return createVNode("uni-icon", { "ref": rootRef }, [value && value.d && createSvgIconVNode(value.d, props$33.color || value.c, rpx2px(props$33.size))], 512);
		};
	}
});
var props$28 = {
...
var image_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$33, { emit: emit$2 }) {
		const rootRef = ref(null);
		const state$3 = useImageState(rootRef, props$33);
		const trigger = useCustomEvent(rootRef, emit$2);
		const { fixSize } = useImageSize(rootRef, props$33, state$3);
		useImageLoader(state$3, props$33, rootRef, fixSize, trigger);
		return () => {
			return createVNode("uni-image", { "ref": rootRef }, [createVNode("div", { "style": state$3.modeStyle }, null, 4), FIX_MODES[props$33.mode] ? createVNode(resize_sensor_default, { "onResize": fixSize }, null, 8, ["onResize"]) : createVNode("span", null, null)], 512);
		};
	}
});
function useImageState(rootRef, props$33) {
	const imgSrc = ref("");
	const modeStyleRef = computed(() => {
		let size = "auto";
		let position = "";
		const opts = IMAGE_MODES[props$33.mode];
		if (!opts) {
			position = "0% 0%";
			size = "100% 100%";
		} else {
	...
	const state$3 = reactive({
		rootEl: rootRef,
		src: computed(() => props$33.src ? getRealPath(props$33.src) : ""),
		origWidth: 0,
		origHeight: 0,
		origStyle: {
			width: "",
	...
	onMounted(() => {
		const rootEl = rootRef.value;
		state$3.origWidth = rootEl.clientWidth || 0;
		state$3.origHeight = rootEl.clientHeight || 0;
	});
	return state$3;
}
function useImageLoader(state$3, props$33, rootRef, fixSize, trigger) {
	let img;
	let draggableImg;
	const setState = (width = 0, height = 0, imgSrc = "") => {
		state$3.origWidth = width;
		state$3.origHeight = height;
		state$3.imgSrc = imgSrc;
	};
	const loadImage = (src) => {
		if (!src) {
			resetImage();
		...
		img.onload = (evt) => {
			...
			img.draggable = props$33.draggable;
		...
		img.onerror = (evt) => {
			setState();
			resetImage();
			trigger("error", evt, { errMsg: `GET ${state$3.src} 404 (Not Found)` });
		};
		img.src = src;
	};
	const resetImage = () => {
	...
	watch(() => state$3.src, (value) => loadImage(value));
	watch(() => state$3.imgSrc, (value) => {
		if (!value && draggableImg) {
			draggableImg.remove();
			draggableImg = null;
		}
	});
	onMounted(() => loadImage(state$3.src));
...
function useImageSize(rootRef, props$33, state$3) {
	const fixSize = () => {
		const { mode: mode$1 } = props$33;
		const names = FIX_MODES[mode$1];
		if (!names) return;
		const { origWidth, origHeight } = state$3;
	...
	const resetSize = () => {
		...
		const { origStyle: { width, height } } = state$3;
		style.width = width;
		style.height = height;
	};
	watch(() => props$33.mode, (value, oldValue) => {
...
function useUserAction() {
	const state$3 = reactive({ userAction: false });
	onMounted(() => {
		addInteractListener(state$3);
	});
	onBeforeUnmount(() => {
		removeInteractListener(state$3);
	});
	return { state: state$3 };
}
function useScopedAttrs() {
	const state$3 = reactive({ attrs: {} });
	onMounted(() => {
		let instance = getCurrentInstance();
		while (instance) {
			const scopeId = instance.type.__scopeId;
			if (scopeId) state$3.attrs[scopeId] = "";
...
const props$16 = /* @__PURE__ */ extend({}, {
	name: {
		type: String,
		default: ""
	},
...
function useBase(props$33, rootRef, emit$2) {
	const fieldRef = ref(null);
	const trigger = useCustomEvent(rootRef, emit$2);
	const selectionStart = computed(() => {
		const selectionStart$1 = Number(props$33.selectionStart);
		return isNaN(selectionStart$1) ? -1 : selectionStart$1;
	});
	const selectionEnd = computed(() => {
		const selectionEnd$1 = Number(props$33.selectionEnd);
		return isNaN(selectionEnd$1) ? -1 : selectionEnd$1;
	});
	const cursor = computed(() => {
		const cursor$1 = Number(props$33.cursor);
		return isNaN(cursor$1) ? -1 : cursor$1;
	});
	const maxlength = computed(() => {
		var maxlength$1 = Number(props$33.maxlength);
		return isNaN(maxlength$1) ? 140 : maxlength$1;
	});
	let value = "";
	value = getValueString(props$33.modelValue, props$33.type) || getValueString(props$33.value, props$33.type);
	const state$3 = reactive({
		value,
		valueOrigin: value,
		maxlength,
		focus: props$33.focus,
		composing: false,
		selectionStart,
		selectionEnd,
		cursor
	});
	watch(() => state$3.focus, (val) => emit$2("update:focus", val));
	watch(() => state$3.maxlength, (val) => state$3.value = state$3.value.slice(0, val), { immediate: false });
	return {
		fieldRef,
		state: state$3,
		trigger
...
function useValueSync(props$33, state$3, emit$2, trigger, fieldRef) {
	let lastUserInputValue = null;
	let valueChangeFn = null;
	valueChangeFn = debounce((val) => {
		const fieldElement = fieldRef.value;
		const newValue = getValueString(val, props$33.type);
		if (fieldElement && document.activeElement === fieldElement && newValue === lastUserInputValue) return;
		state$3.value = newValue;
	}, 100, {
		setTimeout,
		clearTimeout
	});
	watch(() => props$33.modelValue, valueChangeFn);
	watch(() => props$33.value, valueChangeFn);
...
function useAutoFocus(props$33, fieldRef) {
	const { state: userActionState } = useUserAction();
	const needFocus = computed(() => props$33.autoFocus || props$33.focus);
	function focus() {
...
function useEvent(fieldRef, state$3, props$33, trigger, triggerInput, beforeInput) {
	function checkSelection() {
		const field = fieldRef.value;
		if (field && state$3.focus && state$3.selectionStart > -1 && state$3.selectionEnd > -1 && field.type !== "number") {
			field.selectionStart = state$3.selectionStart;
			field.selectionEnd = state$3.selectionEnd;
		}
	}
	function checkCursor() {
		const field = fieldRef.value;
		if (field && state$3.focus && state$3.selectionStart < 0 && state$3.selectionEnd < 0 && state$3.cursor > -1 && field.type !== "number") field.selectionEnd = field.selectionStart = state$3.cursor;
	}
	...
	function initField() {
		...
		const onFocus = function(event) {
			state$3.focus = true;
			trigger("focus", event, { value: state$3.value });
			checkSelection();
			checkCursor();
		};
		const onInput = function(event, force) {
			event.stopPropagation();
			if (isFunction(beforeInput) && beforeInput(event, state$3) === false) return;
			state$3.value = field.value;
			if (!state$3.composing || !props$33.ignoreCompositionEvent) triggerInput(event, {
				value: field.value,
				cursor: getFieldSelectionEnd(field)
			}, force);
		};
		const onBlur = function(event) {
			if (state$3.composing) {
				state$3.composing = false;
				onInput(event, true);
			}
			state$3.focus = false;
			trigger("blur", event, {
		...
		function _onComposition(event) {
			if (!props$33.ignoreCompositionEvent) trigger(event.type, event, { value: event.data });
		}
	}
	watch([() => state$3.selectionStart, () => state$3.selectionEnd], checkSelection);
	watch(() => state$3.cursor, checkCursor);
	watch(() => fieldRef.value, initField);
}
function useField(props$33, rootRef, emit$2, beforeInput) {
	UniViewJSBridgeSubscribe();
	const { fieldRef, state: state$3, trigger } = useBase(props$33, rootRef, emit$2);
	const { triggerInput } = useValueSync(props$33, state$3, emit$2, trigger, fieldRef);
	useAutoFocus(props$33, fieldRef);
	useKeyboard$1(props$33, fieldRef, trigger);
	const { state: scopedAttrsState } = useScopedAttrs();
	useFormField("name", state$3);
	useEvent(fieldRef, state$3, props$33, trigger, triggerInput, beforeInput);
	return {
		fieldRef,
		state: state$3,
		scopedAttrsState,
...
function resolveDigitDecimalPoint(event, cache, state$3, input, resetCache) {
	if (cache.value) {
		if (event.data === ".") {
			if (cache.value.slice(-1) === ".") {
				state$3.value = input.value = cache.value = cache.value.slice(0, -1);
				return false;
			}
			...
			if (resolveDigitDecimalPointDeleteContentBackward()) {
				if (cache.value.slice(-2, -1) === ".") {
					cache.value = state$3.value = input.value = cache.value.slice(0, -2);
					return true;
				}
			}
		}
...
var props$26 = /* @__PURE__ */ extend({}, props$16, {
	placeholderClass: {
		type: String,
...
function useCache(props$33, type) {
	if (type.value === "number") {
		const value = typeof props$33.modelValue === "undefined" ? props$33.value : props$33.modelValue;
		const cache = ref(typeof value !== "undefined" && value !== null ? value.toLocaleString() : "");
		watch(() => props$33.modelValue, (value$1) => {
			cache.value = typeof value$1 !== "undefined" && value$1 !== null ? value$1.toLocaleString() : "";
		});
		watch(() => props$33.value, (value$1) => {
			cache.value = typeof value$1 !== "undefined" && value$1 !== null ? value$1.toLocaleString() : "";
		});
		return cache;
	} else return ref("");
...
var input_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	props: props$26,
	emits: ["confirm", ...emit],
	setup(props$33, { emit: emit$2, expose }) {
		const INPUT_TYPES = [
			"text",
			"number",
			"idcard",
		...
		const type = computed(() => {
			let type$1 = "";
			switch (props$33.type) {
				case "text":
					type$1 = "text";
					if (props$33.confirmType === "search") type$1 = "search";
					break;
				case "idcard":
					type$1 = "text";
					break;
				...
				case "none":
					...
					break;
				default:
					type$1 = INPUT_TYPES.includes(props$33.type) ? props$33.type : "text";
					break;
			}
			return props$33.password ? "password" : type$1;
		});
		const autocomplete = computed(() => {
			const camelizeIndex = AUTOCOMPLETES.indexOf(props$33.textContentType);
			const kebabCaseIndex = AUTOCOMPLETES.indexOf(hyphenate(props$33.textContentType));
			return AUTOCOMPLETES[camelizeIndex !== -1 ? camelizeIndex : kebabCaseIndex !== -1 ? kebabCaseIndex : 0];
		});
		const inputmode = computed(() => {
			if (props$33.inputmode !== void 0) return props$33.inputmode;
			if (INPUT_MODES.includes(props$33.type)) return props$33.type;
			return {
				number: "numeric",
				digit: "decimal",
				idcard: "text"
		...
		let cache = useCache(props$33, type);
		let resetCache = { fn: null };
		const rootRef = ref(null);
		const { fieldRef, state: state$3, scopedAttrsState, fixDisabledColor, trigger } = useField(props$33, rootRef, emit$2, (event, state$4) => {
			...
			if (type.value === "number") {
				...
				if (input.validity && !input.validity.valid) {
					...
					const res = resolveDigitDecimalPoint(event, cache, state$4, input, resetCache);
					if (typeof res === "boolean") return res;
					cache.value = state$4.value = input.value = cache.value === "-" ? "" : cache.value;
					return false;
				} else {
					const res = resolveDigitDecimalPoint(event, cache, state$4, input, resetCache);
					if (typeof res === "boolean") return res;
				...
				if (state$4.maxlength > 0 && input.value.length > state$4.maxlength && !isPaste(event)) {
					input.value = cache.value = state$4.value;
					return false;
				}
			}
		});
		watch(() => state$3.value, (value) => {
			if (props$33.type === "number" && !(cache.value === "-" && value === "")) cache.value = value.toString();
		});
		watch(() => props$33.maxlength, (length) => {
			length = parseInt(length, 10);
			const realValue = state$3.value.slice(0, length);
			realValue !== state$3.value && (state$3.value = realValue);
		});
		const NUMBER_TYPES = ["number", "digit"];
		const step = computed(() => NUMBER_TYPES.includes(props$33.type) ? props$33.step : "");
		function onKeyUpEnter(event) {
			if (event.key !== "Enter") return;
			const input = event.target;
			event.stopPropagation();
			trigger("confirm", event, { value: input.value });
			!props$33.confirmHold && input.blur();
		}
		expose({ $triggerInput: (detail) => {
			emit$2("update:modelValue", detail.value);
			emit$2("update:value", detail.value);
			state$3.value = detail.value;
		} });
		return () => {
			let inputNode = props$33.disabled && fixDisabledColor ? createVNode("input", {
				"key": "disabled-input",
				"ref": fieldRef,
				"value": state$3.value,
				"tabindex": "-1",
				"readonly": !!props$33.disabled,
				"type": type.value,
				"maxlength": state$3.maxlength,
				"step": step.value,
				"class": "uni-input-input",
				"style": props$33.cursorColor ? { caretColor: props$33.cursorColor } : {},
				...
				"onInput": withModifiers((event) => {
					...
					if (type.value === "number" && state$3.maxlength > 0 && value.length > state$3.maxlength) {
						if (isPaste(event)) state$3.value = value.slice(0, state$3.maxlength);
						return;
					}
					if (value.length === 0 && event.inputType === "insertText" && event.data === ".") return;
					state$3.value = value;
				}, ["stop"]),
				"disabled": !!props$33.disabled,
				"type": type.value,
				"maxlength": state$3.maxlength,
				"step": step.value,
				"enterkeyhint": props$33.confirmType,
				"pattern": props$33.type === "number" ? "[0-9]*" : void 0,
				"class": "uni-input-input",
				"style": props$33.cursorColor ? { caretColor: props$33.cursorColor } : {},
			...
			return createVNode("uni-input", { "ref": rootRef }, [createVNode("div", { "class": "uni-input-wrapper" }, [withDirectives(createVNode("div", mergeProps(scopedAttrsState.attrs, {
				"style": props$33.placeholderStyle,
				"class": ["uni-input-placeholder", props$33.placeholderClass]
			}), [props$33.placeholder], 16), [[vShow, !(state$3.value.length || cache.value === "-" || cache.value.includes("."))]]), props$33.confirmType === "search" ? createVNode("form", {
				"action": "",
...
var movable_area_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$33, { slots }) {
		...
		let { setContexts, events: movableAreaEvents } = useMovableAreaState(props$33, rootRef);
...
function useMovableAreaState(props$33, rootRef) {
	const width = ref(0);
	const height = ref(0);
	const gapV = reactive({
		x: null,
	...
	function _updateScale(e$1) {
		if (e$1 && e$1 !== 1) {
			if (props$33.scaleArea) movableViewContexts.forEach(function(item) {
				item._setScale(e$1);
			});
...
function useTouchtrack(element, method, useCancel) {
	...
	let x1 = 0;
	let y1 = 0;
	const fn = function($event, state$3, x, y) {
		if (method({
...
var movable_view_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	emits: ["change", "scale"],
	setup(props$33, { slots, emit: emit$2 }) {
		const rootRef = ref(null);
		const { setParent } = useMovableViewState(props$33, useCustomEvent(rootRef, emit$2), rootRef);
		return () => {
...
function useMovableViewTransform(rootRef, props$33, _scaleOffset, _scale, maxX, maxY, minX, minY, _translateX, _translateY, _SFA, _FA, _adjustScale, trigger) {
	const dampingNumber = computed(() => {
		let val = Number(props$33.damping);
		return isNaN(val) ? 20 : val;
	});
	const xMove = computed(() => props$33.direction === "all" || props$33.direction === "horizontal");
	const yMove = computed(() => props$33.direction === "all" || props$33.direction === "vertical");
	const xSync = ref(_getPx(props$33.x));
	const ySync = ref(_getPx(props$33.y));
	watch(() => props$33.x, (val) => {
		xSync.value = _getPx(val);
	});
	watch(() => props$33.y, (val) => {
	...
	function _animationTo(x, y, scale, source, r, o$1) {
		...
		if (!props$33.scale) scale = _scale.value;
		let limitXY = _getLimitXY(x, y);
		x = limitXY.x;
		y = limitXY.y;
		if (!props$33.animation) {
...
function useMovableViewInit(props$33, rootRef, trigger, _scale, _oldScale, _isScaling, _translateX, _translateY, _SFA, _FA) {
	const scaleMinNumber = computed(() => {
		let val = Number(props$33.scaleMin);
		return isNaN(val) ? .1 : val;
	});
	const scaleMaxNumber = computed(() => {
		let val = Number(props$33.scaleMax);
		return isNaN(val) ? 10 : val;
	});
	const scaleValueSync = ref(Number(props$33.scaleValue) || 1);
	...
	const { FAandSFACancel, _getLimitXY, _animationTo, _setTransform, _revise, dampingNumber, xMove, yMove, xSync, ySync, _STD } = useMovableViewTransform(rootRef, props$33, _scaleOffset, _scale, maxX, maxY, minX, minY, _translateX, _translateY, _SFA, _FA, _adjustScale, trigger);
	...
	function _setScaleMinOrMax() {
		if (!props$33.scale) return false;
...
function useMovableViewState(props$33, trigger, rootRef) {
	...
	const frictionNumber = computed(() => {
		let val = Number(props$33.friction);
		return isNaN(val) || val <= 0 ? 2 : val;
	});
	const _friction = new Friction$1(1, frictionNumber.value);
	watch(() => props$33.disabled, () => {
		__handleTouchStart();
	});
	const { _updateOldScale, _endScale, _setScale, scaleValueSync, _updateBoundary, _updateOffset, _updateWH, _scaleOffset, minX, minY, maxX, maxY, FAandSFACancel, _getLimitXY, _setTransform, _revise, dampingNumber, xMove, yMove, xSync, ySync, _STD } = useMovableViewInit(props$33, rootRef, trigger, _scale, _oldScale, _isScaling, _translateX, _translateY, _SFA, _FA);
	function __handleTouchStart() {
		if (!_isScaling.value) {
			if (!props$33.disabled) {
				disableScrollBounce({ disable: true });
				FAandSFACancel();
	...
	function __handleTouchMove(event) {
		if (!_isScaling.value && !props$33.disabled && _isTouching) {
			...
			if (!_checkCanMove) {
				...
				let source = "touch";
				if (x < minX.value) if (props$33.outOfBounds) {
					source = "touch-out-of-bounds";
					x = minX.value - _declineX.x(minX.value - x);
				} else x = minX.value;
				else if (x > maxX.value) if (props$33.outOfBounds) {
					source = "touch-out-of-bounds";
					x = maxX.value + _declineX.x(x - maxX.value);
				} else x = maxX.value;
				if (y < minY.value) if (props$33.outOfBounds) {
					source = "touch-out-of-bounds";
					y = minY.value - _declineY.x(minY.value - y);
				} else y = minY.value;
				else if (y > maxY.value) if (props$33.outOfBounds) {
					source = "touch-out-of-bounds";
	...
	function __handleTouchEnd() {
		if (!_isScaling.value && !props$33.disabled && _isTouching) {
			...
			_isTouching = false;
			if (!_checkCanMove && !_revise("out-of-bounds") && props$33.inertia) {
		...
		if (!props$33.outOfBounds && !props$33.inertia) FAandSFACancel();
	}
	function setParent() {
		if (!_isMounted.value) return;
		FAandSFACancel();
		let scale = props$33.scale ? scaleValueSync.value : 1;
		_updateOffset();
		_updateWH(scale);
		_updateBoundary();
...
function createNavigatorOnClick(props$33) {
	return () => {
		if (props$33.openType !== "navigateBack" && !props$33.url) {
			console.error("<navigator/> should have url attribute when using navigateTo, redirectTo, reLaunch or switchTab");
			return;
		}
		const animationDuration = parseInt(props$33.animationDuration);
		const onFail = (error) => {
			console.error(error.errMsg);
		};
		switch (props$33.openType) {
			case "navigate":
				uni.navigateTo({
					url: props$33.url,
					animationType: props$33.animationType || "pop-in",
					animationDuration,
					fail: onFail
				});
				break;
			...
			case "navigateBack":
				uni.navigateBack({
					delta: props$33.delta,
					animationType: props$33.animationType || "pop-out",
					animationDuration,
					fail: onFail
				});
				break;
...
var navigator_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$33, { slots }) {
		...
		const { hovering, binding } = useHover(props$33);
		const onClick = createNavigatorOnClick(props$33);
		return () => {
			const { hoverClass, url } = props$33;
			const hasHoverClass = props$33.hoverClass && props$33.hoverClass !== "none";
			const innerNode = props$33.renderLink ? createVNode("a", {
...
function useState$4(props$33) {
	const state$3 = reactive({
		value: reactive([...props$33.value]),
		height: 34
	});
	watch(() => props$33.value, (val, oldVal) => {
		state$3.value.length = val.length;
		val.forEach((val$1, index$4) => {
			if (val$1 !== state$3.value[index$4]) state$3.value.splice(index$4, 1, val$1);
		});
	});
	return state$3;
}
var picker_view_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$33, { slots, emit: emit$2 }) {
		...
		const trigger = useCustomEvent(rootRef, emit$2);
		const state$3 = useState$4(props$33);
		const resizeSensorRef = ref(null);
		const onMountedCallback = () => {
			const resizeSensor = resizeSensorRef.value;
			resizeSensor && (state$3.height = resizeSensor.$el.offsetHeight);
		};
		...
		const getPickerViewColumn = function(columnInstance) {
			return computed({
				get() {
					...
					return state$3.value[index$4] || 0;
				},
				set(current) {
					const index$4 = getItemIndex(columnInstance.vnode);
					if (index$4 < 0) return;
					if (state$3.value[index$4] !== current) {
						state$3.value[index$4] = current;
						const value = state$3.value.map((val) => val);
		...
		provide("pickerViewProps", props$33);
		provide("pickerViewState", state$3);
		return () => {
			...
			return createVNode("uni-picker-view", { "ref": rootRef }, [createVNode(resize_sensor_default, {
				"ref": resizeSensorRef,
				"onResize": ({ height }) => state$3.height = height
...
function createAnimation$1(scroll, onScroll, onEnd) {
	...
	function startAnimation$1(state$4, scroll$1, onScroll$1, onEnd$1) {
		if (!state$4 || !state$4.cancelled) {
			onScroll$1(scroll$1);
			const isDone = scroll$1.done();
			if (!isDone) {
				if (!state$4.cancelled) state$4.id = requestAnimationFrame(startAnimation$1.bind(null, state$4, scroll$1, onScroll$1, onEnd$1));
			}
			if (isDone && onEnd$1) onEnd$1(scroll$1);
		}
	}
	function cancel(state$4) {
		if (state$4 && state$4.id) cancelAnimationFrame(state$4.id);
		if (state$4) state$4.cancelled = true;
	}
	startAnimation$1(state$3, scroll, onScroll, onEnd);
	return {
		cancel: cancel.bind(null, state$3),
		model: scroll
	};
}
var Scroller = class {
...
var picker_view_column_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$33, { slots, emit: emit$2 }) {
		...
		function updatesScroller() {
			if (scroller && !updatesScrollerRequest) {
				...
				nextTick(() => {
					...
					let current = Math.min(state$3.current, state$3.length - 1);
		...
		watch(() => currentRef.value, (current) => {
			if (current !== state$3.current) {
				state$3.current = current;
				updatesScroller();
			}
		});
		watch(() => state$3.current, (current) => currentRef.value = current);
		watch([
		...
		function handleWheel(event) {
			...
			if (Math.abs(deltaY) > 10) {
				oldDeltaY = 0;
				let current = Math.min(state$3.current + (deltaY < 0 ? -1 : 1), state$3.length - 1);
				state$3.current = current = Math.max(current, 0);
		...
		function handleTap({ clientY }) {
			...
			if (!scroller.isScrolling()) {
				...
				if (!(Math.abs(r) <= o$1)) {
					...
					const s = r < 0 ? -a$1 : a$1;
					let current = Math.min(state$3.current + s, state$3.length - 1);
					state$3.current = current = Math.max(current, 0);
					scroller.scrollTo(current * indicatorHeight.value);
				}
			}
		...
		const initScroller = () => {
			...
			const { scroller: scrollerOrigin, handleTouchStart, handleTouchMove, handleTouchEnd } = useScroller(content, {
				...
				onSnap: (index$4) => {
					if (!isNaN(index$4) && index$4 !== state$3.current) state$3.current = index$4;
				}
			});
		...
		return () => {
			const defaultSlots = slots.default && slots.default();
			state$3.length = flatVNode(defaultSlots).length;
...
var progress_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$33) {
		const rootRef = ref(null);
		const state$3 = useProgressState(props$33);
		_activeAnimation(state$3, props$33);
		watch(() => state$3.realPercent, (newValue, oldValue) => {
			state$3.strokeTimer && clearInterval(state$3.strokeTimer);
			state$3.lastPercent = oldValue || 0;
			_activeAnimation(state$3, props$33);
		});
		return () => {
			const { showInfo } = props$33;
			const { outerBarStyle, innerBarStyle, currentPercent } = state$3;
...
function useProgressState(props$33) {
	const currentPercent = ref(0);
	return reactive({
		outerBarStyle: computed(() => `background-color: ${props$33.backgroundColor}; height: ${rpx2px(props$33.strokeWidth)}px;`),
		innerBarStyle: computed(() => {
			const backgroundColor = props$33.color !== PROGRESS_VALUES.activeColor && props$33.activeColor === PROGRESS_VALUES.activeColor ? props$33.color : props$33.activeColor;
			return `width: ${currentPercent.value}%;background-color: ${backgroundColor}`;
		}),
		realPercent: computed(() => {
			if (typeof props$33.percent === "string" && !/^-?\d*\.?\d*$/.test(props$33.percent)) return 0;
			let realValue = parseFloat(props$33.percent);
...
function _activeAnimation(state$3, props$33) {
	if (props$33.active) {
		state$3.currentPercent = props$33.activeMode === PROGRESS_VALUES.activeMode ? 0 : state$3.lastPercent;
		state$3.strokeTimer = setInterval(() => {
			if (state$3.currentPercent + 1 > state$3.realPercent) {
				state$3.currentPercent = state$3.realPercent;
				state$3.strokeTimer && clearInterval(state$3.strokeTimer);
			} else state$3.currentPercent += 1;
		}, parseFloat(props$33.duration));
	} else state$3.currentPercent = state$3.realPercent;
}
...
var radio_group_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$33, { emit: emit$2, slots }) {
		const rootRef = ref(null);
		useProvideRadioGroup(props$33, useCustomEvent(rootRef, emit$2));
		return () => {
			return createVNode("uni-radio-group", { "ref": rootRef }, [slots.default && slots.default()], 512);
		};
	}
});
function useProvideRadioGroup(props$33, trigger) {
	const fields$1 = [];
	onMounted(() => {
...
var radio_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$33, { slots }) {
		const rootRef = ref(null);
		const radioChecked = ref(props$33.checked);
		const radioValue = ref(props$33.value);
		function getRadioStyle(checked) {
			...
			if (radioChecked.value) {
				style.backgroundColor = props$33.activeBackgroundColor || props$33.color;
				style.borderColor = props$33.activeBorderColor || style.backgroundColor;
			} else {
				if (props$33.borderColor) style.borderColor = props$33.borderColor;
				if (props$33.backgroundColor) style.backgroundColor = props$33.backgroundColor;
			}
		...
		const _onClick = ($event) => {
			if (props$33.disabled || radioChecked.value) return;
		...
		return () => {
			...
			return createVNode("uni-radio", mergeProps(booleanAttrs, {
				...
				"style": { "--HOVER-BD-COLOR": !radioChecked.value ? props$33.activeBorderColor : radioStyle.value.borderColor }
			}, [createVNode("div", {
				"class": ["uni-radio-input", { "uni-radio-input-disabled": props$33.disabled }],
				"style": radioStyle.value
			}, [realCheckValue ? createSvgIconVNode(ICON_PATH_SUCCESS_NO_CIRCLE, props$33.disabled ? "#ADADAD" : props$33.iconColor, 18) : ""], 6), slots.default && slots.default()], 4)], 16, ["id", "onClick"]);
		};
	}
});
...
var rich_text_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$33, { emit: emit$2 }) {
		...
		function renderVNode() {
			let nodeList = props$33.nodes;
			if (isString(nodeList)) nodeList = parseHtml(props$33.nodes);
			_vnode.value = nodeList2VNode(scopeId, triggerItemClick, nodeList);
		}
		watch(() => props$33.nodes, renderVNode, {
			immediate: true,
			deep: true
		});
...
var refresher_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$33, { slots }) {
		...
		const rootStyle = computed(() => {
			const style = { backgroundColor: props$33.refresherBackground };
			switch (props$33.refreshState) {
				case "pulling":
					style.height = props$33.refresherHeight + "px";
					break;
				case "refreshing":
					style.height = props$33.refresherThreshold + "px";
		...
		const refreshRotate = computed(() => {
			const route = props$33.refresherHeight / props$33.refresherThreshold;
			return (route > 1 ? 1 : route) * 360;
		});
		return () => {
			const { refreshState, refresherDefaultStyle, refresherThreshold } = props$33;
...
var scroll_view_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$33, { emit: emit$2, slots, expose }) {
		const rootRef = ref(null);
		const main = ref(null);
		const wrap = ref(null);
		const content = ref(null);
		const trigger = useCustomEvent(rootRef, emit$2);
		const { state: state$3, scrollTopNumber, scrollLeftNumber } = useScrollViewState(props$33);
		const { realScrollX, realScrollY, _scrollLeftChanged, _scrollTopChanged } = useScrollViewLoader(props$33, state$3, scrollTopNumber, scrollLeftNumber, trigger, rootRef, main, content, emit$2);
		...
		const scrollBarClassName = computed(() => {
			...
			if (props$33.showScrollbar === false) className += " uni-scroll-view-scrollbar-hidden";
			return className;
		});
		expose({ $getMain() {
			return main.value;
		...
		return () => {
			const { refresherEnabled, refresherBackground, refresherDefaultStyle, refresherThreshold } = props$33;
			const { refresherHeight, refreshState } = state$3;
...
function useScrollViewState(props$33) {
	const scrollTopNumber = computed(() => {
		return Number(props$33.scrollTop) || 0;
	});
	const scrollLeftNumber = computed(() => {
		return Number(props$33.scrollLeft) || 0;
	});
	return {
		state: reactive({
...
function useScrollViewLoader(props$33, state$3, scrollTopNumber, scrollLeftNumber, trigger, rootRef, main, content, emit$2) {
	...
	const realScrollX = computed(() => {
		return props$33.scrollX;
	});
	const realScrollY = computed(() => {
		return props$33.scrollY;
	});
	const upperThresholdNumber = computed(() => {
		let val = Number(props$33.upperThreshold);
		return isNaN(val) ? 50 : val;
	});
	const lowerThresholdNumber = computed(() => {
		let val = Number(props$33.lowerThreshold);
		return isNaN(val) ? 50 : val;
	});
	...
	function _handleScroll($event) {
		...
		trigger("scroll", $event, {
			...
			deltaX: state$3.lastScrollLeft - target.scrollLeft,
			deltaY: state$3.lastScrollTop - target.scrollTop
		});
		if (realScrollY.value) {
			if (target.scrollTop <= upperThresholdNumber.value && state$3.lastScrollTop - target.scrollTop > 0 && $event.timeStamp - state$3.lastScrollToUpperTime > 200) {
				trigger("scrolltoupper", $event, { direction: "top" });
				state$3.lastScrollToUpperTime = $event.timeStamp;
			}
			if (target.scrollTop + target.offsetHeight + lowerThresholdNumber.value >= target.scrollHeight && state$3.lastScrollTop - target.scrollTop < 0 && $event.timeStamp - state$3.lastScrollToLowerTime > 200) {
				trigger("scrolltolower", $event, { direction: "bottom" });
				state$3.lastScrollToLowerTime = $event.timeStamp;
			}
		}
		if (realScrollX.value) {
			if (target.scrollLeft <= upperThresholdNumber.value && state$3.lastScrollLeft - target.scrollLeft > 0 && $event.timeStamp - state$3.lastScrollToUpperTime > 200) {
				trigger("scrolltoupper", $event, { direction: "left" });
				state$3.lastScrollToUpperTime = $event.timeStamp;
			}
			if (target.scrollLeft + target.offsetWidth + lowerThresholdNumber.value >= target.scrollWidth && state$3.lastScrollLeft - target.scrollLeft < 0 && $event.timeStamp - state$3.lastScrollToLowerTime > 200) {
				trigger("scrolltolower", $event, { direction: "right" });
				state$3.lastScrollToLowerTime = $event.timeStamp;
			}
		}
		state$3.lastScrollTop = target.scrollTop;
		state$3.lastScrollLeft = target.scrollLeft;
	}
	function _scrollTopChanged(val) {
		if (realScrollY.value) if (_innerSetScrollTop) _innerSetScrollTop = false;
		else if (props$33.scrollWithAnimation) scrollTo$1(val, "y");
	...
	function _scrollLeftChanged(val) {
		...
		else if (props$33.scrollWithAnimation) scrollTo$1(val, "x");
		...
		if (val) {
			...
			if (element) {
				...
				if (realScrollX.value) {
					...
					if (props$33.scrollWithAnimation) scrollTo$1(x, "x");
					else main.value.scrollLeft = x;
				}
				if (realScrollY.value) {
					let top = elRect.top - mainRect.top;
					let y = main.value.scrollTop + top;
					if (props$33.scrollWithAnimation) scrollTo$1(y, "y");
	...
	function _setRefreshState(_state) {
		if (!props$33.refresherEnabled) return;
		switch (_state) {
			case "refreshing":
				state$3.refresherHeight = props$33.refresherThreshold;
				if (!beforeRefreshing) {
					beforeRefreshing = true;
					trigger("refresherpulling", {}, {
						deltaY: state$3.refresherHeight,
						dy: state$3.refresherHeight
					});
			...
			case "refresherabort":
				...
				state$3.refresherHeight = toUpperNumber = 0;
				if (_state === "restore") {
					triggerAbort = false;
	...
	onMounted(() => {
		...
		_scrollIntoViewChanged(props$33.scrollIntoView);
		...
		let __handleTouchMove = function(event) {
			...
			if (Math.abs(x - touchStart.x) > Math.abs(y - touchStart.y)) if (realScrollX.value) {
				...
				needStop = false;
				if (props$33.refresherEnabled && event.cancelable !== false) event.preventDefault();
			...
			if (props$33.refresherEnabled && state$3.refreshState === "pulling") {
				const dy = y - touchStart.y;
				if (toUpperNumber === 0) toUpperNumber = y;
				if (!beforeRefreshing) {
					state$3.refresherHeight = y - toUpperNumber;
					if (state$3.refresherHeight > 0) {
						triggerAbort = true;
					...
					state$3.refresherHeight = dy + props$33.refresherThreshold;
					triggerAbort = false;
				}
			}
		};
		...
		let __handleTouchEnd = function(event) {
			...
			disableScrollBounce({ disable: false });
			if (state$3.refresherHeight >= props$33.refresherThreshold) _setRefreshState("refreshing");
	...
	onActivated(() => {
		realScrollY.value && (main.value.scrollTop = state$3.lastScrollTop);
		realScrollX.value && (main.value.scrollLeft = state$3.lastScrollLeft);
	});
	...
	watch(() => props$33.scrollIntoView, (val) => {
		_scrollIntoViewChanged(val);
	});
	watch(() => props$33.refresherTriggered, (val) => {
...
var slider_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$33, { emit: emit$2 }) {
		...
		const sliderValue = ref(Number(props$33.value));
		if (sliderValue.value < Number(props$33.min)) sliderValue.value = Number(props$33.min);
		if (sliderValue.value > Number(props$33.max)) sliderValue.value = Number(props$33.max);
		watch(() => props$33.value, (val) => {
			sliderValue.value = Number(val);
		});
		const trigger = useCustomEvent(sliderRef, emit$2);
		const state$3 = useSliderState(props$33, sliderValue);
		const { _onClick, _onTrack } = useSliderLoader(props$33, sliderValue, sliderRef, sliderValueRef, trigger);
		onMounted(() => {
			useTouchtrack(sliderHandleRef.value, _onTrack);
		});
		return () => {
			const { setBgColor, setBlockBg, setActiveColor, setBlockStyle } = state$3;
			...
			}, [sliderValue.value], 512), [[vShow, props$33.showValue]])])], 8, ["onClick"]);
		};
	}
});
...
function useSliderState(props$33, sliderValue) {
	const _getValueWidth = () => {
		return getValueWidth(sliderValue.value, props$33.min, props$33.max);
	};
	const _getBgColor = () => {
		return props$33.backgroundColor !== "#e9e9e9" ? props$33.backgroundColor : props$33.color !== "#007aff" ? props$33.color : "#007aff";
	};
	const _getActiveColor = () => {
		return props$33.activeColor !== "#007aff" ? props$33.activeColor : props$33.selectedColor !== "#e9e9e9" ? props$33.selectedColor : "#e9e9e9";
	};
	return {
		...
		setBlockStyle: computed(() => ({
			width: props$33.blockSize + "px",
			height: props$33.blockSize + "px",
			marginLeft: -props$33.blockSize / 2 + "px",
			marginTop: -props$33.blockSize / 2 + "px",
			left: _getValueWidth(),
			backgroundColor: props$33.blockColor
		}))
	};
}
function useSliderLoader(props$33, sliderValue, sliderRef, sliderValueRef, trigger) {
	const truthStep = computed(() => {
		const step = Number(props$33.step);
	...
	const _onUserChangedValue = (e$1) => {
		const max = Number(props$33.max);
		const min = Number(props$33.min);
		...
		const offsetWidth = slider.offsetWidth - (props$33.showValue ? sliderRightBoxWidth : 0);
	...
	const _onTrack = (e$1) => {
		if (!props$33.disabled) return e$1.detail.state === "move" ? (_onUserChangedValue({ x: e$1.detail.x }), trigger("changing", e$1, { value: sliderValue.value }), !1) : e$1.detail.state === "end" && trigger("change", e$1, { value: sliderValue.value });
	};
	const uniForm = inject(uniFormKey, false);
	if (!!uniForm) {
		const field = {
			reset: () => sliderValue.value = Number(props$33.min),
			submit: () => {
				const data = ["", null];
				if (props$33.name !== "") {
					data[0] = props$33.name;
...
function useState$3(props$33) {
	return reactive({
		interval: computed(() => {
			const interval = Number(props$33.interval);
			return isNaN(interval) ? 5e3 : interval;
		}),
		duration: computed(() => {
			const duration = Number(props$33.duration);
			return isNaN(duration) ? 500 : duration;
		}),
		displayMultipleItems: computed(() => {
			const displayMultipleItems = Math.round(props$33.displayMultipleItems);
			return isNaN(displayMultipleItems) ? 1 : displayMultipleItems;
		}),
		current: Math.round(props$33.current) || 0,
		currentItemId: props$33.currentItemId,
		userTracking: false
	});
}
function useLayout(props$33, state$3, swiperContexts, slideFrameRef, emit$2, trigger) {
	...
	let animationFrame;
	const swiperEnabled = computed(() => swiperContexts.value.length > state$3.displayMultipleItems);
	const circularEnabled = computed(() => props$33.circular && swiperEnabled.value);
	function checkCircularLayout(index$4) {
		if (!invalid) for (let items = swiperContexts.value, n = items.length, i = index$4 + state$3.displayMultipleItems, r = 0; r < n; r++) {
			const item = items[r];
			...
			item.updatePosition(position, props$33.vertical);
		}
	}
	function updateViewport(index$4) {
		...
		const x = props$33.vertical ? "0" : 100 * -index$4 * viewportMoveRatio + "%";
		const y = props$33.vertical ? 100 * -index$4 * viewportMoveRatio + "%" : "0";
		...
		trigger("transition", {}, {
			dx: props$33.vertical ? 0 : index$4 * slideFrame.offsetWidth,
			dy: props$33.vertical ? index$4 * slideFrame.offsetHeight : 0
		});
	}
	...
	function normalizeCurrentValue(current) {
		...
		if (circularEnabled.value) {
			if (length <= state$3.displayMultipleItems) return 0;
		} else if (index$4 > length - state$3.displayMultipleItems) return length - state$3.displayMultipleItems;
		return index$4;
	}
	...
	function animateFrameFuncProto() {
		...
		if (time <= 0) {
			...
			const item = swiperContexts.value[state$3.current];
			if (item) {
				const currentItemId = item.getItemId();
				trigger("animationfinish", {}, {
					current: state$3.current,
					currentItemId,
					source
				});
			}
	...
	function animateViewport(current, source, n) {
		...
		const duration = state$3.duration;
		const length = swiperContexts.value.length;
		...
		else if (source === "click") current = current + state$3.displayMultipleItems - 1 < length ? current : 0;
		animating = {
			toPos: current,
	...
	function scheduleAutoplay() {
		...
		const callback = function() {
			...
			if (circularEnabled.value) state$3.current = normalizeCurrentValue(state$3.current + 1);
			else state$3.current = state$3.current + state$3.displayMultipleItems < items.length ? state$3.current + 1 : 0;
			animateViewport(state$3.current, "autoplay", circularEnabled.value ? 1 : 0);
			timer = setTimeout(callback, state$3.interval);
		};
		if (!(invalid || items.length <= state$3.displayMultipleItems)) timer = setTimeout(callback, state$3.interval);
	}
	function resetLayout() {
		cancelSchedule();
		endViewportAnimation();
		const items = swiperContexts.value;
		for (let i = 0; i < items.length; i++) items[i].updatePosition(i, props$33.vertical);
		viewportMoveRatio = 1;
		const slideFrameEl = slideFrameRef.value;
		if (state$3.displayMultipleItems === 1 && items.length) {
		...
		viewportPosition = -2;
		const current = state$3.current;
		if (current >= 0) {
			invalid = false;
			if (state$3.userTracking) {
				updateViewport(position + current - contentTrackViewport);
				contentTrackViewport = current;
			} else {
				updateViewport(current);
				if (props$33.autoplay) scheduleAutoplay();
			}
		} else {
			invalid = true;
			updateViewport(-state$3.displayMultipleItems - 1);
		}
	}
	watch([
		() => props$33.current,
		() => props$33.currentItemId,
		() => [...swiperContexts.value]
	], () => {
		let current = -1;
		if (props$33.currentItemId) {
			for (let i = 0, items = swiperContexts.value; i < items.length; i++) if (items[i].getItemId() === props$33.currentItemId) {
				current = i;
				break;
			}
		}
		if (current < 0) current = Math.round(props$33.current) || 0;
		current = current < 0 ? 0 : current;
		if (state$3.current !== current) {
			currentChangeSource = "";
			state$3.current = current;
		}
	});
	watch([
		...
		() => state$3.displayMultipleItems,
		() => [...swiperContexts.value]
	], resetLayout);
	watch(() => state$3.interval, () => {
		if (timer) {
			cancelSchedule();
			scheduleAutoplay();
		}
	...
	function currentChanged(current, history$1) {
		...
		if (item) {
			const currentItemId = state$3.currentItemId = item.getItemId();
			trigger("change", {}, {
				current: state$3.current,
				currentItemId,
				source
	...
	watch(() => state$3.current, (val, oldVal) => {
		currentChanged(val, oldVal);
		emit$2("update:current", val);
	});
	watch(() => state$3.currentItemId, (val) => {
	...
	watch(() => props$33.autoplay && !state$3.userTracking, inintAutoplay);
	inintAutoplay(props$33.autoplay && !state$3.userTracking);
	...
	onMounted(() => {
		...
		function handleTrackMove(data) {
			...
			contentTrackT = Date.now();
			const other = swiperContexts.value.length - state$3.displayMultipleItems;
			function calc$1(val) {
				return .5 - .25 / (val + .5);
			}
			...
			const slideFrameEl = slideFrameRef.value;
			if (props$33.vertical) move(-data.dy / slideFrameEl.offsetHeight, -data.ddy / time);
			else move(-data.dx / slideFrameEl.offsetWidth, -data.ddx / time);
		}
		function handleTrackEnd(isCancel) {
			debouncedTrackEndFallback === null || debouncedTrackEndFallback === void 0 || debouncedTrackEndFallback.cancel();
			state$3.userTracking = false;
			...
			if (isCancel) animateViewport(state$3.current, "", 0);
			else {
		...
		useTouchtrack(slideFrameRef.value, (event) => {
			if (props$33.disableTouch) return;
			if (!invalid) {
				if (event.detail.state === "start") {
					state$3.userTracking = true;
				...
				if (state$3.userTracking) {
					if (!userDirectionChecked) {
						...
						if (t$1 >= n && props$33.vertical) state$3.userTracking = false;
						else if (t$1 <= n && !props$33.vertical) state$3.userTracking = false;
						if (!state$3.userTracking) {
							if (props$33.autoplay) scheduleAutoplay();
							return;
						}
	...
	function onSwiperDotClick(index$4) {
		animateViewport(state$3.current = index$4, currentChangeSource = "click", circularEnabled.value ? 1 : 0);
	}
	return {
		onSwiperDotClick,
		circularEnabled,
...
var swiper_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$33, { slots, emit: emit$2 }) {
		...
		const state$3 = useState$3(props$33);
		const slidesStyle = computed(() => {
			let style = {};
			if (props$33.nextMargin || props$33.previousMargin) style = props$33.vertical ? {
				left: 0,
				right: 0,
				top: rpx2px(props$33.previousMargin, true),
				bottom: rpx2px(props$33.nextMargin, true)
			} : {
				top: 0,
				bottom: 0,
				left: rpx2px(props$33.previousMargin, true),
				right: rpx2px(props$33.nextMargin, true)
			};
			return style;
		});
		const slideFrameStyle = computed(() => {
			const value = Math.abs(100 / state$3.displayMultipleItems) + "%";
			return {
				width: props$33.vertical ? "100%" : value,
				height: !props$33.vertical ? "100%" : value
			};
		});
		let swiperItems = [];
		...
		const { onSwiperDotClick, circularEnabled, swiperEnabled } = useLayout(props$33, state$3, swiperContexts, slideFrameRef, emit$2, trigger);
		let createNavigationTsx = () => null;
		createNavigationTsx = useSwiperNavigation(rootRef, props$33, state$3, onSwiperDotClick, swiperContexts, circularEnabled, swiperEnabled);
		return () => {
			const defaultSlots = slots.default && slots.default();
			...
			return createVNode("uni-swiper", { "ref": rootRef }, [createVNode("div", {
				...
				props$33.indicatorDots && createVNode("div", { "class": ["uni-swiper-dots", props$33.vertical ? "uni-swiper-dots-vertical" : "uni-swiper-dots-horizontal"] }, [swiperContexts.value.map((_, index$4, array) => createVNode("div", {
					"onClick": () => onSwiperDotClick(index$4),
					"class": {
						"uni-swiper-dot": true,
						"uni-swiper-dot-active": index$4 < state$3.current + state$3.displayMultipleItems && index$4 >= state$3.current || index$4 < state$3.current + state$3.displayMultipleItems - array.length
					},
					"style": { background: index$4 === state$3.current ? props$33.indicatorActiveColor : props$33.indicatorColor }
				}, null, 14, ["onClick"]))], 2),
				createNavigationTsx()
			], 512)], 512);
		};
...
var useSwiperNavigation = (rootRef, props$33, state$3, onSwiperDotClick, swiperContext, circularEnabled, swiperEnabled) => {
	let isNavigationAuto = false;
	let prevDisabled = false;
	let nextDisabled = false;
	let hideNavigation = ref(false);
	watchEffect(() => {
		isNavigationAuto = props$33.navigation === "auto";
		hideNavigation.value = props$33.navigation !== true || isNavigationAuto;
		swiperAddMouseEvent();
	});
	watchEffect(() => {
		const swiperItemLength = swiperContext.value.length;
		const notCircular = !circularEnabled.value;
		prevDisabled = state$3.current === 0 && notCircular;
		nextDisabled = state$3.current === swiperItemLength - 1 && notCircular || notCircular && state$3.current + state$3.displayMultipleItems >= swiperItemLength;
	...
	function navigationHover(event, type) {
		...
		if (!target) return;
		target.style.backgroundColor = type === "over" ? props$33.navigationActiveColor : "";
	}
	...
	const createNavigationSVG = () => createSvgIconVNode(ICON_PATH_BACK, props$33.navigationColor, 26);
	...
	const _mousemove = (e$1) => {
		...
		let hide = false;
		if (props$33.vertical) hide = !(clientY - top < height / 3 || bottom - clientY < height / 3);
	...
	function createNavigationTsx() {
		const navigationClass = {
			...
			"uni-swiper-navigation-vertical": props$33.vertical
		};
		if (props$33.navigation) return createVNode(Fragment, null, [createVNode("div", mergeProps({
...
var switch_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$33, { emit: emit$2 }) {
		const rootRef = ref(null);
		const switchChecked = ref(props$33.checked);
		const uniLabel = useSwitchInject(props$33, switchChecked);
		const trigger = useCustomEvent(rootRef, emit$2);
		watch(() => props$33.checked, (val) => {
		...
		return () => {
			...
			}, null, 6), [[vShow, type === "switch"]]), withDirectives(createVNode("div", { "class": "uni-checkbox-input" }, [realCheckValue ? createSvgIconVNode(ICON_PATH_SUCCESS_NO_CIRCLE, props$33.color, 22) : ""], 512), [[vShow, type === "checkbox"]])])], 16, ["id", "onClick"]);
		};
	}
});
function useSwitchInject(props$33, switchChecked) {
	const uniForm = inject(uniFormKey, false);
	const uniLabel = inject(uniLabelKey, false);
	const formField = {
		submit: () => {
	...
	setup(props$33, { slots }) {
		...
		return () => {
			...
			if (slots.default) slots.default().forEach((vnode) => {
				if (vnode.shapeFlag & 8 && vnode.type !== Comment) {
					...
					lines = parseText(vnode.children, {
						space: props$33.space,
						decode: props$33.decode
					});
			...
			return createVNode("uni-text", {
				"ref": rootRef,
				"selectable": props$33.selectable ? true : null
			}, [createVNode("span", null, children)], 8, ["selectable"]);
		};
	}
});
var props$17 = /* @__PURE__ */ extend({}, props$16, {
	placeholderClass: {
		type: String,
		default: "input-placeholder"
	},
...
var textarea_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$33, { emit: emit$2, expose }) {
		const rootRef = ref(null);
		const wrapperRef = ref(null);
		const { fieldRef, state: state$3, scopedAttrsState, fixDisabledColor, trigger } = useField(props$33, rootRef, emit$2);
		const valueCompute = computed(() => state$3.value.split(LINEFEED));
		const isDone = computed(() => ConfirmTypes.includes(props$33.confirmType));
		...
		watch(() => heightRef.value, (height) => {
			...
			if (props$33.autoHeight) wrapper$1.style.height = height + "px";
		});
		function onResize$2({ height }) {
			heightRef.value = height;
		}
		function onChange$1(event) {}
		function confirm(event) {
			trigger("confirm", event, { value: state$3.value });
		}
		...
		function onKeyUpEnter(event) {
			...
			if (isDone.value) {
				...
				!props$33.confirmHold && textarea.blur();
			}
		}
		setFixMargin();
		expose({ $triggerInput: (detail) => {
		...
		return () => {
			let textareaNode = props$33.disabled && fixDisabledColor ? createVNode("textarea", {
				...
				"style": {
					overflowY: props$33.autoHeight ? "hidden" : "auto",
					...props$33.cursorColor && { caretColor: props$33.cursorColor }
				},
				"onFocus": (event) => event.target.blur()
			}, null, 46, [
				"value",
			...
			return createVNode("uni-textarea", {
				"ref": rootRef,
				"auto-height": props$33.autoHeight
			}, [createVNode("div", {
				"ref": wrapperRef,
				...
				withDirectives(createVNode("div", mergeProps(scopedAttrsState.attrs, {
					"style": props$33.placeholderStyle,
					"class": ["uni-textarea-placeholder", props$33.placeholderClass]
				}), [props$33.placeholder], 16), [[vShow, !state$3.value.length]]),
				createVNode("div", {
					"ref": lineRef,
				...
				createVNode("div", { "class": {
					...
					"uni-textarea-compute-auto-height": props$33.autoHeight
				...
				props$33.confirmType === "search" ? createVNode("form", {
					"action": "",
					"onSubmit": () => false,
					"class": "uni-input-form"
...
function useContextInfo(_id) {
	...
	const id$1 = _id || vm.id || `context${index$2++}`;
	onMounted(() => {
		const el = vm.$el;
		el.__uniContextInfo = {
			id: id$1,
...
function wrapperComponentSetup(comp, { type, clone, init: init$1, setup, before, options }) {
	...
	comp.setup = (props$33, ctx) => {
		const instance = getCurrentInstance();
		init$1(instance.proxy);
		setup(instance);
		if (oldSetup) return oldSetup(props$33, ctx);
	};
	return comp;
}
	...
	return setupComponent(comp, {
		...
		setup(instance) {
			...
			const onLaunch = () => {
				...
				onLaunch$1 && invokeArrayFns(onLaunch$1, launchOptions$1);
				onShow && invokeArrayFns(onShow, launchOptions$1);
				if (__UNI_FEATURE_PAGES__) {
					if (!route.matched.length) {
						...
						handleBeforeEntryPageRoutes();
						onPageNotFound$1 && invokeArrayFns(onPageNotFound$1, pageNotFoundOptions);
					}
				}
			};
		...
		before(comp$1) {
			...
			comp$1.setup = (props$33, ctx) => {
				const res = setup && setup(props$33, ctx);
...
function useGesture(props$33, videoState, videoRef, fullscreenState) {
	const state$3 = reactive({
		seeking: false,
		gestureType: "none",
		volumeOld: 0,
	...
	const changeToastThin = () => {
		if (state$3.gestureType !== "none" && changeToastThinTimer != null) return;
		changeToastThinTimer = setTimeout(() => {
			state$3.toastThin = true;
		}, 500);
	};
	...
	function onTouchmove(event) {
		...
		const gestureType = state$3.gestureType;
		...
		if (Math.abs(pageX - origin.x) > Math.abs(pageY - origin.y)) {
			if (!props$33.enableProgressGesture) {
				state$3.gestureType = "stop";
				return;
			}
			state$3.gestureType = "progress";
			state$3.currentTimeOld = state$3.currentTimeNew = video.currentTime;
			if (!fullscreenState.fullscreen) stop();
		} else {
			if (!props$33.pageGesture && !props$33.vslideGesture) {
				state$3.gestureType = "stop";
				return;
			}
			changeToastThin();
			state$3.gestureType = "volume";
			state$3.volumeOld = video.volume;
			if (!fullscreenState.fullscreen) stop();
		}
	}
	function onTouchend(event) {
		const video = videoRef.value;
		if (state$3.gestureType !== "none" && state$3.gestureType !== "stop") {
			event.stopPropagation();
			event.preventDefault();
		}
		if (state$3.gestureType === "progress" && state$3.currentTimeOld !== state$3.currentTimeNew) video.currentTime = state$3.currentTimeNew;
		state$3.gestureType = "none";
	}
	function changeProgress(x) {
		const duration = videoState.currentDuration;
		let currentTimeNew = x / 600 * duration + state$3.currentTimeOld;
		if (currentTimeNew < 0) currentTimeNew = 0;
		else if (currentTimeNew > duration) currentTimeNew = duration;
		state$3.currentTimeNew = currentTimeNew;
	}
	function changeVolume(y) {
		const video = videoRef.value;
		const valueOld = state$3.volumeOld;
		let value;
...
function useFullscreen(trigger, containerRef, videoRef, userActionState, rootRef) {
	const state$3 = reactive({ fullscreen: false });
...
function useVideo(props$33, attrs$1, trigger) {
	const videoRef = ref(null);
	const src = computed(() => getRealPath(props$33.src));
	const muted = computed(() => props$33.muted === "true" || props$33.muted === true);
	const state$3 = reactive({
		start: false,
		src,
		playing: false,
	...
	watch(() => src.value, () => {
		...
		state$3.currentTime = 0;
	});
	watch(() => state$3.buffered, (buffered) => {
	...
	watch([() => state$3.duration, () => props$33.duration], () => {
		let _duration = Number(props$33.duration);
		isNaN(_duration) && (_duration = 0);
		state$3.currentDuration = _duration > 0 ? _duration : state$3.duration;
	});
	function onDurationChange({ target }) {
		state$3.duration = target.duration;
	}
	function onLoadedMetadata($event) {
		const initialTime = Number(props$33.initialTime) || 0;
	...
	function onProgress($event) {
		...
		if (buffered.length) state$3.buffered = buffered.end(buffered.length - 1) / video.duration * 100;
	}
	...
	function onTimeUpdate($event) {
		const video = $event.target;
		if (!state$3.pauseUpdatingCurrentTime) state$3.currentTime = video.currentTime;
	...
	function toggle() {
		const video = videoRef.value;
		if (state$3.playing) video.pause();
		else video.play();
	}
	function play() {
		const video = videoRef.value;
...
function useControls(props$33, videoState, seek, seeking) {
	const progressRef = ref(null);
	const ballRef = ref(null);
	const centerPlayBtnShow = computed(() => props$33.showCenterPlayBtn && !videoState.start);
	...
	const state$3 = reactive({
		...
		centerPlayBtnShow,
		controlsShow: computed(() => !centerPlayBtnShow.value && props$33.controls && controlsVisible.value),
		controlsVisible
	});
	...
	function toggleControls() {
		state$3.controlsVisible = !state$3.controlsVisible;
	}
	let hideTiming;
	function autoHideStart() {
		hideTiming = setTimeout(() => {
			state$3.controlsVisible = false;
		}, 3e3);
	}
	...
	watch(() => state$3.controlsShow && videoState.playing && !state$3.controlsTouching, (val) => {
	...
	onMounted(() => {
		...
		function touchend(event) {
			state$3.controlsTouching = false;
			if (state$3.touching) {
...
function useDanmu(props$33, videoState) {
	const danmuRef = ref(null);
	const state$3 = reactive({ enable: Boolean(props$33.enableDanmu) });
	let danmuIndex = {
		time: 0,
		index: -1
	};
	const danmuList = isArray(props$33.danmuList) ? JSON.parse(JSON.stringify(props$33.danmuList)) : [];
	danmuList.sort(function(a$1, b) {
		return (a$1.time || 0) - (b.time || 0);
	});
	function toggleDanmu() {
		state$3.enable = !state$3.enable;
	}
	function updateDanmu(event) {
		...
		if (currentTime > oldDanmuIndex.time) for (let index$4 = oldDanmuIndex.index + 1; index$4 < danmuList.length; index$4++) {
			...
			if (currentTime >= (element.time || 0)) {
				...
				if (videoState.playing && state$3.enable) playDanmu(element);
			} else break;
		}
...
var video_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$33, { emit: emit$2, attrs: attrs$1, slots }) {
		const rootRef = ref(null);
		...
		const { videoRef, state: videoState, play, pause, stop, seek, playbackRate, toggle, onDurationChange, onLoadedMetadata, onProgress, onWaiting, onVideoError, onPlay, onPause, onEnded, onTimeUpdate } = useVideo(props$33, attrs$1, trigger);
		const { state: danmuState, danmuRef, updateDanmu, toggleDanmu, sendDanmu } = useDanmu(props$33, videoState);
		...
		const { state: gestureState, onTouchstart, onTouchend, onTouchmove } = useGesture(props$33, videoState, videoRef, fullscreenState);
		const { state: controlsState, progressRef, ballRef, clickProgress, toggleControls, autoHideEnd, autoHideStart } = useControls(props$33, videoState, seek, (currentTimeNew) => {
		...
		return () => {
			return createVNode("uni-video", {
				...
				createVNode("video", mergeProps({
					"ref": videoRef,
					"style": { "object-fit": props$33.objectFit },
					"muted": !!props$33.muted,
					"loop": !!props$33.loop,
					"src": videoState.src,
					"poster": props$33.poster,
					"autoplay": !!props$33.autoplay
				}, videoAttrs.value, {
					"class": {
				...
				withDirectives(createVNode("div", {
					...
					createVNode("div", { "class": "uni-video-controls" }, [
						...
						}, null, 10, ["onClick"]), [[vShow, props$33.showPlayBtn]]),
						withDirectives(createVNode("div", { "class": "uni-video-current-time" }, [formatTime(videoState.currentTime)], 512), [[vShow, props$33.showProgress]]),
						withDirectives(createVNode("div", {
							"ref": progressRef,
						...
						], 2)], 8, ["onClick"]), [[vShow, props$33.showProgress]]),
						withDirectives(createVNode("div", { "class": "uni-video-duration" }, [formatTime(videoState.currentDuration)], 512), [[vShow, props$33.showProgress]])
					]),
					withDirectives(createVNode("div", {
						"class": {
							"uni-video-icon": true,
					...
					}, null, 10, ["onClick"]), [[vShow, props$33.danmuBtn]]),
					withDirectives(createVNode("div", {
						"class": {
							"uni-video-icon": true,
					...
					}, null, 10, ["onClick"]), [[vShow, props$33.showFullscreenBtn]])
	...
	setup(props$33, { emit: emit$2 }) {
		...
		const renderIframe = () => {
			...
			iframe.onload = function(event) {
				trigger("load", event, { src: props$33.src });
			};
			watchEffect(() => {
				for (const key in $attrs.value) if (hasOwn($attrs.value, key)) iframe[key] = $attrs.value[key];
			});
			watchEffect(() => {
				iframe.src = getRealPath(props$33.src);
			});
			iframeRef.value = iframe;
			_resize = useWebViewSize(rootRef, iframeRef, props$33.fullscreen);
			if (props$33.fullscreen) document.body.appendChild(iframe);
		};
		renderIframe();
		onMounted(() => {
			var _rootRef$value;
			_resize();
			!props$33.fullscreen && ((_rootRef$value = rootRef.value) === null || _rootRef$value === void 0 || _rootRef$value.appendChild(iframeRef.value));
		});
		onActivated(() => {
			props$33.fullscreen && (iframeRef.value.style.display = "block");
		});
		onDeactivated(() => {
			props$33.fullscreen && (iframeRef.value.style.display = "none");
		});
		onBeforeUnmount(() => {
			props$33.fullscreen && document.body.removeChild(iframeRef.value);
		});
		return () => {
			return createVNode(Fragment, null, [createVNode("uni-web-view", mergeProps({ "class": props$33.fullscreen ? "uni-webview--fullscreen" : "" }, $listeners.value, $excludeAttrs.value, { "ref": rootRef }), [createVNode(resize_sensor_default, { "onResize": _resize }, null, 8, ["onResize"])], 16)]);
		};
	}
});
...
var MapMarker_default = /* @__PURE__ */ defineSystemComponent({
	...
	props: props$13,
	setup(props$33) {
		const id$1 = String(!isNaN(Number(props$33.id)) ? props$33.id : "");
		...
		onMapReady((map, maps$1, trigger) => {
			...
			function addMarker(props$34) {
				if (!getIsBMap()) marker = new maps$1.Marker({
					map,
					flat: true,
					autoRotation: false
				});
				updateMarker(props$34);
				...
				if (getIsBMap()) {} else MapsEvent.addListener(marker, "click", () => {
					...
					if (id$1) trigger("markertap", {}, {
						...
						latitude: props$34.latitude,
						longitude: props$34.longitude
					});
				});
			}
			addMarker(props$33);
			watch(props$33, updateMarker);
		});
		if (id$1) {
...
var MapPolyline_default = /* @__PURE__ */ defineSystemComponent({
	...
	setup(props$33) {
		...
		onMapReady((map, maps$1) => {
			...
			addPolyline(props$33);
			watch(props$33, updatePolyline);
		});
...
var MapControl_default = /* @__PURE__ */ defineSystemComponent({
	...
	setup(props$33) {
		const imgPath = computed(() => getRealPath(props$33.iconPath));
		const positionStyle = computed(() => {
			let positionStyle$1 = `top:${props$33.position.top || 0}px;left:${props$33.position.left || 0}px;`;
			if (props$33.position.width) positionStyle$1 += `width:${props$33.position.width}px;`;
			if (props$33.position.height) positionStyle$1 += `height:${props$33.position.height}px;`;
			return positionStyle$1;
		});
		const handleClick = ($event) => {
			if (props$33.clickable) props$33.trigger("controltap", $event, { controlId: props$33.id });
		};
		return () => {
...
function ensureRoot(id$1) {
	let rootEl = document.getElementById(id$1);
	if (!rootEl) {
...
function usePopup(props$33, { onEsc, onEnter }) {
	const visible = ref(props$33.visible);
	const { key, disable } = useKeyboard();
	watch(() => props$33.visible, (value) => visible.value = value);
...
var ImageView_default = /* @__PURE__ */ defineSystemComponent({
	...
	setup(props$33) {
		const state$3 = reactive({ direction: "none" });
		let scale = 1;
		let imgWidth = 0;
		let imgHeight = 0;
		let width = 0;
		...
		function onTouchEnd(event) {
			...
			if (horizontal && vertical) state$3.direction = "all";
			else if (horizontal) state$3.direction = "horizontal";
			else if (vertical) state$3.direction = "vertical";
			else state$3.direction = "none";
			checkDirection(event);
		}
		function checkDirection(event) {
			if (state$3.direction === "all" || state$3.direction === "horizontal") event.stopPropagation();
		}
		return () => {
			const viewStyle = {
				position: "absolute",
...
function getIndex(props$33) {
	let index$4 = typeof props$33.current === "number" ? props$33.current : props$33.urls.indexOf(props$33.current);
...
var ImagePreview_default = /* @__PURE__ */ defineSystemComponent({
	...
	setup(props$33, { emit: emit$2 }) {
		...
		const rootRef = ref(null);
		const indexRef = ref(getIndex(props$33));
		watch(() => props$33.current, () => indexRef.value = getIndex(props$33));
		...
		return () => {
			...
			}, _isSlot$2(_slot = props$33.urls.map((src) => createVNode(swiper_item_default, null, { default: () => [createVNode(ImageView_default, { "src": src }, null, 8, ["src"])] }))) ? _slot : {
				default: () => [_slot],
				_: 1
...
function useState$2(props$33) {
	...
	function updatePosition() {
		if (props$33.latitude && props$33.longitude) {
			state$3.center.latitude = props$33.latitude;
			state$3.center.longitude = props$33.longitude;
			state$3.marker.latitude = props$33.latitude;
			state$3.marker.longitude = props$33.longitude;
		}
	}
	watch([() => props$33.latitude, () => props$33.longitude], updatePosition);
	updatePosition();
	return state$3;
}
var LocationView_default = /* @__PURE__ */ defineSystemComponent({
	...
	emits: ["close"],
	setup(props$33, { emit: emit$2 }) {
		const state$3 = useState$2(props$33);
		usePreventScroll();
		getLocation({
			type: "gcj02",
			success: ({ latitude, longitude }) => {
				state$3.location.latitude = latitude;
				state$3.location.longitude = longitude;
			}
		});
		function onRegionChange(event) {
			const centerLocation = event.detail.centerLocation;
			if (centerLocation) {
				state$3.center.latitude = centerLocation.latitude;
				state$3.center.longitude = centerLocation.longitude;
			}
		}
		function nav() {
			const mapInfo = getMapInfo();
			let url = "";
			if (mapInfo.type === MapType.GOOGLE) url = `https://www.google.com/maps/dir/?api=1${state$3.location.latitude ? `&origin=${state$3.location.latitude}%2C${state$3.location.longitude}` : ""}&destination=${props$33.latitude}%2C${props$33.longitude}`;
			else if (mapInfo.type === MapType.QQ) url = `https://apis.map.qq.com/uri/v1/routeplan?type=drive${state$3.location.latitude ? `&fromcoord=${state$3.location.latitude}%2C${state$3.location.longitude}&from=${encodeURIComponent("我的位置")}` : ""}&tocoord=${props$33.latitude}%2C${props$33.longitude}&to=${encodeURIComponent(props$33.name || "目的地")}&ref=${mapInfo.key}`;
			else if (mapInfo.type === MapType.AMAP) url = `https://uri.amap.com/navigation?${state$3.location.latitude ? `from=${state$3.location.longitude},${state$3.location.latitude},${encodeURIComponent("我的位置")}&` : ""}to=${props$33.longitude},${props$33.latitude},${encodeURIComponent(props$33.name || "目的地")}`;
			window.open(url);
		}
		function back() {
			emit$2("close");
		...
		function setCenter({ latitude, longitude }) {
			state$3.center.latitude = latitude;
			state$3.center.longitude = longitude;
		}
		return () => {
			return createVNode("div", { "class": "uni-system-open-location" }, [
				createVNode(map_default, {
					"latitude": state$3.center.latitude,
					"longitude": state$3.center.longitude,
					"class": "map",
					"markers": [state$3.marker, state$3.location],
					"onRegionchange": onRegionChange
				}, { default: () => [createVNode("div", {
					"class": "map-move",
					"onClick": () => setCenter(state$3.location)
				...
				createVNode("div", { "class": "info" }, [
					createVNode("div", {
						"class": "name",
						"onClick": () => setCenter(state$3.marker)
					}, [props$33.name], 8, ["onClick"]),
					createVNode("div", {
						"class": "address",
						"onClick": () => setCenter(state$3.marker)
					}, [props$33.address], 8, ["onClick"]),
					createVNode("div", {
						"class": "nav",
						"onClick": nav
...
function distance(distance$1) {
	if (distance$1 > 100) return `${distance$1 > 1e3 ? (distance$1 / 1e3).toFixed(1) + "k" : distance$1.toFixed(0)}m | `;
	else if (distance$1 > 0) return "<100m | ";
	else return "";
}
function useState$1(props$33) {
	const state$3 = reactive({
		latitude: 0,
		longitude: 0,
		keyword: "",
	...
	function updatePosition() {
		if (props$33.latitude && props$33.longitude) {
			state$3.latitude = props$33.latitude;
			state$3.longitude = props$33.longitude;
		}
	}
	watch([() => props$33.latitude, () => props$33.longitude], updatePosition);
	updatePosition();
...
function useList(state$3) {
	...
	const adcodeRef = ref("");
	const boundaryRef = computed(() => adcodeRef.value ? `region(${adcodeRef.value},1,${state$3.latitude},${state$3.longitude})` : `nearby(${state$3.latitude},${state$3.longitude},5000)`);
	...
	function getList() {
		...
		if (mapInfo.type === MapType.GOOGLE) {
			...
			new google.maps.places.PlacesService(document.createElement("div"))[state$3.searching ? "textSearch" : "nearbySearch"]({
				location: {
					lat: state$3.latitude,
					lng: state$3.longitude
				},
				query: state$3.keyword,
				radius: 5e3
			}, (results, state$4, page) => {
		...
		} else if (mapInfo.type === MapType.QQ) getJSONP(state$3.searching ? `https://apis.map.qq.com/ws/place/v1/search?output=jsonp&key=${key}&boundary=${boundaryRef.value}&keyword=${state$3.keyword}&page_size=${listState.pageSize}&page_index=${listState.pageIndex}` : `https://apis.map.qq.com/ws/geocoder/v1/?output=jsonp&key=${key}&location=${state$3.latitude},${state$3.longitude}&get_poi=1&poi_options=page_size=${listState.pageSize};page_index=${listState.pageIndex}`, { callback: "callback" }, (res) => {
			listState.loading = false;
			if (state$3.searching && "data" in res && res.data.length) pushData(res.data);
			...
			const keyword = state$3.searching ? state$3.keyword : "";
			const radius = state$3.searching ? 5e4 : 5e3;
			placeSearch.searchNearBy(keyword, [state$3.longitude, state$3.latitude], radius, function(status, result) {
...
var LoctaionPicker_default = /* @__PURE__ */ defineSystemComponent({
	...
	setup(props$33, { emit: emit$2 }) {
		...
		const { t: t$1 } = useI18n();
		const state$3 = useState$1(props$33);
		const { list, listState, loadMore, reset, getList } = useList(state$3);
		const search = debounce(() => {
			reset();
			if (state$3.keyword) getList();
		}, 1e3, {
			setTimeout,
			clearTimeout
		});
		watch(() => state$3.searching, (val) => {
			reset();
			if (!val) getList();
		});
		function onInput(event) {
			state$3.keyword = event.detail.value;
			search();
		}
		function onChoose() {
			emit$2("close", extend({}, listState.selected));
		}
		function onBack() {
			emit$2("close");
		}
		...
		function move({ latitude, longitude }) {
			state$3.latitude = latitude;
			state$3.longitude = longitude;
			if (!state$3.searching) {
				reset();
				getList();
			}
		}
		if (!state$3.latitude || !state$3.longitude) moveToLocation();
		return () => {
			const content = list.map((item, index$4) => {
				return createVNode("div", {
					"key": index$4,
					"class": {
						"list-item": true,
						selected: listState.selectedIndex === index$4
					},
					"onClick": () => {
						listState.selectedIndex = index$4;
						state$3.latitude = item.latitude;
						state$3.longitude = item.longitude;
					}
				}, [
			...
			return createVNode("div", { "class": "uni-system-choose-location" }, [
				createVNode(map_default, {
					"latitude": state$3.latitude,
					"longitude": state$3.longitude,
					"class": "map",
					"show-location": true,
					"libraries": ["places"],
					"onUpdated": getList,
				...
				createVNode("div", { "class": "menu" }, [createVNode("div", { "class": "search" }, [createVNode(input_default, {
					"value": state$3.keyword,
					"class": "search-input",
					"placeholder": t$1("uni.chooseLocation.search"),
					"onFocus": () => state$3.searching = true,
					"onInput": onInput
				}, null, 8, [
					"value",
					"placeholder",
					"onFocus",
					"onInput"
				]), state$3.searching && createVNode("div", {
					"class": "search-btn",
					"onClick": () => {
						state$3.searching = false;
						state$3.keyword = "";
					}
...
var modal_default = /* @__PURE__ */ defineComponent({
	...
	setup(props$33, { emit: emit$2 }) {
		...
		const cancel = () => (close(), emit$2("close", "cancel"));
		const confirm = () => (close(), emit$2("close", "confirm", editContent.value));
		const visible = usePopup(props$33, {
			onEsc: cancel,
			onEnter: () => {
				!props$33.editable && confirm();
			}
		});
		const cancelColor = useOnThemeChange$1(props$33);
		return () => {
			const { title, content, showCancel, confirmText, confirmColor, editable, placeholderText } = props$33;
			...
			return createVNode(Transition, { "name": "uni-fade" }, { default: () => [withDirectives(createVNode("uni-modal", { "onTouchmove": onEventPrevent }, [VNODE_MASK, createVNode("div", { "class": "uni-modal" }, [
				...
				editable ? createVNode("textarea", {
					...
					"value": content,
					"onInput": (e$1) => editContent.value = e$1.target.value
				}, null, 40, [
					"placeholder",
					"value",
					"onInput"
				...
				createVNode("div", { "class": "uni-modal__ft" }, [showCancel && createVNode("div", {
					...
					"onClick": cancel
				}, [props$33.cancelText], 12, ["onClick"]), createVNode("div", {
...
function useOnThemeChange$1(props$33) {
	const cancelColor = ref(props$33.cancelColor);
	const _onThemeChange = ({ theme }) => {
		setCancelColor(theme, cancelColor);
	};
	watchEffect(() => {
		if (props$33.visible) {
			cancelColor.value = props$33.cancelColor;
			if (props$33.cancelColor === "#000") {
...
var toast_default = /* @__PURE__ */ defineComponent({
	...
	setup(props$33) {
		...
		const { Icon } = useToastIcon(props$33);
		const visible = usePopup(props$33, {});
		return () => {
			const { mask, duration, title, image } = props$33;
...
function usePopupStyle(props$33) {
	...
	const popupStyle = computed(() => {
		...
		const popover = props$33.popover;
...
var actionSheet_default = /* @__PURE__ */ defineComponent({
	...
	setup(props$33, { emit: emit$2 }) {
		...
		const main = ref(null);
		const { t: t$1 } = useI18n();
		const { _close } = useActionSheetLoader(props$33, emit$2);
		const { popupStyle } = usePopupStyle(props$33);
		let scroller;
		onMounted(() => {
			const { scroller: _scroller, handleTouchStart, handleTouchMove, handleTouchEnd } = useScroller(content.value, {
				...
				spring: new Spring(2, 90, 20),
				onScroll: (e$1) => {
					scrollTop.value = e$1.target.scrollTop;
				}
			});
			scroller = _scroller;
			useTouchtrack(content.value, (e$1) => {
				if (_scroller) switch (e$1.detail.state) {
					case "start":
						handleTouchStart(e$1);
						disableScrollBounce({ disable: true });
						break;
		...
		watch(() => props$33.visible, () => {
			nextTick(() => {
				if (props$33.title) titleHeight.value = document.querySelector(".uni-actionsheet__title").offsetHeight;
				scroller.update();
		...
		const actionSheetTheme = useOnThemeChange(props$33);
		return () => {
			return createVNode("uni-actionsheet", { "onTouchmove": onEventPrevent }, [createVNode(Transition, { "name": "uni-fade" }, { default: () => [withDirectives(createVNode("div", {
				...
				"onClick": () => _close(-1)
			}, null, 8, ["onClick"]), [[vShow, props$33.visible]])] }), createVNode("div", {
				"class": ["uni-actionsheet", { "uni-actionsheet_toggle": props$33.visible }],
				...
				}, [props$33.title ? createVNode(Fragment, null, [createVNode("div", {
					"class": "uni-actionsheet__cell",
					"style": { height: `${titleHeight.value}px` }
				}, null, 4), createVNode("div", { "class": "uni-actionsheet__title" }, [props$33.title])]) : "", createVNode("div", { "style": {
					maxHeight: `${HEIGHT.value}px`,
					overflow: "hidden"
				} }, [createVNode("div", { "ref": content }, [props$33.itemList.map((itemTitle, index$4) => createVNode("div", {
					"key": index$4,
					"style": { color: actionSheetTheme.listItemColor },
					"class": "uni-actionsheet__cell",
					"onClick": () => _close(index$4)
				...
				createVNode("div", { "class": "uni-actionsheet__action" }, [createVNode("div", {
					...
					"class": "uni-actionsheet__cell",
					"onClick": () => _close(-1)
				}, [t$1("uni.showActionSheet.cancel")], 12, ["onClick"])]),
				createVNode("div", { "style": popupStyle.value.triangle }, null, 4)
			], 6)], 40, ["onTouchmove"]);
		};
	}
});
function useActionSheetLoader(props$33, emit$2) {
	function _close(tapIndex) {
		emit$2("close", tapIndex);
	}
	const { key, disable } = useKeyboard();
	watch(() => props$33.visible, (value) => disable.value = !value);
	watchEffect(() => {
		const { value } = key;
...
function useOnThemeChange(props$33) {
	...
	watchEffect(() => {
		if (props$33.visible) {
			actionSheetTheme.listItemColor = actionSheetTheme.cancelItemColor = props$33.itemColor;
			if (props$33.itemColor === "#000") {
...
function setProperties(item, props$33, propsData) {
	props$33.forEach(function(name) {
...
const showTopWindow = /* @__PURE__ */ defineAsyncApi("showTopWindow", (_, { resolve, reject }) => {
	const state$3 = getLayoutState();
	if (!state$3) {
		reject();
		return;
	}
	state$3.apiShowTopWindow = true;
	nextTick(resolve);
});
const hideTopWindow = /* @__PURE__ */ defineAsyncApi("hideTopWindow", (_, { resolve, reject }) => {
	...
	state$3.apiShowTopWindow = false;
	nextTick(resolve);
});
const showLeftWindow = /* @__PURE__ */ defineAsyncApi("showLeftWindow", (_, { resolve, reject }) => {
	...
	state$3.apiShowLeftWindow = true;
	nextTick(resolve);
});
const hideLeftWindow = /* @__PURE__ */ defineAsyncApi("hideLeftWindow", (_, { resolve, reject }) => {
	...
	state$3.apiShowLeftWindow = false;
	nextTick(resolve);
});
const showRightWindow = /* @__PURE__ */ defineAsyncApi("showRightWindow", (_, { resolve, reject }) => {
	...
	state$3.apiShowRightWindow = true;
	nextTick(resolve);
});
	...
	setup() {
		...
		});
		{
			...
			function compassChangeHandler(res) {
				state$3.rotate = res.direction;
			}
			function updateLocation() {
				getLocation({
					type: "gcj02",
					success: (res) => {
						state$3.latitude = res.latitude;
						state$3.longitude = res.longitude;
					},
					complete: () => {
		...
		return () => {
			return state$3.latitude ? createVNode(MapMarker_default, mergeProps({
				"anchor": {
					x: .5,
					y: .5
				},
	...
	setup(props$33) {
		...
		inject("onMapReady")((map, maps$1, trigger) => {
			function drawPolygon() {
				const { points, strokeWidth, strokeColor, dashArray, fillColor, zIndex } = props$33;
...
function useMap(props$33, rootRef, emit$2) {
	const trigger = useCustomEvent(rootRef, emit$2);
	const mapRef = ref(null);
	let maps$1;
	let map;
	const state$3 = reactive({
		latitude: Number(props$33.latitude),
		longitude: Number(props$33.longitude),
		includePoints: getPoints(props$33.includePoints)
	});
	...
	watch([() => props$33.latitude, () => props$33.longitude], ([latitudeVlaue, longitudeVlaue]) => {
		const latitude = Number(latitudeVlaue);
		const longitude = Number(longitudeVlaue);
		if (latitude !== state$3.latitude || longitude !== state$3.longitude) {
			state$3.latitude = latitude;
			state$3.longitude = longitude;
			if (map) {
				const centerPosition = getMapPosition(maps$1, state$3.latitude, state$3.longitude);
				map.setCenter(centerPosition);
			}
		}
	});
	watch(() => props$33.includePoints, (points) => {
		state$3.includePoints = getPoints(points);
	...
	function updateBounds() {
		if (getIsAMap()) {
			const points = [];
			state$3.includePoints.forEach((point) => {
			...
			state$3.includePoints.forEach(({ latitude, longitude }) => {
	...
	function initMap() {
		...
		const center = getMapPosition(maps$1, state$3.latitude, state$3.longitude);
		const event = maps$1.event || maps$1.Event;
		const map$1 = new maps$1.Map(mapEl, {
			center,
			zoom: Number(props$33.scale),
		...
		if (getIsBMap()) {
			map$1.centerAndZoom(center, Number(props$33.scale));
			map$1.enableScrollWheelZoom();
			map$1._printLog && map$1._printLog("uniapp");
		}
		watch(() => props$33.scale, (scale) => {
			map$1.setZoom(Number(scale) || 16);
		});
		onBoundsReady(() => {
			if (state$3.includePoints.length) {
				updateBounds();
				updateCenter();
			}
		});
		...
		useSubscribe((type, data = {}) => {
			switch (type) {
				...
				case "translateMarker":
					...
					break;
				case "includePoints":
					state$3.includePoints = getPoints(data.includePoints);
	...
	onMounted(() => {
		loadMaps(props$33.libraries, (result) => {
			maps$1 = result;
			map = initMap();
			emitMapReady();
			trigger("updated", {}, {});
...
var map_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$33, { emit: emit$2, slots }) {
		const rootRef = ref(null);
		const { mapRef, trigger } = useMap(props$33, rootRef, emit$2);
		return () => {
			return createVNode("uni-map", {
				...
				props$33.markers.map((item) => createVNode(MapMarker_default, mergeProps({ "key": item.id }, item), null, 16)),
				props$33.polyline.map((item) => createVNode(MapPolyline_default, item, null, 16)),
				props$33.circles.map((item) => createVNode(MapCircle_default, item, null, 16)),
				props$33.controls.map((item) => createVNode(MapControl_default, mergeProps(item, { "trigger": trigger }), null, 16, ["trigger"])),
				props$33.showLocation && createVNode(MapLocation_default, null, null),
				props$33.polygons.map((item) => createVNode(map_polygon_default, item, null, 16)),
...
var cover_view_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$33, { slots }) {
		...
		onMounted(() => {
			setScrollTop(props$33.scrollTop);
		});
		return () => {
			return createVNode("uni-cover-view", {
				"scroll-top": props$33.scrollTop,
				"ref": root
			}, [createVNode("div", {
				"ref": content,
...
function getDefaultStartValue(props$33) {
	if (props$33.mode === mode.TIME) return "00:00";
	if (props$33.mode === mode.DATE) {
		const year = (/* @__PURE__ */ new Date()).getFullYear() - 150;
		switch (props$33.fields) {
...
function getDefaultEndValue(props$33) {
	if (props$33.mode === mode.TIME) return "23:59";
	if (props$33.mode === mode.DATE) {
		const year = (/* @__PURE__ */ new Date()).getFullYear() + 150;
		switch (props$33.fields) {
			case fields.YEAR: return year.toString();
...
function getDateValueArray(props$33, state$3, valueStr, defaultValue) {
	const splitStr = props$33.mode === mode.DATE ? "-" : ":";
	const array = props$33.mode === mode.DATE ? state$3.dateArray : state$3.timeArray;
	let max;
	if (props$33.mode === mode.TIME) max = 2;
	else switch (props$33.fields) {
		case fields.YEAR:
			max = 1;
			break;
		case fields.MONTH:
	...
	if (value.indexOf(-1) >= 0) value = defaultValue ? getDateValueArray(props$33, state$3, defaultValue) : value.map(() => 0);
	return value;
}
var mode = {
	SELECTOR: "selector",
...
var picker_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	props: {
		...
		start: {
			type: String,
			default: (props$33) => {
				return getDefaultStartValue(props$33);
			}
		},
		end: {
			type: String,
			default: (props$33) => {
				return getDefaultEndValue(props$33);
			}
		},
		disabled: {
			type: [Boolean, String],
	...
	setup(props$33, { emit: emit$2, slots }) {
		...
		const { state: state$3, rangeArray } = usePickerState(props$33);
		const { system, selectorTypeComputed, _show, _l10nColumn, _l10nItem, _input, _fixInputPosition, _pickerViewChange, _cancel, _change, _resetFormData, _getFormData, _createTime, _createDate, _setValueSync } = usePickerMethods(props$33, state$3, useCustomEvent(rootRef, emit$2), rootRef, pickerRef, selectRef, inputRef);
		usePickerWatch(state$3, _cancel, _change);
		usePickerForm(_resetFormData, _getFormData);
		_createTime();
		_createDate();
		_setValueSync();
		const popup = usePopupStyle(state$3);
		watchEffect(() => {
			state$3.isDesktop = popup.isDesktop.value;
			state$3.popupStyle = popup.popupStyle.value;
		});
		onBeforeUnmount(() => {
		...
		return () => {
			let _slot2;
			const { visible, contentVisible, valueArray, popupStyle, valueSync } = state$3;
			const { rangeKey, mode: mode$1, start, end } = props$33;
			const booleanAttrs = useBooleanAttr(props$33, "disabled");
...
function usePickerState(props$33) {
	const state$3 = reactive({
		valueSync: void 0,
		visible: false,
	...
	return {
		state: state$3,
		rangeArray: computed(() => {
			let val = props$33.range;
			switch (props$33.mode) {
				case mode.SELECTOR: return [val];
				case mode.MULTISELECTOR: return val;
				case mode.TIME: return state$3.timeArray;
				case mode.DATE: {
					const dateArray = state$3.dateArray;
					switch (props$33.fields) {
...
function usePickerMethods(props$33, state$3, trigger, rootRef, pickerRef, selectRef, inputRef) {
	const isiPad = useIsiPad();
	const _system = useSystem();
	const selectorTypeComputed = computed(() => {
		const type = props$33.selectorType;
		if (Object.values(selectorType).includes(type)) return type;
		return isiPad.value ? selectorType.PICKER : selectorType.SELECT;
	});
	const system = computed(() => {
		if (props$33.mode === mode.DATE && !Object.values(fields).includes(props$33.fields) && state$3.isDesktop) return _system.value;
		return "";
	});
	const startArray = computed(() => {
		return getDateValueArray(props$33, state$3, props$33.start, getDefaultStartValue(props$33));
	});
	const endArray = computed(() => {
		return getDateValueArray(props$33, state$3, props$33.end, getDefaultEndValue(props$33));
	});
	function _show(event) {
		if (props$33.disabled) return;
		state$3.valueChangeSource = "";
		...
		setTimeout(() => {
			state$3.visible = true;
		}, 20);
	}
	function _getFormData() {
		return {
			value: state$3.valueSync,
			key: props$33.name
		};
	}
	function _resetFormData() {
		switch (props$33.mode) {
			...
			case mode.MULTISELECTOR:
				state$3.valueSync = props$33.value.map((val) => 0);
				break;
			case mode.DATE:
			case mode.TIME:
				state$3.valueSync = "";
	...
	function _createTime() {
		...
		state$3.timeArray.push(hours, minutes);
	}
	function getYearStartEnd() {
		...
		let end = year + 150;
		if (props$33.start) {
			const _year = new Date(props$33.start).getFullYear();
			if (!isNaN(_year) && _year < start) start = _year;
		}
		if (props$33.end) {
			const _year = new Date(props$33.end).getFullYear();
	...
	function _createDate() {
		...
		state$3.dateArray.push(years, months, days);
	}
	...
	function _setValueSync() {
		let val = props$33.value;
		switch (props$33.mode) {
			case mode.MULTISELECTOR:
				{
					if (!isArray(val)) val = state$3.valueArray;
					if (!isArray(state$3.valueSync)) state$3.valueSync = [];
					const length = state$3.valueSync.length = Math.max(val.length, props$33.range.length);
					for (let index$4 = 0; index$4 < length; index$4++) {
						const val0 = Number(val[index$4]);
						const val1 = Number(state$3.valueSync[index$4]);
						const val2 = isNaN(val0) ? isNaN(val1) ? 0 : val1 : val0;
						const maxVal = props$33.range[index$4] ? props$33.range[index$4].length - 1 : 0;
						state$3.valueSync.splice(index$4, 1, val2 < 0 || val2 > maxVal ? 0 : val2);
					}
				}
				break;
			case mode.TIME:
			case mode.DATE:
				state$3.valueSync = String(val);
				break;
			default: {
				const valueSync = Number(val);
				state$3.valueSync = valueSync < 0 ? 0 : valueSync;
				break;
			}
		}
	}
	function _setValueArray() {
		let val = state$3.valueSync;
		let valueArray;
		switch (props$33.mode) {
			case mode.MULTISELECTOR:
				valueArray = [...val];
				break;
			case mode.TIME:
				valueArray = getDateValueArray(props$33, state$3, val, formatDateTime({ mode: mode.TIME }));
				break;
			case mode.DATE:
				valueArray = getDateValueArray(props$33, state$3, val, formatDateTime({ mode: mode.DATE }));
				break;
			default:
				valueArray = [val];
				break;
		}
		state$3.oldValueArray = [...valueArray];
		state$3.valueArray = [...valueArray];
	}
	function _getValue() {
		let val = state$3.valueArray;
		switch (props$33.mode) {
			case mode.SELECTOR: return val[0];
			case mode.MULTISELECTOR: return val.map((val$1) => val$1);
			case mode.TIME: return state$3.valueArray.map((val$1, i) => state$3.timeArray[i][val$1]).join(":");
			case mode.DATE: return state$3.valueArray.map((val$1, i) => state$3.dateArray[i][val$1]).join("-");
		}
	}
	function _change() {
		_close();
		state$3.valueChangeSource = "click";
		const value = _getValue();
		state$3.valueSync = isArray(value) ? value.map((val) => val) : value;
		trigger("change", {}, { value });
	}
	function _cancel($event) {
		if (system.value === "firefox" && $event) {
			const { top, left, width, height } = state$3.popover;
	...
	function _select() {
		if (props$33.mode === mode.SELECTOR && selectorTypeComputed.value === selectorType.SELECT) selectRef.value.scrollTop = state$3.valueArray[0] * 34;
	}
	function _input($event) {
		state$3.valueSync = $event.target.value;
		nextTick(() => {
			_change();
		});
	}
	...
	function _pickerViewChange(event) {
		state$3.valueArray = _l10nColumn(event.detail.value, true);
	}
	function _l10nColumn(array, normalize) {
		const { getLocale: getLocale$1 } = useI18n();
		if (props$33.mode === mode.DATE) {
			const locale = getLocale$1();
			if (!locale.startsWith("zh")) switch (props$33.fields) {
	...
	function _l10nItem(item, index$4) {
		...
		if (props$33.mode === mode.DATE) {
			...
			else if (props$33.fields !== fields.YEAR && index$4 === (props$33.fields !== fields.MONTH && (locale === "es" || locale === "fr") ? 1 : 0)) {
				let array;
				switch (locale) {
					case "es":
						array = [
	...
	watch(() => state$3.visible, (val) => {
		if (val) {
			clearTimeout(__contentVisibleDelay);
			state$3.contentVisible = val;
			_select();
	...
	watch(() => state$3.valueSync, _setValueArray, { deep: true });
	watch(() => state$3.valueArray, (val) => {
		if (props$33.mode === mode.TIME || props$33.mode === mode.DATE) {
			const getValue = props$33.mode === mode.TIME ? _getTimeValue : _getDateValue;
			const valueArray = state$3.valueArray;
		...
		val.forEach((value, column) => {
			if (value !== state$3.oldValueArray[column]) {
				state$3.oldValueArray[column] = value;
				if (props$33.mode === mode.MULTISELECTOR) trigger("columnchange", {}, {
					column,
					value
				});
			}
...
function usePickerWatch(state$3, _cancel, _change) {
	const { key, disable } = useKeyboard();
	watchEffect(() => {
		disable.value = !state$3.visible;
	});
	watch(key, (value) => {
...
var AdRender = class {
	constructor(props$33, trigger, rootRef, options) {
		...
		this._adpid = props$33.adpid;
		this._adpidWidescreen = props$33.adpidWidescreen;
		this._widescreenWidth = props$33.widescreenWidth;
...
var ad_default = /* @__PURE__ */ defineBuiltInComponent({
	...
	setup(props$33, { emit: emit$2, slots }) {
		...
		const ad = new AdRender(props$33, useCustomEvent(rootRef, emit$2), rootRef, {
		...
		watch(() => props$33.adpid, (val) => {
			ad.load(val);
		});
		watch(() => props$33.adpidWidescreen, (val) => {
			ad.load(val);
		});
		onMounted(() => {
		...
		return () => {
			const { adpid, adpidWidescreen, widescreenWidth } = props$33;
...
function usePageHeadTransparent(headRef, { id: id$1, navigationBar: { titleColor, coverage, backgroundColor } }) {
	...
	useOn(id$1 + ".onPageScroll", ({ scrollTop }) => {
...
function usePageHeadButtons({ id: id$1, navigationBar }) {
	...
	if (isArray(buttons)) {
		...
		buttons.forEach((btn, index$4) => {
			...
			const pageHeadBtn = usePageHeadButton(id$1, index$4, btn, isTransparent);
...
function usePageHeadSearchInput({ id: id$1, navigationBar: { searchInput } }) {
	...
	if (disabled) {
		const onClick = () => {
			invokeHook(id$1, ON_NAVIGATION_BAR_SEARCH_INPUT_CLICKED);
		};
		return {
			focus,
			text,
	...
	const onFocus = () => {
		focus.value = true;
		invokeHook(id$1, ON_NAVIGATION_BAR_SEARCH_INPUT_FOCUS_CHANGED, { focus: true });
	};
	const onBlur = () => {
		focus.value = false;
		invokeHook(id$1, ON_NAVIGATION_BAR_SEARCH_INPUT_FOCUS_CHANGED, { focus: false });
	};
	const onInput = (evt) => {
		text.value = evt.detail.value;
		invokeHook(id$1, ON_NAVIGATION_BAR_SEARCH_INPUT_CHANGED, { text: text.value });
	};
	const onConfirm = (evt) => {
		invokeHook(id$1, ON_NAVIGATION_BAR_SEARCH_INPUT_CONFIRMED, { text: text.value });
	};
	return {
		focus,
		text,
...
var __plugin_vue_export_helper_default = (sfc, props$33) => {
	const target = sfc.__vccOpts || sfc;
	for (const [key, val] of props$33) target[key] = val;
	return target;
};
...
function usePageRefresh(refreshRef) {
	const pageMeta = usePageMeta();
	const { id: id$1, pullToRefresh } = pageMeta;
	...
	}, API_START_PULL_DOWN_REFRESH, false, id$1);
	useSubscribe(() => {
		if (!pageMeta.enablePullDownRefresh) return;
		if (state$3 === REFRESHING) {
			removeClass();
			state$3 = RESTORING;
			addClass();
			restoring(() => {
				removeClass();
				state$3 = distance$1 = offset = null;
			});
		}
	}, API_STOP_PULL_DOWN_REFRESH, false, id$1);
	...
	function toggleClass(type) {
		if (!state$3) return;
		if (refreshContainerElem) refreshContainerElem.classList[type]("uni-page-refresh--" + state$3);
	}
	function addClass() {
		toggleClass("add");
	}
	...
	const onTouchstartPassive = withWebEvent((ev) => {
		...
		if ([
			...
			REFRESHING,
			RESTORING
		].indexOf(state$3) >= 0) canRefresh = false;
	...
	const onTouchmove = withWebEvent((ev) => {
		...
		distance$1 = deltaY;
		if (deltaY >= range && state$3 !== REACHED || deltaY < range && state$3 !== PULLING) {
			removeClass();
			state$3 = state$3 === REACHED ? PULLING : REACHED;
			addClass();
		}
		pulling(deltaY);
	});
	...
	function refreshing() {
		...
		invokeHook(id$1, ON_PULL_DOWN_REFRESH);
	}
