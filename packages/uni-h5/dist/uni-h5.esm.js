import {isFunction, isArray, hasOwn as hasOwn$1, isString, isPlainObject, isObject as isObject$1, capitalize, toRawType, makeMap as makeMap$1, extend, isPromise, invokeArrayFns as invokeArrayFns$1, hyphenate} from "@vue/shared";
...
import {removeLeadingSlash, getLen, passive, invokeArrayFns, NAVBAR_HEIGHT, parseQuery, decodedQuery, plusReady, debounce, PRIMARY_COLOR as PRIMARY_COLOR$1} from "@dcloudio/uni-shared";
import {useRoute as useRoute$1, createRouter, createWebHistory, createWebHashHistory, isNavigationFailure, RouterView} from "vue-router";
...
const createIntersectionObserver$1 = defineSyncApi("createIntersectionObserver", (context, options) => {
...
const i18n$1 = initVueI18n(getBaseSystemInfo().language);
function useI18n() {
  return i18n$1;
}
const i18n = useI18n();
...
function initPageMeta() {
  if (__UNI_FEATURE_PAGES__) {
    return reactive(normalizePageMeta(JSON.parse(JSON.stringify(mergePageMeta(useRoute$1().meta)))));
  }
...
var TabBar = /* @__PURE__ */ defineComponent({
  ...
  setup() {
    const tabBar2 = useTabBar();
    const onSwitchTab = useSwitchTab(useRoute$1(), tabBar2);
    const {
      style,
      borderStyle,
      placeholderStyle
