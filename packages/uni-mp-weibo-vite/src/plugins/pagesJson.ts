...
function generateCssCode(config: ResolvedConfig) {
  ...
  const cssFiles = [MP_WEIBO_FRAMEWORK_STYLE_PATH + 'base.css']
  if (config.isProduction) {
    cssFiles.push(MP_WEIBO_FRAMEWORK_STYLE_PATH + 'shadow.css')
  }
  // if (define.__UNI_FEATURE_PAGES__) {
  cssFiles.push(MP_WEIBO_FRAMEWORK_STYLE_PATH + 'async.css')
  // }
  if (define.__UNI_FEATURE_RESPONSIVE__) {
    cssFiles.push(MP_WEIBO_FRAMEWORK_STYLE_PATH + 'layout.css')
  }
  if (define.__UNI_FEATURE_NAVIGATIONBAR__) {
    cssFiles.push(MP_WEIBO_FRAMEWORK_STYLE_PATH + 'pageHead.css')
  }
  if (define.__UNI_FEATURE_TABBAR__) {
    cssFiles.push(MP_WEIBO_FRAMEWORK_STYLE_PATH + 'tabBar.css')
  }
  if (define.__UNI_FEATURE_NVUE__) {
    cssFiles.push(MP_WEIBO_FRAMEWORK_STYLE_PATH + 'nvue.css')
  }
  if (define.__UNI_FEATURE_PULL_DOWN_REFRESH__) {
    cssFiles.push(MP_WEIBO_FRAMEWORK_STYLE_PATH + 'pageRefresh.css')
  }
  ...
  if (config.command === 'serve' || !enableTreeShaking) {
    ...
    Object.keys(WEIBO_API_DEPS_CSS).forEach((name) => {
      const styles = WEIBO_API_DEPS_CSS[name as keyof typeof WEIBO_API_DEPS_CSS]
