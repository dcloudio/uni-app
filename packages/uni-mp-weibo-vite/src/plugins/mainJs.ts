...
function createApp(code: string) {
  return `import { plugin as __plugin } from '@dcloudio/uni-mp-weibo';${code.replace(
    'createSSRApp',
...
function createLegacyApp(code: string) {
  return `import { plugin as __plugin } from '@dcloudio/uni-mp-weibo';function createApp(rootComponent,rootProps){return createVueApp(rootComponent, rootProps).use(__plugin)};${code.replace(
    'createApp',
    'createVueApp'
  )}`
}
