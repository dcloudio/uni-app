...
declare namespace ts {
    ...
    interface Printer {
         ...
         * @param sourceFile
         */
        printSourceFile(sourceFile: SourceFile, options: {
            host: UTSEmitHost;
            map?: UTSSourceMapOptions;
        }): {
            code: string;
            map?: string;
        };
    ...
    function parseNodeWithCustomTransformer<T extends Node>(node: T, parser: CustomParsers, options: ParseTransformNodesOptions): Node;
    interface CustomParser {
        SourceFile?: ParseSourceFileTransformerFactory;
        InterfaceDeclaration?: ParseNodeTransformerFactory<InterfaceDeclaration>;
        TypeAliasDeclaration?: ParseNodeTransformerFactory<TypeAliasDeclaration>;
    }
    ...
    interface ParseNodeTransformationContext extends ParseTransformNodesOptions {
        factory: NodeFactory;
    }
    type ParseNodeTransformerFactory<T extends Node> = (context: ParseNodeTransformationContext) => (node: T) => Node | undefined;
    type ParseSourceFileTransformerFactory = (context: ParseSourceFileTransformationContext) => ParseTransformer<SourceFile>;
    type ParseTransformer<T extends Node> = (node: T) => T;
    interface ParseTransformNodesOptions {
        fileName: string;
        parseErrorAtCurrentToken(message: DiagnosticMessage, ...args: DiagnosticArguments): DiagnosticWithDetachedLocation | undefined;
    }
    interface ParseSourceFileTransformationContext extends ParseTransformNodesOptions, TransformationContext {
        readonly factory: NodeFactory;
        addParseDiagnostic(diag: DiagnosticWithLocation): void;
        /**
         * fail
         * @param diag
         */
        addBindDiagnostic(diag: DiagnosticWithLocation): void;
        /**
         * fail
         * @param diag
         */
        addBindSuggestionDiagnostic(diag: DiagnosticWithLocation): void;
    }
    function printUTSSourceFile(sourceFile: SourceFile, host: UTSEmitHost, writeFile: (sourceFile: SourceFile, writer: EmitTextWriter, sourceMapGenerator: SourceMapGenerator | undefined) => void, beginPrint: () => EmitTextWriter, endPrint: () => string, printerOptions: PrinterOptions, mapOptions?: UTSSourceMapOptions): {
        code: string;
        map: string | undefined;
    };
    type UTSEmitHost = Pick<EmitHost, "getCanonicalFileName" | "getCurrentDirectory">;
    interface UTSSourceMapOptions {
        file: string;
        sourceRoot: string;
