...
function genSetEvent(oper, context) {
	...
	function genHandler() {
		var _context$options$genE, _context$options;
		if (handler) return handler;
		const generated = genEventHandler(context, [value], modifiers);
		return handler = ((_context$options$genE = (_context$options = context.options).genEventHandler) === null || _context$options$genE === void 0 ? void 0 : _context$options$genE.call(_context$options, oper, context, generated)) || generated;
	}
	function genInvoker() {
		return [
...
function genDynamicProps(oper, context, helperName = "setDynamicProps") {
	const { helper } = context;
	...
	return [NEWLINE, ...genCall(helper(helperName), `n${oper.element}`, genMulti(DELIMITERS_ARRAY, ...values), isSVG && "true")];
}
...
function genDynamicComponentFlags(root, once, slotRoot, ns) {
	let flags = 0;
	const names = [];
	if (root) {
		flags |= 1;
...
function genRawProps(props, context, directStaticLiteralProps = false) {
	...
	if (isArray$1(staticProps)) {
		...
		return genStaticProps(staticProps, context, genDynamicProps$1(props.slice(1), context, directStaticLiteralProps), directStaticLiteralProps);
	} else if (props.length) return genStaticProps([], context, genDynamicProps$1(props, context, directStaticLiteralProps), directStaticLiteralProps);
}
...
function genDynamicProps$1(props, context, directStaticLiteralProps = false) {
	const { helper } = context;
	const frags = [];
...
function genOperation(oper, context) {
	var _context$options$genO, _context$options;
	const custom = (_context$options$genO = (_context$options = context.options).genOperation) === null || _context$options$genO === void 0 ? void 0 : _context$options$genO.call(_context$options, oper, context);
	if (custom) return custom;
	switch (oper.type) {
		case 2: return genSetBlockKey(oper, context);
		case 3: return genSetProp(oper, context);
		case 4: return genDynamicProps(oper, context);
...
const transformElement = (node, context) => {
	...
	return function postTransformElement() {
		...
		if (isComponent) transformComponentElement(node, propsResult, staticKey, singleRoot, context, isDynamicComponent, useCreateElement);
			...
			var _context$options$tran, _context$options;
			(_context$options$tran = (_context$options = context.options).transformElementProps) === null || _context$options$tran === void 0 || _context$options$tran.call(_context$options, propsResult, node, context, getEffectIndex);
			transformNativeElement(node, propsResult, staticKey, singleRoot, context, getEffectIndex, context.root === context.effectiveParent || canOmitEndTag(node, context));
		}
...
const transformSlotOutlet = (node, context) => {
	...
	return () => {
		...
		if (node.__uniTextSlot) flags |= 256;
		context.dynamic.operation = _objectSpread2(_objectSpread2({
			type: 12,
			id
		}, context.effectBoundary()), {}, {
...
//#endregion
export { CodegenContext, DynamicFlag, IRDynamicPropsKind, IRNodeTypes, IRSlotType, NEWLINE, TemplateRegistry, VaporErrorCodes, VaporErrorMessages, buildCodeFragment, codeFragmentToString, compile, createStructuralDirectiveTransform, createVaporCompilerError, foldClassValues, genCall, genDynamicProps, genExpression, genMulti, generate, getBaseTransformPreset, isBlockOperation, parse, transform, transformChildren, transformComment, transformElement, transformKey, transformSlotOutlet, transformTemplateRef, transformText, transformTransition, transformVBind, transformVFor, transformVHtml, transformVIf, transformVModel, transformVOn, transformVOnce, transformVShow, transformVSlot, transformVText, wrapTemplate };
