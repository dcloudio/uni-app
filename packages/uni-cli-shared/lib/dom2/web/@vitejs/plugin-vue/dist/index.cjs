...
function createDescriptor(filename, source, { root, isProduction, sourceMap, compiler, template, features }, hmr = false) {
	...
	const normalizedPath = (0, vite.normalizePath)(node_path.default.relative(root, filename));
...
function getDescriptor(filename, options, createIfNotFound = true, hmr = false, code) {
	...
	if (createIfNotFound) {
		const { descriptor, errors } = createDescriptor(filename, code ?? node_fs.default.readFileSync(filename, "utf-8"), options, hmr);
...
function getHash(text) {
	return node_crypto.default.hash("sha256", text, "hex").substring(0, 8);
}
//#endregion
...
function resolveTemplateCompilerOptions(descriptor, options, filename, ssr) {
	...
	if (transformAssetUrls === false) {} else if (options.devServer) {
		if (filename.startsWith(options.root)) {
			...
			assetUrlOptions = {
				base: (options.devServer.config.server?.origin ?? "") + devBase + slash(node_path.default.relative(options.root, node_path.default.dirname(filename))),
...
//#endregion
//#region ../../node_modules/.pnpm/obug@3.0.0/node_modules/obug/dist/core.js
/**
* Coerce `value`.
*/
...
//#endregion
//#region ../../node_modules/.pnpm/obug@3.0.0/node_modules/obug/dist/ansi.js
let env = {};
try {
	process.env.DEBUG;
	env = process.env;
...
*/
function useColors() {
	return "colors" in inspectOpts ? Boolean(inspectOpts.colors) : (0, node_tty.isatty)(process.stderr.fd);
}
function getDate() {
...
function log(...args) {
	process.stderr.write(`${(0, node_util.formatWithOptions)(this.inspectOpts, ...args)}\n`);
}
	...
	formatters: {
		...
		*/
		o(v) {
			...
			return (0, node_util.inspect)(v, this.inspectOpts).split("\n").map((str) => str.trim()).join(" ");
		},
		/**
		...
		*/
		O(v) {
			this.inspectOpts.colors = this.useColors;
			return (0, node_util.inspect)(v, this.inspectOpts);
		}
	},
	inspectOpts,
	log,
...
async function handleHotUpdate({ file, modules, read }, options, customElement, typeDepModules) {
	...
	if (needRerender) {
		...
		else if (mainModule && !affectedModules.has(mainModule)) [...mainModule.importers].filter((m) => (0, vite.isCSSRequest)(m.url)).forEach((m) => affectedModules.add(m));
	}
...
//#endregion
//#region src/main.ts
const emptyScriptLangRE = /<script[^>]*\slang\s*=\s*["']?(tsx?)\b[^>]*?(?:\/>|>\s*<\/script\s*>)/;
async function transformMain(code, filename, options, pluginContext, ssr, customElement) {
	const { devServer, isProduction, devToolsEnabled } = options;
	const prevDescriptor = getPrevDescriptor(filename);
	const { descriptor, errors } = createDescriptor(filename, code, options);
	if (node_fs.default.existsSync(filename)) getDescriptor(filename, options, true, true, filename.endsWith(".vue") ? void 0 : code);
	if (errors.length) {
	...
	if (devToolsEnabled || devServer && !isProduction) attachedProps.push([`__file`, JSON.stringify(isProduction ? node_path.default.basename(filename) : filename)]);
	if (devServer && devServer.config.server.hmr !== false && !ssr && !isProduction) {
		...
		if (prevDescriptor && isOnlyTemplateChanged(prevDescriptor, descriptor)) output.push(`export const _rerender_only = __VUE_HMR_RUNTIME__.CHANGED_FILE === ${JSON.stringify((0, vite.normalizePath)(filename))}`);
	...
	if (ssr) {
		const normalizedFilename = (0, vite.normalizePath)(node_path.default.relative(options.root, filename));
	...
	const lang = descriptor.scriptSetup?.lang || descriptor.script?.lang || emptyScriptLangRE.exec(code)?.[1];
	if (lang && /tsx?$/.test(lang) && !descriptor.script?.src) {
		...
		if (transformWithOxc) {
			...
			const { code, map } = await (0, vite.transformWithEsbuild)(resolvedCode, filename, {
				target: "esnext",
				charset: "utf8",
...
async function transformStyle(code, descriptor, index, options, pluginContext, filename) {
	...
	const map = result.map ? await (0, vite.formatPostcssSourceMap)(result.map, filename) : { mappings: "" };
	return {
		code: result.code,
		map,
...
function vuePlugin(rawOptions = {}) {
	clearScriptCache();
	const options = (0, vue.shallowRef)({
	...
	const include = (0, vue.shallowRef)(rawOptions.include ?? /\.vue$/);
	const exclude = (0, vue.shallowRef)(rawOptions.exclude);
	let optionsHookIsCalled = false;
	const filter = (0, vue.computed)(() => (0, vite.createFilter)(include.value, exclude.value));
	const customElementFilter = (0, vue.computed)(() => {
		const customElement = options.value.features?.customElement || options.value.customElement;
		return typeof customElement === "boolean" ? () => customElement : (0, vite.createFilter)(customElement);
	});
	...
	const plugin = {
		...
		handleHotUpdate(ctx) {
			ctx.server.ws.send({
				type: "custom",
				event: "file-changed",
				data: { file: (0, vite.normalizePath)(ctx.file) }
			});
		...
		options() {
			...
			plugin.transform.filter = { id: {
				include: [...(0, _rolldown_pluginutils.makeIdFiltersToMatchWithQuery)(ensureArray(include.value)), /[?&]vue\b/],
		...
		resolveId: {
			filter: { id: [(0, _rolldown_pluginutils.exactRegex)(EXPORT_HELPER_ID), /[?&]vue\b/] },
			handler(id) {
		...
		load: {
			...
			handler(id, opt) {
				...
				if (query.vue) {
					if (query.src) return node_fs.default.readFileSync(filename, "utf-8");
...
exports["module.exports"] = vuePluginCjs;
