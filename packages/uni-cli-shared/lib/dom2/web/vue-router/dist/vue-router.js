...
import { computed, createComponent, createDynamicComponent, createPlainElement, createTemplateRefSetter, defineComponent, defineVaporComponent, getCurrentInstance, h, inject, nextTick, provide, reactive, ref, shallowReactive, shallowRef, unref, watch, watchEffect } from "vue";
...
const VaporRouterLink = defineVaporComponent({
	...
	setup(props, { slots, attrs }) {
		...
		return createDynamicComponent(() => {
			if (props.custom && slots.default) return slots.default(link);
			return createPlainElement("a", {
				"aria-current": () => link.isExactActive ? props.ariaCurrentValue : null,
				href: () => link.href,
				class: () => elClass.value,
				$: [() => attrs, { onClick: () => attrs.onClick ? [link.navigate, attrs.onClick] : link.navigate }]
...
//#endregion
//#region src/VaporRouterView.ts
const VaporRouterView = /*#__PURE__*/ defineVaporComponent({
	...
	setup(props, { attrs, slots }) {
		...
		const routeProps = computed(() => {
			...
			return routePropsOption ? routePropsOption === true ? route.params : typeof routePropsOption === "function" ? routePropsOption(route) : routePropsOption : null;
		});
		const setRef = createTemplateRefSetter();
		const initComponent = () => {
			if (!ViewComponent.value) return [];
			const instance = createComponent(ViewComponent.value, { $: [() => assign({}, routeProps.value, attrs)] });
...
//#endregion
export { NavigationFailureType, RouterLink, RouterView, START_LOCATION_NORMALIZED as START_LOCATION, VaporRouterLink, VaporRouterView, createMemoryHistory, createRouter, createRouterMatcher, createWebHashHistory, createWebHistory, isNavigationFailure, loadRouteLocation, matchedRouteKey, onBeforeRouteLeave, onBeforeRouteUpdate, parseQuery, routeLocationKey, routerKey, routerViewLocationKey, stringifyQuery, useLink, useRoute, useRouter, viewDepthKey };
