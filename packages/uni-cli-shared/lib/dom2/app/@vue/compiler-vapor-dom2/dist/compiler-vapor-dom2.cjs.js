...
function generate$1(ir, options) {
  ...
  if (options.componentType === "page") {
    ...
    push(
      compilerVapor.NEWLINE,
      `__sharedDataScope = ${context.helper("useSharedDataScope")}(__sharedData)`
    );
  } else {
    push(
      compilerVapor.NEWLINE,
...
function buildElementStatements(tag, context, nodeAttrs, children) {
  ...
  if (attrs && !isEmptyAttrs(attrs)) {
    ...
    for (const [name, value] of Object.entries(attrs)) {
      if (name === "style") {
        const { obj: styleObj, messages } = context.options.parseStaticStyle(
...
function normalizeOptions(options) {
  ...
  if (resolvedOptions.parseStaticStyle) {
    const parseStaticStyle = resolvedOptions.parseStaticStyle;
    const cache = /* @__PURE__ */ new Map();
    resolvedOptions.parseStaticStyle = (platform, target, style) => {
      const key = `${platform}-${target}-${style}`;
      if (cache.has(key)) {
        return cache.get(key);
      }
      const result = parseStaticStyle(platform, target, style);
...
`;
  ...
  for (let i = 0; i < members.length; i++) {
    ...
    codeCppInit += indent(2) + `${defineMacroCppCommonInitMember}(${memberName})
`;
