...
function injectHook(type, hook, target = currentInstance, prepend = false) {
  if (target) {
    if (uniShared.isRootHook(type) && target.root && target !== target.root) {
      target = target.root;
      if (uniShared.isRootImmediateHook(type)) {
        ...
        callWithAsyncErrorHandling(
          ...
          target,
          type,
          uniShared.ON_LOAD === type ? [proxy.$page.options] : []
        );
      }
    }
    ...
    const apiName = shared.toHandlerKey(
...
const createHook = (lifecycle) => (hook, target = currentInstance) => {
  if (!exports.isInSSRComponentSetup || lifecycle === "sp") {
  ...
  get({ _: instance }, key) {
    ...
    if (key[0] !== "$") {
      ...
      if (n !== void 0) {
        ...
        (normalizedProps = instance.propsOptions && instance.propsOptions[0]) && shared.hasOwn(normalizedProps, key)
      ) {
...
const computed = (getterOrOptions, debugOptions) => {
  return reactivity.computed(getterOrOptions, debugOptions, exports.isInSSRComponentSetup);
};
