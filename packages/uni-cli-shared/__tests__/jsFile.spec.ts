...
describe('miniProgram:jsonFile', () => {
  ...
  test('moves uni_modules component into one normal subpackage and shares it from main when reused', () => {
    ...
    resetMiniProgramJsonFiles()
    addMiniProgramAppJson(createSubPackagesAppJson(['pages-a']), {
      subPackages: false,
    })
    addMiniProgramComponentPackageRoot(
      'uni_modules/foo/components/foo/foo',
    ...
    const disabledJsonFiles = findChangedJsonFiles()
    expect(
      disabledJsonFiles.has('pages-a/uni_modules/foo/components/foo/foo')
    ).toBe(true)
    expect(disabledJsonFiles.has('uni_modules/foo/components/foo/foo')).toBe(
      false
    )
