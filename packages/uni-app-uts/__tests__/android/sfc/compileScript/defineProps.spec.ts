  ...
  test('w/ external definition', () => {
    expect(() => {
      ...
      const props = defineProps(propsModel)
        ...
        `)
    }).toThrow(`defineProps() argument must be an object or array literal.`)
...
const props = defineProps({ foo: String })
    ...
    `)
    assertCode(content)
    expect(content).toMatch(`props: { foo: String }`)
    //   expect(content).toMatch(`export default /*#__PURE__*/_defineComponent({
    // props: { foo: String },
    // setup(_ctx, { expose: __expose }) {`)
  })
  ...
  test('withDefaults (dynamic)', () => {
    ...
    `)
      ...
      `withDefaults does not support spread properties in the second argument.`
    )
    //   const { content } = compile(`
    //   <script setup lang="ts">
    //   import { defaults } from './foo'
  ...
  test('withDefaults (reference)', () => {
    ...
    `)
    }).toThrow(`withDefaults' second argument must be an object literal.`)
    //   const { content } = compile(`
    //   <script setup lang="ts">
    //   import { defaults } from './foo'
    //   const props = withDefaults(defineProps<{
  ...
  test('custom element retains the props type & production mode', () => {
    const { content } = compile(
      ...
      {
        isProd: true,
        customElement: (filename) => /\.ce\.vue$/.test(filename),
      },
      { filename: 'app.ce.vue' }
    )
