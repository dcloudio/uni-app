import { invokeArrayFns, SLOT_DEFAULT_NAME, EventChannel, MINI_PROGRAM_PAGE_RUNTIME_HOOKS, ON_LOAD, ON_SHOW, ON_HIDE, ON_UNLOAD, ON_RESIZE, ON_TAB_ITEM_TAP, ON_REACH_BOTTOM, ON_PULL_DOWN_REFRESH, ON_ADD_TO_FAVORITES, isUniLifecycleHook, ON_READY, ON_LAUNCH, ON_ERROR, ON_THEME_CHANGE, ON_PAGE_NOT_FOUND, ON_UNHANDLE_REJECTION, customizeEvent, addLeadingSlash, stringifyQuery } from '@dcloudio/uni-shared';
...
function parseApp(instance, parseAppOptions) {
    ...
    const onErrorHandlers = xhs.$onErrorHandlers;
    if (onErrorHandlers) {
...
function initAppLifecycle(appOptions, vm) {
    ...
    if (isFunction(appOptions.onShow) && xhs.onAppShow) {
        xhs.onAppShow((args) => {
            vm.$callHook('onShow', args);
        });
    }
    if (isFunction(appOptions.onHide) && xhs.onAppHide) {
        xhs.onAppHide((args) => {
...
const mocks = ['nodeId', 'componentPath', 'componentId'];
...
xhs.EventChannel = EventChannel;
xhs.createApp = global.createApp = createApp;
xhs.createPage = createPage;
xhs.createComponent = createComponent;
xhs.createSubpackageApp = global.createSubpackageApp =
    createSubpackageApp;
