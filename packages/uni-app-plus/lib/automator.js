...
function toHTML(node, type) {
    ...
    return formatHTML(node.pureChildren.map(function (child) { return child.toString(); }).join(""));
}
var FUNCTIONS = {
    input: {
...
var NativeAdapter = adapter;

function initNativeApi() {
    return Object.assign({}, initPage(NativeAdapter), initElement(NativeAdapter));
}
