...
function initWrapper(protocols) {
        ...
        if (isPlainObject(fromArgs)) {
            ...
            for (const key in fromArgs) {
                if (hasOwn(argsOption, key)) {
                    ...
                    if (!keyOption) {
                        // 不支持的参数
                        console.warn(`京东小程序 ${methodName} 暂不支持 ${key}`);
                    }
    ...
    return function wrapper(methodName, method) {
        ...
        if (!hasProtocol && typeof jd[methodName] !== 'function') {
            return method;
        }
        ...
        if (!hasProtocol && !method) {
            // 暂不支持的 api
            return function () {
                console.error(`京东小程序 暂不支持${methodName}`);
            };
        }
...
let deviceId;
function useDeviceId(global = jd) {
...
function initEventChannel(events, cache = true) {
    id++;
    const eventChannel = new jd.EventChannel(id, events);
    if (cache) {
...
const onError = {
    args(fromArgs) {
        ...
        if (!app.$vm) {
            if (!jd.$onErrorHandlers) {
                jd.$onErrorHandlers = [];
            }
            jd.$onErrorHandlers.push(fromArgs);
        }
        else {
...
const offError = {
    args(fromArgs) {
        ...
        if (!app.$vm) {
            ...
            const index = jd.$onErrorHandlers.findIndex((fn) => fn === fromArgs);
            if (index !== -1) {
                jd.$onErrorHandlers.splice(index, 1);
            }
        }
...
const onSocketOpen = {
    args() {
        if (jd.__uni_console__) {
            if (jd.__uni_console_warned__) {
                return;
            }
            jd.__uni_console_warned__ = true;
...
function initUni(api, protocols, platform = jd) {
