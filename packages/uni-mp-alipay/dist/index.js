...
const VidOptionCache = {};

function findVmByVueId(vm, vuePid) {
	if (vuePid == null && VidOptionCache.root) {
		return VidOptionCache.root.$vm;
	} else {
		if (VidOptionCache[vuePid] == null || VidOptionCache[vuePid] == undefined) {
			return null;
		} else {
			return VidOptionCache[vuePid].$vm;
		}
	}
...
function handleLink(event) {
	...
	if (vuePid) {
		parentVm = findVmByVueId(this, vuePid);
	}
...
function getVueId(mpInstance) {
	if (mpInstance._$vueId == undefined ||
		mpInstance._$vueId == null) {
		return "root";
	} else {
		return mpInstance._$vueId;
	}
}
...
function initChildVues(mpInstance) {
	...
	const _vueid = getVueId(mpInstance);
	const debtmp=[
		
	];
	if (debtmp.indexOf(_vueid)>=0) {
		debugger;
	}
	for (let var1 in VidOptionCache) {
		if (var1 != _vueid &&
			VidOptionCache[var1].vuePid == _vueid &&
			VidOptionCache[var1].$vm == null) {
			const childMPInstance = VidOptionCache[var1].mpInstance;
			const vueOptions = VidOptionCache[var1].vueOptions;
			const VueComponent = VidOptionCache[var1].VueComponent;
			vueOptions.parent = mpInstance.$vm;
			childMPInstance.$vm = new VueComponent(vueOptions);
			VidOptionCache[var1].$vm = childMPInstance.$vm;
			initSpecialMethods(childMPInstance);

			handleRef.call(vueOptions.ownerVm, childMPInstance);
...
const handleLink$1 = (function() {
	...
	return function handleLink$1(detail) {
		...
		if (debtmp.indexOf(detail.mpInstance._$vueId)>=0
		) {
			debugger;
		}
		if (detail.mpInstance._$vueId == undefined) { //是根节点
			return;
		} else {
			let _vpid = detail.vuePid;
			if (detail.vuePid == null || detail.vuePid == undefined) {
				if(this.props&&this.props.vueId!=null){
					_vpid=this.props.vueId.split(",")[0];
					detail.vuePid=_vpid;
				}
				else{
					detail.vueOptions.parent = this.$page.$vm != null ? this.$page.$vm : this.$vm;
					detail.vueOptions.ownerVm=this;
					VidOptionCache[detail.mpInstance._$vueId] = detail;
					return;
				}
			}

			const vpm = findVmByVueId(this.$vm, detail.vuePid);
			if (vpm == null) { //父未出现，且初始化了
				VidOptionCache[detail.mpInstance._$vueId] = detail;
				detail.vueOptions.ownerVm=this;
				return;
			} else {
...
function initVm(VueComponent) {
	...
	if(false){
		...
		if (options.parent) { // 父组件已经初始化，直接初始化子，否则放到父组件的 didMount 中处理
			...
			if (debtmp.indexOf(this._$vueId)>=0) {
				debugger;
			}
			// 初始化 vue 实例
			this.$vm = new VueComponent(options);
			VidOptionCache[this._$vueId].$vm=this.$vm;
			handleRef.call(options.ownerVm, this);
			// 触发首次 setData
			this.$vm.$mount();
...
function parseComponent(vueComponentOptions) {
	...
	const componentOptions = {
		...
		didUnmount() {
			if(this.$vm){
				if(this._$vueId!=null){
					VidOptionCache[this._$vueId]=null;
					delete VidOptionCache[this._$vueId];
				}
				this.$vm.$destroy();
			}
		},
