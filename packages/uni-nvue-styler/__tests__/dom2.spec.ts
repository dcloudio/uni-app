...
describe('dom2', () => {
  ...
  describe('parseDom2StaticStyle', () => {
    ...
    test('should handle different platforms', () => {
      ...
      const androidResult = parseDom2StaticStyle(input, {
        ...baseOptions,
        platform: DOM2_APP_PLATFORM.APP_ANDROID,
        target: DOM2_APP_TARGET.DOM_C,
      })

      const iosResult = parseDom2StaticStyle(input, {
        ...baseOptions,
        platform: DOM2_APP_PLATFORM.APP_IOS,
        target: DOM2_APP_TARGET.DOM_C,
      })

      expect(androidResult).toHaveProperty('width')
      expect(iosResult).toHaveProperty('width')
    })
  ...
  describe('processors', () => {
    describe('defineStyleVariableProcessor', () => {
      test('should create correct processor for CSS variable definition', () => {
    ...
    describe('setStyleVariableProcessor', () => {
      test('should create correct processor for CSS variable usage', () => {
    ...
    describe('createSetStyleUnitValueProcessor', () => {
    ...
    describe('createSetStyleEnumValueProcessor', () => {
  ...
  describe('edge cases', () => {
    ...
    test('should handle CSS with whitespace', () => {
      const input = '  width  :  100px  ;  position  :  static  ;  '
      const result = parseDom2StaticStyle(input, baseOptions)
  ...
  describe('complex scenarios', () => {
    test('should handle complex CSS with multiple properties, variables, and platforms', () => {
      const complexCSS = `
        /* CSS Variables */
      ...
      const result = parseDom2StaticStyle(complexCSS, {
        platform: DOM2_APP_PLATFORM.APP,
        target: DOM2_APP_TARGET.DOM_C,
      })
    ...
    test('should handle complex CSS with different platforms and targets', () => {
      const complexCSS = `
        --theme-color: #FF6B6B;
        --size: 24px;
      ...
      // Test Android platform with DOM_KT target
      const androidResult = parseDom2StaticStyle(complexCSS, {
        platform: DOM2_APP_PLATFORM.APP_ANDROID,
        target: DOM2_APP_TARGET.DOM_KT,
      })
      expect(androidResult).toMatchSnapshot('android-dom-kt')

      // Test iOS platform with DOM_OC target
      const iosResult = parseDom2StaticStyle(complexCSS, {
        platform: DOM2_APP_PLATFORM.APP_IOS,
        target: DOM2_APP_TARGET.DOM_OC,
      })
      expect(iosResult).toMatchSnapshot('ios-dom-oc')

      // Test Harmony platform with DOM_TS target
      const harmonyResult = parseDom2StaticStyle(complexCSS, {
        platform: DOM2_APP_PLATFORM.APP_HARMONY,
        target: DOM2_APP_TARGET.DOM_TS,
      })
      expect(harmonyResult).toMatchSnapshot('harmony-dom-ts')
    })

    test('should handle CSS with mixed variable usage and regular values', () => {
      const mixedCSS = `
        --base-size: 20px;
        --color-red: #FF0000;
        --color-blue: #0000FF;
