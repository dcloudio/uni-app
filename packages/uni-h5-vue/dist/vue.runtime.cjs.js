...
function watch$1(source, cb, options = shared.EMPTY_OBJ) {
...
function setRef(rawRef, oldRawRef, parentSuspense, vnode, isUnmount = false) {
  ...
  const refValue = vnode.shapeFlag & 4 && true ? getComponentPublicInstance(vnode.component) : vnode.el;
  ...
  const canSetSetupRef = setupState === shared.EMPTY_OBJ ? shared.NO : (key) => {
    {
      if (shared.hasOwn(rawSetupState, key) && !isRef(rawSetupState[key])) {
        warn$1(
  ...
  setup(props, { slots }) {
    ...
    sharedContext.activate = (vnode, container, anchor, namespace, optimized) => {
      ...
      if (instance2.ba) {
        ...
        shared.invokeArrayFns(instance2.ba);
...
function withAsyncContext(getAwaitable) {
  ...
  const inSSRSetup = exports.isInSSRComponentSetup;
  if (!ctx) {
    warn$1(
...
function normalizeWxsEventArgs(instance, handler, args) {
  ...
  if ($sysEvent && shared.hasOwn($sysEvent, "type") && shared.hasOwn($sysEvent, "timeStamp") && shared.hasOwn($sysEvent, "target") && shared.hasOwn($sysEvent, "currentTarget") && shared.hasOwn($sysEvent, "detail")) {
...
const computed = (getterOrOptions, debugOptions) => {
  const c = computed$1(getterOrOptions, debugOptions, exports.isInSSRComponentSetup);
  {
    const i = getCurrentInstance();
...
class VueElement extends BaseClass {
  ...
  _createVNode() {
    ...
    const vnode = createVNode(this._def, shared.extend(baseProps, this._props));
    if (!this._instance) {
      vnode.ce = (instance) => {
...
exports.effectScope = effectScope;
exports.getComponentPublicInstance = getComponentPublicInstance;
