...
function normalizeWxsEventArgs(instance, handler, args) {
  ...
  if ($sysEvent && hasOwn($sysEvent, "type") && hasOwn($sysEvent, "timeStamp") && hasOwn($sysEvent, "target") && hasOwn($sysEvent, "currentTarget") && hasOwn($sysEvent, "detail")) {
...
const KeepAliveImpl = {
  ...
  setup(props, { slots }) {
    ...
    function pruneCacheEntry(cached) {
      if (!current || !isSameVNodeType(cached, current) || props.matchBy === "key" && cached.key !== current.key) {
        unmount(cached);
      } else if (current) {
    ...
    onBeforeUnmount(() => {
      cache.forEach((cached, key) => {
        ...
        if (cached.type === vnode.type && (props.matchBy !== "key" || cached.key === vnode.key)) {
...
function createHydrationFunctions(rendererInternals) {
  ...
  - expected on client: ${JSON.stringify(vnode.children)}`
      ...
      default:
        if (shapeFlag & 1) {
          if ((domType !== 1 /* ELEMENT */ || vnode.type.toLowerCase() !== node.tagName.toLowerCase()) && (domType !== 1 /* ELEMENT */ || vnode.type.toLowerCase() !== "uni-" + node.tagName.toLowerCase() && vnode.type !== "uni-cloud-db-element" && node.tagName !== "UNICLOUD-DB") && !isTemplateNode(node)) {
            nextNode = onMismatch();
          } else {
...
function setVarsOnNode(el, vars) {
  if (el.nodeType === 1) {
    ...
    for (const key in vars) {
      const value = normalizeRpx(vars[key]);
