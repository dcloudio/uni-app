...
function setRef(rawRef, oldRawRef, parentSuspense, vnode, isUnmount = false) {
  ...
  const refValue =
    vnode.shapeFlag & 4 && !vnode.component.type.rootElement
      ...
      if (props) {
        if (
          ...
          for (const key in props) {
            if (
              (!!(process.env.NODE_ENV !== 'production') ||
                __VUE_PROD_HYDRATION_MISMATCH_DETAILS__) && // fixed by xxxxxx 暂时不对比style，因为服务器style会rpx2unit，而客户端是在setStyle的时候处理的，此时还没有格式化
              // 后续考虑调整客户端rpx2unit的时机，提前到vnode处
              key !== 'style' && // #11189 skip if this node has directives that have created hooks
...
const KeepAliveImpl = {
  ...
  setup(props, { slots }) {
    ...
    sharedContext.activate = (
      ...
      if (instance2.ba) {
        const currentState = instance2.isDeactivated
        instance2.isDeactivated = false
        invokeArrayFns(instance2.ba)
        instance2.isDeactivated = currentState
      }
    ...
    sharedContext.deactivate = vnode => {
      const instance2 = vnode.component
      if (instance2.bda) {
        invokeKeepAliveHooks(instance2.bda)
      }
      move(vnode, storageContainer, null, 1, parentSuspense)
      queuePostRenderEffect(() => {
        if (instance2.bda) {
          resetHookState(instance2.bda)
        }
        if (instance2.da) {
    ...
    return () => {
      ...
      if (children.length > 1) {
        ...
        !isVNode(rawVNode) ||
        (!(rawVNode.shapeFlag & 4) && !isSuspense(rawVNode.type))
      ) {
        current = null
        return rawVNode
      }
...
function injectHook(type, hook, target = currentInstance, prepend = false) {
  if (target) {
    ...
    const apiName = toHandlerKey(
      (ErrorTypeStrings$1[type] || type.replace(/^on/, '')).replace(
        / hook$/,
        '',
      ),
    )
...
const getPublicInstance = i => {
  ...
  if (isStatefulComponent(i)) {
    const p = getComponentPublicInstance(i)
...
const createForceUpdate = i => {
  return function () {
    ...
    children.forEach(child => {
      const p = getComponentPublicInstance(child) || child.proxy
      if (isUniBuiltInComponentInstance(p)) {
        p.$forceUpdate()
      }
    })
...
function useModel(props, name, options = EMPTY_OBJ) {
  ...
  if (
    !!(process.env.NODE_ENV !== 'production') &&
    !i.propsOptions[0][camelizedName]
  ) {
    ...
    for (const key in raw) {
      ...
      if (validatePropName(normalizedKey)) {
        ...
        if (
          ...
          comp.__externalClassesOptions.includes(key)
        ) {
...
const CSS_VAR_TEXT = /* @__PURE__ */ Symbol(
  !!(process.env.NODE_ENV !== 'production') ? 'CSS_VAR_TEXT' : '',
)
...
function setVarsOnNode(el, vars) {
  if (el.nodeType === 1) {
    ...
    for (const key in vars) {
      const value = normalizeRpx(normalizeCssVarValue(vars[key]))
...
function patchEvent(el, rawName, prevValue, nextValue, instance = null) {
  ...
  if (nextValue && existingInvoker) {
    existingInvoker.value = !!(process.env.NODE_ENV !== 'production')
...
function useShadowRoot() {
  const el = !!(process.env.NODE_ENV !== 'production')
    ? useHost('useShadowRoot')
    : useHost()
