      ...
      : {
          add(value) {
            ...
            const hadKey =
              ...
              (shared.hasChanged(value, valueToAdd) &&
                proto.has.call(target, value)) ||
              (shared.hasChanged(rawValue, valueToAdd) &&
...
const toReactive = value =>
  shared.isObject(value) ? /* @__PURE__ */ reactive(value) : value
const toReadonly = value =>
  shared.isObject(value) ? /* @__PURE__ */ readonly(value) : value
  ...
  const canSetSetupRef =
    setupState === shared.EMPTY_OBJ
      ...
      : key => {
          {
            if (
              shared.hasOwn(rawSetupState, key) &&
      ...
      if (props) {
        {
          ...
          for (const key in props) {
            if (
              // fixed by xxxxxx 暂时不对比style，因为服务器style会rpx2unit，而客户端是在setStyle的时候处理的，此时还没有格式化
            ...
            if (
              ...
              key[0] === '.' ||
              (isCustomElement && !shared.isReservedProp(key))
            ) {
...
const requestIdleCallback =
  shared.getGlobalThis().requestIdleCallback || (cb => setTimeout(cb, 1))
const cancelIdleCallback =
  shared.getGlobalThis().cancelIdleCallback || (id => clearTimeout(id))
const hydrateOnIdle =
  (timeout = 1e4) =>
  hydrate => {
...
function renderSlot(slots, name, props = {}, fallback, noSlotted) {
  ...
  const rendered = createBlock(
    ...
    {
      key:
        (slotKey && !shared.isSymbol(slotKey) ? slotKey : `_${name}`) + // #7256 force differentiate fallback content from actual content
...
function hasPropValueChanged(nextProps, prevProps, key) {
  ...
  if (
    key === 'style' &&
    shared.isObject(nextProp) &&
    shared.isObject(prevProp)
  ) {
...
function h(type, propsOrChildren, children) {
  try {
    ...
    if (l === 2) {
      if (
        shared.isObject(propsOrChildren) &&
        !shared.isArray(propsOrChildren)
      ) {
  ...
  if (key === 'class') {
    ...
    (shouldSetAsPropForVueCE(el, key) || // @ts-expect-error _def is private
      (el._def.__asyncLoader &&
        (/[A-Z]/.test(key) || !shared.isString(nextValue))))
  ) {
...
function shouldSetAsPropForVueCE(el, key) {
  ...
  return Array.isArray(props)
    ? props.some(prop => shared.camelize(prop) === camelKey)
    : Object.keys(props).some(prop => shared.camelize(prop) === camelKey)
}
