/**
* @dcloudio/uni-x-h5-vue v3.5.35
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/
import { extend, hasChanged, isArray, isIntegerKey, isSymbol, isMap, hasOwn, isObject, makeMap, capitalize, toRawType, def, isFunction, EMPTY_OBJ, isSet, isPlainObject, remove, NOOP, isString, isPromise, getGlobalThis, isBuiltInDirective, NO, isOn, isReservedProp, normalizeClass, stringifyStyle, normalizeStyle, isKnownSvgAttr, isBooleanAttr, isKnownHtmlAttr, includeBooleanAttr, isRenderableAttrValue, normalizeCssVarValue, getEscapedCssVarName, invokeArrayFns, isRegExp, toHandlerKey, camelize, isGloballyAllowed, hyphenate, looseToNumber, isModelListener, looseEqual, EMPTY_ARR, toNumber, isSpecialBooleanAttr, looseIndexOf, isHTMLTag, isSVGTag, isMathMLTag } from '@vue/shared';
export { camelize, capitalize, toDisplayString, toHandlerKey } from '@vue/shared';
import { isRootHook, isRootImmediateHook, ON_LOAD, normalizeClass as normalizeClass$1, normalizeStyle as normalizeStyle$1, createRpx2Unit, defaultRpx2Unit } from '@dcloudio/uni-shared';
      ...
      if (props) {
        if (!!(process.env.NODE_ENV !== "production") || __VUE_PROD_HYDRATION_MISMATCH_DETAILS__ || forcePatch || !optimized || patchFlag & (16 | 32)) {
          ...
          for (const key in props) {
            if ((!!(process.env.NODE_ENV !== "production") || __VUE_PROD_HYDRATION_MISMATCH_DETAILS__) && // fixed by xxxxxx 暂时不对比style，因为服务器style会rpx2unit，而客户端是在setStyle的时候处理的，此时还没有格式化
...
function baseCreateRenderer(options, createHydrationFns) {
  ...
  const setScopeId = (el, vnode, scopeId, slotScopeIds, parentComponent) => {
    ...
    if (process.env.UNI_APP_X_NEW_STYLE_ISOLATION && vnode.ctx) {
      const ctx = vnode.ctx;
      const styleIsolation = ctx.type.styleIsolation;
...
function createComponentInstance(vnode, parent, suspense) {
  ...
  {
    if (parent) {
      instance.page = parent.$pageInstance && parent.$pageInstance.page;
    }
  }
  if (vnode.ce) {
    vnode.ce(instance);
