...
describe('mp:compiler-mp-baidu', () => {
  ...
  it('generate scoped slot with scopedSlotsCompiler: augmented', () => {
    assertCodegen(
      ...
      '<custom-view vue-id="551070e6-1" vue-slots="{{[\'name\']}}"><block slot="name" s-if="{{test}}"><block s-for="$root.l1" s-for-item="item1" s-for-index="index1"><block s-if="{{item1.m0}}"><block s-for="item1.l0" s-for-item="item2" s-for-index="index2"><view>{{item2.m1}}</view></block></block></block></block></custom-view>',
