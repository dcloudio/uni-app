...
function reportNewSession(
  ...
  if (attachVisit && !firstVisitEmittedInProcess) {
    ...
    visit = tryRun(() => buildVisitFields(now), undefined as unknown as {
      fvts: number
      lvts: number
      tvc: number
    })
...
export function handleLaunch(
  app: StatApp,
  options: { scene?: string | number; path?: string; query?: Record<string, unknown> } = {},
...
export function handleAppHide(app: StatApp): void {
  ...
  const stayed = state.lastRouteEnterTime > 0 ? Math.max(0, now - state.lastRouteEnterTime) : 0
  c.report({
    lt: LT.Hide,
    t: now,
 ...
 */
export function handlePageShow(app: StatApp, vm: PageVm | undefined, opts: LifecycleOptions = {}): void {
  const c = safeCollector(app)
  if (!c) return
  const now = nowSec()
...
export function handlePageHide(app: StatApp, vm: PageVm | undefined): void {
  ...
  c.report({
    ...
    t: now,
    url,
    urlref: state.lastRoute && state.lastRoute !== route ? state.lastRoute : route,
    urlref_ts: stayed,
...
interface UniLifecycleApi {
  onAppShow?: (cb: (e: { scene?: string | number; path?: string }) => void) => void
  onAppHide?: (cb: () => void) => void
  offAppShow?: (cb: (e: { scene?: string | number; path?: string }) => void) => void
 ...
 *     生命周期 1:1，便于上层调试。
 */
export function bindLifecycle(app: StatApp, opts: LifecycleOptions = {}): BindLifecycleResult {
  let bound = true
  const mixin: Record<string, unknown> = {
    onLaunch(this: unknown, options: { scene?: string | number; path?: string } = {}): void {
  ...
  const u = getUni()
  let appShowCb: ((e: { scene?: string | number; path?: string }) => void) | undefined
