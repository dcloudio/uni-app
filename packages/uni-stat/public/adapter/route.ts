...
interface PageVmLike {
  ...
  $scope?: { route?: string; is?: string; $page?: { route?: string; fullPath?: string } }
}
...
export function getTopPageVm(): PageVmLike | undefined {
  const fn = (globalThis as unknown as { getCurrentPages?: () => PageEntry[] }).getCurrentPages
 ...
 *   - 失败返回 `{}`，不抛。
 */
export function parseQuery(input: string | undefined | null): Record<string, string> {
