...
interface UniNetworkApi {
  ...
  onNetworkStatusChange?: (cb: (res: { networkType: string; isConnected?: boolean }) => void) => void
  offNetworkStatusChange?: (cb: (res: { networkType: string; isConnected?: boolean }) => void) => void
}
...
export function onChange(cb: (r: NetResult) => void): () => void {
  ...
  const wrapped = (res: { networkType: string; isConnected?: boolean }): void => {
    const raw = res?.networkType ?? ''
