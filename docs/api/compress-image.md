...
| 名称 | 类型 | 必填 | 默认值 | 兼容性 | 描述 |
| :- | :- | :- | :- |  :-: | :- |
| options | **CompressImageOptions** | 是 |  | Web: x; 微信小程序:  ; Android(VDOM):  ; iOS:  ; HarmonyOS(VDOM):   |  |

#### options 的属性描述

| 名称 | 类型 | 必备 | 默认值 | 兼容性 | 描述 |
| :- | :- | :- | :- |  :-: | :- |
| src | [string.ImageURIString](/uts/data-type.md#ide-string) | 是 |  | Web: x; 微信小程序: 4.41; Android(VDOM): 4.18; iOS: 4.25; HarmonyOS(VDOM): 4.61; HarmonyOS(Vapor): 5.0 | 图片路径，图片的路径，可以是相对路径、临时文件路径、存储文件路径 |
| quality | number | 否 |  | Web: x; 微信小程序: 4.41; Android(VDOM): 4.18; iOS: 4.25; HarmonyOS(VDOM): 4.61; HarmonyOS(Vapor): 5.0 | 压缩质量，范围0～100，数值越小，质量越低，压缩率越高（仅对jpg有效） |
| rotate | number | 否 |  | Web: x; 微信小程序:  ; Android(VDOM): 4.18; iOS: 4.25; HarmonyOS(VDOM): 4.61; HarmonyOS(Vapor): 5.0 | 旋转度数，范围0～360 |
| compressedHeight | number | 否 |  | Web: x; 微信小程序: 4.41; Android(VDOM): 4.18; iOS: 4.25; HarmonyOS(VDOM): 4.61; HarmonyOS(Vapor): 5.0 | 压缩后图片的高度，单位为px，若不填写则默认以compressedWidth为准等比缩放 |
| compressedWidth | number | 否 |  | Web: x; 微信小程序: 4.41; Android(VDOM): 4.18; iOS: 4.25; HarmonyOS(VDOM): 4.61; HarmonyOS(Vapor): 5.0 | 压缩后图片的宽度，单位为px，若不填写则默认以compressedHeight为准等比缩放。 |
| success | (callback: [CompressImageSuccess](#compressimagesuccess-values)) => void | 否 |  | Web: x; 微信小程序: 4.41; Android(VDOM): 4.18; iOS: 4.25; HarmonyOS(VDOM): 4.61; HarmonyOS(Vapor): 5.0 | 接口调用成功的回调函数 |
| fail | (callback: [CompressImageFail](#compressimagefail-values)) => void | 否 |  | Web: x; 微信小程序: 4.41; Android(VDOM): 4.18; iOS: 4.25; HarmonyOS(VDOM): 4.61; HarmonyOS(Vapor): 5.0 | 接口调用失败的回调函数 |
| complete | (callback: any) => void | 否 |  | Web: x; 微信小程序: 4.41; Android(VDOM): 4.18; iOS: 4.25; HarmonyOS(VDOM): 4.61; HarmonyOS(Vapor): 5.0 | 接口调用结束的回调函数（调用成功、失败都会执行） |
| ~~width~~ | string | 否 |  | Web: x; 微信小程序: 4.41; Android(VDOM):  ; iOS:  ; HarmonyOS(VDOM):   | 缩放图片的宽度  **已废弃** |
| ~~height~~ | string | 否 |  | Web: x; 微信小程序: 4.41; Android(VDOM):  ; iOS:  ; HarmonyOS(VDOM):   | 缩放图片的高度  **已废弃** | 

#### CompressImageSuccess 的属性值 @compressimagesuccess-values 

| 名称 | 类型 | 必备 | 默认值 | 兼容性 | 描述 |
| :- | :- | :- | :- |  :-: | :- |
| tempFilePath | string | 是 |  | Web: x; 微信小程序: 4.41; Android(VDOM):  ; iOS:  ; HarmonyOS(VDOM):   | 压缩后图片的临时文件路径 |
...
| 1101002 | Web: x; 微信小程序:  ; Android(VDOM):  ; iOS:  ; HarmonyOS(VDOM):   | urls至少包含一张图片地址 |
| 1101003 | Web: x; 微信小程序:  ; Android(VDOM):  ; iOS:  ; HarmonyOS(VDOM):   | 文件不存在 |
| 1101004 | Web: x; 微信小程序:  ; Android(VDOM):  ; iOS:  ; HarmonyOS(VDOM):   | 图片加载失败 |
| 1101005 | Web: x; 微信小程序:  ; Android(VDOM):  ; iOS:  ; HarmonyOS(VDOM):   | 未获取权限 |
| 1101006 | Web: x; 微信小程序:  ; Android(VDOM):  ; iOS:  ; HarmonyOS(VDOM):   | 图片或视频保存失败 |
| 1101007 | Web: x; 微信小程序:  ; Android(VDOM):  ; iOS:  ; HarmonyOS(VDOM):   | 图片裁剪失败 |
| 1101008 | Web: x; 微信小程序:  ; Android(VDOM):  ; iOS:  ; HarmonyOS(VDOM):   | 拍照或录像失败 |
| 1101009 | Web: x; 微信小程序:  ; Android(VDOM):  ; iOS:  ; HarmonyOS(VDOM):   | 图片压缩失败 |
...
- Android平台仅支持对JPG格式图片进行压缩，其他格式会被压缩为JPG格式。如果对带有透明度的png压缩成jpg会丢失透明度变成黑色。iOS平台支持对JPG和PNG格式进行压缩。
