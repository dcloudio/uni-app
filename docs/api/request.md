...
| dataType | string | 否 | "json" | Web:  ; 微信小程序: 4.41; Android: x; Android(Vapor): x; iOS: x; iOS(Vapor): x; HarmonyOS: x | 如果设为 json，会对返回的数据进行一次 JSON.parse，非 json 不会进行 JSON.parse |
| responseType | string | 否 |  | Web:  ; 微信小程序: 4.41; Android: x; Android(Vapor): x; iOS: x; iOS(Vapor): x; HarmonyOS: x | 设置响应的数据类型。 |
| sslVerify | boolean | 否 |  | Web:  ; 微信小程序:  ; Android: x; Android(Vapor): x; iOS: x; iOS(Vapor): x; HarmonyOS: x | 验证 ssl 证书 |
...
| cookies | Array&lt;string&gt; | 是 |  | Web: x; 微信小程序: 4.41; Android: 3.91; iOS: 4.11; iOS uni-app x UTS 插件: 4.11; HarmonyOS: 4.61 | 开发者服务器返回的 cookies，格式为字符串数组 |
...
| profile | **RequestSuccessProfile** | 否 |  | Web:  ; 微信小程序: 4.41; Android:  ; iOS:  ; HarmonyOS:   | 需要基础库： `2.10.4`<br/><br/>网络请求过程中一些调试信息，[查看详细说明](https://developers.weixin.qq.com/miniprogram/dev/framework/performance/network.html)<br/> |
