...
| 名称 | 类型 | 必备 | 默认值 | 兼容性 | 描述 |
| :- | :- | :- | :- |  :-: | :- |
| supportMode | Array&lt;string&gt; | 是 |  | Web: x | 接口调用成功的回调函数 |
| errMsg | string | 是 |  | Web: x | 接口调用结果 |
...
| success | (result: [CheckIsSoterEnrolledInDeviceSuccess](#checkissoterenrolledindevicesuccess-values)) => void | 否 |  | Web: x | 接口调用成功的回调函数 |
