...
| offerId | string | 否 | Web: x; 微信小程序: 4.41; Android: x | mp-支付基础配置中的offerid<br/> |
...
| activitySellingPrice | number | 否 | Web: x; 微信小程序: 4.41; Android: x | 道具优惠价格（分），**非必填，该字段需与goodsPrice一起传入**。如用户使用优惠券、积分等，需要以低于道具价格下单时可传入，传入后该价格即为实际下单价格。<br/> |
| env | number | 否 | Web: x; 微信小程序: 4.41; Android: x | 环境配置, 0 正式环境, 1 沙箱环境, 默认为 0<br/> |
...
| 名称 | 类型 | 必备 | 兼容性 | 描述 |
| :- | :- | :- |  :-: | :- |
| productId | string | 是 | Web: x; Android: x; iOS 系统版本: 15.0; iOS: 4.25; HarmonyOS: x | 产品id，和苹果开发者中心配置的一样 |
| appAccountToken | string | 否 | Web: x; Android: x; iOS 系统版本: 15.0; iOS: 4.25; HarmonyOS: x | 透传参数，一般用于标记订单和用户的关系，可以用来验证和关联用户账户和购买记录 |
| quantity | number | 是 | Web: x; Android: x; iOS 系统版本: 15.0; iOS: 4.25; HarmonyOS: x | 购买数量 |
| transactionDate | Date | 是 | Web: x; Android: x; iOS 系统版本: 15.0; iOS: 4.25; HarmonyOS: x | 交易日期，示例 2022-01-01 08:00:00 |
| transactionIdentifier | string | 是 | Web: x; Android: x; iOS 系统版本: 15.0; iOS: 4.25; HarmonyOS: x | 交易唯一标识 |
| originalTransactionDate | Date | 是 | Web: x; Android: x; iOS 系统版本: 15.0; iOS: 4.25; HarmonyOS: x | 原始交易日期，示例 2022-01-01 08:00:00 |
| originalTransactionIdentifier | string | 是 | Web: x; Android: x; iOS 系统版本: 15.0; iOS: 4.25; HarmonyOS: x | 原始交易唯一标识 |
| jsonRepresentation | string | 是 | Web: x; Android: x; iOS 系统版本: 15.0; iOS: 4.25; HarmonyOS: x | 支付票据 |
...
| data | any | 否 | Web: x; Android: x; HarmonyOS: x | 错误信息中包含的数据 |
| cause | [Error](/err-spec.md#unierror) | 否 |   | 源错误信息，可以包含多个错误，详见SourceError |
| errMsg | string | 是 | Web: x; Android: x; HarmonyOS: x |  |
...
| 700602 | Web: x; Android: x; iOS 系统版本: 15.0; iOS: 4.25; HarmonyOS: x | 网络连接出错。 |
...
| fail | (result: [AppleIAPFinishTransactionFail](#appleiapfinishtransactionfail-values)) => void | 否 | Web: x; Android: x; iOS 系统版本: 15.0; iOS: 4.25; HarmonyOS: x | 接口调用失败的回调函数 |
| complete | (result: any) => void | 否 | Web: x; Android: x; iOS 系统版本: 15.0; iOS: 4.25; HarmonyOS: x | 接口调用结束的回调函数（调用成功、失败都会执行） | 
