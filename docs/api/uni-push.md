...
| 名称 | 类型 | 必备 | 默认值 | 兼容性 | 描述 |
| :- | :- | :- | :- |  :-: | :- |
| cid | string | 是 |  | Web: 4.27; 微信小程序:  ; Android(VDOM): 3.98; iOS: 4.18; HarmonyOS(VDOM):   | 个推客户端推送id，对应uni-id-device表的push_clientid<br/> |
| errMsg | string | 是 |  | Web: 4.27; 微信小程序:  ; Android(VDOM): 3.98; iOS: 4.18; HarmonyOS(VDOM):   | 错误描述<br/> |
...
| 名称 | 类型 | 必备 | 默认值 | 兼容性 | 描述 |
| :- | :- | :- | :- |  :-: | :- |
| type | string | 是 |  | Web: 4.27; 微信小程序:  ; Android(VDOM): 3.98; iOS: 4.18; HarmonyOS(VDOM):   | 事件类型<br/>- click 从系统推送服务点击消息启动应用事件<br/>- receive 应用从推送服务器接收到推送消息事件 |
| data | [UTSJSONObject](/uts/buildin-object-api/utsjsonobject.md) | 是 |  | Web: 4.27; 微信小程序:  ; Android(VDOM): 3.98; iOS: 4.18; HarmonyOS(VDOM):   | 消息内容<br/> |
...
| 名称 | 类型 | 必填 | 默认值 | 兼容性 | 描述 |
| :- | :- | :- | :- |  :-: | :- |
| options | **CreatePushMessageOptions** | 是 |  | Web: x; 微信小程序:  ; Android(VDOM):  ; iOS:  ; HarmonyOS(VDOM):   |  |

#### options 的属性描述

| 名称 | 类型 | 必备 | 默认值 | 兼容性 | 描述 |
| :- | :- | :- | :- |  :-: | :- |
| cover | boolean | 否 | false | Web: x; 微信小程序:  ; Android(VDOM): 3.98; iOS: 4.18; HarmonyOS(VDOM):   | 是否覆盖上一次提示的消息 |
| delay | number | 否 | 0 | Web: x; 微信小程序:  ; Android(VDOM): 3.98; iOS: 4.18; HarmonyOS(VDOM):   | 提示消息延迟显示的时间,单位为s |
| icon | string | 否 | null | Web: x; 微信小程序:  ; Android(VDOM): 3.98; iOS: x; HarmonyOS(VDOM):   | 推送消息的图标 |
| sound | string | 否 | "system" | Web: x; 微信小程序:  ; Android(VDOM): 3.98; iOS: 4.18; HarmonyOS(VDOM):   | 推送消息的提示音<br/>- system: 使用系统通知提示音（默认值）<br/>- none: 不使用提示音 |
| title | string | 否 | App的名称 | Web: x; 微信小程序:  ; Android(VDOM): 3.98; iOS: 4.18; HarmonyOS(VDOM):   | 推送消息的标题 |
| content | string | 是 |  | Web: x; 微信小程序:  ; Android(VDOM): 3.98; iOS: 4.18; HarmonyOS(VDOM):   | 消息显示的内容，在系统通知中心中显示的文本内容。鸿蒙系统中，此字段为必填字段。<br/> |
| payload | any | 否 | null | Web: x; 微信小程序:  ; Android(VDOM): 3.98; iOS: 4.18; HarmonyOS(VDOM):   | 消息承载的数据，可根据业务逻辑自定义数据格式，在点击通知消息时`onPushMessage`回调中会返回此字段的数据。 |
| when | number | 否 | 当前时间 | Web: x; 微信小程序:  ; Android(VDOM): 3.98; iOS: x; HarmonyOS(VDOM):   | 消息上显示的提示时间，需要传入时间戳。 |
| channelId | string | 否 | "DcloudChannelID" | Web: x; 微信小程序:  ; Android(VDOM): 3.98; iOS: x; HarmonyOS(VDOM):   | 渠道id，Android特有字段，[通知渠道介绍](https://developer.android.com/develop/ui/views/notifications/channels?hl=zh-cn)，<br/>创建通知渠道请使用`getPushChannelManager`获取PushChannelManager对象，调用`setPushChannel`方法配置渠道。 |
| category | string | 否 | null | Web: x; 微信小程序:  ; Android(VDOM): 3.98; iOS: x; HarmonyOS(VDOM):   | 通知类别，Android特有字段，[通知渠道介绍](https://developer.android.com/develop/ui/views/notifications/channels?hl=zh-cn)，<br/>标识通知的类别，应用场景为对于离线推送厂商配置的支持，比如[华为消息分类](https://developer.huawei.com/consumer/cn/doc/HMSCore-Guides/message-classification-0000001149358835#section5101818813) |
| success | (result: CreatePushMessageSuccess) => void | 否 | null | Web: x; 微信小程序:  ; Android(VDOM):  ; iOS:  ; HarmonyOS(VDOM):   | 接口调用成功的回调函数 |
| fail | (result: [UniError](https://uniapp.dcloud.net.cn/tutorial/err-spec.html#unierror)) => void | 否 | null | Web: x; 微信小程序:  ; Android(VDOM):  ; iOS:  ; HarmonyOS(VDOM):   | 接口调用失败的回调函数 |
| complete | (result: any) => void | 否 | null | Web: x; 微信小程序:  ; Android(VDOM):  ; iOS:  ; HarmonyOS(VDOM):   | 接口调用结束的回调函数（调用成功、失败都会执行） | 
...
| 名称 | 类型 | 必填 | 默认值 | 兼容性 | 描述 |
| :- | :- | :- | :- |  :-: | :- |
| options | **SetPushChannelOptions** | 是 |  | Web: x; 微信小程序:  ; Android(VDOM):  ; iOS: x; HarmonyOS(VDOM):   |  |

#### options 的属性描述

| 名称 | 类型 | 必备 | 默认值 | 兼容性 | 描述 |
| :- | :- | :- | :- |  :-: | :- |
| soundName | string | 否 | null | Web: x; 微信小程序:  ; Android(VDOM):  ; iOS: x; HarmonyOS(VDOM):   | 添加的声音文件，注意raw目录下必须要有 ，不传此字段将使用默认铃音。 |
| channelId | string | 是 |  | Web: x; 微信小程序:  ; Android(VDOM):  ; iOS: x; HarmonyOS(VDOM):   | 通知渠道id |
| channelDesc | string | 是 |  | Web: x; 微信小程序:  ; Android(VDOM):  ; iOS: x; HarmonyOS(VDOM):   | 通知渠道描述 |
| enableLights | boolean | 否 | false | Web: x; 微信小程序:  ; Android(VDOM):  ; iOS: x; HarmonyOS(VDOM):   | 呼吸灯闪烁 |
| enableVibration | boolean | 否 | false | Web: x; 微信小程序:  ; Android(VDOM):  ; iOS: x; HarmonyOS(VDOM):   | 震动 |
| importance | number | 否 | 3 | Web: x; 微信小程序:  ; Android(VDOM):  ; iOS: x; HarmonyOS(VDOM):   | 通知的重要性级别，可选范围IMPORTANCE_LOW：2、IMPORTANCE_DEFAULT：3、IMPORTANCE_HIGH：4 。 |
| lockscreenVisibility | number | 否 | -1000 | Web: x; 微信小程序:  ; Android(VDOM):  ; iOS: x; HarmonyOS(VDOM):   | 锁屏可见性，可选范围VISIBILITY_PRIVATE：0、VISIBILITY_PUBLIC：1、VISIBILITY_SECRET：-1、VISIBILITY_NO_OVERRIDE：-1000。 | 
...
| 名称 | 类型 | 必填 | 默认值 | 兼容性 | 描述 |
| :- | :- | :- | :- |  :-: | :- |
| num | number | 是 |  | Web: x; 微信小程序:  ; Android(VDOM):  ; iOS:  ; HarmonyOS(VDOM):   | 要显示的角标数字值，参数为0则表示清除角标数字。 |
| options | **BadgeOptions** | 否 |  | Web: x; 微信小程序:  ; Android(VDOM):  ; iOS:  ; HarmonyOS(VDOM):   | 小米手机显示角标需要在系统消息中心显示一条通知，此参数用于设置通知的标题（title）和内容（content）。 |

#### options 的属性描述

| 名称 | 类型 | 必备 | 默认值 | 兼容性 | 描述 |
| :- | :- | :- | :- |  :-: | :- |
| title | string | 否 | 应用的名称 | Web: x; 微信小程序:  ; Android(VDOM): 4.25; iOS: 4.25; HarmonyOS(VDOM):   | 消息的标题 |
| content | string | 否 | '您有x条未读消息'，其中x为设置的角标数字值。 | Web: x; 微信小程序:  ; Android(VDOM): 4.25; iOS: 4.25; HarmonyOS(VDOM):   | 消息的内容 | 
