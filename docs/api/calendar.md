...
| success | (res: [AddPhoneRepeatCalendarSuccess](#addphonerepeatcalendarsuccess-values)) => void | 否 |  | Web: x; 支付宝小程序: x | 接口调用成功的回调函数 |
| fail | (res: [AddPhoneRepeatCalendarFail](#addphonerepeatcalendarfail-values)) => void | 否 |  | Web: x; 支付宝小程序: x | 接口调用失败的回调函数 |
| complete | (res: [AddPhoneRepeatCalendarSuccess](#addphonerepeatcalendarsuccess-values) \| [AddPhoneRepeatCalendarFail](#addphonerepeatcalendarfail-values)) => void | 否 |  | Web: x; 支付宝小程序: x | 接口调用结束的回调函数（调用成功、失败都会执行） | 
...
| path | string | 否 |  | Web: x; 微信小程序: √; 支付宝小程序: x; Android: 5.09; iOS: 5.09; HarmonyOS: 5.09 | 跳转小程序路径，必须要和 signature 一起使用，填入后会自动生成跳转链接拼接在事件说明中 |
| signature | string | 否 |  | Web: x; 微信小程序: √; 支付宝小程序: x; Android: 5.09; iOS: 5.09; HarmonyOS: 5.09 | 仅微信小程序支持，App 平台保留该字段但不会使用，跳转小程序路径签名，必须要和 path 一起使用，用 session_key 对 path 签名得到的结果，即 hmac_sha256(session_key, path)。 |
| success | (res: [AddPhoneCalendarSuccess](#addphonecalendarsuccess-values)) => void | 否 |  | Web: x; 支付宝小程序: x | 接口调用成功的回调函数 |
| fail | (res: [AddPhoneCalendarFail](#addphonecalendarfail-values)) => void | 否 |  | Web: x; 支付宝小程序: x | 接口调用失败的回调函数 |
| complete | (res: [AddPhoneCalendarSuccess](#addphonecalendarsuccess-values) \| [AddPhoneCalendarFail](#addphonecalendarfail-values)) => void | 否 |  | Web: x; 支付宝小程序: x | 接口调用结束的回调函数（调用成功、失败都会执行） | 
