...
| 名称 | 类型 | 必填 | 默认值 | 兼容性 | 描述 |
| :- | :- | :- | :- |  :-: | :- |
| options | **InstallApkOptions** | 是 |  | Web: x; 微信小程序: x; Android(VDOM):  ; iOS: x; HarmonyOS(VDOM): x |  |

#### options 的属性描述

| 名称 | 类型 | 必备 | 默认值 | 兼容性 | 描述 |
| :- | :- | :- | :- |  :-: | :- |
| filePath | string | 是 |  | Web: x; 微信小程序: x; Android(VDOM):  ; iOS: x; HarmonyOS(VDOM): x | apk文件地址（仅支持本地文件路径） |
| success | (res: [InstallApkSuccess](#installapksuccess-values)) => void | 否 | null | Web: x; 微信小程序: x; Android(VDOM):  ; iOS: x; HarmonyOS(VDOM): x | 接口调用成功的回调函数 |
| fail | (err: [InstallApkFail](#installapkfail-values)) => void | 否 | null | Web: x; 微信小程序: x; Android(VDOM):  ; iOS: x; HarmonyOS(VDOM): x | 接口调用失败的回调函数 |
| complete | (res: any) => void | 否 | null | Web: x; 微信小程序: x; Android(VDOM):  ; iOS: x; HarmonyOS(VDOM): x | 接口调用结束的回调函数（调用成功、失败都会执行） | 

#### InstallApkSuccess 的属性值 @installapksuccess-values 

| 名称 | 类型 | 必备 | 默认值 | 兼容性 | 描述 |
| :- | :- | :- | :- |  :-: | :- |
| errMsg | string | 是 |  | Web: x; 微信小程序: x; Android(VDOM):  ; iOS: x; HarmonyOS(VDOM): x | 安装成功消息 |

#### InstallApkFail 的属性值 @installapkfail-values 

| 名称 | 类型 | 必备 | 默认值 | 兼容性 | 描述 |
| :- | :- | :- | :- |  :-: | :- |
| errCode | number | 是 |  | Web: x; 微信小程序: x; Android(VDOM):  ; iOS: x; HarmonyOS(VDOM): x | 错误码<br/>- 1300002 找不到文件 |
| errSubject | string | 是 |  | Web: x; 微信小程序: x; Android(VDOM):  ; iOS: x; HarmonyOS(VDOM): x | 统一错误主题（模块）名称 |
| data | any | 否 |  | Web: x; 微信小程序: x; Android(VDOM):  ; iOS: x; HarmonyOS(VDOM): x | 错误信息中包含的数据 |
| cause | [Error](https://uniapp.dcloud.net.cn/tutorial/err-spec.html#unierror) | 否 |  |   | 源错误信息，可以包含多个错误，详见SourceError |
| errMsg | string | 是 |  | Web: x; 微信小程序: x; Android(VDOM):  ; iOS: x; HarmonyOS(VDOM): x |  |
