...
| 名称 | 类型 | 必备 | 默认值 | 兼容性 | 描述 |
| :- | :- | :- | :- |  :-: | :- |
| filePath | [string.VideoURIString](/uts/data-type.md#ide-string) | 是 |  | Web: x; Android: 4.18; iOS: 4.18 | 视频文件路径，可以是临时文件路径也可以是永久文件路径 |
| success | (callback: SaveVideoToPhotosAlbumSuccess) => void | 否 |  | Web: x; Android: 4.18; iOS: 4.18 | 接口调用成功的回调函数 |
| fail | (callback: [SaveVideoToPhotosAlbumFail](#savevideotophotosalbumfail-values)) => void | 否 |  | Web: x; Android: 4.18; iOS: 4.18 | 接口调用失败的回调函数 |
| complete | (callback: any) => void | 否 |  | Web: x; Android: 4.18; iOS: 4.18 | 接口调用结束的回调函数（调用成功、失败都会执行） | 
