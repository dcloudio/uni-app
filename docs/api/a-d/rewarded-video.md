...
激励视频广告是一种常见于游戏内的广告样式，多出现在游戏的复活、登录等位置，或者网服类app的一些增值服务场景。
...
|adUnitId	|string	|	是|广告位 id |微信小程序2.6.0+, QQ0.1.26+， 头条1.57.0+|
...
|属性名|类型|描述|最低支持版本|
|:-:|:-:|:-:|:-:|
|show|Function|广告创建后默认是隐藏的，可以通过该方法显示广告|App 2.5.11+, 微信小程序2.6.0+, QQ0.1.26+，头条1.57.0+|
|onLoad|Function|绑定广告 load 事件的监听器|App 2.5.11+, 微信小程序2.6.0+, QQ0.1.26+，头条1.57.0+|
|offLoad|Function|解除绑定 load 事件的监听器|QQ0.1.26+，头条1.57.0+|
|load|Function|当广告素材加载出现错误时，可以通过 load 方法手动加载|App 2.5.11+, 微信小程序2.6.0+, QQ0.1.26+，头条1.57.0+|
|onError|Function|绑定 error 事件的监听器	|App 2.5.11+, 微信小程序2.6.0+, QQ0.1.26+，头条1.57.0+|
|offError|Function|解除绑定 error 事件的监听器|QQ0.1.26+，头条1.57.0+|
|onClose|Function|绑定 close 事件的监听器|App 2.5.11+, 微信小程序2.6.0+, QQ0.1.26+，头条1.57.0+|
|offClose|Function|解除绑定 close 事件的监听器|QQ0.1.26+，头条1.57.0+|
...
2. apk加固。即便前端代码加密，原生层引擎的java代码仍然可能被反编译，需要对apk加固。市面上很多加固服务，比如360加固、爱加密加固均可以选择。
