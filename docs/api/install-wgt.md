...
| 名称 | 类型 | 必填 | 描述 |
| :- | :- | :- | :- |
| options | **InstallWgtOptions** | 是 | 安装应用参数 |

#### options 的属性描述

| 名称 | 类型 | 必备 | 描述 |
| :- | :- | :- | :- |
| filePath | string | 是 | 应用资源文件（wgt）路径 |
| ignoreVersion | boolean | 否 | 忽略版本号校验，设置为true则不校验wgt版本号，默认值为false |
| success | (result: InstallWgtSuccess) => void | 否 | 应用资源文件安装成功回调 |
| fail | (result: [InstallWgtFail](#installwgtfail-values)) => void | 否 | 应用资源文件安装失败回调 |
| complete | (result: any) => void | 否 | 应用资源文件安装完成回调 | 

#### InstallWgtFail 的属性值 @installwgtfail-values 

| 名称 | 类型 | 必备 | 描述 |
| :- | :- | :- | :- |
| errCode | InstallWgtErrorCode | 是 | 错误码 |
