...
| 名称 | 类型 | 必备 | 默认值 | 兼容性 | 描述 |
| :- | :- | :- | :- |  :-: | :- |
| pageOrientation | string | 否 |  | Web: x; 微信小程序: x; Android(VDOM): 4.33; iOS: 4.33; HarmonyOS(VDOM): x | 屏幕方向。默认为page.json中的pageOrientation。 |
| albumMode | string | 否 | "custom" | Web: x; 微信小程序: x; Android(VDOM): 4.33; iOS: x; HarmonyOS(VDOM): x | 图片选择模式 |
| count | number | 否 | 9 | Web:  ; 微信小程序: 4.41; Android(VDOM): 3.9; iOS: 4.11; HarmonyOS(VDOM):   | 最多可以选择的图片张数，app端不限制，微信小程序最多可支持20个。 |
| sizeType | Array&lt;string&gt; | 否 | ['original','compressed'\] | Web: x; 微信小程序: 4.41; Android(VDOM): 3.9; iOS: 4.11; HarmonyOS(VDOM): 4.61; HarmonyOS(Vapor): 5.0 | original 原图，compressed 压缩图，默认二者都有 |
| sourceType | Array&lt;string&gt; | 否 | ['album','camera'\] | Web:  ; 微信小程序: 4.41; Android(VDOM): 3.9; iOS: 4.11; HarmonyOS(VDOM):   | album 从相册选图，camera 使用相机，默认二者都有 |
| extension | Array&lt;string&gt; | 否 |  | Web: 4.0; 微信小程序: x; Android(VDOM): x; iOS: x; HarmonyOS(VDOM): x | 根据文件拓展名过滤，每一项都不能是空字符串。默认不过滤。仅H5支持 |
| crop | **ChooseImageCropOptions** | 否 |  | Web: x; 微信小程序:  ; Android(VDOM): 3.9; iOS: 4.11; HarmonyOS(VDOM): x | 图像裁剪参数，设置后 sizeType 失效。 |
| success | (callback: [ChooseImageSuccess](#chooseimagesuccess-values)) => void | 否 |  | Web:  ; 微信小程序: 4.41; Android(VDOM): 3.9; iOS: 4.11; HarmonyOS(VDOM):   | 成功则返回图片的本地文件路径列表 tempFilePaths |
| fail | (callback: [ChooseImageFail](#chooseimagefail-values)) => void | 否 |  | Web:  ; 微信小程序: 4.41; Android(VDOM): 3.9; iOS: 4.11; HarmonyOS(VDOM):   | 接口调用失败的回调函数 |
| complete | (callback: any) => void | 否 |  | Web:  ; 微信小程序: 4.41; Android(VDOM): 3.9; iOS: 4.11; HarmonyOS(VDOM):   | 接口调用结束的回调函数（调用成功、失败都会执行） | 
...
| 名称 | 类型 | 必备 | 默认值 | 兼容性 | 描述 |
| :- | :- | :- | :- |  :-: | :- |
| width | number | 是 |  | Web:  ; 微信小程序:  ; Android(VDOM): 3.9; iOS: 4.11; HarmonyOS(VDOM):   | 裁剪的宽度，单位为px，用于计算裁剪宽高比。 |
| height | number | 是 |  | Web:  ; 微信小程序:  ; Android(VDOM): 3.9; iOS: 4.11; HarmonyOS(VDOM):   | 裁剪的高度，单位为px，用于计算裁剪宽高比。 |
| quality | number | 否 | 80 | Web:  ; 微信小程序:  ; Android(VDOM): 3.9; iOS: 4.11; HarmonyOS(VDOM):   | 取值范围为1-100，数值越小，质量越低（仅对jpg格式有效）。默认值为80。 |
| resize | boolean | 否 |  | Web:  ; 微信小程序:  ; Android(VDOM): 3.9; iOS: 4.11; HarmonyOS(VDOM):   | 是否将width和height作为裁剪保存图片真实的像素值。默认值为true。注：设置为false时在裁剪编辑界面显示图片的像素值，设置为true时不显示。 |

#### ChooseImageSuccess 的属性值 @chooseimagesuccess-values 

| 名称 | 类型 | 必备 | 默认值 | 兼容性 | 描述 |
| :- | :- | :- | :- |  :-: | :- |
| errSubject | string | 是 |  | Web:  ; 微信小程序:  ; Android(VDOM): 3.9; iOS: 4.11; HarmonyOS(VDOM):   | 调用API的名称 |
| errMsg | string | 是 |  | Web:  ; 微信小程序:  ; Android(VDOM): 3.9; iOS: 4.11; HarmonyOS(VDOM):   | 描述信息 |
| tempFilePaths | Array&lt;string&gt; | 是 |  | Web:  ; 微信小程序: 4.41; Android(VDOM): 3.9; iOS: 4.11; HarmonyOS(VDOM):   | 图片的本地文件路径列表 |
| tempFiles | Array&lt;**ChooseImageTempFile**&gt; | 是 |  | Web:  ; 微信小程序: 4.41; Android(VDOM): 3.9; iOS: 4.11; HarmonyOS(VDOM):   | 图片的本地文件列表 |

#### tempFiles 的属性描述

| 名称 | 类型 | 必备 | 默认值 | 兼容性 | 描述 |
| :- | :- | :- | :- |  :-: | :- |
| path | string | 是 |  | Web:  ; 微信小程序:  ; Android(VDOM): 3.9; iOS: 4.11; HarmonyOS(VDOM):   | 本地文件路径 |
| size | number | 是 |  | Web:  ; 微信小程序:  ; Android(VDOM): 3.9; iOS: 4.11; HarmonyOS(VDOM):   | 本地文件大小，单位：B |
| name | string | 否 |  | Web:  ; 微信小程序:  ; Android(VDOM): x; iOS: x; HarmonyOS(VDOM):   | 包含扩展名的文件名称，仅H5支持 |
| type | string | 否 |  | Web:  ; 微信小程序:  ; Android(VDOM): x; iOS: x; HarmonyOS(VDOM):   | 文件类型，仅H5支持 |
