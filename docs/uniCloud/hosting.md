...
开发者不用再安装服务器操作系统、配置Nginx等web服务器、配置https证书、处理负载均衡、处理大并发、处理DDoS攻击......您什么都不用管，直接上传前端页面，即可拥有无限性能、无安全隐患的网站系统。

`前端网站部署`不但打不垮、黑不掉，同时自带cdn功能，这使得网站在访问时更加的快速。网站的浏览者可以在距离其最近的cdn边缘节点快速获取到网页内容。
...
腾讯云前端网站部署需付费开通，提供以下三档预付费套餐（请以购买时展示的套餐信息为准），按量计费套餐还在开发中。
...
阿里云前端网站部署目前可以免费使用，具体限制后续会在此处展示。
...
- 安全起见阿里云、腾讯云提供的默认域名有以下限制，绑定域名后不受下面的限制影响
