...
然后选择或创建一个腾讯云的服务空间。目前本功能仅腾讯云支持，阿里云暂不支持。

最后在上述网页左侧导航点击`前端网站部署`，即可开始使用。

前端网站部署需付费开通，提供以下三档套餐（请以购买时展示的套餐信息为准）。
...
`前端网站部署`，腾讯云自带一个测试域名，该域名限速。如商用，请配置自己的域名。
...
3. 添加后，系统会自动分配一个以 .cdn.dnsv1.com 为后缀的 CNAME 域名，CNAME 域名不能直接访问，您需要在域名服务提供商处完成 CNAME 配置（将添加的域名CNAME到此域名），配置生效后，新域名即可使用。
