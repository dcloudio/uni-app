...
`前端网站部署`基于uniCloud的能力，为开发者的html网页提供**更快速、更安全、更省心**的网站发布。

开发者无需再购买虚拟机、安装操作系统、配置web服务器、处理负载均衡、处理大并发、处理DDoS攻击......您什么都不用管，直接上传前端页面，即可拥有无限性能、无安全隐患的网站系统。

`前端网站部署`不但打不垮、黑不掉，同时自带cdn功能，这使得网站在访问时更快。网站的浏览者可以在距离其最近的cdn边缘节点快速获取到网页内容。
...
最后在上述网页左侧导航点击`前端网站部署`，即可开始使用。

`前端网站部署`和云函数没有绑定关系，可以和云函数部署在一个服务空间，也可以是不同的空间，甚至是不同云服务商的空间。

- 阿里云`前端网站部署`免费。
- 腾讯云`前端网站部署`需付费开通，定价由腾讯云提供。腾讯云的不同档套餐有不同规格，见下表：
...
`前端网站部署`，自带一个测试域名，该域名限速。如商用，请配置自己的域名。

1. 登录[uniCloud控制台](https://unicloud.dcloud.net.cn/)。
2. 进入前端网站部署页面，选择【基础设置】，单击【添加域名】，进行域名添加，如下图所示：
...
注意：uni-app项目发布H5，使用history模式时，做如下配置：

- 腾讯云：配置重定向规则将404错误码重定向至index.html
- 阿里云：配置错误文档为index.html
