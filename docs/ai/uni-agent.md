...
hash: ff91444969b01a8fba2f0382af80d86cf97443aa65e3af5fe11abbb8ea83285f
-->
...
3. **在设置中选择模型来源与档位**
   ...
   - 如果当前来源下提供了多个模型档位，可继续在 **模型档位** 中选择具体模型，例如 `1x`、`2x`、`4x`、`Flash`、`Pro`。
...
- **模型来源与档位**：在同一个下拉菜单的二级列表中，可以切换 **默认模型**、**聚合中转**、**DeepSeek** 及其对应档位，例如 `1x`、`2x`、`4x`、`Flash`、`Pro`。
...
点击聊天视图右上角【设置】进入配置面板后，先在 **模型选择** 中选择当前要使用的模型来源：**默认模型**、**聚合中转**、**DeepSeek** 或 **自定义Key**。如果当前订阅来源下提供了多个模型档位，可继续在 **模型档位** 中选择具体项；如果选择 **自定义Key**，再按 `KEY=VALUE` 的格式逐行填写环境变量（以 `#` 开头的行会被忽略）。设置页下方还可以开启 **权限自动允许**。点击【保存】后，所选模型来源、模型档位、权限设置和自定义环境变量会写入插件的全局存储，并在后续会话中生效。
...
当选择 **默认模型**、**聚合中转** 或 **DeepSeek** 时，如果当前模型目录下提供了多个可选模型，设置窗口中会出现 **模型档位** 选项。

- **默认模型**：常见档位如 `1x`、`2x`
- **聚合中转**：常见档位如 `1x`、`2x`、`4x`
- **DeepSeek**：常见档位如 `Flash`、`Pro`

这些档位不是写死在客户端里的固定常量，后续可能随插件内置模型目录和服务端目录动态调整，因此请以当前界面显示为准。
