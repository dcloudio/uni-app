...
hash: 11ac1c94018a2504f1c67213d337f52dae15e8ed3cf2e21c109116f82eaf73db
-->
...
| --- | --- | --- | --- | --- |
| **默认模型**<br><br>*(推荐优先使用)* | **0.8x-260923** | **0.8x** | 直连最新一代国际顶尖模型，使用更高能力档位，适合复杂需求拆解、跨文件重构、长上下文分析、疑难错误排查和高风险代码修改。 | DCloud 订阅原生模型。 |
...
|  | **2.1x** | **2.1x** | 上一代模型，适合复杂需求拆解、跨文件重构、长上下文分析、疑难错误排查和高风险代码修改。 | DCloud 订阅原生模型。 |
...
|  | **0.1x** | **0.1x** | 上一代模型，极低消耗，可处理一些日常问题、轻量代码生成。 | DCloud 订阅原生模型。 |
| **DeepSeek**<br><br>*(国产化/敏感出境)* | **V4.1 Flash** | 约 **闲时0.35x，高峰0.7x** | 使用 `deepseek-flash`。响应速度快，消耗低，能力全面超载 `V4 pro`，适合复杂需求拆解、长上下文分析、跨文件修改和高难度排障 | ✅ 支持图片识别。 |
