...
hash: 7450c2099c534d16fe7114b7f758f907f2e05e1aa472e23385092cfb65244526
-->
...
3. **在设置中选择模型来源与档位**
   ...
   - 如果当前来源下提供了多个模型档位，可继续在 **模型档位** 中选择具体模型，例如 `1x`、`2x`、`Flash`、`Pro`、`GLM-5.2`。
...
- `GLM 5.2`：国产智谱顶尖档位，提供较强的逻辑推理能力，适合复杂问题分析和高难度排障，消耗较高，建议按需调用。
...
| **智谱**<br><br>*(国产 GLM 线路)* | **GLM 5.2** | 约 **7x** | 国产智谱顶尖档位。提供较强的逻辑推理能力，适合解决复杂问题或高难度排障。 | ❌ 不支持图片识别。消耗较高，建议按需调用。 |
|  | **GLM 5.3** | 约 **7x** | 国产智谱顶尖档位。提供较强的逻辑推理能力，适合解决复杂问题或高难度排障。 | ❌ 不支持图片识别。消耗较高，建议按需调用。 |

> **注意**: 
> 1. 默认模型上下文超过272KB后，消耗会翻倍。
> 2. Deepseek自2026年8月17日0时起采用峰谷定价，空闲时段价格为高峰时段价格的一半。高峰时段为北京时间 9:00 - 12:00、14:00 - 18:00（其余为空闲时段）。
...
- **模型来源与档位**：在同一个下拉菜单的二级列表中，可以切换 **默认模型**、**聚合中转**、**DeepSeek**、**智谱** 及其对应档位，例如 `1x`、`2x`、`Flash`、`Pro`、`GLM-5.2`。
