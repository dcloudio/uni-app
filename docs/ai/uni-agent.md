...
hash: ce8e39c022435530f5fb6380b7c8843e34947631a565c100d39e592c824f1f5a
-->
...
3. **在设置中选择模型来源与档位**
   ...
   - 如果当前来源下提供了多个模型档位，可继续在 **模型档位** 中选择具体模型，例如 `1x`、`2x`、`4x`、`Flash`、`Pro`、`GLM-5.2`。
...
- **模型来源与档位**：在同一个下拉菜单的二级列表中，可以切换 **默认模型**、**聚合中转**、**DeepSeek**、**智谱** 及其对应档位，例如 `1x`、`2x`、`4x`、`Flash`、`Pro`、`GLM-5.2`。
...
- **智谱**：使用 DCloud订阅的智谱 GLM 模型，当前支持 `GLM-5.2`。
...
- 如果希望使用智谱 GLM 模型，可以切换到 **智谱**，并以当前界面显示的模型档位为准。
...
当选择 **默认模型**、**聚合中转**、**DeepSeek** 或 **智谱** 时，如果当前模型目录下提供了多个可选模型，设置窗口中会出现 **模型档位** 选项。
...
### 升级版本后，点击右上角AI图标无法打开 uni-agent 视图
