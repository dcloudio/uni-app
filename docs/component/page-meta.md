...
| scroll-top | string | Web: x; 微信小程序: 4.41; Android: x; iOS: x | 滚动位置，可以使用 px 或者 rpx 为单位，在被设置时，页面会滚动到对应位置（WebView 特有属性） |
| scroll-duration | number | Web: x; 微信小程序: 4.41; Android: x; iOS: x | 滚动动画时长（WebView 特有属性） |
...
| @scroll | (event: [UniEvent](/component/common.md#unievent)) => void | Web: x; 微信小程序: 4.41; Android: x; iOS: x | 页面滚动时会触发 scroll 事件， event.detail = { scrollTop }（WebView 特有属性） |
| @scrolldone | (event: [UniEvent](/component/common.md#unievent)) => void | Web: x; 微信小程序: 4.41; Android: x; iOS: x | 如果通过改变 scroll-top 属性来使页面滚动，页面滚动结束后会触发 scrolldone 事件（WebView 特有属性） |
