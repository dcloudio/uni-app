...
| 名称 | 类型 | 默认值 | 兼容性 | 描述 |
| :- | :- | :- |  :-: | :- |
| type | string |   | Web: 4.0; 微信小程序: 4.41; Android(VDOM): x; iOS(VDOM): x; HarmonyOS(VDOM): x | icon的类型，有效值：success, success_no_circle, info, warn, waiting, cancel, download, search, clear |
| size | number |   | Web: 4.0; 微信小程序: 4.41; Android(VDOM): x; iOS(VDOM): x; HarmonyOS(VDOM): x | icon的大小，单位px |
| color | string([string.ColorString](/uts/data-type.md#ide-string)) |   | Web: 4.0; 微信小程序: 4.41; Android(VDOM): x; iOS(VDOM): x; HarmonyOS(VDOM): x | icon的颜色，同css的color |
