...
- 支持 Base64 格式图片。
- 支持网络路径图片。
- 小程序不支持在 CSS 中使用本地文件，包括本地的背景图和字体文件。需以Base64方式方可使用。App端在 v3 模式以前，也有相同限制。v3 编译模式起支持直接使用本地背景图和字体。
- 使用本地路径背景图片需注意：
...
- 网络路径必须加协议头 ``https``。
- 从 [https://www.iconfont.cn](https://www.iconfont.cn) 上拷贝的代码，默认是没加协议头的。 
- 从 [https://www.iconfont.cn](https://www.iconfont.cn) 上下载的字体名称（font-family）默认是 iconfont，在 nvue 内使用时需要注意，此字体名重复可能会显示不正常，可以在 iconfont 项目的「更多操作」→「编辑项目」→「Font family」中设置。
- 使用本地路径图标字体需注意：
...
`nvue` 中不可直接使用 CSSS 的方式引入字体文件，需要使用以下方式在 JS 内引入。nvue 内不支持本地路径引入字体，请使用网络链接或者 `Base64` 形式。**`src` 属性的 `url()` 函数括号内一定要使用单引号。**
