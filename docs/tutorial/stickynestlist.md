...
uni-app x的滚动视图组件有scroll-view和list-view。scroll-view比较灵活，但没有自带recycle复用机制。
...
- [tab为下划线方式的swiper](https://gitcode.com/dcloud/hello-uni-app-x/blob/master/pages/template/swipe-tabs-underline/swipe-tabs-underline.uvue)
- [tab为字体放大方式的swiper](https://gitcode.com/dcloud/hello-uni-app-x/blob/master/pages/template/swipe-tabs-scale-highlight/swipe-tabs-scale-highlight.uvue)
上面2个是比较简单的样例，没有实际放入数据的列表。

下面的例子是实际放入数据的swiper-list，但这2个列表除了所有滑动，还加入了吸顶和nested嵌套。这些后面章节会详细讲述。
- [表头吸顶的可左右滑动长列表](https://gitcode.com/dcloud/hello-uni-app-x/blob/master/pages/template/banner-tabs-long-list-nested-scroll/banner-tabs-long-list-nested-scroll.uvue)
...
得益于uni-app x的没有通信性能问题，上述做法也可以流畅实现。

源码见：[scroll-view吸顶](https://gitcode.com/dcloud/hello-uni-app-x/blob/master/pages/template/scroll-view-sticky-section/scroll-view-sticky-section.uvue)

### 2. 使用sticky-header组件

上面那种始终固定top的做法，在scroll-view里可以实现，但在list-view里有问题。因为list-view底层是有复用，有一些特殊限制。
...
源码见：[列表到详情](https://gitcode.com/dcloud/hello-uni-app-x/blob/master/pages/template/news-feed-list/news-feed-list.uvue)
