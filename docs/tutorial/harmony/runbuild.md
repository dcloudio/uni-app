...
hash: 876707215920962cac7845885526552d34ccea6bc4a10a8c34b8cf9a1c85e45f
-->
...
### 报错 `Error message:Cannot read property appId of null`

新增了崩溃日志的收集操作，有概率性获取不到appId，然后报出这种错误。

升级版本可以解决这个问题，HBuilderX在5.22.2026072503-alpha版本上已经修复，建议升级到最新稳定的版本，如果是ohpm直接依赖，建议升级到5.2.32026080401以上版本。

### 应用安装后运行闪退？可能跟项目根目录的位置有关

[前面的文档](#project-path)提到，在 Windows 系统中，如果项目根目录所在的路径比较长，则 HBuilderX 内部创建的鸿蒙原生工程目录的路径长度就有可能超过系统限制，导致鸿蒙工具链构建失败。

但是，还有另一种隐藏更深的错误，结果是构建过程本身看上去是成功的，但运行到手机上之后就直接闪退了。

原因在于鸿蒙工具链（hvigor）在打包时解析依赖包位置的代码存在缺陷，当依赖包在 `oh_modules` 中的展开路径超过一定长度时静默回退，导致把开发环境的绝对路径写到了安装包内，而在手机里面运行的代码当然无法加载这个依赖包，于是应用就闪退了。
虽然问题的根本原因仍然是项目根目录所在的路径长度过长，但还没有长到触发构建失败，却已经触发了 hvigor 的解析缺陷。

因这种原因导致的应用闪退，在设备侧拿到的崩溃日志里往往会出现类似下面这样的内容：
```
Error message:cannot find record '&@dcloudio/uni-app-harmony/index&1.0.0',please check the request path.'/data/storage/el1/bundle/entry/ets/modules.abc'.
```

更有一种尤其诡异的故障现象，就是在 HBuilderX 里面“运行到鸿蒙”的时候是正常的，而在“发行打包”之后得到的 .app 却因为上述原因出现闪退。
因为在 HBuilderX 内部为调试运行而构建的鸿蒙原生工程目录比发行打包时的原生工程目录的路径长度短了两个字节，如果两者碰巧卡在触发 hvigor 解析缺陷的边界两侧，就会出现调试运行正常而发行打包时闪退的异常现象。

**规避方案：** 开发者可以调整项目根目录的位置，改到一个路径短一点的位置。或者使用 DevEco Studio 26.0.0+，经过验证这个版本已经不存在解析缺陷了（路径过长导致编译失败的限制依旧存在）。
