...
> - 移动账号认证 SDK  
>   + 涉及的个人信息类型：网络类型、网络地址（ip地址）、运营商类型、本机号码信息、SIM卡状态；手机设备类型、手机操作系统、硬件厂商；H5场景下同时收集浏览器指纹，浏览器指纹具体包括：操作系统、用户代理（浏览器类型）、浏览器版本号、浏览器是否启动cookie、cpu等级、cpu虚拟核心数、系统语言、浏览器插件、屏幕可用宽度、屏幕可用高度、颜色质量、时差区  
>   + 使用场景：向最终用户提供一键登录认证服务时使用  
>   + 使用目的：为App提供风控和认证服务  
>   + 收集方式：SDK自行采集  
>   + 第三方主体：中移互联网有限公司  
