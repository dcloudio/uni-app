...
- uni-oauth
  uni-oauth 三方登录模块 （`HBuilderX5.08+`）
    + 包括API：[uni.login](../api/sign-in.md#login)
    + 依赖的模块：无
...
- uni-canvas
  [canvas画布组件](../component/canvas.md)模块（`HBuilderX4.25+`）
    ...
    + 包括API：[uni.createCanvasContextAsync](../api/create-canvas-context-async.md)、[CanvasRenderingContext2D](../api/canvasrenderingcontext2d.md)、[UniCanvasElement](../api/dom/unicanvaselement.md)
    + 依赖的模块：无
