...
如在自己的项目下打开蒸汽模式，需要确保使用 uni-app x 项目，在**manifest.json的可视化界面首页中勾选蒸汽模式**。
...
但蒸汽模式，通过更强大和复杂的编译器，把vue的语法编译成了包含DOM操作最佳实践的JS代码。在不依赖虚拟DOM的情况下实现了更高性能。
...
对于开发者而言，写条件编译时仅需一个条件，即`// #ifdef VUE3-VAPOR`。（一般用于插件同时兼容VDOM模式和蒸汽模式）
...
|原生 Compose UI				|625.8(645 623 593 656 612)	|124,145	|
...
Android上数据有较多维度，有初次安装、闲时优化；uni-app x也有拍平和非拍平。但不管哪个维度，uni-app x 均比 Android 原生View 和 Compose UI 快。
...
由于原生没有相应方案，故无法对比原生。只能设计一个压力测试，测试 uni-app x 蒸汽模式的 rich-text组件在各平台的体验。

用一个 rich-text组件加载5万字长文，其中包括59张插图。可以看到：
...
当然并非 uni-app x 的几十个组件，每个都比原生组件的性能高。
有些组件在页面中不会大量使用，DCloud并未专项优化，比如video组件使用的就是exoplayer，map组件使用的也是原生的sdk，它们和原生的性能一样。\
但高频使用、有性能压力的组件，uni-app x 均会优化到比原生组件更快，始终保持综合性能第一。
...
- **仅支持组合式，不支持选项式**
    ...
    选项式转组合式，AI可以帮忙。hello uni-app x 里大量的选项式页面都是用 uni-agent 转成了组合式，以适配蒸汽模式。详见[uni-agent](https://doc.dcloud.net.cn/uni-app-x/ai/)
- 不再支持mixin
...
- 变更：css的样式隔离策略，仅支持样式隔离策略2.0。它相对于1.0有较大调整。并且也是跨端中最合理的应用、页面、组件样式隔离管理模式。 [详见](css/common/style-isolation.md)
...
注意，如果项目之前是选项式，使用过早期的 hello uni-app x 或 uni-id-pages 里的废弃组件，需要升级这些项目或插件，更换为新写法：
- uni-loading，改用内置组件[loading](./component/loading.md)
- uni-popup，改用内置组件[page-container](./component/page-container.md)

如果想从 uni-app 升级到 uni-app x 蒸汽模式，[另见升级指南](./uniapptox.md)
