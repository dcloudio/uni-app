...
| fit-content | Web: 4.0; Android: x; Android(Vapor): x; iOS: x; iOS(Vapor): x; HarmonyOS: x | 取以下两种值中的较大值：<br/>    <br/>      固有的最小宽度<br/>      固有首选宽度（max-content）和可用宽度（available）两者中的较小值<br/>    <br/>    可表示为：min(max-content, max(min-content, \<length-percentage>)) |
| max-content | Web: 4.0; Android: x; Android(Vapor): x; iOS: x; iOS(Vapor): x; HarmonyOS: x | 元素内容固有的（intrinsic）合适宽度。 |
| min-content | Web: 4.0; Android: x; Android(Vapor): x; iOS: x; iOS(Vapor): x; HarmonyOS: x | 元素内容固有的最小宽度。 |
