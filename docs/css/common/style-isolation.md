...
- 引用
指的是否能引用到某个class，比如组件里是否可以引用到页面中的class、全局的class。
- 覆盖
...
## HBuilderX 5.0+ 全平台统一样式隔离策略2.0

uni-app x 5.0起，提供了 `样式隔离策略2.0`

蒸汽模式仅支持`样式隔离策略2.0`

VDOM 模式，考虑到向下兼容问题，目前没有废弃老版样式隔离策略，暂时默认是老策略。需要显式开启2.0策略。
...
推荐插件作者在自己的项目下打开样式隔离策略2.0，尽快完成插件适配。主流插件作者陆续改造完毕后，HBuilderX将再次升级，调整为 VDOM 模式也默认使用样式隔离策略2.0，并逐步废弃老版样式隔离策略。
...
推荐开发者尽快升级选项式为组合式，但样式隔离策略2.0在 VDOM 模式下也支持选项式写法。
...
如果组件作者，希望保持向下兼容，即兼容HBuilderX 5.0以前的样式方案。可以在组件内配置app-and-page，允许全局和页面进行同名class合并。
...
## 注意
再次提醒，以上介绍的styleIsolation、external-class等内容，均属于样式隔离策略2.0的内容。VDOM 模式的应用未开启 `manifest.json->uni-app-x->styleIsolationVersion: "2"` 是不会生效的。
