...
uni-app x ，尤其是蒸汽模式，拥有快过原生的渲染引擎。[评测另见](./app-vapor.md#性能说明)
...
hello uni-app x 示例中，有一个日历的模板页面，vdom模式下就是通过draw api来绘制的，性能很高。[源码详见](https://gitcode.com/dcloud/hello-uni-app-x/blob/master/pages/template/calendar/calendar.uvue)

- 在蒸汽模式下，支持了view、text、image的拍平。

Draw API使用复杂，且无法跨端到web和小程序。

uni-app x 蒸汽模式提供了拍平（flatten），允许开发者编写跨平台的view、text、image代码，只需给这些组件加上flatten属性。

被拍平的view、text、image，将被绘制在父上，在审查元素边界时看不到边界。其性能远超原生的view、text、image。实现了跨平台和性能兼得。
...
当然耗时数据不能以真机运行为准，调试基座因为热更新和sourcemap追踪等很多调试功能，导致性能比真实打包差很多。正式打包后性能更优。

## Android蒸汽模式选择so库不压缩

Android蒸汽模式的manifest的Android配置中有一个选项“压缩so库”。默认是压缩的。

压缩so库，apk包体积变小，但性能下降。

这是一个体积和性能的平衡策略，在意体积就压缩，在意性能就要去manifest里配成不压缩。

## 界面元素动画

动画，有2种，预定义不可编程动画、可编程动画。

预定义不可编程动画，包括css transition动画、以及 [animation-view组件](https://ext.dcloud.net.cn/plugin?id=10674)，这个组件本质是lottie动画。

可编程动画，包括 [UniElement.animate](./api/dom/unielement.md)帧动画方法，或者监听手势、滚动、帧回调[requestAnimationFrame](./api/animation-frame.md)，在回调中操作元素的transform。
...
源码参考hello uni-app x中的吸顶示例，这充分体现了 uni-app x 引擎底层的通信性能足够优秀。[详见](https://gitcode.com/dcloud/hello-uni-app-x/blob/master/pages/template/scroll-view-sticky-section/scroll-view-sticky-section.uvue)
...
复杂计算逻辑，可以使用[worker](./api/create-worker.md)
...
分批加载没有封装具体的组件，hello uni-app-x内提供了一个示例[详见](https://gitcode.com/dcloud/hello-uni-app-x/blob/alpha/pages/template/batch-load-long-list/batch-load-long-list.uvue)
...
HBuilderX真机运行到Android vdom时，每个页面进入时会都打印页面初始化的数据：dom数量、排版次数、渲染耗时等。
