...
hash: 5dee93a114ec00103a6e72fdd7033c51ee3ef78bdb51c8960d06859db1a29695
-->
...
`原生混编`出现之前，开发者只能使用[UTS语言](https://doc.dcloud.net.cn/uni-app-x/uts/) 来开发[UTS插件](./uts-plugin.md)
...
UTS的[内置对象](https://doc.dcloud.net.cn/uni-app-x/uts/buildin-object-api/number.html)和[平台专用对象](https://doc.dcloud.net.cn/uni-app-x/uts/utsios.html)均可以在原生环境使用，
...
`UTS内置对象`在具体的平台会有一个对应的类名，举例： UTS内置的[Set](https://doc.dcloud.net.cn/uni-app-x/uts/buildin-object-api/set.html) 对应 `swift`中的`UTSSet`
...
const utsGetMemory = () => {
  ...
  memInfo.value = "可用内存:" + array[0] + "MB \n整体内存:" + array[1] + "MB"
  console.log('getMemoryInfo', array)
}

const utsStartMemoryWatch = () => {
  onMemoryInfoChange((res: Array<number>) => {
    memInfo.value = "可用内存:" + res[0] + "MB \n整体内存:" + res[1] + "MB"
    console.log('onMemoryInfoChange', res)
  })
}

const utsStopMemoryWatch = () => {
  offMemoryInfoChange()
  memInfo.value = "已停止监听"
