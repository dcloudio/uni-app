---
source: https://gitcode.com/dcloud/docs-common/edit/main/plugin/uts-for-ios.md
---

<!-- GENERATED BY docs-sync
hash: 67ff871dc087aa6a621e6abd88cf832d881ab7730b135109763faf46ecb89226
-->
...
有些第三方的 SDK 使用 OC 语言开发，且产物 .framework 文件里不包含 Modules 文件夹。这就造成该 SDK 不支持 use module 模式，不能直接在 Swift 文件中导入。
...
单例是指一个 class 只能创建出一个全局唯一且线程安全的实例。swift 中使用 ` static let shared = FontFaceManager()` 的方式创建。 目前 uts class 中不支持类似 swift 中创建单例的写法，如果你的自定义 class `只在 uts 环境中使用，而不需要向 js export`, 那么可以在混编的 swift 代码中实现 class 的单例功能
示例代码如下：
...
- uts 插件开发者在选择比打包机更低版本XCode编译Swift库时请在Target->buildSettings设置Build Libraries for Distribution 为Yes；
