---
source: https://gitcode.com/dcloud/docs-common/edit/main/plugin/uts-ios-cocoapods.md
---

<!-- GENERATED BY docs-sync
hash: e0803e0be050ef47e95313ad946fe30fd2199e1f9e436fbf038eee6e1910d0c6
-->
...
- dependencies-pod-resources：HBuilderX5.25版本新增支持，指定依赖的 pod 库配置的资源文件（如 bundles ）打包后保存的位置，uni-app项目中的默认值为"all"，uni-app x项目中的默认值为"framework"
  + 配置为"app"时，将资源保存到主应用（main bundle）中
  + 配置为"framework"时，将资源保存到uts插件的动态库（framework）中
  + 配置为"all"时，同时将资源保存到主应用和动态库中
...
说明： 出现此类错误是因为网络问题无法正常访问 [CDN 服务器](https://cdn.cocoapods.org)
