...
hash: 52cf059bbed4868a9a163efdadbe622efc737aaf5bc229bd2e452d3dda7e2139
-->
...
### interceptor.js@interceptorjs

> iOS平台，需HBuilderX 5.11+；Android平台，需蒸汽模式，HBuilderX 5.21+
...
```js
// interceptor.js
export function initTest(test) { // initTest为约定名称，init + capitalize(要拦截的方法名)。test为原始方法
	return (str) => { // return结果会作为正式对页面组件暴露的test方法
		return '#' + test('_' + str)
	}
}
```
...
<template>
	<view>
		<button @click="callTest">callTest</button>
	</view>
</template>

<script setup lang="uts">
	import { test } from '@/uni_modules/test-utssdk'
	function callTest() {
		console.log(test('test')) // 输出 "#_test;"
	}
</script>
```
...
- project
	...
	+ plugins
		...
		注意：plugins配置仅支持以下白名单插件："com.google.gms.google-services"，"com.huawei.agconnect"，"com.hihonor.mcs.asplugin"。如需使用其他插件，请通过[技术支持](https://ask.dcloud.net.cn/explore/guidance/paid_technical_support)沟通。
...
- ios-extension.json文件中不能包含注释信息，包含注释信息将会导致配置文件失效  
...
- [Android debug教程](https://doc.dcloud.net.cn/uni-app-x/tutorial/uni-uts-debug.html)
- [iOS debug教程](https://doc.dcloud.net.cn/uni-app-x/tutorial/uni-uts-debug-ios.html)
- [ArkTS debug教程](https://doc.dcloud.net.cn/uni-app-x/tutorial/uni-uts-debug-harmony.html)
...
- This code uses error suppression for 'INVISIBLE_REFERENCE'. While it might compile and work, the compiler behavior is UNSPECIFIED and WILL NOT BE PRESERVED

此告警通常出现在安卓蒸汽模式，插件中存在疑似隐式protected可见性的方法或属性，改为明确可见性即可。

```uts
export class A extends B {
	override method() { // 编译器无法分析出method的可见性，会生成抑制kotlin编译报错的代码，kotlin编译器会给出上面的警告
		// do sth
	}
}
```
...
```uts
export class A extends B {
	override public method() { // 此处需使用method的真实可见性，public、protected、private
		// do sth
	}
}
```
