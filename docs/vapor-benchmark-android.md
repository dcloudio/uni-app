...
- 蒸汽模式提供了大量自研高性能组件，如view、text、image、list-view、rich-text、swiper、slider、picker等

uni-app x 蒸汽模式依次推出鸿蒙版、iOS版、Android版。
鸿蒙和iOS的评测报告之前已经公布。

本报告重点在于Android平台的性能评测。
...
- rich-text组件
App平台的rich-text过去一直没有太好的解决方案。原生下很多时候不得不基于webview渲染的，存在加载慢、内存占用高、快滑白屏等问题。

uni-app x 蒸汽模式 提供更快的rich-text组件。以下测试，加载5万字长文、59张插图。可以看到无等待进入页面。上下快滑不掉帧、除了联网图片加载外滑不出白屏。

[视频链接](https://www.bilibili.com/video/BV134u26CE2s)

- swiper组件

加载100个item。无等待进入页面。在上述5万字长文中点击图片，可以看到swiper中无等待呈现59张图片，左右切换图片无延迟
