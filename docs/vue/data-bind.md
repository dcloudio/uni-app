...
2. 响应式变量有diff更新机制。比如对于一个大列表数据，其中一项变更时，框架底层会自动计算diff，给UI层差量同步数据。这在大多数情况是很好的，但注意diff这个计算过程本身也会增加耗时。
...
需要给 `ref` 标注类型时，在uts/ts中可通过泛型的写法，如：`ref<string>()`， 或使用 `as` 的写法。

当然 uts/ts 有一定的自动推导能力，对于特别简单的布尔值/数字/字符串的字面量，不写泛型或as，也可以自动推导类型。

`ref()` 接收参数，并将其包裹在一个带有 `.value` 属性的 `ref` 对象中返回。这个对象，

- ref定义的响应式变量，在script代码中取值时，需要使用 `.value`属性。
...
这个语法同样也适用于 `<script setup>`，且支持表达式 (需要用引号包裹起来)

`v-bind` 也可在样式中使用，可以很方便的在 script 中改变样式，如下所示：
...
而使用组合式 API 时，可以直接在 `<script setup>` 中定义方法
