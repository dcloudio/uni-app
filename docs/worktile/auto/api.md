---
source: https://gitcode.com/dcloud/docs-common/edit/main/worktile/auto/api.md
---

<!-- GENERATED BY docs-sync
hash: 386415cc613b48b2919b4ab4e95952b715e9868442c65e799a7e5c4c4d45658e
-->
...
未设置 `options.path` 时，该方法返回图片数据的 base64 编码；设置 `options.path` 时，将图片写入指定路径。

options 字段定义如下：

|字段|类型|必填|默认值|说明|
|:-:|:-:|:-:|:-:|:-:|
|path|string|否|-|图片保存路径，相对路径基于项目根目录|
|fullPage|boolean|否|false|是否截取当前页面的全部内容|
|area|Object|否|-|指定截图区域，uni-app x 的 App 平台支持，见下方 area 说明。HBuilderX 5.23+|

area 字段定义如下，坐标和尺寸单位均为逻辑像素：

|字段|类型|必填|默认值|说明|
|:-:|:-:|:-:|:-:|:-:|
|x|number|否|0|截图区域左上角的 x 坐标|
|y|number|否|0|截图区域左上角的 y 坐标|
|width|number|否|从 x 到截图右边界|截图区域宽度，必须大于 0|
|height|number|否|从 y 到截图下边界|截图区域高度，必须大于 0|

> uni-app x 的 App 平台使用 `area` 截图需 HBuilderX 5.23+。`area` 在截图完成后进行裁剪，可与 `fullPage` 同时使用。截图区域超出原图边界时，会按原图边界裁剪；裁剪后区域无效时，方法将抛出异常。
...
> `program.device` API 仅支持 App 平台，需 HBuilderX 5.23+。以下 API 中的坐标和尺寸单位均为逻辑像素，时长单位均为毫秒。

##### program.device.screenshot

截取设备屏幕，截图中可以包含状态栏、导航栏等系统界面。

> HBuilderX 5.23+

`program.device.screenshot(options?: Object): Promise<string | void>`

未设置 `options.path` 时，该方法返回图片数据的 base64 编码；设置 `options.path` 时，将图片写入指定路径并返回 `void`。
...
|:-:|:-:|:-:|:-:|:-:|
|path|string|否|-|图片保存路径，相对路径基于项目根目录|
|area|Object|否|-|指定截图区域，字段定义同 `program.screenshot` 的 area|

> iOS 仅支持模拟器，iOS 真机不支持该方法。
...
```js
  ...
  await program.device.screenshot({
    path: 'static/device-screenshot.png',
    area: {
      x: 0,
      y: 100,
      width: 300,
...
`program.device.tap(x: number, y: number, options?: Object): Promise<void>`

|字段|类型|必填|默认值|说明|
|:-:|:-:|:-:|:-:|:-:|
|x|number|是|-|点击位置的 x 坐标|
|y|number|是|-|点击位置的 y 坐标|
|options|Object|否|-|点击选项|
|options.duration|number|否|0|按压时长|

> HarmonyOS App 上，`duration` 仅用于区分点击和长按：小于 350 时执行点击，大于等于 350 时执行长按，不表示实际按压时长。

示例代码：

```js
  await program.device.tap(100, 200)
  await program.device.tap(100, 200, { duration: 500 })
```


##### program.device.swipe

在设备屏幕上从起点滑动到终点。

> HBuilderX 5.23+

`program.device.swipe(startX: number, startY: number, endX: number, endY: number, options?: Object): Promise<void>`

|字段|类型|必填|默认值|说明|
|:-:|:-:|:-:|:-:|:-:|
|startX|number|是|-|起点的 x 坐标|
|startY|number|是|-|起点的 y 坐标|
|endX|number|是|-|终点的 x 坐标|
|endY|number|是|-|终点的 y 坐标|
|options|Object|否|-|滑动选项|
|options.duration|number|否|100|滑动时长|

示例代码：

```js
  await program.device.swipe(100, 500, 100, 200, {
    duration: 300
  })
```


##### program.device.setScreenOrientation

设置设备屏幕方向。

> HBuilderX 5.23+

`program.device.setScreenOrientation(orientation: string): Promise<void>`

|字段|类型|必填|默认值|说明|
|:-:|:-:|:-:|:-:|:-:|
|orientation|string|是|-|屏幕方向，可取值为 `portrait`、`portraitUpsideDown`、`landscapeLeft`、`landscapeRight`|

> 该方法仅支持 Android 模拟器和 iOS 模拟器，不支持 Android 真机、iOS 真机及 HarmonyOS。

示例代码：

```js
  await program.device.setScreenOrientation('landscapeLeft')
```


##### program.device.keyboard.input

向设备输入文本。

> HBuilderX 5.23+

`program.device.keyboard.input(text: string, options?: Object): Promise<void>`

|字段|类型|必填|默认值|说明|
|:-:|:-:|:-:|:-:|:-:|
|text|string|是|-|要输入的文本|
|options|Object|HarmonyOS：是；其他平台：否|-|输入选项|
|options.x|number|HarmonyOS：是；其他平台：否|-|输入位置的 x 坐标，需与 y 同时设置才会生效|
|options.y|number|HarmonyOS：是；其他平台：否|-|输入位置的 y 坐标，需与 x 同时设置才会生效|
|options.delay|number|否|0|逐字符输入时相邻字符之间的间隔；大于 0 时启用逐字符输入|

> Android 和 iOS 未设置 `options.x`、`options.y` 时，会向当前已聚焦的输入控件输入文本；同时设置两者时，会先点击指定位置再输入。只设置其中一个坐标不会执行点击。

> HarmonyOS App 必须同时设置 `options.x` 和 `options.y`，缺少任一坐标都会调用失败。

示例代码：

```js
  await program.device.keyboard.input('hello uni-app', {
    x: 100,
    y: 200,
    delay: 50
  })
...
|方法|Android App|iOS 模拟器|iOS 真机|HarmonyOS App|H5|小程序|说明|
|--|--|--|--|--|--|--|--|
|screenshot|√|√|x|√|x|x|截取设备屏幕，支持通过 area 指定截图区域|
|tap|√|√|√|√|x|x|点击设备屏幕上的指定位置|
|swipe|√|√|√|√|x|x|在设备屏幕上滑动|
|setScreenOrientation|仅模拟器|√|x|x|x|x|设置设备屏幕方向|
|keyboard.input|√|√|√|√|x|x|输入文本，HarmonyOS App 必须指定 x 和 y|
